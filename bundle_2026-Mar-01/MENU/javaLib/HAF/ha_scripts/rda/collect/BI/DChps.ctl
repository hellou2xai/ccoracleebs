# DChps.ctl:582:Collects Oracle Hyperion Performance Scorecard Information
# $Id: DChps.ctl,v 1.6 2015/08/21 16:04:40 RDA Exp $
# ARCS: $Header: /home/cvs/cvs/RDA_8/src/scripting/lib/collect/BI/DChps.ctl,v 1.6 2015/08/21 16:04:40 RDA Exp $
#
# Change History
...
BI:DChps - Collects Oracle Hyperion Performance Scorecard Information

=head1 DESCRIPTION

This module collects information for Oracle Hyperion Performance Scorecard.
...
echo tput('bold'),'Processing BI.HPS module ...',tput('off')
...
pretoc '1:Hyperion Performance Scorecard'
...
Displays the RDA abbreviations defined for the Hyperion Performance Scorecard
home collection.

=cut

debug ' Inside HPS module, collecting defined home abbreviations'
report abbr
prefix
{write '---+ Hyperion Performance Scorecard Home Abbreviations'
...
For Windows, collects Hyperion Performance Scorecard-related Registry
information.

=cut

if or(isWindows(),isCygwin())
{debug ' Inside HPS module, gathering HPS registry information'
 report registry
 prefix
 {write '---+!! Hyperion Performance Scorecard Registry Information'
  write $TOC
 }
 if hasRegOption()
 ...
 if writeRegistry('HKLM\SYSTEM\CurrentControlSet\Services\HyS9HPSAlerter')
  write $TOP
 if writeRegistry('HKLM\SYSTEM\CurrentControlSet\Services\HyS9WebReports')
  write $TOP
 if isCreated(true)
  toc '2:[[',getFile(),'][rda_report][Registry Information]]'

=head2 eventx - HPS Events

Extracts Oracle Hyperion Performance Scorecard events from the application event
log using the F<wevtutil> command (only available for Windows Vista,
Windows Server 2008, and Windows 7).

=cut

 debug ' Inside HPS module, gathering event log information'
 ...
 {if ?findCommand('wevtutil')
  {var $evt = last
   ...
   report eventx
    write '---+!! Oracle Hyperion Performance Scorecard Events'
   prefix
    call beginBlock(true)
   if createBuffer('EVT','R',$tmp)
   {call parseReset()
    call parseBegin('TOP',\
      '^<Event.*<System><Provider Name=.*?HyS9FinancialClose.*/>','Event')
   ...
   if hasOutput(true)
    call endBlock(['C',\
                   'wevtutil qe Application | grep -i HyS9FinancialClose'])
   else
    write '**No Oracle Hyperion Performance Scorecard events found.**%BR%'
   toc '2:[[',getFile(),'][rda_report][HPS Events]]'
   call unlinkTemp('dat')
  }
 }

=head2 events - HPS Events
...
=cut
 ...
 {# Get the event file
  ...
  # Extract the Oracle Hyperion Performance Scorecard events
  if ?testFile('r',$evt)
  {report events
   write '---+ Application Events'
   if !writeEvents($evt,'HyS9FinancialClose',$AGE)
    write '**No Oracle Hyperion Performance Scorecard events found.**%BR%'
   toc '2:[[',getFile(),'][rda_report][HPS Events]]'
  }
  if $flg
...
debug ' Inside HPS module, gathering log files'
pretoc '2:Log Files'
...
=cut
  ...
  debug ' Inside HPS module, collecting defined instance abbreviations'
  report abbr
  prefix
  {write '---+ Hyperion Performance Scorecard Instance Abbreviations'
   write '|*Abbreviation*|*Location*|'
  }
  var %hsh = getSymbols()
  loop $key (keys(%hsh))
...
=cut
  ...
  debug ' Inside HPS module, getting the CSS Configuration'
...
=cut
  ...
  debug ' Inside HPS module, getting the logging configuration files'
  pretoc '2:Configuration Files'
  call sort_files(3,0,\
...
=cut
  ...
  debug ' Inside HPS module, getting the diagnostic log files'
  report diaglogs
  var $log = catDir($ins,'diagnostics','logs')
  var %pat = (starter  => '^(start|stop)-HyS9HPS(Alerter|WebReports)-out\.log$')
  prefix
...
=cut
  ...
  if ${CUR.O_SETUP}->search(concat('^WREQ_BI_HPS_',replace($uid,'^OI','DOM')))
  {var ($req) = last
   ...
   toc '%INCLUDE("OFM_WREQ_BI_HPS_',$oid,'_TF.toc",1)%'
   toc '%POP2%'
   toc '%PUSH("%SPLIT%")%'
...
=cut
 ...
 {var $dom = basename(last)
  toc '%PUSH("%SPLIT%")%'
  toc '%PUSH("1+:Oracle WebLogic Server Overview")%'
  toc '%INCLUDE("OFM_WREQ_BI_HPS_WH_TF.toc")%'
  toc '%POP2%'
  toc '%PUSH("%SPLIT%")%'
  toc '%PUSH("1+:',"'",$dom,"'",' Domain")%'
  toc '%INCLUDE("OFM_WREQ_BI_HPS_DOM_TF.toc")%'
  toc '%POP2%'
 }
}
