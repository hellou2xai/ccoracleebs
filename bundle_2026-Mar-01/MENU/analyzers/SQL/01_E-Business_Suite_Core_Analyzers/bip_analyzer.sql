...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for BIP for EBS Analyzer                   |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE ebs_bip_analyzer_pkg AUTHID DEFINER AS
...
END ebs_bip_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY ebs_bip_analyzer_pkg AS
-- $Id: bip_analyzer.sql, 200.55 2026/02/25 16:18:02 dterrell Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2032715.1));
...
g_s_xdo_runtime_release  VARCHAR2(10);
g_pqp_status             VARCHAR2(75);
g_instance               V$INSTANCE.INSTANCE_NAME%TYPE;
g_opp_process_timeout    number;
g_opp_response_timeout   number;
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'BIP_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'BIP_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>BIP Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2032715.1#feedback");
...
-- page title
    print_out('        <div class="header_title">'||p_analyzer_title|| ' Report <br>
        ...
        <img class="header_version" src="https://www.oracle.com/webfolder/s/analyzer/bip_latest_version.gif" title="Click here to download the latest version of Analyzer" alt="Latest Version Icon"></a>');
    print_cloud_image;
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,202,EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="thumb_up_lg" title="Analyzer Feedback: Thumbs Up"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,202,EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="thumb_dn_lg" title="Analyzer Feedback: Thumbs Down"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},202,EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="af_idea_lg" title="Analyzer Idea"></span></a>');
    print_out('        </div>
    </div>');
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/mosc/discussion/3800139" class="blacklink" target="new">
...
<div><div><div><div><p>8/11/25: Updated signature EBS_ATG_CT_BIP_OAP_PATCHES and replaced patch 35381710 with the patch 37640849.</p><p>7/2/25: Signature EBS_ATG_UGLE_32BIT_SEQUENCES_CLOSE_TO_MAX has been changed.</p><p>5/07/25: Added the signature CP1_PROFILES to the OPP section to show a list of profiles and it&apos;s value</p><p>4/16/25: There are 2 new features:<br>
- Fixed the input is too long error when running an analyzer in SQL*Plus<br>
- Added the click to highlight rows in tables</p><p><span style="font-family:&apos;Calibri&apos;,sans-serif;font-size:16px;">The analyzer is now executed as a Package instead of an Anonymous Block. Please see the instructions in Doc ID </span>2032715.1<span style="font-family:&apos;Calibri&apos;,sans-serif;font-size:16px;"> for running the analyzer from Sql*Plus and from Concurrent Manager as a Concurrent Program.</span></p><p>The framework was updated to framework version 4.5.2</p><p>The Data Masking feature has been expanded.&nbsp;&nbsp;&nbsp;&nbsp;<br>
...
3.&nbsp;&nbsp;&nbsp;The existing “Lock Icon” still provides customers the manual column scrambling capabilities, which can be used <strong>in addition to</strong> the newly introduced pre-masked data feature.&nbsp;&nbsp;</p></div>
</div>
<ul><li>&nbsp; ORA-29532: Java call terminated by uncaught Java exception: java.lang.NullPointerException</li>
<li>&nbsp; Print only not applied patches in DX for recommended patch signatures</li>
<li>&nbsp; Put the Framework version in the html output</li>
</ul>
</div>
<div>We’re listening to your feedback and made the following changes:</div>
<ul><li>Persistent table headers.&nbsp;<br>
If sql output has lot of rows, you will still see the column headers when scrolling down.</li>
<li>Print parameters in the log even when validation fails and the process stops without creating an output</li>
<li>Data scrambling can be enabled for selected columns based on a user-entered key</li>
<li>Hamburger icon in section (tile) view is moved and made more visible</li>
<li>Start/End Time in Execution Details showing down to seconds only</li>
<li>New features for MGD compatibility:&nbsp;<br>
&nbsp;&nbsp; - Tags can be added to signatures and used for filtering<br>
&nbsp;&nbsp; - Extra information tokens can be defined and their values will be replaced in the signature solution</li>
<li>Change Feedback URL dynamically and point to internal KM note when analyzer output is opened in ISS viewer</li>
</ul>
</div>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,202,'||p_sig.sigrepo_id||',EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,202,'||p_sig.sigrepo_id||',EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},202,'||p_sig.sigrepo_id||',EBS,EBS ATG,EBS - Component Tools" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
FUNCTION check_rec_patches_1 RETURN VARCHAR2 IS
  /* Signature BIPXDO_chk_rec_patches */
...
BEGIN
    ...
    print_log('Processing recommended patches signature: BIPXDO_chk_rec_patches (check_rec_patches_1) "Recommended Patches" (version 23)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_col_rows(1)(l_rows) := '35381710';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '35381710: OAP: ADD SUPPORT FOR ORACLE ANALYTICS PUBLISHER IN EBS 12.2';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34365607';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '34365607: Oracle Analytics Publisher 6.4.0 Re-hosting Patch for EBS 12.2';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '33236811';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '33236811: XML BURSTING PROGRAM FAILING WITH : JAVA.UTIL.HASHTABLE.PUT(HASHTABLE.JAVA:514)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '31966103';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '31966103: BI-PUBLISHER 10G RE-HOSTING ROLL-UP PATCH FOR E-BUSINESS SUITE 12.2.X';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28029438';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP PATCH FOR E-BUSINESS SUITE 12.2.X';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '22615957';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP PATCH FOR E-BUSINESS SUITE 12.2.X TXK';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '22083976';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G REHOSTING PATCH FOR BUG 21888922 FOR E-BUSINESS SUITE 12.2.X TXK';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21255570';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING PATCH FOR E-BUSINESS SUITE 12.2.X TXK';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21127389';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G REHOSTING PATCH (20216531,20573476) FOR E-BUSINESS SUITE 12.2.X TXK';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '20605452';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C BIP: BI-Publisher 10G Rehosting Patch (19010320,20512587) For E-Business Suite 12.2.4';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '20121649';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10g REHOSTING PATCH (BUG 19788249) FOR E-BUSINESS SUITE 12.2.4';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '17997557';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.C Duplicate of Templates Not Working in 12.2 Online Patching Envs';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28728678';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Sync to BIP 12g SFTP code including latest ciphers and algorithms';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '22312109';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'CONFIGURE OF XDODELIVERY.CFG FILE FOR TLS AUTHENTICATION IN EBS FOR BI PUBLISHER';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18935728';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B XML Publsher EBS 5.6.3.1 Patch';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18350925';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Hyperlinks Incorrect Format After Application Of All XDO Patches';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18044389';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B java.lang.NullPointerException at oracle.apps.xdo.delivery.ssh2.transport.TransportProtocolClient.negotiateVersion';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '17862411';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B NOSUCHMETHODERROR While Using SFTP GET';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '16729676';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Set Timestamp Which Is Different From SYSDATE In Date Field In Digital Signature';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '13848568';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.B Inclusion of osdt_cert.jar For CP Tier';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '13791540';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Override Template Locale In TemplateHelper.processTemplate';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '12415414';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B 1OFF:11873050:12.1.3:12.1.3:FRM-41830, FRM-40212 Submitting Program - Publisher FSG (adds Excel Template definition)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '12382509';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B PROGRAM CALLS XDODTEXE,ERRORS - JAVA.LANG.NUMBERFORMATEXCEPTION:FOR INPUT STRING';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '10405679';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B XML PUBLISHER LIMITATION OF 2^31 ON CONCURRENT PROGRAM PARAMETERS';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9914102';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B SUPPORT DIGITAL SIGNATURE FOR PDF AS PART OF BURSTING CONTROL FILE';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9501440';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Field Mappings Are Not Mapped In The Final Output When Reports Are Bursted Using PDF Template';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9162140';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Right Fax Channel Support With Provision For Delivery Status At A Later Time';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9003873';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B XDO_CURRENCY_FORMAT_SETS_PKG.ADD_LANGUAGE Fails';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '8919489';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.B.delta.3';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '7687414';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.B Rehosting xdoparser 10.1.0.5 With Fix For Bug 7586025, 7339075 (incl. in patch 8919489:R12.TXK.B.delta.3)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.0' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '14004181';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A The Concurrent Completed With Warning Status If Repeating Field Have Value';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Override Template Locale In TemplateHelper.processTemplate';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '13146709';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A TemplateHelper Produces A Garbled Output When Invoked By Multiple Threads';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Program Calls XDODTEXE, errors - java.lang.NumberFormatException: For input string';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '10432202';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Base bug 10377266 : Backport request for oracle.xdo.template.formprocessor';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A XML Publisher Limitation Of 2^31 On Concurrent Program Parameters';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '10065439';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A API TemplateHelper.processTemplate Takes Long Time';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9851616';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A 1OFF:9752717:12.1.1:Accented Characters (French) Are Not Appearing Correctly in Bursting Email Body';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Field Mappings Are Not Mapped In The Final Output When Reports Are Bursted Using PDF Template';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9450112';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Sales Journal By GL Account Report (XML) -Incorrect GL Dates Displayed';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '9197676';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Backport:9113190:5.6.3:12.0.4:Field Alignment Of Justify Not Working When PDF Output';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Right Fax Channel Support With Provision For Delivery Status At A Later Time';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A XDO_CURRENCY_FORMAT_SETS_PKG.ADD_LANGUAGE Fails';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '8869321';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Error - Privatekey Is Not Supported - When Transmitting Payment File To Back';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '8793202';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Need Ability To Chose To Ignore Invalid Characters Or Not Ignore';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '8745414';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A Total By Page In The RTF Layout To Be Printed In Words';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '8359991';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A An XDO Property To Enable Debug Logging';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '7274371';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.A eText Update For XML Publisher 5.6.3';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28950024';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'UNABLE TO CONNECT TO SERVER: UNKNOWN MSG WITH ID 0/1';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '25828573';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.10';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28840822';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.11';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '30735865';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.12';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '32392507';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.13';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '33602997';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.14';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '37311063';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'XDOTRANS:JCP4TEMPLATETRANSLATOR GENERATES MLS_TEMPLATE THAT DID NOT EXIST BEFORE';
   l_col_rows(5)(l_rows) := '[3097824/KB706386]';

END IF;

   l_extra_info('##SHOW_SQL##'):= 'Y';

   l_sig.sigrepo_id := '421';
   l_sig.title := 'Recommended Patches';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please click the expand-collapse arrow above to check the recommended patches that are not applied on this instance';
   l_sig.solution := '<p>Patches are documented in the note Overview of Available Patches for Oracle XML Publisher embedded in the Oracle E-Business Suite [1138602/KB684460] The patches listed in the note are also recommended to resolve template and report generation issues. </p>
              <p><strong>To get a current accurate list of recommended EBS product patches that are applied or not applied to your instance, the Oracle Best Practice is to run the Patch Wizard utility.</strong><br /> See [976188/KB739927] - Patch Wizard Utility, [10940175/KA580] FAQ, or [1077813/KB627816] Videos for more information.</p>
            <ul>
                <li>The list of recommended product patches are as of the date the script was written to show the patches not on this instance.</li>
               <li>Please review the list to schedule and apply any patches as soon as possible</li>
               <li>Refer to the MOS Note indicated for more information about each patch</li>
           </ul>
            <p>Note: BI Publisher Desktop 10.1.3.4.2 is the compatible version for EBS Embedded BI Publisher 12.2.x Applications. 
              The BI Publisher Desktop 10.1.3.4.2 for E-Business Suite 12.2.x can be downloaded via the following OTN link for BI Publisher Desktop 10.1.3.4.2 for Windows: 
              <a href="http://www.oracle.com/technetwork/middleware/bi-publisher/downloads/index.html">OTN BI Publisher Download</a></p>';
   l_sig.success_msg := '<p>There are no recommended patches at the time this script was written for this release.<strong><br /> </strong></p>
                         <p>An overview of available patches for Oracle XML Publisher embedded in the Oracle E-Business Suite [1138602/KB684460] The patches listed in the note are also recommended to resolve template and report generation issues.</p>
                         <p><strong>To get a current accurate list of recommended EBS product patches that are applied/not applied to your instance, please run the Patch Wizard utility.</strong><br /> See [976188/KB739927] - Patch Wizard Utility, [10940175/KA580] FAQ, or [1077813/KB627816] Videos for more information.</p>
                         <p>Note: BI Publisher Desktop 10.1.3.4.2 is the compatible version for EBS Embedded BI Publisher 12.2.x Applications. 
                         The BI Publisher Desktop 10.1.3.4.2 for E-Business Suite 12.2.x can be downloaded via the following OTN link for BI Publisher Desktop 10.1.3.4.2 for Windows: 
                         <a href="http://www.oracle.com/technetwork/middleware/bi-publisher/downloads/index.html">OTN BI Publisher Download</a></p>';
  ...
  -- Register
  l_step := '60';
  g_signatures('BIPXDO_chk_rec_patches') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'BIPXDO_chk_rec_patches',     -- sig ID
...
FUNCTION check_rec_patches_2 RETURN VARCHAR2 IS
  /* Signature EBS_ATG_CT_BIPXDO_REC_PATCHES */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: EBS_ATG_CT_BIPXDO_REC_PATCHES (check_rec_patches_2) "BIP XDO Recommended Patches" (version 4)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34785677';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'TXK:R12.TXK.C.DELTA.15';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34456759';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BIP 10G BUILD.145 REHOSTING ROLLUP FOR EBS 12.2.X
 (xdo delivery smtp updates Javamail debug for xdo10g.zip < 120.0.12020000.26)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34419550';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BIP 10G BUILD.144 REHOSTING ROLLUP FOR EBS 12.2.X  R12.TXK.C (xdo delivery smtp updates Javamail debug for xdo10g.zip < 120.0.12020000.25)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '33765613';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BIP 10G BUILD.143 REHOSTING ROLLUP FOR EBS 12.2.X  R12.TXK.C (CONSUME NLSUTIL.INDEXOFIGNORECASE() for xdo10g.zip < 120.0.12020000.24)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.2:R12.TXK.C.delta.14';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '33535981';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BIP 10G BUILD.142 REHOSTING ROLLUP FOR EBS 12.2.X  R12.TXK.C (SFTP BURSTING RTF output Backport hlie_bug-13057014 for xdo10g.zip < 120.0.12020000.23)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '32817004';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BIP 10G BUILD.141 REHOSTING ROLLUP FOR EBS 12.2.X  R12.TXK.C (Sept 2021 Backports for xdo10g.zip < 120.0.12020000.22)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.2:R12.TXK.C.delta.13';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP 12.2:FOR E-BUSINESS SUITE 12.2.X';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '31074449';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'SPECIAL ROMANIAN CHARACTER NOT DISPLAYING ON PDF INVOICE  R12.TXK.C (Albany 7.40 fonts - requires post-install AutoConfig for xdo10g.zip < 120.0.12020000.21)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.2:R12.TXK.C.delta.12';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '29538401';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP 12.2:FOR E-BUSINESS SUITE 12.2.X  R12.TXK.C (data grouping for xdo10g.zip < 120.0.12020000.20)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '29521630';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP 12.2:FOR E-BUSINESS SUITE 12.2.X  R12.TXK.C (SecureRandom for xdo10g.zip < 120.0.12020000.18)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BI-PUBLISHER 10G RE-HOSTING ROLL-UP 12.2:FOR E-BUSINESS SUITE 12.2.X  R12.TXK.C (sftp upgrades for xdo10g.zip < 120.0.12020000.19)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28020622';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.XDO.B BACKPORT:23233792:5.6.3:12.1.3:FTP DELIVERY FAILS WITH ORACLE.XDO.DELIVERY.SSH2';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21443957';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'BACKPORT:14680717:5.6.3:12.1.3: BI PUBLISHER 11.1.1.6:TIMES NEW ROMAN FONT DISP';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '20763632';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B Bursting Fails With Illegal Whitespace In Address In String';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.XDO.B XML Publsher EBS 5.6.3.1 12.1:--> This 12.1:replaces many of the previously released XDO patches.';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.B Rehosting xdoparser 10.1.0.5 With Fix For Bug 7586025, 7339075 (incl. in 12.1:8919489:R12.TXK.B.delta.3)';
   l_col_rows(5)(l_rows) := '[1138602/KB684460]';
...
END IF;
   ...
   l_sig.sigrepo_id := '27824';
   l_sig.title := 'BIP XDO Recommended Patches';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please click the expand-collapse arrow above to check the recommended patches that are not applied on this instance';
   l_sig.solution := '<p>Patches are documented in the note Overview of Available Patches for Oracle XML Publisher embedded in the Oracle E-Business Suite [1138602/KB684460] The patches listed in the note are also recommended to resolve template and report generation issues. </p>
              <p><strong>To get a current accurate list of recommended EBS product patches that are applied or not applied to your instance, the Oracle Best Practice is to run the Patch Wizard utility.</strong><br /> See [976188/KB739927] - Patch Wizard Utility, [10940175/KA580] FAQ, or [1077813/KB627816] Videos for more information.</p>
   ...
   l_sig.success_msg := '<p>There are no recommended patches at the time this script was written for this release.<strong><br /> </strong></p>
                         <p>An overview of available patches for Oracle XML Publisher embedded in the Oracle E-Business Suite [1138602/KB684460] The patches listed in the note are also recommended to resolve template and report generation issues.</p>
                         <p><strong>To get a current accurate list of recommended EBS product patches that are applied/not applied to your instance, please run the Patch Wizard utility.</strong><br /> See [976188/KB739927] - Patch Wizard Utility, [10940175] FAQ, or [1077813/KB627816] Videos for more information.</p>
                         <p>Note: BI Publisher Desktop 10.1.3.4.2 is the compatible version for EBS Embedded BI Publisher 12.2.x Applications. 
                         The BI Publisher Desktop 10.1.3.4.2 for E-Business Suite 12.2.x can be downloaded via the following OTN link for BI Publisher Desktop 10.1.3.4.2 for Windows: 
                         <a href="http://www.oracle.com/technetwork/middleware/bi-publisher/downloads/index.html">OTN BI Publisher Download</a></p>';
   l_sig.print_condition := 'ALWAYS';
  ...
  -- Register
  l_step := '60';
  g_signatures('EBS_ATG_CT_BIPXDO_REC_PATCHES') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'EBS_ATG_CT_BIPXDO_REC_PATCHES',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
FUNCTION check_rec_patches_3 RETURN VARCHAR2 IS
  /* Signature EBS_ATG_CT_BIP_OAP_PATCHES */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: EBS_ATG_CT_BIP_OAP_PATCHES (check_rec_patches_3) "Oracle Analytics Publisher Recommended Patches on XDO Runtime Release '||mask_text(g_s_xdo_runtime_release, nvl( upper(''), 'NO_MASK') )||'" (version 14)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_col_rows(5)(l_rows) := '[2230914/KB717858]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '33600809';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.AD.C.Delta.14';
   l_col_rows(5)(l_rows) := '[11177530/KA1054]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34668508';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.AD.C.Delta.14</b> - QRE12212.7:AD:FINALIZE FAILURE: HIGH TEMPORARY TABLESPACE USAGE ISSUE WHILE RUNNING ADOP FINALIZE';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34669333';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.AD.C.Delta.14</b> - BUNDLE PATCH ON TOP OF R12.AD.C.DELTA.14';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34681299';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.AD.C.Delta.14</b> - ADOP PHASE=APPLY PATCHES=31817501 FAILS TO APPLY - ADPATCH MAY BE TERMINATING';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '35280947';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.AD.C.Delta.14</b> - CHANGE TAG FOR OBSOLETE COLUMN TO AD_OBSOLETE IN TABLE MANAGER (ADZDTMB.PLS)';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34708635';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.TXK.C.Delta.14</b> - BNE and FRM PRODUCTS HAVING ISSUES WITH UPTAKE TXK DELTA 14 (HAVING LATEST POI LIBRARIES 3.17) IF EBS RELEASE 12.2.11 OR LOWER';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34654260';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '&#x09;&#x09;<b>Critical Post R12.TXK.C.Delta.14</b> - BUNDLE PATCH ON TOP OF R12.TXK.C.DELTA.14';
   l_col_rows(5)(l_rows) := '[2857317/KB408399]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '37640849';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'OAP: Add Support for Oracle Analytics Publisher in EBS 12.2 (v2)';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34695811';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.AD.C.DELTA.15';
   l_col_rows(5)(l_rows) := '[11177530/KA1054]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.TXK.C.DELTA.15';
   l_col_rows(5)(l_rows) := '[11177530/KA1054]';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Oracle Analytics Publisher 6.4.0 Re-hosting Patch for EBS 12.2';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '35620323';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Consolidated patch for Oracle Analytics Publisher';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '34986658';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := ''||g_pqp_status||'R12.PQP.C TST12212: BIP12C:TP MCR Recon report excel has formatting issue';
   l_col_rows(5)(l_rows) := '[11217188/KA1181]';
...
END IF;
   ...
   l_sig.sigrepo_id := '26581';
   l_sig.title := 'Oracle Analytics Publisher Recommended Patches on XDO Runtime Release '||mask_text(g_s_xdo_runtime_release, nvl( upper(''), 'NO_MASK') )||'';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please click the expand-collapse arrow above to check the recommended patches that are not applied on this instance '||mask_text(g_instance, nvl( upper(''), 'NO_MASK') )||'.';
   l_sig.solution := 'Required Patches are documented in the [11217188/KA1181] - Using Oracle Analytics Publisher with Oracle E-Business Suite Release 12.2. The patches listed in the note are also recommended to provide support for Excel 2007 (xlsx) format. <br>
<ul>
                <li>The list of recommended product patches are as of the date the script was written to show the patches not on this instance.</li>
               <li>Please review the list to schedule and apply any patches as soon as possible</li>
               <li>Refer to the MOS Note indicated for more information about each patch</li>
           </ul>
Note: BI Publisher Desktop 10.1.3.4.2 is the compatible version for EBS Embedded BI Publisher 12.2.x Applications. 
              The BI Publisher Desktop 10.1.3.4.2 for E-Business Suite 12.2.x can be downloaded via the following OTN link for BI Publisher Desktop 10.1.3.4.2 for Windows: 
              <a href="http://www.oracle.com/technetwork/middleware/bi-publisher/downloads/index.html">OTN BI Publisher Download</a><br>
[11217188/KA1181] - Using Oracle Analytics Publisher with Oracle E-Business Suite Release 12.2.<br>
[1138602/KB684460] - Overview of Available Patches for Oracle XML Publisher embedded in the Oracle E-Business Suite.<br>
[2032715/KB684257] - EBS BI Publisher (BIP) Analyzer.<br>
[1640073/KB829153] - Notes for Using Oracle Business Intelligence Publisher 10g in Oracle E-Business Suite Release 12.2.<br>
[11220163/KA1195] - Using Microsoft 365 and Microsoft Office with Oracle E-Business Suite Releases 12 and 11i.<br>
[11178562/KA1050] - Setting Up Client Tier Products for Oracle E-Business. Suite<br>
[11165167/KA1016] - Using Java Web Start with Oracle E-Business Suite.<br><br>
To get a current accurate list of recommended EBS product patches that are applied or not applied to your instance, the Oracle Best Practice is to run the Patch Wizard utility.</strong><br /> See [976188/KB739927] - Patch Wizard Utility, [10940175/KA580] FAQ, or [1077813/KB627816] Videos for more information.';
   l_sig.success_msg := 'There are no recommended patches at the time this script was written for this release on this instance '||mask_text(g_instance, nvl( upper(''), 'NO_MASK') )||'.<br>
For more information, please refer to :<br>
 [11217188/KA1181] - Using Oracle Analytics Publisher with Oracle E-Business Suite Release 12.2.  The patches listed in the note are also recommended to resolve template and report generation issues.
';
   l_sig.print_condition := 'ALWAYS';
   l_sig.fail_type := 'W';
   l_sig.print_sql_output := 'Y';
  ...
  -- Register
  l_step := '60';
  g_signatures('EBS_ATG_CT_BIP_OAP_PATCHES') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'EBS_ATG_CT_BIP_OAP_PATCHES',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
l_s_xdo_runtime_release  VARCHAR2(10);
l_pqp_status             VARCHAR2(75);
l_opp_process_timeout    number;
l_opp_response_timeout   number;
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/02/25 16:18:01 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'bip_analyzer.sql';
...
print_log('Debug: l_apps_short_version = '||l_apps_short_version);

select count(PROPERTY_NAME) into l_prop_cnt from (SELECT xcpt.property_name PROPERTY_NAME, 
...
print_log('Debug: l_prop_cnt = '||l_prop_cnt);

if (l_apps_short_version = '12.1') then
    l_s_xdo_runtime_release := 'noxdo121';
else if (l_apps_short_version = '12.2') then
	select distinct nvl(extractValue(XMLType(TEXT),'//xdo_runtime_release[@oa_var="s_xdo_runtime_release"]'),'noxdo') "XDO RUNTIME RELEASE"
	into l_s_xdo_runtime_release
...
print_log('Debug: l_s_xdo_runtime_release = '||l_s_xdo_runtime_release);

SELECT Decode(b.Status,'I','Public Sector Payroll is installed so patch 34986658 is required, ','N','Public Sector Payroll is NOT Installed so patch 34986658 can be skipped, ','S','Shared') Status
into l_pqp_status
	FROM fnd_application_vl a, fnd_product_installations b
	WHERE a.application_id = b.application_id
    and a.application_short_name = 'PQP';

print_log('Debug: l_pqp_status = '||l_pqp_status);

SELECT sys_context('USERENV','DB_NAME') "instance_name", sys_context('USERENV','SERVER_HOST') "HOST" 
INTO l_instance, l_host
from dual;

print_log('Debug: l_instance = '||l_instance);
print_log('Debug: l_host = '||l_host);

select v.PROFILE_OPTION_VALUE Value
into l_opp_process_timeout
	...
	and (z.PROFILE_OPTION_NAME = t.PROFILE_OPTION_NAME)
        ...
        and z.language = 'US'
	and z.PROFILE_OPTION_NAME = 'CONC_PP_PROCESS_TIMEOUT';

print_log('Debug: l_opp_process_timeout = '||l_opp_process_timeout);

select v.PROFILE_OPTION_VALUE Value
into l_opp_response_timeout
	from fnd_profile_options t, fnd_profile_option_values v, fnd_profile_options_tl z
	where (v.PROFILE_OPTION_ID (+) = t.PROFILE_OPTION_ID)
	and (z.PROFILE_OPTION_NAME = t.PROFILE_OPTION_NAME)
        and v.level_id = '10001'
        and z.language = 'US'
	and z.PROFILE_OPTION_NAME = 'CONC_PP_RESPONSE_TIMEOUT';

print_log('Debug: l_opp_response_timeout = '||l_opp_response_timeout);

	--Set the count of other languages installed
	select count(language_code) "LANGUAGES" into l_languages_installed 
	FROM FND_LANGUAGES WHERE INSTALLED_FLAG in ('I');

print_log('Debug: l_languages_installed = '||l_languages_installed);
...
print_log('Debug: Multiple languages l_lang_list_installed = '||l_lang_list_installed);
...
g_prop_cnt   := l_prop_cnt;
g_db_version := l_dbversion;
g_s_xdo_runtime_release := l_s_xdo_runtime_release;
g_pqp_status := l_pqp_status;
g_instance := l_instance;
g_opp_process_timeout := l_opp_process_timeout;
g_opp_response_timeout := l_opp_response_timeout;
...
print_log('l_prop_cnt = '||l_prop_cnt);  
print_log('g_prop_cnt = '||g_prop_cnt);
print_log('l_dbversion	= '||l_dbversion);
print_log('g_db_version	= '||g_db_version);
print_log('g_s_xdo_runtime_release = '||g_s_xdo_runtime_release);
print_log('g_opp_process_timeout = '||g_opp_process_timeout);  
print_log('g_opp_response_timeout = '||g_opp_response_timeout);  
...
debug('begin add_signature: BIPXDO_INVALIDS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '422',
      p_sig_id                 => 'BIPXDO_INVALIDS',
      p_sig_sql                => 'select OWNER, OBJECT_NAME, OBJECT_TYPE, STATUS , LAST_DDL_TIME
from dba_objects 
where 
status = ''INVALID''
and object_name like ''XDO%''',
      p_title                  => 'XDO Invalid Objects',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Invalid XDO objects exist in this this environment',
      p_solution               => '<ul>
...
The following note may be useful in troubleshooting lingering issues:<br> 
 [1325394/KB401905#aref_section21] - Troubleshooting Guide - Invalid Objects in the E-Business Suite Environment 11i and 12.',
      p_success_msg            => 'There are not any invalid XDO objects presently in this environment',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_INVALIDS');



debug('begin add_signature: BIPXDO_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '424',
      p_sig_id                 => 'BIPXDO_SUMMARY',
      p_sig_sql                => 'Select nvl(dummy,''High level setup requirements for BIP'') BIP_Setup
From dual 
where dummy != ''X''',
      p_title                  => 'BI Publisher in EBS Troubleshooting',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'BI Publisher (formerly XML Publisher) setups in E-Business Suite that can impact file creation and formatting.',
      p_solution               => '<p>The following are common BI Publisher setups within the E-Business Suite that will impact EBS Printing and Reporting</p>
<ul>
<li>BIP related patchset and one-off patches</li>
...
<p>Here is an example of BI Publisher Exception Troublshooting:</p>
<p style="padding-left: 30px;">XDOException is designed as the superclass of all exceptions occurred inside XML Publisher. XDOException also has a capability of containing lower level Throwable to let the application know the reason of exception.</p>
<p><em><strong>Action: </strong></em>To begin troubleshooting find any errors or exceptions that occurs due to BIP it is required to view the following log for the errors and/or exceptions in particular look for the string <em>oracle.apps.xdo.XDOException </em>and the other strong mentioned below:</p>
<p><strong>OACORE</strong></p>
<p style="padding-left: 30px;">$IAS_ORACLE_HOME/j2ee/oacore/logs</p>
<p><strong>CP/OPP</strong></p>
<p style="padding-left: 30px;">1. All debug files generated in LogDir<br />2. OPP log file<br />SELECT fcpp.concurrent_request_id req_id, fcp.node_name, fcp.logfile_name<br /> FROM fnd_conc_pp_actions fcpp, fnd_concurrent_processes fcp<br /> WHERE fcpp.processor_id = fcp.concurrent_process_id<br /> AND fcpp.action_type = 6<br /> AND fcpp.concurrent_request_id = 1234567</p>
<p style="padding-left: 30px;">3. $APPLCSF/$APPLLOG/*123456* of (4) &amp; (5)<br />4. $APPLCSF/$APPLOUT/</p>
<p><strong>ADOP logs</strong></p>
<p style="padding-left: 30px;">ADOP (R12.2)<br />s_ne_base/EBSapps/log/adop/&lt;adop_session_id&gt;/&lt;phase&gt;_&lt;date&gt;_&lt;time&gt;/&lt;context_name&gt;/log</p>
<p>For cases where&nbsp;XDO is invoked via its Java API&nbsp;oracle.apps.xdo.XDOException errors can also be found via&nbsp;FND logging and log messages</p>
<p><em><strong>Example Errors in the Logfiles:</strong></em></p>
<p style="padding-left: 30px;"><em>oracle.apps.xdo.XDOException</em>: Error creating lock file</p>
<p style="padding-left: 60px;">2/10/15 4:28:52 PM Output file was found but is zero sized - Deleted</p>
<p style="padding-left: 60px;">2/10/14 4:28:52 PM UNEXPECTED oracle.apps.xdo.XDOException: Error creating lock file</p>
<p style="padding-left: 30px;"><em>Timeout issue</em>:</p>
<p style="padding-left: 60px;">java.sql.SQLException: Closed Connection at oracle.jdbc.driver.DatabaseError.throwSqlException ...</p>
<p style="padding-left: 30px;">An example error due to template being old and the new version has more fields</p>
<p style="padding-left: 60px;">SQLException java.sql.SQLException: ORA-01722: invalid number</p>
<p style="padding-left: 30px;">Out of Memory Exception</p>
<p style="padding-left: 60px;">oracle.apps.xdo.oa.schema.server.TemplateHelper.processTemplate(TemplateHelper.java:1278)</p>
<p style="padding-left: 60px;">at oracle.apps.xdo.oa.cp.JCP4XMLPublisher.runProgram(JCP4XMLPublisher.java:807)</p>
<p style="padding-left: 60px;">at oracle.apps.fnd.cp.request.Run.main(Run.java:148)</p>
<p style="padding-left: 60px;">Caused by: java.lang.OutOfMemoryError</p>
<p>For Troubleshooting BIP in EBS issues review [364547/KB697288]&nbsp;</p>',
      p_success_msg            => '<p>The following are common BI Publisher setups within the E-Business Suite that will impact EBS Printing and Reporting</p>
<ul>
<li>BIP related patchset and one-off patches</li>
<li>Memory setting for Output Post Processor concurrent manager that handles BI requests</li>
<li>Memory setting at concurrent program definition level (in options field)</li>
...
<p>For Troubleshooting BIP in EBS issues review [364547/KB697288]&nbsp;</p>',
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_SUMMARY');



debug('begin add_signature: BIPXDO_RTF');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '425',
      p_sig_id                 => 'BIPXDO_RTF',
      p_sig_sql                => 'Select nvl(dummy,''RTF Template Troubleshooting'') RTF_troubleshoot From dual Where dummy != ''X''',
      p_title                  => 'RTF Template Troubleshooting',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'RTF Template Troubleshooting',
      p_solution               => '<p>In using and designing BI Publisher templates within EBS there are several template items that&nbsp;can impact EBS Printing and Reporting, those can be <strong>performance</strong> related, <strong>design</strong> related, or <strong>data</strong> related. The following actions are some best practices in mitigation and troubleshooting.</p>
<p>As a best practice to improve report&nbsp;performance in the template design, leverage back-end database system resources whenever possible specifically for the following areas:</p>
<ul>
<li>Joining data&nbsp;Using Filter, grouping data</li>
<li>Perform expensive calculations &amp; data transformations</li>
<li>Sort data</li>
</ul>
<p><strong>Troubleshooting RTF Layouts</strong></p>
<p>In troubleshooting the reporting it is important to note the environment:</p>
<p> Make sure the certified BIP Desktop and MS Word is installed by referring to Installation and configuration of Oracle BI Publisher Desktop with Oracle E-Business Suite Release 12.1 and 12.2 [11226170/KA1227].</p>
<p>Do you have a valid layout, sample data, and are able to produce a PDF output?</p>
<p style="padding-left: 30px;">Other examples:</p>
<ul>
<li>Is the issue reproducible using latest Desktop version for EBS (5.6.3/10.1.3.4.2)? Use this Analyzer to help gather some EBS environment information</li>
<li>In troubleshooting the templates it is advisable to use color to determine values mapped to form fields, or to diagnose a column in a table. As an example use (printf() debugging).</li>
<li>Has the template been validated to ensure that it is free from errors in page breaks, headers, page numbers, etc?</li>
<li>Check field browser for program flow Is the issue clear, can a test case be produced that demonstrates the reproducibility of the issue?</li>
<li>If the XML Publisher Template Re-Generator concurrent program was recently run look at the logfile to determine if there were any failures using [1941138/KB437721]</li>
</ul>
<p>Generate test case. They are beneficial for many reasons:</p>
<ul>
<li>Test Cases can help with regressions as well as lifecycle management when moving reports from Dev to Prod</li>
<li>A simple test case can prevent an issue from becoming to embedded in the layout and help provide a clear idea of what needs to happen</li>
<li>Sometimes when an error is just &ldquo;too odd&rdquo; and cannot be replicated in a new layout, which can be due to a corrupt RTF (ex: hidden characters in Word document)</li>
<li>Create a new RTF cut and paste sections in testing at each step</li>
</ul>
<p>Other areas to look into:</p>
<ul>
<li>Is the subject or topic addressed in the manual?</li>
<li>Does the simple test case work?</li>
<li>Is there any information reported in the Oracle Knowledge base?</li>
</ul>
<p>Other Avenues for help:</p>
<ul>
<li>Check the <a href="https://blogs.oracle.com/xmlpublisher/">blog</a> or <a href="https://community.oracle.com/community/support/oracle_e-business_suite/bi_publisher">community</a>/forums, the issue may already be discussed and solved there</li>
<li>File an SR with Oracle Support using the test case and other information</li>
</ul>
<footer></footer>
<p>Contact information: <a href="support.oracle.com"> Oracle Support</a>.</p>',
      p_success_msg            => '<p>In using and designing BI Publisher templates within EBS there are several template items that&nbsp;can impact EBS Printing and Reporting, those can be <strong>performance</strong> related, <strong>design</strong> related, or <strong>data</strong> related. The following actions are some best practices in mitigation and troubleshooting.</p>
<p>As a best practice to improve report&nbsp;performance in the template design, leverage back-end database system resources whenever possible specifically for the following areas:</p>
<ul>
<li>Joining data&nbsp;Using Filter, grouping data</li>
<li>Perform expensive calculations &amp; data transformations</li>
...
<p>Contact information: <a href="support.oracle.com"> Oracle Support</a>.</p>
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_RTF');



debug('begin add_signature: BIPXDO_CHK_OPP_CONFIG');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5063',
      p_sig_id                 => 'BIPXDO_CHK_OPP_CONFIG',
      p_sig_sql                => 'SELECT  v.concurrent_queue_name,v.concurrent_queue_id,
        v.user_concurrent_queue_name,v.max_processes, v.running_processes,
        v.control_code,v.target_node,
        TO_CHAR(v.last_update_date, ''DD-MON-RR HH24:MI:SS'') "Last_Update_Date",
        v.node_name,v.node_name2, s.service_parameters, s.sleep_seconds,
        c.developer_parameters
      ...
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The output provided is for review and confirmation by your teams, and serves as a baseline regarding your current OPP configuration. ',
      p_solution               => '<p><span style="color: #ff0000;"><strong>OPP is NOT configured</strong></span>: 
See [465036/KB773327] - Concurrent Processing - Unable to Find the Concurrent Request Output Post Processor (OPP) Service</p>
<p>The output provided is for review and confirmation by your teams, and serves as a baseline regarding your current OPP configuration. You can cross reference the collected information with existing notes on OPP best practices : 
[1399454/KB627107] - Tuning Output Post Processor (OPP) to Improve Performance</p>
[1057802/KB263169] - Concurrent Processing - Best Practices for Performance for Concurrent Managers in E-Business Suite',
      p_success_msg            => '<p>OPP is currently configured, identifying the: Service ID, Service Handle, and Parameters used.</p>
<p>The output provided is for review and confirmation by your teams, and serves as a baseline regarding your current OPP configuration. You can cross reference the collected information with existing notes on OPP best practices : [1399454/KB627107] - Tuning Output Post Processor (OPP) to Improve Performance [1057802/KB263169] - Concurrent Processing - Best Practices for Performance for Concurrent Managers in E-Business Suite</p>',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_CHK_OPP_CONFIG');



debug('begin add_signature: BIPXDO_CP_HEAP_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '442',
      p_sig_id                 => 'BIPXDO_CP_HEAP_CHECK',
      p_sig_sql                => 'select a.USER_CONCURRENT_PROGRAM_NAME, b.EXECUTION_FILE_PATH, a.CONCURRENT_PROGRAM_NAME, nvl(a.EXECUTION_OPTIONS,''Not Set'') Execution_Options, fcr.ACTUAL_START_DATE, fcr.ACTUAL_COMPLETION_DATE
      FROM FND_CONCURRENT_PROGRAMS_VL a, FND_CONCURRENT_REQUESTS fcr,
           FND_EXECUTABLES b
     WHERE a.concurrent_program_id = fcr.concurrent_program_id
     and a.executable_id = b.executable_id     
     and b.EXECUTION_FILE_PATH = ''oracle.apps.xdo.oa.cp''
     AND ( a.EXECUTION_OPTIONS is NULL OR instr(a.EXECUTION_OPTIONS,''-Xmx512M'') < 2048) 
     and fcr.ACTUAL_COMPLETION_DATE > sysdate-1095',
      p_title                  => 'BIP/Java Based Concurrent Programs Maximum Heap is Unset or Too Low',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The maximum heap for BIP/Java based concurrent programs are either unset or too low.',
      p_solution               => 'This section covers the Java Heap space for the concurrent programs that use XML as an output.<br>
If the programs are run frequently and are set too low or unset it "can" result in "java.lang.OutOfMemoryError: Java heap space" errors.
<ul>
<li>The programs listed here have been run within the past 3 years and are either unset or set below 2048M</li>
<li>A warning is to indicate the above criteria was met. Although it is not urgent it is beneficial to review the programs and the memory setting while taking the recommendations below into consideration:
<ul>
<li>For normal processing 1024MB is recommended</li>
<li>For large volume processing 2048MB is recommended</li>
</ul>
</li>
<li>Please follow instructions on [315477/KB187330] or [1630732/KB625904] to update the heap space to any of the values recommended above</li>
<li>This heap is used by concurrent program java executable when generating the data/xml file</li>
<li>As an example on troubleshooting BIP integration see BIP/IBY [1410160/KB789809]</li>
</ul>',
      p_success_msg            => 'These programs run frequently within the 3 years period meet the 1024MB recommendation.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_CP_HEAP_CHECK');



debug('begin add_signature: BIPXDO_JAVA_HEAP_CHILD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5030',
      p_sig_id                 => 'BIPXDO_JAVA_HEAP_CHILD',
      p_sig_sql                => 'select developer_parameters from fnd_cp_services WHERE service_id = (SELECT manager_type FROM fnd_concurrent_queues WHERE concurrent_queue_name = ''FNDCPOPP'')',
      p_title                  => 'BIP OPP Java Heap Check child',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'When setting the BIP for EBS Java Heap the memory setting of less than or equal to 512MB is not considered optimal for most customers. ',
      p_solution               => 'When setting the BIP for EBS Java Heap the memory setting of less than or equal to 512MB is not considered optimal for most customers.',
      p_success_msg            => 'When setting the BIP for EBS Java Heap the memory setting of less than or equal to 512MB is not considered optimal for most customers.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_JAVA_HEAP_CHILD');



debug('begin add_signature: BIPXDO_OPP_HEAP_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '441',
      p_sig_id                 => 'BIPXDO_OPP_HEAP_CHECK',
      p_sig_sql                => 'select SERVICE_HANDLE, DEVELOPER_PARAMETERS,
                  substr(developer_parameters,   
                  instr(developer_parameters,''-mx'') + 3, 
     ...
     select ''Parameter not set'' SERVICE_HANDLE, ''Heap memory not set'' DEVELOPER_PARAMETERS,
      ...
      where not exists (select 1 from FND_CP_SERVICES 
                         WHERE SERVICE_HANDLE  LIKE ''%OPP%'')',
      p_title                  => 'BIP OPP Java Heap Check',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The Java Heap space for the OPP Concurrent Manager is lower than the recommended value or it is unset which can cause error "java.lang.OutOfMemoryError: Java heap space" in processing.

The java heap is the memory "container" of you runtime Java program which provides to your Java program the proper memory spaces it needs.',
      p_solution               => '<ul>
     <li>For normal processing 1024MB of heap is recommended</li>
     <li>For large volume processing 2048MB of heap is recommended</li>
     <li>Please follow instructions on [1268217/KB792333] to update the heap space to any of the values recommended above</li>
     <li>Ensure that concurrent managers are bounced/restarted using adcmcctl.sh after increasing the heap space</li>
     <li>For additional information on troubleshooting BIP integration with IBY see [1410160/KB789809]</li>
     <li> [1630732/KB625904] is recommended for BIP within EBS sizing </li>
     </ul>',
      p_success_msg            => 'No rows were selected, which indicates that OPP Java heap is at least set to the recommended value of 1024MB for normal OPP processing.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('BIPXDO_JAVA_HEAP_CHILD'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '9'
      );
   l_info.delete;
debug('end add_signature: BIPXDO_OPP_HEAP_CHECK');



debug('begin add_signature: BIPXDO_OPP_PROC-THRD_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '443',
      p_sig_id                 => 'BIPXDO_OPP_PROC-THRD_CHECK',
      p_sig_sql                => 'SELECT  SERVICE_PARAMETERS,
	MIN_PROCESSES, 
	MAX_PROCESSES,  
	SLEEP_SECONDS, 
...
Errors that may be encountered include timeout error waiting for OPP and OPP Fails Due To java.lang.ThreadDeath',
      p_solution               => '<p>BIP in EBS is integrated within Concurrent Processing by means of a specialized concurrent manager called the Output Post Processor (OPP).</p>
<p>If a request is submitted which has an XML Publisher template specified as a layout for the output, after completion the concurrent program&nbsp;will contact the OPP to apply the XML Publisher template and create the final output.</p>
<ul>
<li>The default value for <strong>number of processes</strong> is <strong>1</strong></li>
</ul>
<p style="margin-left: 80px;">There should always be at least one OPP process active in the system. Even when using RAC or PCP a single OPP process across the whole environment is sufficient. If no OPP service is available to process concurrent requests, completed requests that require OPP post-processing will complete with a status of <em>WARNING</em>, once it reached the value specified for the profile option <strong>Concurrent:OPP Response Timeout</strong>.</p>
<p style="margin-left: 80px;">One service instance of the OPP service is seeded by default. This seeded OPP service instance has one workshift with one process.</p>
<ul style="line-height: 20.7999992370605px;">
<li>The default value for <strong>number of&nbsp;thread</strong> is&nbsp;<strong>5</strong></li>
</ul>
<p style="margin-left: 80px;">The OPP Service is multi-threaded and will start a new thread for each concurrent request it processes. You can control the number of simultaneous threads for an OPP Service Instance by adjusting the Threads per Process parameter for the instance. <br /> <br /> If all the OPP process has reached their respective maximum number of threads, the requests waiting to be processed remain in a queue to be processed as soon as threads become available. If request throughput has become slow, you may want to increase the number of Threads per Process for the OPP.</p>
<p>The number of concurrent requests that the Output Post Processor can handle in parallel depends upon:</p>
<ul>
<li>the number of Processes and&nbsp;the number of Threads Per Process</li>
<li>If the default values are 2 Processes and 5 Threads per Process it means about 10 reports can be processed in parallel</li>
</ul>
<p>Generation of a single report is single threaded, so more threads and more processes will not help to improve the generation of large reports.<br />Increasing the number of processes will increase the maximum memory usage. The specified heap value (e.g. 2048MB) is the maximum value PER process (3 processes = combined max of 6GB).<br /><strong>Best practice</strong>: start by increasing the heap size, having less OPP processes but more heap space available.</p>
<p><br /> Once the Output Post Processor picks up the request, the BI Publisher engine is invoked to generate the final output file. The time that this takes will depends on various elements such as:</p>
<ul>
<li style="margin-left: 80px;">size of the XML Data File</li>
<li style="margin-left: 80px;">complexity of the template</li>
<li style="margin-left: 80px;">performance of the server</li>
<li>Errors that may be encountered include
<ul>
<li>timeout error waiting for OPP which may be resolved by&nbsp;adding more processes</li>
<li>Output Post Processing Fails Due To java.lang.ThreadDeath, may be solved by increasing the value of the <strong>Concurrent:OPP Timeout profile option</strong> to 10800 seconds&nbsp;and/or enabling the scalability feature of XML Publisher</li>
</ul>
</li>
<li>If any changes were required or made [1630732/KB625904] to increase processes and/or threads restart OPP via [1637492/KB625582]</li>
<li>For troubleshooting OPP please see [1268217/KB792333] and [364547/KB697288]</li>
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_OPP_PROC-THRD_CHECK');



debug('begin add_signature: EBS_ATG_CT_BIPXDO_OPP_PROCESS_TIMEOUT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '27303',
      p_sig_id                 => 'EBS_ATG_CT_BIPXDO_OPP_PROCESS_TIMEOUT',
      p_sig_sql                => 'select
    ...
    from
      ...
      , FND_APPLICATION h
    where
a.profile_option_name = ''CONC_PP_PROCESS_TIMEOUT''
      ...
      ,''10004'',nvl(e.user_name,to_char(c.level_value))
      ,''Unknown'')',
      p_title                  => 'Output Post Processor (OPP) Process Timeout Likely Too Low',
      p_fail_condition         => '['||mask_text(g_opp_process_timeout, nvl( upper(''), 'NO_MASK') )||'] < [300]',
      p_problem_descr          => 'The Process Timeout that the OPP uses when waiting for external requests is likely too low',
      p_solution               => 'The Site Level Process Timeout profile for OPP is set to '||mask_text(g_opp_process_timeout, nvl( upper(''), 'NO_MASK') )||' seconds.
<ul>
If your request still completes in warning status, the next step is to increase the number of processes and threads available for OPP.
<li><strong>Concurrent:OPP Process Timeout</strong> - specifies the amount of time the manager waits for the OPP to actually process the request</li>
...
For more information see [352518/KB717296] - Concurrent Requests Fail Due To Output Post Processing (OPP) Timeout.',
      p_success_msg            => 'The Process Timeout that the OPP uses when waiting for external requests is set to '||mask_text(g_opp_process_timeout, nvl( upper(''), 'NO_MASK') )||' seconds at Site level which is fine.
<br><br>
Please review additional level settings in the above output.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_ATG_CT_BIPXDO_OPP_PROCESS_TIMEOUT');



debug('begin add_signature: EBS_ATG_CT_BIPXDO_OPP_RESPONSE_TIMEOUT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '27304',
      p_sig_id                 => 'EBS_ATG_CT_BIPXDO_OPP_RESPONSE_TIMEOUT',
      p_sig_sql                => 'select
      b.user_profile_option_name profile_name
      , a.profile_option_name profile_short_name
      , decode(to_char(c.level_id),''10001'',''Site''
    ...
    from
      ...
      , FND_APPLICATION h
    where
a.profile_option_name = ''CONC_PP_RESPONSE_TIMEOUT''
      and a.profile_option_name = b.profile_option_name
      and a.profile_option_id = c.profile_option_id
      and a.application_id = c.application_id
      and c.last_updated_by = d.user_id (+)
      ...
      ,''10004'',nvl(e.user_name,to_char(c.level_value))
      ,''Unknown'')',
      p_title                  => 'Output Post Processor (OPP) Response Timeout Likely Too Low',
      p_fail_condition         => '['||mask_text(g_opp_response_timeout, nvl( upper(''), 'NO_MASK') )||'] < [240]',
      p_problem_descr          => 'The Response Timeout that the OPP uses when waiting for external requests is likely too low',
      p_solution               => 'The Site Level Response Timeout profile for OPP is set to '||mask_text(g_opp_response_timeout, nvl( upper(''), 'NO_MASK') )||' seconds.
<ul>
<li><strong>Concurrent:OPP Response Timeout</strong> - specifies the amount of time a manager waits for OPP to respond to its request for post processing
<ul>
...
<li>Default setting for response timeout is 120 seconds. Try increasing to 240 seconds.</li>
</ul>

For more information see [862644/FAQ3670] - XML Publisher Report Issues, Recommendations and Errors.',
      p_success_msg            => 'The Response Timeout that the OPP uses when waiting for external requests is set to '||mask_text(g_opp_response_timeout, nvl( upper(''), 'NO_MASK') )||' seconds at Site level which is fine.
<br><br>
Please review additional level settings in the above output.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_ATG_CT_BIPXDO_OPP_RESPONSE_TIMEOUT');
...
debug('begin add_signature: BIPXDO_TEMPLATE_VERSIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '447',
      p_sig_id                 => 'BIPXDO_TEMPLATE_VERSIONS',
      p_sig_sql                => 'select template_code, template_type_code, TEMPLATE_NAME, DESCRIPTION, TEMPLATE_STATUS, OBJECT_VERSION_NUMBER from XDO_TEMPLATES_VL
    where LAST_UPDATE_DATE > sysdate-180',
      p_title                  => 'BI Publisher Template Versions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Errors and Issues could result from updated templates',
      p_solution               => 'The listed templates were updated within the past 3 months
Search specifically for the given Template Code or the problematic template to ensure it was regenerated. 
     <ul>
        <li>If issues are encountered with the templates listed or other templates, check the logfiles of the program that was run and re-generate the template using [1941138/KB437721][1567558/KB595550] </li>
        <p>There are some known issues while defining data definition or errors generated from the concurrent program, the following are some examples:</p>
            <p style="padding-left: 30px;">Error: The uploaded file is invalid. the file should be in xml-data-template format</p>
            <p style="padding-left: 30px;">Solution : Double check your data definition xml file, format must be correct.</p>
            <p style="padding-left: 30px;">Example Error in the Log file of concurrent request</p>
            <p style="padding-left: 30px;">Calling XDO Data Engine...
                                                java.lang.NullPointerException
                                                    at oracle.apps.xdo.oa.util.DataTemplate.getDataTemplate(DataTemplate.java:379)</p>
            <p style="padding-left: 30px;">Solution: Ensure that the DataTemplate code is in Uppercase and same as Concurrent program Short Code.</p>
     <ul>',
      p_success_msg            => 'There are no templates in this environment that have been updated in the past 3 months',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_TEMPLATE_VERSIONS');



debug('begin add_signature: BIPXDO_TRIGGERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '448',
      p_sig_id                 => 'BIPXDO_TRIGGERS',
      p_sig_sql                => 'SELECT trigger_name, table_name, OWNER, status FROM dba_triggers where table_name like ''XDO%'' and status = ''ENABLED''',
      p_title                  => 'BIP Triggers',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The XML Publisher (XDO) Triggers are missing or not enabled in this environment',
      p_solution               => 'The XML Publisher (XDO) Triggers are missing or not enabled. Please follow [1570873/KB391986] to correct this issue

Note: On R12.2 there are 22 triggers expected. 

There have been some reported problems due to missing triggers, which were resolved via [1570873/KB391986]
R12 - XML Publisher Data Definition JBO-26041: Failed to post data to database during "Insert": SQL Statement "INSERT INTO XDO_DS_DEFINITIONS_VL.',
      p_success_msg            => 'If more than 10 rows are selected the XML Publisher (XDO) triggers are installed and enabled.

Note: On version R12.2, 22 triggers are expected.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_TRIGGERS');



debug('begin add_signature: BIPXDO_DFLT_TEMPLATE_OUTPUT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5028',
      p_sig_id                 => 'BIPXDO_DFLT_TEMPLATE_OUTPUT',
...
like upper(''%default%%template%%output%''))
...
order by short_name, user_profile_option_name, level_id, level_set',
      p_title                  => 'Default Template Output Type',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'This query selects the profile option "FND: Default Template Output Type" It will display the results of the profile option. 

Note the default output type is RTF',
      p_solution               => 'This query selects the profile option "FND: Default Template Output Type" It will display the results of the profile option. 

Note the default output type is RTF',
      p_success_msg            => 'This query selects the profile option "FND: Default Template Output Type" It will display the results of the profile option. 
...
Note the default output type is RTF',
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_DFLT_TEMPLATE_OUTPUT');



debug('begin add_signature: BIPXDO_SCALABLE_FLAG');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5061',
      p_sig_id                 => 'BIPXDO_SCALABLE_FLAG',
      p_sig_sql                => 'SELECT p.USER_CONCURRENT_PROGRAM_NAME "NAME",
...
--and (f.DESCRIPTIVE_FLEXFIELD_NAME like ''$SRS$.FNDWF%'')
AND p.LANGUAGE = ''US''
and f.END_USER_COLUMN_NAME = ''P_SCALABLE_FLAG''
order by f.DESCRIPTIVE_FLEXFIELD_NAME, f.COLUMN_SEQ_NUM',
      p_title                  => 'ScalableFlag Parameter Option',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'ScalableFlag Parameter Option: Set the Scalable Option to on for issues of high data volume causing memory exceptions.
',
      p_solution               => '<p>No rows were returned where the ScalableFlag Options were set</p>
<p>See the [1410160/KB789809] to Set the Scalable Option to on for issues of high data volume causing memory exceptions.</p>
<p>As an Example: This prevents this error in the Subledger Accounting program''s log: Calling XDO Data Engine... <span style="color: #ff0000;"><em>****Warning!!! Due to high volume of data, got out of memory exception...*** ****Please retry with scalable option or modify the Data template to run in scalable mode...*** </em></span></p>
<p><strong>Action:</strong></p>
<p>1. As System Administrator: Navigate to Concurrent-&gt;Program-&gt;Define.</p>
<p>2. Query up the report: Account Analysis Report (do this for both the General Ledger and the Subledger Accounting program definitions).</p>
<p>3. Add a parameter: ScalableFlag. Value set should be yes_no Default value should be Y.</p>
<p>Select checkboxes Enable and Required.</p>
<p>Do not select the check box Displayed, or users could turn this off at runtime.</p>
<p>Token needs to be ScalableFlag (this is a case sensitive value).</p>',
      p_success_msg            => '<p>One or more rows were returned where the ScalableFlag Options is set</p> 
<p>See the [1410160/KB789809] to Set the Scalable Option to on for issues of high data volume causing memory exceptions.</p>
<p>As an Example: This prevents this error in the Subledger Accounting program''s log: Calling XDO Data Engine... <span style="color: #ff0000;"><em>****Warning!!! Due to high volume of data, got out of memory exception...*** ****Please retry with scalable option or modify the Data template to run in scalable mode...*** </em></span></p>
<p><strong>Action:</strong></p>
<p>1. As System Administrator: Navigate to Concurrent-&gt;Program-&gt;Define.</p>
...
<p>Token needs to be ScalableFlag (this is a case sensitive value).</p>',
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_SCALABLE_FLAG');
...
debug('begin add_signature: EBS_ATG_BIP_XDO10G_ZIP_FILE_VERSION_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25123',
      p_sig_id                 => 'EBS_ATG_BIP_XDO10G_ZIP_FILE_VERSION_CHECK',
      p_sig_sql                => 'select *  from
(select "Filename", "US Subdir", "US Version"
  from ( select af1.filename "Filename", af1.subdir "US Subdir", afv1.version "US Version",
                   rank()over(partition by af1.filename
  ...
  from ad_files af1, ad_file_versions afv1
where af1.file_id = afv1.file_id
  and af1.filename like ''xdo10g.zip''
  and af1.subdir like ''%3rdparty%'')  
where rankUS = 1
and "US Version" < ''120.0.12020000.26'')',
      p_title                  => 'BI Publisher xdo10g.zip file Version Check',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The version of xdo10g.zip  is below 120.0.12020000.26. ',
      p_solution               => 'According to Bug 34456759 - BIP 10G BUILD.145 REHOSTING ROLLUP FOR EBS 12.2.X, the xdo10g.zip file modified to turn off Javamail debug in OPP log when no xdodebug.cfg is found. <br><br>

Please update xdo10g.zip to version 120.0.12020000.26 by following the steps in [2891038/KB734369] - JavaMail Debug Messages In Output Post Processor Manager Log File.',
      p_success_msg            => 'You are currently at  xdo10g.zip version 120.0.12020000.26 or higher as expected.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_ATG_BIP_XDO10G_ZIP_FILE_VERSION_CHECK');



debug('begin add_signature: BIPXDO_OPP_RUNTIME_PROPS_BAD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7745',
      p_sig_id                 => 'BIPXDO_OPP_RUNTIME_PROPS_BAD',
   ...
   ORDER BY xcv.config_level, xcp.CATEGORY, xcp.sort_order',
      ...
      p_fail_condition         => 'RS',
      p_problem_descr          => '"java.lang.OutOfMemoryError" errors in the Output Post Processor logs due to BI Publisher runtime properties Settings not being set. Optimize these setting to prevent errors. ',
      p_solution               => '<p>BIP settings were not found</p>
<ul>
<li>Modify XML Publisher (XDO) runtime parameters to use optimization settings:</li>
<ol>
<li>As XML Publisher Administrator navigate to Administration-&gt;Configuration</li>
<li>Under Temporary Directory pick a temporary file location on your concurrent processing node. This should be at least 5GB or 20x larger than largest XML data file you generate
<ul>
<li>The Temp dir can be *any* directory and there is no need for it to be shared on multi-node environments</li>
<li><strong>Recommended: &nbsp;</strong>for I/O purposes it is not recommended to use /var/tmp or /usr/tmp or applcsf</li>
</ul>
</li>
<li>Under FO Processing, set the following:</li>
<ul>
<li>Use XML Publisher''s XSLT processor set to True</li>
<li>Enable scalable feature of XSLT processor set to False
<ul>
<li>Enable scalable feature of XSLT processor: this MUST be set to true in order to fix java.lang.OutOfMemoryError. The recommendation is to set it to false on the Administration --&gt; Configuration page, so that''s the default (processing in memory of most (smaller) reports) and then setting it to true for larger reports. This is essential.</li>
<li>Note: If Patch 7599031 is applied, Enable scalable feature of XSLT processor can be set to True at template level for large reports to enable improved heap utilization</li>
</ul>
</li>
<li>Enable XSLT runtime optimization set to True</li>
<li>Note: It is recommended to modify these settings at Template level instead of site level</li>
</ul>
</ol>
<li>For more information see [737311/KB358344] and [1410160/KB789809]</li>
<li>The Results should look like:</li>
</ul>
<blockquote>
<table>
<tbody>
...
<tr>
<td>
<p>Use XML Publisher''s XSLT processor</p>
</td>
<td>
<p>xslt-xdoparser</p>
</td>
...
<tr>
<td>
<p>Enable scalable feature of XSLT processor</p>
</td>
<td>
<p>xslt-scalable</p>
</td>
...
<tr>
<td>
<p>Enable XSLT runtime optimization</p>
</td>
<td>
<p>xslt-runtime-optimization</p>
</td>
<td>
<p>Site</p>
</td>
...
<p>true</p>
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_OPP_RUNTIME_PROPS_BAD');



debug('begin add_signature: BIPXDO_OPP_RUNTIME_PROPS_GOOD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7746',
      p_sig_id                 => 'BIPXDO_OPP_RUNTIME_PROPS_GOOD',
      p_sig_sql                => 'SELECT xcpt.property_name PROPERTY_NAME, 
         xcp.xdo_cfg_name CONFIGURATION_NAME,
         DECODE (TO_CHAR (xcv.config_level), ''10'', ''Site'',''30'', ''DataSource'',''50'', ''Template'', ''???'') "LEVEL",
         DECODE (TO_CHAR (xcv.config_level), ''10'', '''',''30'', xd.application_short_name || ''|'' || xd.data_source_code, ''50'', xt.application_short_name || ''|'' || xt.template_code,''???'') "CONTEXT_SHORT",
   ...
   ORDER BY xcv.config_level, xcp.CATEGORY, xcp.sort_order',
      p_title                  => 'BIP Runtime Properties',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no temp directory setup.',
      p_solution               => 'Please setup a temporary file location on your concurrent processing node.<br>
This should be at least 5GB or 20x larger than largest XML data file you generate as defined in [737311/KB358344] and [1410160/KB789809].',
      p_success_msg            => 'BI Publisher runtime properties Settings. Optimize these setting to prevent "java.lang.OutOfMemoryError" errors in the Output Post Processor log
<ul>
<li>Modify XML Publisher (XDO) runtime parameters to use optimization settings:</li>
<ol>
<li>As XML Publisher Administrator navigate to Administration-&gt;Configuration</li>
...
<li>For more information see [737311/KB358344] and [1410160/KB789809]</li>
   ...
   l_info.delete;
debug('end add_signature: BIPXDO_OPP_RUNTIME_PROPS_GOOD');
...
BEGIN
  ...
  analyzer_title := 'BIP for EBS Analyzer';
...
start_section('Proactive Recommendations', 'Proactive_Recommendations');
   ...
   set_item_result(check_rec_patches_1); /* Signature BIPXDO_chk_rec_patches */
   set_item_result(check_rec_patches_2); /* Signature EBS_ATG_CT_BIPXDO_REC_PATCHES */
   IF (g_rep_info('Apps Version') > '12.2') THEN
      set_item_result(run_stored_sig('BIPXDO_BIPXDOZIP_VER'));
      set_item_result(run_stored_sig('EBS_ATG_BIP_XDO10G_ZIP_FILE_VERSION_CHECK'));
   END IF;
   set_item_result(run_stored_sig('BIPXDO_INVALIDS'));
   set_item_result(run_stored_sig('BIPXDO_SUMMARY'));
   set_item_result(run_stored_sig('BIPXDO_RTF'));
end_section;
debug('end section: Proactive_Recommendations');

debug('begin section: CHECK_OAP');
IF substr(g_rep_info('Apps Version'),1,4)='12.2'
and g_s_xdo_runtime_release in ('xdo10g', 'xdo12c') THEN
   start_section('Oracle Analytics Publisher (OAP) '||g_s_xdo_runtime_release||'', 'CHECK_OAP');
      set_item_result(check_rec_patches_3); /* Signature EBS_ATG_CT_BIP_OAP_PATCHES */
   end_section;
END IF;
debug('end section: CHECK_OAP');

debug('begin section: BIPOPP');
start_section('BIP OPP Integration', 'BIPOPP');
   set_item_result(run_stored_sig('BIPXDO_CHK_OPP_CONFIG'));
   set_item_result(run_stored_sig('BIPXDO_CP_HEAP_CHECK'));
   set_item_result(run_stored_sig('BIPXDO_JAVA_HEAP_CHILD'));
   set_item_result(run_stored_sig('BIPXDO_OPP_HEAP_CHECK'));
   set_item_result(run_stored_sig('BIPXDO_OPP_PROC-THRD_CHECK'));
   set_item_result(run_stored_sig('EBS_ATG_CT_BIPXDO_OPP_PROCESS_TIMEOUT'));
   set_item_result(run_stored_sig('EBS_ATG_CT_BIPXDO_OPP_RESPONSE_TIMEOUT'));
   set_item_result(run_stored_sig('CP1_PROFILES'));
end_section;
debug('end section: BIPOPP');

debug('begin section: BI_Publisher_Integration');
start_section('BI Publisher Integration', 'BI_Publisher_Integration');
   set_item_result(run_stored_sig('BIPXDO_TEMPLATE_VERSIONS'));
   set_item_result(run_stored_sig('BIPXDO_TRIGGERS'));
   set_item_result(run_stored_sig('BIPXDO_DFLT_TEMPLATE_OUTPUT'));
   IF g_prop_cnt < 4 THEN
      set_item_result(run_stored_sig('BIPXDO_OPP_RUNTIME_PROPS_BAD'));
   END IF;
   IF g_prop_cnt > 3 THEN
      set_item_result(run_stored_sig('BIPXDO_OPP_RUNTIME_PROPS_GOOD'));
   END IF;
   set_item_result(run_stored_sig('BIPXDO_SCALABLE_FLAG'));
end_section;
debug('end section: BI_Publisher_Integration');
