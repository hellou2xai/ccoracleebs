...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Receivables Document Sequence Analyzer |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE ar_docseq_analyzer_pkg AUTHID DEFINER AS
...
(            p_user_name                    IN VARCHAR2    DEFAULT NULL
           ...
           ,p_id_categ                     IN VARCHAR2    DEFAULT NULL
           ,p_trx_id_cp                    IN VARCHAR2    DEFAULT NULL
           ,p_trx_type_cp                  IN VARCHAR2    DEFAULT NULL
           ,p_rct_id_cp                    IN VARCHAR2    DEFAULT NULL
           ,p_rct_meth_cp                  IN VARCHAR2    DEFAULT NULL
...
END ar_docseq_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY ar_docseq_analyzer_pkg AS
-- $Id: ar_docseq_analyzer.sql, 200.44 2026/01/29 15:19:19 mamoreir Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2021711.1));
...
----------------------------------------------------------------
--patch recommendation related
g_bug_number           NUMBER := NULL;

--parameter related
g_org_id               NUMBER := NULL;
g_resp_id              NUMBER := NULL;
g_customer_trx_id      NUMBER := NULL;
g_cash_receipt_id      NUMBER := NULL;
g_document_type        VARCHAR2(30) := NULL;
g_doc_string           VARCHAR2(30) := NULL;
g_cat_string           VARCHAR2(30) := NULL;
g_category_type        VARCHAR2(30) := NULL;
g_type_name            VARCHAR2(30) := NULL;

g_org_id_tmp           NUMBER := NULL;
g_resp_id_tmp          NUMBER := NULL;
g_id_categ_tmp         VARCHAR2(30) := NULL;
g_user_name_tmp            VARCHAR2(100) := 0;

p_trx_id               NUMBER  := NULL;
p_trx_type_name        VARCHAR2(30) := NULL;
p_cash_receipt_id      NUMBER  := NULL;
p_rct_method_name      VARCHAR2(30) := NULL;
p_trx_type_class       VARCHAR2(5) := NULL;


--transaction - document sequence parameters
g_cust_trx_type_id     NUMBER:= NULL;
g_doc_sequence_id      NUMBER:= NULL;
g_doc_sequence_value   NUMBER:= NULL;
g_batch_source_id      NUMBER:= NULL;
g_complete_flag        VARCHAR2(1):= NULL;
g_categ_code           VARCHAR2(30):= NULL;
g_source_name          VARCHAR2(30):= NULL;

g_use_prof_name        VARCHAR2(240);
g_prof_name            VARCHAR2(80);
g_prof_level           VARCHAR2(240);
g_prof_context         VARCHAR2(240);
g_prof_value           VARCHAR2(240);
g_prof_resp            VARCHAR2(1) := 'N';
g_prof_app             VARCHAR2(1) := 'N';
g_prof_site            VARCHAR2(1) := 'N';
g_prof_not_set         VARCHAR2(1) := 'N';


g_trx_type_name        VARCHAR2(20):= NULL;
g_assign_code_name     VARCHAR2(30):= NULL;
g_trx_type_class       VARCHAR2(5):= NULL;

g_dsamethod            VARCHAR2(1):= NULL;
g_message_string       VARCHAR2(4000):= NULL;
g_trx_date             VARCHAR2(25):= NULL;

g_sysparam_sob_id      NUMBER:= NULL;
g_assign_sob_id        NUMBER:= NULL;

g_seq_name             VARCHAR2(30):= NULL;
g_seq_type             VARCHAR2(1):= NULL;
g_dbseq_name           VARCHAR2(30):= NULL;
g_seq_table            VARCHAR2(30):= NULL;
g_doc_seq_id           NUMBER:= NULL;
       
g_def_trx_type         NUMBER:= NULL;

g_application_id       NUMBER:= NULL;
g_table_name           VARCHAR2(30):= NULL;
g_apps_version         FND_PRODUCT_GROUPS.RELEASE_NAME%TYPE;

--receipt - document sequence parameters
g_receipt_class_id            NUMBER:= NULL;
g_creation_date               DATE:= NULL;
g_creation_method_code        VARCHAR2(30):= 'XXX';
g_last_update_date            DATE:= NULL;
g_class_name                  VARCHAR2(50):= NULL;
g_creation_status             VARCHAR2(10):= NULL;
g_remit_method_code           VARCHAR2(30):= NULL;
g_receipt_method_id           NUMBER:= NULL;
g_rct_method_name             VARCHAR2(30):= NULL;

CURSOR prof_value (resp_id IN NUMBER) is    
       SELECT po.user_profile_option_name user_prof_name,
              po.profile_option_name prof_name,
              DECODE (TO_CHAR (pov.level_id), '10001', 'SITE', '10002', 'APP', '10003', 'RESP', '10004', 'USER') lev,
              pov.profile_option_value opt_val
       ...
       ORDER by pov.level_id desc;    


l_continue   VARCHAR2(1) := 'Y';

l_use_prof_name VARCHAR2(240);
l_prof_name     VARCHAR2(80);
l_lev           VARCHAR2(10);
l_opt_val       VARCHAR2(240);
l_notset        VARCHAR2(1) DEFAULT 'X';
l_got_one       VARCHAR2(1) DEFAULT 'N'; 
l_halt          VARCHAR2(1) DEFAULT 'N'; -- SS look

l_gen_lev      VARCHAR2(80);
 
l_type_upd     VARCHAR2(20):= NULL;
l_cat_date     VARCHAR2(20):= NULL;
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'ARDOCSEQ_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'ARDOCSEQ_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>ARDOCSEQ Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2021711.1#feedback");
...
-- page title
    print_out('        <div class="header_title">'||p_analyzer_title|| ' Report <br>
        ...
        <img class="header_version" src="https://www.oracle.com/webfolder/s/analyzer/ar_docseq_latest_version.gif" title="Click here to download the latest version of Analyzer" alt="Latest Version Icon"></a>');
    print_cloud_image;
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,124,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_up_lg" title="Analyzer Feedback: Thumbs Up"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,124,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_dn_lg" title="Analyzer Feedback: Thumbs Down"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},124,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="af_idea_lg" title="Analyzer Idea"></span></a>');
    print_out('        </div>
    </div>');
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/3914263" class="blacklink" target="new">
...
<ul><li>&nbsp;Changes in ATG signatures, Template changes</li>
</ul>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,124,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,124,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},124,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
FUNCTION check_rec_patches_1 RETURN VARCHAR2 IS
  /* Signature AR_DS_SEC2_PATCH_REC_ACCT-RECON */
...
BEGIN
    ...
    print_log('Processing recommended patches signature: AR_DS_SEC2_PATCH_REC_ACCT-RECON (check_rec_patches_1) "AR Recommended Patches - Accounting-Reconcilation Issues" (version 3)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_col_rows(1)(l_rows) := '22185457';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Unable to Post Receipts. The Subledger Journal Entry Does Not Balance in the Entered Currency';
   l_col_rows(5)(l_rows) := '[2102791.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21836947';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Changing Revenue Account Causes Accounting Error - The Total Debits and the Total Credits Do Not Balance for this Subledger Journal Entry';
   l_col_rows(5)(l_rows) := '[2110981.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21689422';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Journal Entries ARRGTA Not Displaying Upgraded 11i Reversed Receipts and all the SLA Data was Migrated for all Periods';
   l_col_rows(5)(l_rows) := NULL;

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21679074';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Late Charges Are Not Being Calculated Correctly if There is a Manual Adjustment Against the Invoice';
   l_col_rows(5)(l_rows) := '[2059589.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21673711';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables now recognizes revenue properly for the invoice lines that have over payment and a revenue contingency';
   l_col_rows(5)(l_rows) := '[1994699.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21672840';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Create Accounting Error: MFAR Account Balancing Rule - 95353 Subledger Accounting was unable to derive an accounting code combination using the account derivation rule Transaction Distribution GL Account owned by Oracle';
   l_col_rows(5)(l_rows) := '[2046467.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21512459';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables now prevents duplicate accounting of the receipt reversal event if the receipt was applied to a transaction with an application general ledger date greater than the receipt reversal general ledger date.';
   l_col_rows(5)(l_rows) := '[1558241.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21461852';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Performance improvement for Accounting Program';
   l_col_rows(5)(l_rows) := '[2061515.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21365773';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Change Customer On Receipt Only When All Previous Accounting Entries Are Been Posted to GL';
   l_col_rows(5)(l_rows) := '[2054953.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21287731';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'AR To GL Reconciliation Report Format Issue and Period Totals Do Not Match in 12.2.4';
   l_col_rows(5)(l_rows) := '[2030037.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21263871';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables Reports: UNAPPLIED RECEIPTS JOURNAL DISPLAYS INCORRECT DATA FOR REVERSED RECEIPTS';
   l_col_rows(5)(l_rows) := '[2046545.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21212398';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receipts Workbench Issue: Third Party Balance Not Correct When Applying Bills Receivables (BR) to Related Customer';
   l_col_rows(5)(l_rows) := '[2024692.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18187446';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables now creates multi-fund accounting entries correctly for receipts with currency rounding';
   l_col_rows(5)(l_rows) := '[2046467.1]';
...
END IF;
   ...
   l_sig.sigrepo_id := '2450';
   l_sig.title := 'AR Recommended Patches - Accounting-Reconcilation Issues';
   ...
   l_sig.solution := '<ul><li>NOTE: Since you have not yet applied the RPC, check the list of patches above.</li> 
<li>Refer to the note indicated for more information about each patch.</li>
<li>Please consider applying these patches, or the RPC (which includes them all) soon.</li>
<li>Patch {22471504} is the latest RPC patch and is included in the RPC Feb 2016.</li>
</ul>
...
This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('AR_DS_SEC2_PATCH_REC_ACCT-RECON') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'AR_DS_SEC2_PATCH_REC_ACCT-RECON',     -- sig ID
...
FUNCTION check_rec_patches_2 RETURN VARCHAR2 IS
  /* Signature AR_DS_SEC2_PATCH_REC_FUNDS_CAPTURE */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: AR_DS_SEC2_PATCH_REC_FUNDS_CAPTURE (check_rec_patches_2) "AR Recommended Patches - Funds Capture Issues" (version 3)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21643531';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'SEPA Mandate Not Defaulted in Transactions Form Based on Legal Entity';
   l_col_rows(5)(l_rows) := '[2068375.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18307364';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Submit Offline Transactions program now correctly updates the receipt status from Remitted to Confirmed when the transaction fails or gets rejected.';
   l_col_rows(5)(l_rows) := NULL;
...
END IF;
   ...
   l_sig.sigrepo_id := '2452';
   l_sig.title := 'AR Recommended Patches - Funds Capture Issues';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please check if any of the recommended patches were not applied in this instance';
   l_sig.solution := '<ul><li>NOTE: Since you have not yet applied the RPC, check the list of patches above.</li> 
<li>Refer to the note indicated for more information about each patch.</li>
...
This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('AR_DS_SEC2_PATCH_REC_FUNDS_CAPTURE') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'AR_DS_SEC2_PATCH_REC_FUNDS_CAPTURE',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
FUNCTION check_rec_patches_3 RETURN VARCHAR2 IS
  /* Signature AR_DS_SEC2_PATCH_REC_NO_RPC */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: AR_DS_SEC2_PATCH_REC_NO_RPC (check_rec_patches_3) "AR Recommended Patches - RPC Patches" (version 3)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'RECEIVABLES RECOMMENDED PATCH COLLECTION (RPC), FEB 2016 ';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'R12.1: RECEIVABLES RECOMMENDED PATCH COLLECTION (RPC), AUG 2015';
   ...
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'AR:R12.1:Receivables Recommended Patch Collection (RPC), Feb 2015';
...
END IF;
   ...
   l_sig.sigrepo_id := '2449';
   l_sig.title := 'AR Recommended Patches - RPC Patches';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please check if any of the recommended patches were not applied in this instance';
   l_sig.solution := '<ul><li>NOTE: Since you have not yet applied the RPC, check the list of patches above.</li> 
<li>Refer to the note indicated for more information about each patch.</li>
...
This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('AR_DS_SEC2_PATCH_REC_NO_RPC') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'AR_DS_SEC2_PATCH_REC_NO_RPC',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
FUNCTION check_rec_patches_4 RETURN VARCHAR2 IS
  /* Signature AR_DS_SEC2_PATCH_REC_RECEIPTS */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: AR_DS_SEC2_PATCH_REC_RECEIPTS (check_rec_patches_4) "AR Recommended Patches - Receipts Issues" (version 3)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '22304981';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Error "No Invoice Line" When Creating Refunds';
   l_col_rows(5)(l_rows) := '[2095067.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21659645';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables now allow users to upgrade the accounting entries of a receipt, for the secondary ledger, on the fly when the reversal/unapply action is performed.';
   l_col_rows(5)(l_rows) := NULL;

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21620611';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Change Customer On Receipt Only When All Previous Accounting Entries Are Been Posted to GL';
   l_col_rows(5)(l_rows) := '[2054953.1]';
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21065572';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'The Clear Record Button Clears Two (2) Records in the Remittance Workbench';
   l_col_rows(5)(l_rows) := '[2020447.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '18079691';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receipts Workbench: Mass Apply Allows Application to Be Saved Prior to Invoice GL Date';
   l_col_rows(5)(l_rows) := '[1638780.1]';

END IF;

   l_extra_info('##SHOW_SQL##'):= 'Y';

   l_sig.sigrepo_id := '2453';
   l_sig.title := 'AR Recommended Patches - Receipts Issues';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please check if any of the recommended patches were not applied in this instance';
   l_sig.solution := '<ul><li>NOTE: Since you have not yet applied the RPC, check the list of patches above.</li> 
<li>Refer to the note indicated for more information about each patch.</li>
...
This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('AR_DS_SEC2_PATCH_REC_RECEIPTS') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'AR_DS_SEC2_PATCH_REC_RECEIPTS',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
FUNCTION check_rec_patches_5 RETURN VARCHAR2 IS
  /* Signature AR_DS_SEC2_PATCH_REC_TRANSACTIONS */
  l_col_rows   COL_LIST_TBL := col_list_tbl(); -- Row values
  l_rel_rows   COL_LIST_TBL := col_list_tbl(); -- Row release
  l_hdr        VARCHAR_TBL  := varchar_tbl(); -- Column headings
  l_app_date   DATE;         -- Patch applied date
...
BEGIN
    ...
    print_log('Processing recommended patches signature: AR_DS_SEC2_PATCH_REC_TRANSACTIONS (check_rec_patches_5) "AR Recommended Patches - Transactions Issues" (version 4)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21417336';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Tax Not Calculated On Credit Memo Lines On Credits Created With ar_credit_memo_api_pub (missing tax_classification_code)';
   l_col_rows(5)(l_rows) := '[2025893.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '21313018';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Receivables now allows users to assign transactions having positive and negative line amounts (line / tax / freight) to the bill receivable successfully.';
   l_col_rows(5)(l_rows) := NULL;

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '20919438';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Application And Unapplication Of Credit Note To Invoice Changes The Tax And The Line Balances.';
   l_col_rows(5)(l_rows) := '[2045801.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '19950690';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Tax Only Credit Memo Errors - This Action Has Been Blocked to Avoid Inaccurate Accounting Entries';
   l_col_rows(5)(l_rows) := '[2029104.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '19950119';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'SLA Exchange Rate Gain & Loss are created for Credit Memo when Distribution Amount is 0.00 and Accounted Amount is not 0.00';
   l_col_rows(5)(l_rows) := '[2025196.1]';

END IF;

   l_extra_info('##SHOW_SQL##'):= 'Y';

   l_sig.sigrepo_id := '2454';
   l_sig.title := 'AR Recommended Patches - Transactions Issues';
   l_sig.fail_condition := '[Applied] = [No]';
   l_sig.problem_descr := 'Please check if any of the recommended patches were not applied in this instance';
   l_sig.solution := '<ul><li>NOTE: Since you have not yet applied the RPC, check the list of patches above.</li> 
<li>Refer to the note indicated for more information about each patch.</li>
...
This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('AR_DS_SEC2_PATCH_REC_TRANSACTIONS') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'AR_DS_SEC2_PATCH_REC_TRANSACTIONS',     -- sig ID
    l_sig,                              -- signature information
    l_col_rows,                         -- data
    l_hdr);                             -- headers
...
debug('begin populate parameters hash table');
   ...
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_document_type,'NO_MASK');
   IF NOT g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'ID or Category';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_id_categ,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Customer Trx ID';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_trx_id_cp,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Transaction Type';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_trx_type_cp,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Cash Receipt ID';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_rct_id_cp,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Receipt Method';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_rct_meth_cp,'NO_MASK');
   END IF;
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  l_revision := rtrim(replace('$Revision: 200.44  $','$',''));
  l_revision := ltrim(replace(l_revision,'Revision:',''));
  l_date_char := rtrim(replace('$Date: 2026/01/29 15:19:18 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'ar_docseq_analyzer.sql';
...
debug('begin parameter validation: p_document_type');
IF p_document_type IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Document Type is required.');
   raise invalid_parameters;
END IF;
IF p_document_type IS NOT NULL AND p_document_type NOT IN ( 'RECEIPT','TRANSACTION') THEN
   print_error('INVALID ARGUMENT: Invalid Document Type.  You may have entered the wrong type.  Please try again.');
   raise invalid_parameters;
END IF;
debug('end parameter validation: p_document_type');

debug('begin parameter validation: p_id_categ');
IF NOT g_is_concurrent THEN
IF p_id_categ IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter ID or Category is required.');
   raise invalid_parameters;
END IF;
END IF;
debug('end parameter validation: p_id_categ');
...
This analyzer first asks user to identify if they want to run for TRANSACTION or RECEIPT
If run for TRANSACTION, then we need Transaction Type or Transaction ID
If run for RECEIPT, then we need Receipt Method or Receipt ID

Note that the parameters requested are different based on how the analyzer is run:
When run as a concurrent program, we expose four fields each with LOVs: (1)Transaction Type (2)Transaction Number (3)Receipt Method (4)Receipt Number
When run as a script, we just ask for ID or Category only

When processing starts, we check the ID/Category first, If it is TRANSACTION, then validate trx type/number. If RECEIPT, then validate receipt method/number

*/

g_document_type := ltrim(rtrim(p_document_type));
g_doc_string    := initcap(g_document_type);
g_org_id_tmp    := p_org_id;
g_resp_id_tmp   := p_resp_id;
g_user_name_tmp := p_user_name;

if p_document_type = 'TRANSACTION' then
   g_cat_string := 'Transaction Type Name';
elsif p_document_type = 'RECEIPT' then
   g_cat_string := 'Receipt Method Name';
end if; 

print_log('START of Additional Validation code');
print_log('START of Verify Org ID');

IF g_org_id_tmp is null THEN
   print_log('The org_id parameter is mandatory.');
   raise invalid_parameters;
ELSE
   BEGIN
      select org_id, document_seq_gen_level, set_of_books_id
        into g_org_id, l_gen_lev, g_sysparam_sob_id
        from ar_system_parameters_all
       where org_id = g_org_id_tmp;
     
       print_log('Set policy context to ' || g_org_id_tmp);
   ...
   EXCEPTION
   WHEN NO_DATA_FOUND then
       print_log('The org_id parameter = ' || g_org_id_tmp ||' is invalid.  The process cannot continue.');
       raise invalid_parameters;        
   END;
END IF;

print_log('END of Verify Org ID');


print_log('Retrieve profile options');
begin
   SELECT 'Y'
     into g_prof_resp
     FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_value = g_resp_id_tmp;

   print_log('Retrieved profile option at RESP level');
...
when no_data_found then
   ...
   begin
      SELECT 'Y'
       ...
       AND   pov.level_value = 222;

      print_log('Retrieved profile option at APP level');
   ...
   when no_data_found then
      ...
      begin
         ...
         print_log('Retrieved profile option at SITE level');
...
print_log('End of Retrieve profile options');


print_log('START of Document Type Selection');
-- P_DOCUMENT_TYPE validated from list of values: TRANSACTION or RECEIPT

if p_document_type = 'TRANSACTION' then

   begin

      print_log('Parameters received: ' || 
                ' p_id_categ = ' || p_id_categ || 
                ' p_trx_id_cp = ' || p_trx_id_cp || 
                ' p_trx_type_cp = ' || p_trx_type_cp);

      if p_trx_id_cp is NOT null AND p_trx_type_cp is not null then
         print_log('You have incorrectly provided both a Transaction Number and a Transaction Type. The code will IGNORE Transaction Type and will process Transaction Number value instead.');
      end if;
      
      g_id_categ_tmp := nvl(p_id_categ, nvl(p_trx_id_cp, p_trx_type_cp));
      print_log('g_id_categ_tmp = ' || g_id_categ_tmp);

      -- determine what value is in g_id_categ_tmp
      -- is it a NUMBER?
      p_trx_id := to_number(g_id_categ_tmp);

      -- if we get to this line, p_trx_id is numeric, now we check if it is a CUSTOMER_TRX_ID

      begin

         select trx.customer_trx_id, trx.cust_trx_type_id, trx.doc_sequence_id, trx.doc_sequence_value,
                trx.batch_source_id, trx.complete_flag, to_char(trx.trx_date,'DD-MON-YYYY'),
                typ.name, typ.type, typ.cust_trx_type_id, bat.name
         into g_customer_trx_id, g_cust_trx_type_id, g_doc_sequence_id, g_doc_sequence_value, 
              g_batch_source_id, g_complete_flag, g_trx_date,
              g_trx_type_name, g_trx_type_class, g_cust_trx_type_id, g_source_name
         from ra_customer_trx trx , ra_cust_trx_types typ, ra_batch_sources bat
         ...
         g_categ_code := g_trx_type_name;

         print_log('Case A - customer_trx_id was provided');
         print_log('A1. g_customer_trx_id = ' || g_customer_trx_id ||' has been validated against the Transactions table.');
         print_log('A2. g_trx_type_name = ' || g_trx_type_name ||', has been validated against the Transactions Type table.');
         print_log('A3. g_trx_type_class = ' || g_trx_type_class ||', has been validated against the Transactions Type table.');
         print_log('A4. g_cust_trx_type_id = ' || g_cust_trx_type_id ||', has been validated against the Transactions Type table.');
         print_log('A5. g_categ_code = ' || g_categ_code);


      exception 
      when no_data_found then

         -- value is numeric but it's not a CUSTOMER_TRX_ID, check if it's a trx type name
         p_trx_type_name := ltrim(rtrim(g_id_categ_tmp));

         begin
            select name, type, cust_trx_type_id
            into g_trx_type_name, g_trx_type_class, g_cust_trx_type_id
            from RA_CUST_TRX_TYPES
            where upper(name) = upper(p_trx_type_name);

            g_categ_code := g_trx_type_name;
            g_doc_string := g_doc_string || ' Type';

            print_log('Case B - a *numeric* transaction type name was provided');
            print_log('B1. g_trx_type_name = ' || g_trx_type_name ||', has been validated against the Transactions Type table.');
            print_log('B2. g_trx_type_class = ' || g_trx_type_class ||', has been validated against the Transactions Type table.');
            print_log('B3. g_cust_trx_type_id = ' || g_cust_trx_type_id ||', has been validated against the Transactions Type table.');
            print_log('B4. g_categ_code = ' || g_categ_code);

         exception
         when no_data_found then
            print_log('ERROR: Value provided for g_id_categ_tmp = ' || g_id_categ_tmp||
                      ', is neither a CUSTOMER_TRX_ID or a Transaction Type.  The process cannot continue.');
            raise invalid_parameters;
         end;
   ...
   -- value provided is NON NUMERIC, check if it's a trx type name
      ...
      begin
         ...
         print_log('Case C - transaction type was provided');
         print_log('C1. g_trx_type_name = ' || g_trx_type_name ||', has been validated against the Transactions Type table.');
         print_log('C2. g_trx_type_class = ' || g_trx_type_class ||', has been validated against the Transactions Type table.');
         print_log('C3. g_cust_trx_type_id = ' || g_cust_trx_type_id ||', has been validated against the Transactions Type table.');
         print_log('C4. g_categ_code = ' || g_categ_code);
   ...
   begin
      ...
      print_log('Parameters received: ' ||
                ' p_id_categ = ' || p_id_categ || 
                ' p_rct_id_cp = ' || p_rct_id_cp || 
                ' p_rct_meth_cp = ' || p_rct_meth_cp);

      if p_rct_id_cp is NOT null AND p_rct_meth_cp is not null then
         print_log('You have incorrectly provided both a Receipt Number and a Receipt Method. The code will IGNORE Receipt Method and will process Receipt Number value instead.');
      end if;


      g_id_categ_tmp := nvl(p_id_categ, nvl(p_rct_id_cp, p_rct_meth_cp));
      print_log('g_id_categ_tmp = ' || g_id_categ_tmp);

      -- first check if it's a NUMBER
      p_cash_receipt_id := to_number(g_id_categ_tmp);

      -- if the above does not raise an exception, we get to this line and know it's numeric
      -- check if it's a CASH_RECEIPT_ID

      begin

         select cr.receipt_method_id, cr.cash_receipt_id, method.name, to_char(cr.receipt_date,'DD-MON-YYYY'), method.name, rc.creation_method_code
         into g_receipt_method_id, g_cash_receipt_id, g_rct_method_name, g_trx_date, g_rct_method_name, g_creation_method_code
         from AR_CASH_RECEIPTS cr,
              AR_RECEIPT_METHODS method,
              AR_RECEIPT_CLASSES rc
         where method.receipt_method_id = cr.receipt_method_id
         and   method.receipt_class_id = rc.receipt_class_id
         and cash_receipt_id = p_cash_receipt_id;

         g_categ_code := g_rct_method_name;

         print_log('Case D - cash_receipt_id was provided');
         print_log('D1. g_cash_receipt_id = ' || g_cash_receipt_id || ' has been validated against the Receipts table.');
         print_log('D2. g_categ_code = ' || g_categ_code);

      exception 
      when no_data_found then

         -- value is numeric but it's not a CASH_RECEIPT_ID, check if it's a receipt method name
         p_rct_method_name := ltrim(rtrim(g_id_categ_tmp));

         begin
            select name
            into g_rct_method_name
            from AR_RECEIPT_METHODS
            where upper(name) = upper(g_rct_method_name)
            and receipt_method_id = g_receipt_method_id;

            g_categ_code := g_rct_method_name;
            g_doc_string := g_doc_string || ' Method';

            print_log('Case E - *numeric* receipt method name was provided');      
            print_log('E1. g_rct_method_name = ' || g_rct_method_name || ' has been validated against the Receipt Methods table.');
            print_log('E2. g_categ_code = ' || g_categ_code);

         exception
         when no_data_found then
    
            print_log('ERROR:  The value provided for g_id_categ_tmp = ' || g_id_categ_tmp||', is neither a CASH_RECEIPT_ID or a Receipt Method.  The process cannot continue.');
            raise invalid_parameters;
         end;
      end;
   ...
   when value_error then
      ...
      begin
         ...
         into g_rct_method_name
         from AR_RECEIPT_METHODS
         where upper(name) = upper(g_id_categ_tmp);

         g_categ_code := g_rct_method_name;
         g_doc_string := g_doc_string || ' Method';

         print_log('Case F - receipt method name was provided');       
         print_log('F1. g_rct_method_name = ' || g_rct_method_name || ' has been validated against the Receipt Methods table.');
         print_log('F2. g_categ_code = ' || g_categ_code);

      exception
      when no_data_found then
         print_log('ERROR:  The g_id_categ_tmp ' || g_id_categ_tmp ||', this Receipt Method does not exist.  The process cannot continue.');
         raise invalid_parameters;
      end; 
   end;
end if;

print_log('END Document Type Selection');

  
print_log('Check for RPC');
...
begin
   ...
   into g_bug_number
   from ad_bugs
   where bug_number = to_char(22471504); 

   print_log('g_bug_number = ' || g_bug_number);
          
exception
when no_data_found then
          
   print_log('Could not find g_bug_number  ' || g_bug_number ||' in ad_bugs.');
end;
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$RESPID$$##') := g_resp_id_tmp;
   g_masked_tokens('##$$RESPID$$##') := mask_text( g_sql_tokens('##$$RESPID$$##'), 'NO_MASK');
   g_sql_tokens('##$$TRXDATE$$##') := g_trx_date;
   g_masked_tokens('##$$TRXDATE$$##') := mask_text( g_sql_tokens('##$$TRXDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$ACODE$$##') := g_assign_code_name;
   g_masked_tokens('##$$ACODE$$##') := mask_text( g_sql_tokens('##$$ACODE$$##'), 'NO_MASK');
   g_sql_tokens('##$$APPID$$##') := '222';
   g_masked_tokens('##$$APPID$$##') := mask_text( g_sql_tokens('##$$APPID$$##'), 'NO_MASK');
   g_sql_tokens('##$$SYSSOBID$$##') := g_sysparam_sob_id;
   g_masked_tokens('##$$SYSSOBID$$##') := mask_text( g_sql_tokens('##$$SYSSOBID$$##'), 'NO_MASK');
   g_sql_tokens('##$$DOCSEQID$$##') := g_doc_sequence_id;
   g_masked_tokens('##$$DOCSEQID$$##') := mask_text( g_sql_tokens('##$$DOCSEQID$$##'), 'NO_MASK');
   g_sql_tokens('##$$CASHRECEIPTID$$##') := g_cash_receipt_id;
   g_masked_tokens('##$$CASHRECEIPTID$$##') := mask_text( g_sql_tokens('##$$CASHRECEIPTID$$##'), 'NO_MASK');
   g_sql_tokens('##$$METHOD$$##') := g_rct_method_name;
   g_masked_tokens('##$$METHOD$$##') := mask_text( g_sql_tokens('##$$METHOD$$##'), 'NO_MASK');
   g_sql_tokens('##$$BUGNUM$$##') := g_bug_number;
   g_masked_tokens('##$$BUGNUM$$##') := mask_text( g_sql_tokens('##$$BUGNUM$$##'), 'NO_MASK');
   g_sql_tokens('##$$TRXTYPE$$##') := g_trx_type_name;
   ...
   g_sql_tokens('##$$CATEGCODE$$##') := g_categ_code;
   g_masked_tokens('##$$CATEGCODE$$##') := mask_text( g_sql_tokens('##$$CATEGCODE$$##'), 'NO_MASK');
   g_sql_tokens('##$$TYPECLASS$$##') := g_trx_type_class;
   g_masked_tokens('##$$TYPECLASS$$##') := mask_text( g_sql_tokens('##$$TYPECLASS$$##'), 'NO_MASK');
   g_sql_tokens('##$$METHODCODE$$##') := g_creation_method_code;
   g_masked_tokens('##$$METHODCODE$$##') := mask_text( g_sql_tokens('##$$METHODCODE$$##'), 'NO_MASK');
   g_sql_tokens('##$$SOURCE$$##') := g_source_name;
   ...
   g_sql_tokens('##$$TRXTYPEID$$##') := g_cust_trx_type_id;
   g_masked_tokens('##$$TRXTYPEID$$##') := mask_text( g_sql_tokens('##$$TRXTYPEID$$##'), 'NO_MASK');
   g_sql_tokens('##$$METHODID$$##') := g_receipt_method_id;
   ...
   g_sql_tokens('##$$A$$##') := 'AR_DOCSEQ_ANALYZER_PKG';
   ...
   g_sql_tokens('##$$CAT_STRING$$##') := g_cat_string;
   g_masked_tokens('##$$CAT_STRING$$##') := mask_text( g_sql_tokens('##$$CAT_STRING$$##'), 'NO_MASK');
   g_sql_tokens('##$$CATEGID$$##') := p_id_categ;
   g_masked_tokens('##$$CATEGID$$##') := mask_text( g_sql_tokens('##$$CATEGID$$##'), 'NO_MASK');
...
debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_CHANGE_PRINTED_INVOICE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '465',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_CHANGE_PRINTED_INVOICE',
      p_sig_sql                => 'select change_printed_invoice_flag
    FROM AR_SYSTEM_PARAMETERS
    WHERE nvl(change_printed_invoice_flag,''N'') = ''Y''',
      p_title                  => 'System Options - Allow Changes to Printed Transactions Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'System Options Allow Change to Printed Transactions is selected.',
      p_solution               => '<b>You have a potential risk.</b> Sequences could be changed or potential sequences can be skipped.<br>
This should not be checked, if you are using document sequencing.<br>
<br>
Use the following steps to change this setting.<br>
<ol>
<li><b>Responsibility</b>:  Receivables Manager</li>
<li><b>Navigation:</b> Setup > System > System Options</li>
<li>Go To: Trans and Customers Tab</li>
<li>Uncheck <i>Allow Changes to Printed Transactions</i> Box and Save</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_CHANGE_PRINTED_INVOICE');



debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DELETE_INVOICE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '466',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_DELETE_INVOICE',
      p_sig_sql                => 'select invoice_deletion_flag
    FROM AR_SYSTEM_PARAMETERS
    WHERE nvl(invoice_deletion_flag,''N'') = ''Y''',
      p_title                  => 'System Options - Allow Transaction Deletion Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'System Options - Allow Transaction Deletion is selected.',
      p_solution               => '<b>You have a potential risk.</b> Transactions can be deleted and sequence numbers can be skipped.<br>
This should not be set if you are using document sequencing.<br>
<br>
Use the following steps to change this setting:<br>
<ol>
<li><b>Responsibility</b>:  Receivables Manager</li>
<li><b>Navigation:</b> Setup > System > System Options</li>
<li>Go To: Trans and Customers Tab</li>
<li>Uncheck <i>Allow Transaction Deletion</i> Box and Save</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DELETE_INVOICE');



debug('begin add_signature: AR_DS_SEC5_FDS_SEQ');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '475',
      p_sig_id                 => 'AR_DS_SEC5_FDS_SEQ',
      p_sig_sql                => 'select DOC_SEQUENCE_ID,     
       NAME,   
       APPLICATION_ID,  
       AUDIT_TABLE_NAME,
       START_DATE,     
       END_DATE,
       TYPE, 
       decode(TYPE, ''A'', ''Your sequence type uses <b>Automatic</b> numbering, which assigns a unique system-generated number to each document. '' ||  
                                 ''Automatic numbering is sequential by date and time of creation.'',
                    ''M'', ''Your sequence type uses Manual</b> numbering, which requires a user to enter a unique number for each document created. '' ||
                                 ''With Manual numbering, numerical ordering and completeness are not enforced. '' ||
                                 ''Users can skip or omit numbers when entering the sequence value.'',
                    ''G'', ''Your sequence type uses <b>Gapless</b> numbering, which automatically generates a unique number for each document, '' ||
                                 ''but ensures that the document was successfully generated before assigning the number. '' ||
                                 ''With Gapless numbering, no sequence numbers are lost due to incomplete or failed document creation. '' ||
                                 ''This is a legal requirement in some countries.'' ||
                                 ''<br><b>IMPORTANT:</b> We recommend that you choose this type only when gapless numbering is essential, '' ||
                                 ''as it may affect the performance of your system.'') "DESCRIPTION",
       DB_SEQUENCE_NAME,
       INITIAL_VALUE,    
       TABLE_NAME
from FND_DOCUMENT_SEQUENCES seq
where seq.doc_sequence_id in 
         (select doc_sequence_id 
          from FND_DOC_SEQUENCE_ASSIGNMENTS 
          where category_code = ''##$$CATEGCODE$$##''
          and application_id = 222
          and set_of_books_id = ##$$SYSSOBID$$##)',
      p_title                  => 'Document Sequence Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate the document sequence assigned.',
      p_solution               => 'The application and set of books associated to the document sequence, should match the category you are using.<br>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'Displaying details of the document sequence assigned.',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDS_SEQ');



debug('begin add_signature: AR_DS_SEC5_FDSA_SOBID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '472',
      p_sig_id                 => 'AR_DS_SEC5_FDSA_SOBID',
      p_sig_sql                => 'select doc_sequence_id, set_of_books_id              
from FND_DOC_SEQUENCE_ASSIGNMENTS assign
where assign.category_code = ''##$$CATEGCODE$$##'' 
and assign.set_of_books_id = ##$$SYSSOBID$$##',
      p_title                  => 'Ledger for Document Sequence Assignment',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The ledger assigned to this Document Sequence Assignment is incorrect.',
      p_solution               => 'The Document Sequence Assignment Ledger should match the Ledger used by the Operating Unit you are creating the transaction in.<br>
<br>
You will need to create a new Document Sequence Assignment for the correct Ledger.<br>
<br>
Use the following steps:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Assign</li>
<li><b>NOTE:</b> If you are using Document Sequencing for multiple ledgers, you need to setup a Document Assignment for each Ledger wherein you want to use the document sequence.</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'Displaying details of the ledger for this document sequence assignment.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSA_SOBID');



debug('begin add_signature: AR_DS_SEC5_FDS_APPID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '477',
      p_sig_id                 => 'AR_DS_SEC5_FDS_APPID',
      p_sig_sql                => 'select fds.doc_sequence_id, fds.application_id              
from FND_DOCUMENT_SEQUENCES fds
where fds.application_id <> 222 
and doc_sequence_id in 
   (select assign.doc_sequence_id
    from   FND_DOC_SEQUENCE_ASSIGNMENTS assign
    where  assign.category_code = ''##$$CATEGCODE$$##''
    and    assign.application_id = 222
    and    set_of_books_id = ##$$SYSSOBID$$##)',
      p_title                  => 'Application for the Document Sequence',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The application assigned to this Document Sequence is incorrect.',
      p_solution               => 'The Document Sequence application used should be Receivables.<br>
You will need to create a new Document Sequence for the Receivables application.<br>
<br>
Use the following steps:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Define</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'The application assigned to this document sequence is correct.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDS_APPID');



debug('begin add_signature: AR_DS_SEC5_FDS_CACHE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '478',
      p_sig_id                 => 'AR_DS_SEC5_FDS_CACHE',
      p_sig_sql                => 'select fds.doc_sequence_id, dbaseq.increment_by, dbaseq.cache_size
    from FND_DOCUMENT_SEQUENCES fds,
         DBA_SEQUENCES dbaseq
    where doc_sequence_id in 
     (select assign.doc_sequence_id
      ...
      and assign.application_id = 222
      and set_of_books_id = ##$$SYSSOBID$$##)
    and (dbaseq.cache_size <> 0
         or dbaseq.increment_by <> 1)
    and dbaseq.sequence_name = fds.db_sequence_name',
      p_title                  => 'Cache and Increment By Values',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This document sequence has caching set to a number greater than zero and/or is incrementing more than one number at a time.',
      p_solution               => 'You may have a risk of gaps in your document numbering.<br>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'This document sequence has the recommended caching setup.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDS_CACHE');



debug('begin add_signature: AR_DS_SEC5_FDSC_CATEGORY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '468',
      p_sig_id                 => 'AR_DS_SEC5_FDSC_CATEGORY',
      p_sig_sql                => 'select fdsc.code, fdsc.name, fdsc.table_name, fdsc.application_id, fdsc.description, fdsc.creation_date
     from FND_DOC_SEQUENCE_CATEGORIES fdsc
     where fdsc.code = ''##$$CATEGCODE$$##''
     and fdsc.application_id = 222',
      p_title                  => 'Document Sequence Category',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The Document Sequence Category should be setup to match the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' + Application.',
      p_solution               => 'You can use the following steps to verify Document Sequence Category setup:<br>
<br>
<b>Responsibility</b>:  System Administrator<br>
<b>Navigation:</b> Application > Sequential Numbering > Categories<br>
<br>
Following conditions must be true:<br>
<li>Document Category Code <b>MUST MATCH</b> the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' <b>EXACTLY</b>.</li>  
<li>The application must be for Oracle Receivables.</li>
<li>If the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' was changed, after the Category was created, you need to:</li>
<ul>
<li>Change the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' back to the original name, in the case of a transaction type, incomplete the transaction and complete it <b>OR</b></li>
<li>Create a New Category for the new '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||'.</li>
</ul>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'Displaying setup for document sequence category.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSC_CATEGORY');



debug('begin add_signature: AR_DS_SEC5_FDSA_ASSIGNMENT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '469',
      p_sig_id                 => 'AR_DS_SEC5_FDSA_ASSIGNMENT',
      p_sig_sql                => 'select DOC_SEQUENCE_ASSIGNMENT_ID,
       CATEGORY_CODE, 
       METHOD_CODE,
       decode(METHOD_CODE, ''M'', 
                   ''Method Code is M (Manual). This method setup indicates a document within the category is manually created using a form in an application.'',
                           ''A'',
                   ''Method Code is A (Automatic). This method setup indicates documents are created via a concurrent process, such as an external program of something like AutoInvoice which creates transaction without manual data entry.'',
                           null,
                    ''Method Code is NULL. This method setup indicates documents can be created either Automatically or Manually.'') "DESCRIPTION",
       SET_OF_BOOKS_ID,
       DOC_SEQUENCE_ID,
       CREATION_DATE,
       START_DATE,
...
and    application_id = 222
and    set_of_books_id = ##$$SYSSOBID$$##',
      p_title                  => 'Document Sequence Assignment',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Document sequence assignment is missing.',
      p_solution               => 'You can use the following steps to create a Document Sequence Assignment:<br>
<ol>
<li>Responsibility</b>:  System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Assignment<br></li>
</ol>
<b>NOTE:</b> Document Categories <b>MUST MATCH</b> the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' <b>EXACTLY</b>.<br>  
<br>
Also, the Ledger should match the Operating Unit you are using for Document Sequencing.<br>
If your instance uses multiple Ledgers, you will need to set up the document sequence assignment for each Ledger.<br>
<ul>
<li>If the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' was changed, after the Category and Assignment was created, you need to:</li>
</ul>
<ol>
<li>Change the '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||' back to the original name, incomplete the '||mask_text(g_doc_string, nvl( upper(''), 'NO_MASK') )||'and complete it <b>OR</b></li>
<li>Create a Sequential Numbering Category and Sequential Numbering Assignment for the new '||mask_text(g_cat_string, nvl( upper(''), 'NO_MASK') )||'.</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'Displaying setup for document sequence assignment.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSA_ASSIGNMENT');



debug('begin add_signature: AR_DS_SEC5_FDSA_APPID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '471',
      p_sig_id                 => 'AR_DS_SEC5_FDSA_APPID',
      p_sig_sql                => 'select CATEGORY_CODE,
...
from FND_DOC_SEQUENCE_ASSIGNMENTS
    ...
    and application_id = 222 
    and set_of_books_id = ##$$SYSSOBID$$##',
      p_title                  => 'Ledger and Application Setup for Document Sequence Assignment',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The application and/or ledger assigned to this Document Sequence Assignment is incorrect.',
      p_solution               => 'The Document Sequence Assignment application used should be Receivables.<br>
You will need to create a new Document Sequence Assignment for the Receivables application.<br>
<br>
Use the following steps:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
...
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'Displaying the application and ledger assigned to this document sequence assignment.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSA_APPID');



debug('begin add_signature: AR_DS_FND_DOC_SEQUENCE_CATEGORIES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19371',
      p_sig_id                 => 'AR_DS_FND_DOC_SEQUENCE_CATEGORIES',
      p_sig_sql                => 'select * 
from FND_DOC_SEQUENCE_CATEGORIES
where application_id = 222
order by name',
      p_title                  => 'FND_DOC_SEQUENCE_CATEGORIES',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no document sequence categories defined in Receivables.',
      p_solution               => '',
      p_success_msg            => 'Displaying all document sequence categories in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_FND_DOC_SEQUENCE_CATEGORIES');



debug('begin add_signature: AR_DS_FND_DOC_SEQUENCE_ASSIGNMENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19372',
      p_sig_id                 => 'AR_DS_FND_DOC_SEQUENCE_ASSIGNMENTS',
      p_sig_sql                => 'select * 
from FND_DOC_SEQUENCE_ASSIGNMENTS
where application_id = 222
order by set_of_books_id, category_code',
      p_title                  => 'FND_DOC_SEQUENCE_ASSIGNMENTS',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no document sequence assignments defined for Receivables.',
      p_solution               => '',
      p_success_msg            => 'Displaying all document sequence assignments in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_FND_DOC_SEQUENCE_ASSIGNMENTS');



debug('begin add_signature: AR_DS_FND_DOC_SEQUENCE_USERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22092',
      p_sig_id                 => 'AR_DS_FND_DOC_SEQUENCE_USERS',
      p_sig_sql                => 'select * 
from       FND_DOC_SEQUENCE_USERS 
WHERE  doc_sequence_id in (select doc_sequence_id 
                                                       from   FND_DOC_SEQUENCE_ASSIGNMENTS
                                                       where application_id = 222)',
      p_title                  => 'FND_DOC_SEQUENCE_USERS',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no document sequence users defined in Receivables.',
      p_solution               => 'There are no document sequence users defined in Receivables.',
      p_success_msg            => 'Displaying all document sequence users in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_FND_DOC_SEQUENCE_USERS');



debug('begin add_signature: AR_DS_FND_DOCUMENT_SEQUENCES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19373',
      p_sig_id                 => 'AR_DS_FND_DOCUMENT_SEQUENCES',
      p_sig_sql                => 'select * 
from       FND_DOCUMENT_SEQUENCES fds
...
ORDER BY  table_name, name',
      p_title                  => 'FND_DOCUMENT_SEQUENCES',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no document sequences defined in Receivables.',
      p_solution               => '',
      p_success_msg            => 'Displaying all document sequences in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_FND_DOCUMENT_SEQUENCES');



debug('begin add_signature: EBS_FIN_AR_ARDOCSEQ_AR_DS_FND_DOCUMENT_SEQUENCES_GAPLESS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22837',
      p_sig_id                 => 'EBS_FIN_AR_ARDOCSEQ_AR_DS_FND_DOCUMENT_SEQUENCES_GAPLESS',
      p_sig_sql                => 'SELECT A.NAME, B.*
 FROM FND_DOCUMENT_SEQUENCES A, FND_DOC_SEQUENCE_USERS B
WHERE EXISTS  ( SELECT ''Y'' 
                               FROM FND_DOC_SEQUENCE_ASSIGNMENTS  FDSA
                               WHERE FDSA.DOC_SEQUENCE_ID = A.DOC_SEQUENCE_ID
                               AND FDSA.APPLICATION_ID = 222 ) 
    AND A.DOC_SEQUENCE_ID = B.DOC_SEQUENCE_ID
ORDER BY A.NAME
',
      p_title                  => 'GAPLESS SEQUENCES',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no gapless sequences defined in Receivables.',
      p_solution               => '',
      p_success_msg            => 'Displaying all gapless sequences information in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_ARDOCSEQ_AR_DS_FND_DOCUMENT_SEQUENCES_GAPLESS');



debug('begin add_signature: AR_DS_DBA_SEQUENCES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19374',
      p_sig_id                 => 'AR_DS_DBA_SEQUENCES',
      p_sig_sql                => 'SELECT    * 
FROM DBA_SEQUENCES 
WHERE   sequence_name IN ( SELECT   db_sequence_name  
                                                       FROM FND_DOCUMENT_SEQUENCES  fds
                                                       WHERE exists  ( SELECT  ''Y''    
                                                                                     FROM FND_DOC_SEQUENCE_ASSIGNMENTS fdsa
                                                                                     WHERE fdsa.doc_sequence_id = fds.doc_sequence_id
...
',
      p_title                  => 'DBA_SEQUENCES',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no database sequences associated with Receivables.',
      p_solution               => '',
      p_success_msg            => 'Displaying all database sequences used in Receivables.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_DBA_SEQUENCES');



debug('begin add_signature: AR_DS_CHECK_MISSING_CATEGORY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12562',
      p_sig_id                 => 'AR_DS_CHECK_MISSING_CATEGORY',
      p_sig_sql                => 'select ''TRANSACTION'' "DOCUMENT_CATEGORY", cust_trx_type_id, name, start_date, end_date
from RA_CUST_TRX_TYPES typ
where nvl(end_date, sysdate+1) > sysdate
and not exists 
  (select ''Y''
   from FND_DOC_SEQUENCE_CATEGORIES fdsc
   where fdsc.application_id = 222
   and typ.name = fdsc.code)
union all
select ''RECEIPT'', receipt_method_id, name, start_date, end_date
from AR_RECEIPT_METHODS rm
where nvl(rm.end_date, sysdate+1) > sysdate
and not exists 
  (select ''Y''
   from FND_DOC_SEQUENCE_CATEGORIES fdsc
   where fdsc.application_id = 222
   and rm.name = fdsc.code)',
      p_title                  => 'Check for Missing Category Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found transaction types / receipt methods that are active but do not have corresponding document categories defined.
You will encounter errors if you use these transaction types / receipt methods and expect a document sequence to be generated.',
      p_solution               => 'You can use the following steps to verify Document Sequence Category setup:<br>
<br>
<b>Responsibility</b>:  System Administrator<br>
<b>Navigation:</b> Application > Sequential Numbering > Categories<br>
<br>
Following conditions must be true:<br>
<li>Document Category Code <b>MUST MATCH</b> the Transaction Type Name <b>EXACTLY</b>.</li>  
<li>The application must be for Oracle Receivables.</li>
<li>If the Transaction Type Name was changed, after the Category was created, you need to:</li>
<ul>
<li>Change the Transaction Type Name back to the original name, incomplete the transaction and complete it <b>OR</b></li>
<li>Create a New Category for the new Transaction Type Name.</li>
</ul>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'All active transaction types / receipt methods have corresponding document categories defined.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_CHECK_MISSING_CATEGORY');



debug('begin add_signature: AR_DS_SEC6_CATEG_SIZE_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '3482',
      p_sig_id                 => 'AR_DS_SEC6_CATEG_SIZE_CHECK',
      p_sig_sql                => 'select fdsc.code, fdsc.name, fdsc.table_name, fdsc.application_id, fdsc.description
     from FND_DOC_SEQUENCE_CATEGORIES fdsc
     where fdsc.code = ''##$$CATEGCODE$$##''
     and fdsc.application_id = 222
     and length(rtrim(ltrim(''##$$CATEGCODE$$##''))) > 30',
      p_title                  => 'Check Length of Category Name',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The category name exceeds 30 characters in length.',
      p_solution               => 'The document category is automatically created when you define a new Transaction Type or Receipt Method. <br>
The document category name will be used to create the database sequence, and when the name is too long, you may encounter errors when the system attempts to build the sequence. <br>
The value should not exceed 30 characters.<br><br>
<br>
Please use the following steps to check the name used:<br><br>

<ol>
<li><b>Responsibility:</b>Receivables</li>
<li>For Transaction Type: <b>Navigation:</b> Setup > Transactions > Types</li>
<li>For Receipt Methods: <b>Navigation:</b> Setup > Receipts > Classes</li>
</ol>
Read [1086565/KB298647#Characters] for more information.',
      p_success_msg            => 'The length of the category name has no issues.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC6_CATEG_SIZE_CHECK');



debug('begin add_signature: AR_DS_SEC1_AR_INVALIDS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '2444',
      p_sig_id                 => 'AR_DS_SEC1_AR_INVALIDS',
...
AND a.status = ''INVALID''',
      p_title                  => 'AR Invalid Objects',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Receivables Invalid Objects',
...
<li>If you still have invalid objects, please review steps from [1325394/KB401905] Troubleshooting Guide - Invalid Objects in the E-Business Suite Environment 11i and 12</li>
      ...
      p_success_msg            => 'There are no Receivables related invalid objects exist in the database',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC1_AR_INVALIDS');



debug('begin add_signature: AR_DS_SEC2_PATCH_REC_RPC_APPLIED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '2457',
      p_sig_id                 => 'AR_DS_SEC2_PATCH_REC_RPC_APPLIED',
      p_sig_sql                => 'select bug_number
from ad_bugs
where bug_number = to_char(22471504)',
      p_title                  => 'AR Recommended Patches',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Validating if RPC Recommendation has been applied.',
      p_solution               => 'It is recommended that you apply the RPC Patch {22471504}, which is the latest patch that includes all the patches listed under the recommended patch sections and is included in the RPC Feb 2016.<br>
...
Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.',
      p_success_msg            => 'You are on the latest RPC patch, no action required.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC2_PATCH_REC_RPC_APPLIED');
...
debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_NULL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '462',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_NULL',
      p_sig_sql                => 'select document_seq_gen_level
    FROM AR_SYSTEM_PARAMETERS
    WHERE document_seq_gen_level = ''null''',
      p_title                  => 'System Options Document Number Generation Level Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Document Number Generation Level is not setup',
      p_solution               => 'This setting indicates you may not be using document sequencing.<br> 
It must be set to completed or saved when using document sequencing.<br>

<br>
Check your setup, using the following steps:<br>
<ol>
<li><b>Responsibility</b>:  Receivables Manager</li>
<li><b>Navigation:</b> Setup > System > System Options</li>
<li>Go To: Trans and Customers Tab</li>
<li>Set Document Number Generation Level</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_NULL');



debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_SAVED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '463',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_SAVED',
      p_sig_sql                => 'select document_seq_gen_level
    FROM AR_SYSTEM_PARAMETERS
    WHERE document_seq_gen_level = ''WHEN_TRX_COMMITTED''',
      p_title                  => 'System Options Document Number Generation Level Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'System Options Document Number Generation Level is set to SAVED.',
      p_solution               => 'Your System Options setup indicates your document sequence will be created when transactions are <b>SAVED</b>.<br>
This is one of the recommended settings when using document sequencing.<br>
<br>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_SAVED');



debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_COMPLETED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '464',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_COMPLETED',
      p_sig_sql                => 'select document_seq_gen_level
    FROM AR_SYSTEM_PARAMETERS
    WHERE document_seq_gen_level = ''WHEN_TRX_COMPLETED''',
      p_title                  => 'System Options Document Number Generation Level Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'System Options Document Number Generation Level is set to COMPLETE',
      p_solution               => 'Your System Options setup indicates your document sequence will be created when transactions are <b>COMPLETED</b>.<br>
This is one of the recommended settings when using document sequencing.<br>
<br>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_COMPLETED');



debug('begin add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_BR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '3390',
      p_sig_id                 => 'AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_BR',
      p_sig_sql                => 'select type
from RA_CUST_TRX_TYPES_ALL
where cust_trx_type_id = ##$$TRXTYPEID$$##',
      p_title                  => 'Bills Receivable',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'This is a Bills Receivable Transaction type. ',
      p_solution               => 'null',
      p_success_msg            => 'The transaction ID you provided is for a <b>Bills Receivable</b> type. Therefore, the document number will be generated when the transaction is <B>COMPLETED</B>, regardless of what your setting is in the System Options form as shown above.<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_BR');



debug('begin add_signature: AR_DS_SEC5_FDSA_TRX_DATES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '470',
      p_sig_id                 => 'AR_DS_SEC5_FDSA_TRX_DATES',
      p_sig_sql                => 'select assign.category_code, 
           assign.start_date,
           assign.end_date, 
           docseq.NAME, 
           docseq.DB_SEQUENCE_NAME,
           ''##$$TRXDATE$$##'' "Date of Trx"
    from   FND_DOC_SEQUENCE_ASSIGNMENTS assign, FND_DOCUMENT_SEQUENCES docseq
    where  assign.doc_sequence_id = docseq.doc_sequence_id
    and    assign.application_id = 222    
    and    assign.category_code = ''##$$TRXTYPE$$##''
    and    assign.set_of_books_id = ##$$SYSSOBID$$##
    and    to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') not between assign.start_date and 
                                                            nvl(assign.end_date, to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'')+1)
    and    not exists (select ''X'' from FND_DOC_SEQUENCE_ASSIGNMENTS assign2 
                        where assign2.category_code = ''##$$TRXTYPE$$##'' 
                          and assign2.application_id = ''222''
                          and (to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') >= assign2.start_date AND to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') <= assign2.end_date))',
      p_title                  => 'Check Transaction is Within Document Sequence Assignment Date Range',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The date of the transaction falls outside of the document Sequence Assignment setup.',
      p_solution               => 'Please review the Document Sequence Assignment start date, and the start date of your transaction.<br>
If this Document sequence Assignment is end-dated, and your transaction is after this date, you will need to create a new Document Sequence Assignment.<br>
<br>
Use the following steps, to Review the Assignment:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Assign</li>
<li>If the transaction date is before the Document Sequence Assignment start date, incomplete the transaction, change it to the appropriate dates and complete it again.</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'The date of your transaction is within the Document Sequence Assignment date range setup.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSA_TRX_DATES');



debug('begin add_signature: AR_DS_SEC5_FDS_DATES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '476',
      p_sig_id                 => 'AR_DS_SEC5_FDS_DATES',
      p_sig_sql                => 'select
  fds.doc_sequence_id,
  fds.name,
...
and
(
  to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') < fds.start_date
  OR
  to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') >= nvl(fds.end_date, to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'')+1)
)
and not exists
(
  select ''X''
  from FND_DOCUMENT_SEQUENCES fds2
  where  fds2.application_id = 222
  and fds2.doc_sequence_id in
  (
	  select assign2.doc_sequence_id
      from   FND_DOC_SEQUENCE_ASSIGNMENTS assign2
      where  assign2.category_code = ''##$$CATEGCODE$$##''
      and    assign2.application_id = 222
      and    assign2.set_of_books_id = ##$$SYSSOBID$$##
   )
   and
   (
    to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') >= fds2.start_date
    AND
    to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') <= nvl(fds2.end_date, to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY''))
   )
)',
      p_title                  => 'Check Document Sequence Date Range',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The date of the '||mask_text(g_doc_string, nvl( upper(''), 'NO_MASK') )||' falls outside of the Document Sequence used.',
      p_solution               => 'Please review the Document Sequence start date, and the date of your '||mask_text(g_doc_string, nvl( upper(''), 'NO_MASK') )||'.<br>
If this Document sequence is end-dated, and your '||mask_text(g_doc_string, nvl( upper(''), 'NO_MASK') )||' is after this date, you will need to create a new Document Sequence.<br>
<br>
Use the following steps, to Review the Sequence:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Define</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDS_DATES');



debug('begin add_signature: AR_DS_SEC6_TRX_BATCH_SOURCE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '479',
      p_sig_id                 => 'AR_DS_SEC6_TRX_BATCH_SOURCE',
      p_sig_sql                => 'select bsrc.name "Batch Source", 
       bsrc.batch_source_id "Batch Source ID", 
       nvl(bsrc.copy_doc_number_flag, ''N'') "Copy Doc Number Flag",
       nvl(bsrc.auto_trx_numbering_flag, ''N'') "Automatic Trx Numbering Flag",
       nvl(bsrc.auto_batch_numbering_flag, ''N'') "Automatic Batch Numbering Flag"
from   ra_batch_sources_all bsrc
where  bsrc.name = ''##$$SOURCE$$##''',
      p_title                  => 'Transaction Batch Source',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Transaction Batch Source details impacting Document Sequencing.
',
      p_solution               => 'The Batch Source used does not have the <b>Copy Document Number to Transaction Number</b> set. 
This is not a requirement for Document Sequencing, but you may want to consider selecting the <b>Copy Document Number to Transaction Number</b>.<br>

<br>Some information about using a Batch Source and Document Sequencing:<br>

<li>The <b>Copy Document Number to Transaction Number</b> - By checking this, you are indicating that you want the Document Sequence associated to the Transaction Type in this batch to replace the Transaction Number generated by the Transaction Batch Source.</li>
<li>If you set this to ''Y,'' the current transaction number will be replaced by the system generated document number, regardless of whether it was manually entered or automatically generated via your setting for Automatic Transaction Numbering.</li>
<li>This setting enables users to implement Gapless Transaction Numbering, if required, but you can still use this feature even if you do not require Gapless Transaction Numbering.</li>

<br>Use the following steps to review:<br>

<ol>
<li><b>Responsibility:</b> Receivables</li>
<li><b>Navigation:</b> Setup > Transactions > Sources</li>
<li>Create a new Source</li>
<li>Check <i>Copy Document Number to Transaction Number</i> and Save</li>
</ol>

Read [1087122/KB728629] or [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'The Batch Source used has the <b>Copy Document Number to Transaction Number</b> set. 
<br>
Some information about using a Batch Source and Document Sequencing:<br>
<li>The <b>Copy Document Number to Transaction Number</b> - By checking this, you are indicating that you want the Document Sequence associated to the Transaction Type in this batch to replace the Transaction Number generated by the Transaction Batch Source.</li>
<li>If you set this to ''Y,'' the current transaction number will be replaced by the system generated document number, regardless of whether it was manually entered or automatically generated via your setting for Automatic Transaction Numbering.</li>
<li>This setting enables users to implement Gapless Transaction Numbering, if required, but you can still use this feature even if you do not require Gapless Transaction Numbering.</li>',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC6_TRX_BATCH_SOURCE');



debug('begin add_signature: AR_DS_SEC5_FDSA_RCT_DATES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '2412',
      p_sig_id                 => 'AR_DS_SEC5_FDSA_RCT_DATES',
      p_sig_sql                => 'select assign.category_code, 
       assign.start_date,
       assign.end_date,
       docseq.NAME,
    ...
    from   FND_DOC_SEQUENCE_ASSIGNMENTS assign, FND_DOCUMENT_SEQUENCES docseq
    where  assign.doc_sequence_id = docseq.doc_sequence_id
    and    assign.category_code = ''##$$METHOD$$##''
    and    assign.set_of_books_id = ##$$SYSSOBID$$##
    and    to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'') not between assign.start_date AND
                                                            nvl(assign.end_date, to_date(''##$$TRXDATE$$##'',''DD-MON-YYYY'')+1)',
      p_title                  => 'Check Receipt is Within Document Sequence Assignment Date Range',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The date of your receipt falls outside of the document Sequence Assignment setup.
',
      p_solution               => 'Please review the Document Sequence Assignment start date, and the date of your receipt.<br>
If this Document Sequence Assignment is end-dated, and your receipt is after this date, you will need to create a new Document Sequence Assignment.<br>
<br>
Use the following steps, to Review the Assignment:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering > Assign</li>
<li>If the receipt date is before the Document Sequence Assignment start date, you will need to reverse this receipt and create a new one.</li>
</ol>
Read [1086565/KB298647] for more information.<br>',
      p_success_msg            => 'The date of the receipt is within the Document Sequence Assignment date range setup.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC5_FDSA_RCT_DATES');



debug('begin add_signature: AR_DS_SEC3_RESP_DOC_PROF_NOT_SET');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '247',
      p_sig_id                 => 'AR_DS_SEC3_RESP_DOC_PROF_NOT_SET',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
         fnd_profile_option_values pov
    ...
    AND   pov.profile_option_id = po.profile_option_id
    AND   po.user_profile_option_name = ''Sequential Numbering''
    AND   pov.level_id = ''10003'' AND pov.level_value = ##$$RESPID$$##
    AND   nvl(pov.profile_option_value,''N'') = ''N''',
      p_title                  => 'The profile option: Sequential Numbering Responsibility Level = NULL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is not set',
      p_solution               => 'The Profile option for Sequential Numbering is set to NULL or Not Used.  Due to this, the document type will not generate a document sequence number.<br>
<br>
Please review your setup:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Profile > System</li>
<li>Query Application = Receivables, Responsibility = <enter the AR responsibility used when creating the document></li>
<li>The value has to be set to Always or Partially used</li>
</ol>
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_RESP_DOC_PROF_NOT_SET');



debug('begin add_signature: AR_DS_SEC3_RESP_DOC_PROF_PARTIAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '341',
      p_sig_id                 => 'AR_DS_SEC3_RESP_DOC_PROF_PARTIAL',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   po.user_profile_option_name = ''Sequential Numbering''
    AND   pov.level_id = ''10003'' AND pov.level_value = ##$$RESPID$$##
    AND   nvl(pov.profile_option_value,''N'') = ''P''',
      p_title                  => 'The profile option: Sequential Numbering Responsibility Level = Partially Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is set to Partially Used',
      p_solution               => 'This setting is good.<br>
The profile option Sequential Numbering must be enabled in order to use the Document Sequence feature. This can be set at the site, application or responsibility level.<br>
<ul>
<li>If you are creating transactions via APIs, and you have the profile "Sequential Numbering" set to Partially Used (and you have not assigned an *Automatic* document sequence to the document category associated to the transaction type, you may not create a document sequence for the transaction created through the APIs.</li>  
<li>You need to ensure that the transaction types you are using to create transactions via APIs have been set up for Automatic document sequencing. Even though you use a *Manual* Batch Source for APIs, the creation happens automatically because it is handled by a program as opposed to being entered manually by a user via the transaction form.</li>
</ul>
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_RESP_DOC_PROF_PARTIAL');



debug('begin add_signature: AR_DS_SEC3_RESP_DOC_PROF_ALWAYS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '342',
      p_sig_id                 => 'AR_DS_SEC3_RESP_DOC_PROF_ALWAYS',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   po.user_profile_option_name = ''Sequential Numbering''
    AND   pov.level_id = ''10003'' AND pov.level_value = ##$$RESPID$$##
    AND   nvl(pov.profile_option_value,''N'') = ''A''',
      p_title                  => 'The profile option: Sequential Numbering Responsiblity Level = Always Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is set to Always Used',
      p_solution               => 'This setting is good.<br>
<b>IMPORTANT:</b>  We recommend that you choose this type only when Gapless numbering is essential, as it may affect the performance of your system <br>
The profile option Sequential Numbering must be enabled in order to use the Document Sequence feature. This can be set at the site, application or responsibility level.<br>
<ul>
<li>If you are creating transactions via APIs, and you have the profile "Sequential Numbering" set to Partially Used (and you have not assigned an *Automatic* document sequence to the document category associated to the transaction type, you may not create a document sequence for the transaction created through the APIs.</li>  
<li>You need to ensure that the transaction types you are using to create transactions via APIs have been set up for Automatic document sequencing. Even though you use a *Manual* Batch Source for APIs, the creation happens automatically because it is handled by a program as opposed to being entered manually by a user via the transaction form.</li>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_RESP_DOC_PROF_ALWAYS');



debug('begin add_signature: AR_DS_SEC3_APP_DOC_PROF_NOT_SET');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '343',
      p_sig_id                 => 'AR_DS_SEC3_APP_DOC_PROF_NOT_SET',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.profile_option_id = po.profile_option_id
    AND   po.user_profile_option_name = ''Sequential Numbering''
    AND   pov.level_id = ''10002'' AND pov.level_value = 222
    AND   nvl(pov.profile_option_value,''N'') = ''N''',
      p_title                  => 'The profile option: Sequential Numbering Application Level = NULL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is not set',
      p_solution               => 'The Profile option for Sequential Numbering is set to NULL or Not Used. Due to this, the document type will not generate a document sequence number.<br>
<br>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information.<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_APP_DOC_PROF_NOT_SET');



debug('begin add_signature: AR_DS_SEC3_APP_DOC_PROF_PARTIAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '344',
      p_sig_id                 => 'AR_DS_SEC3_APP_DOC_PROF_PARTIAL',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_id = ''10002'' AND pov.level_value = 222
    AND   nvl(pov.profile_option_value,''N'') = ''P''',
      p_title                  => 'The profile option: Sequential Numbering Application Level = Partially Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is to Partially Used',
      p_solution               => 'This setting is good.<br>
The profile option Sequential Numbering must be enabled in order to use the Document Sequence feature. This can be set at the site, application or responsibility level.<br>
<ul>
<li>If you are creating transactions via APIs, and you have the profile "Sequential Numbering" set to Partially Used (and you have not assigned an *Automatic* document sequence to the document category associated to the transaction type, you may not create a document sequence for the transaction created through the APIs.</li>  
...
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_APP_DOC_PROF_PARTIAL');



debug('begin add_signature: AR_DS_SEC3_APP_DOC_PROF_ALWAYS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '444',
      p_sig_id                 => 'AR_DS_SEC3_APP_DOC_PROF_ALWAYS',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_id = ''10002'' AND pov.level_value = 222
    AND   nvl(pov.profile_option_value,''N'') = ''A''',
      p_title                  => 'The profile option: Sequential Numbering Application Level = Always Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is set to Always Used
',
      p_solution               => 'This setting is good.<br>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_APP_DOC_PROF_ALWAYS');



debug('begin add_signature: AR_DS_SEC3_SITE_DOC_PROF_NOT_SET');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '457',
      p_sig_id                 => 'AR_DS_SEC3_SITE_DOC_PROF_NOT_SET',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_id = ''10001''
    AND   nvl(pov.profile_option_value,''N'') = ''N''',
      p_title                  => 'The profile option: Sequential Numbering Site Level = NULL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is not set',
      p_solution               => 'The Profile option for Sequential Numbering is set to NULL or Not Used.  Due to this, the document type will not generate a document sequence number.<br>
<br>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information.<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_SITE_DOC_PROF_NOT_SET');



debug('begin add_signature: AR_DS_SEC3_SITE_DOC_PROF_PARTIAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '460',
      p_sig_id                 => 'AR_DS_SEC3_SITE_DOC_PROF_PARTIAL',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_id = ''10001''
    AND   nvl(pov.profile_option_value,''N'') = ''P''',
      p_title                  => 'The profile option: Sequential Numbering Site Level = Partially Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is to Partially Used',
      p_solution               => 'This setting is good.<br>
The profile option Sequential Numbering must be enabled in order to use the Document Sequence feature. This can be set at the site, application or responsibility level.<br>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_SITE_DOC_PROF_PARTIAL');



debug('begin add_signature: AR_DS_SEC3_SITE_DOC_PROF_ALWAYS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '461',
      p_sig_id                 => 'AR_DS_SEC3_SITE_DOC_PROF_ALWAYS',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   pov.level_id = ''10001''
    AND   nvl(pov.profile_option_value,''N'') = ''A''',
      p_title                  => 'The profile option: Sequential Numbering Site Level = Always Used',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The profile option: Sequential Numbering is set to Always Used
',
      p_solution               => 'This setting is good.<br>
...
Read [1087122/KB728629] or [1086565/KB298647] for more information.<br>',
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_SITE_DOC_PROF_ALWAYS');



debug('begin add_signature: AR_DS_SEC3_SITE_JOURNAL_ENTRIES_SETUP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '3486',
      p_sig_id                 => 'AR_DS_SEC3_SITE_JOURNAL_ENTRIES_SETUP',
      p_sig_sql                => 'SELECT po.user_profile_option_name , po.profile_option_name,
           DECODE (TO_CHAR (pov.level_id), ''10001'', ''SITE'', ''10002'', ''APP'', ''10003'', ''RESP'', ''10004'', ''USER'') lev,
           pov.profile_option_value opt_val
    FROM fnd_profile_options_vl po,
    ...
    AND   po.user_profile_option_name = ''Sequential Numbering''
    AND   pov.level_id = ''10001''',
      p_title                  => 'Journal Entry Categories Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Sequential Numbering is set only at the Site Level.  In this case, you must define sequences, categories, and assignments for the following journal entry categories before you can post.
',
      p_solution               => '<b>NOTE:</b>Since you have enabled Sequential Numbering only at the Site Level, and did not set it up for specific Applications or Responsibilities, then this would mean that you have enabled it across all applications including Oracle General Ledger. In this case, you must define sequences, categories, and assignments for your following journal entry categories before you can post.<br>
<br>
<li>Adjustments</li>
<li>Bills Receivable</li>
<li>Chargebacks</li>
<li>Credit Memo Applications</li>
<li>Credit Memos</li>
<li>Cross Currency</li>
<li>Debit Memos</li>
<li>Miscellaneous Receipts</li>
<li>Sales Invoices</li>
<li>Trade Receipts</li>
<br>
Use the following navigation to review/create:<br>
<ol>
<li><b>Responsibility:</b> System Administrator</li>
<li><b>Navigation:</b> Application > Sequential Numbering</li>
</ol>
Read [1087122/KB728629] or [1086565/KB298647] for more information<br>',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC3_SITE_JOURNAL_ENTRIES_SETUP');



debug('begin add_signature: AR_DS_SEC7_RCT_INHERIT_INV_NUM_BR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '3523',
      p_sig_id                 => 'AR_DS_SEC7_RCT_INHERIT_INV_NUM_BR',
      p_sig_sql                => 'select nvl(c.RECEIPT_INHERIT_INV_NUM_FLAG,''N''), c.NAME, a.creation_method_code
FROM AR_RECEIPT_CLASSES a,
     AR_RECEIPT_METHODS c
  WHERE c.name = ''##$$METHOD$$##''
   AND a.receipt_class_id = c.receipt_class_id
   AND c.RECEIPT_INHERIT_INV_NUM_FLAG = ''Y''',
      p_title                  => 'Bills Receivable Maturity and Risk Elimination Program Information',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The Receipts Inherit Transaction Number for your Bills Receivable Remittance.',
      p_solution               => 'When a Bills Receivable (BR) matures, you may run the Bills Receivable Maturity and Risk Elimination program to create receipts for factored with recourse BRs or the Bills Receivable Remittance program to remit matured BRs to collect payment.<br>
<br>
In these processes you may encounter errors like:<br>
<br>
ORA-06512: at ''APPS.ARP_BR_HOUSEKEEPER_PKG''<br>
ORA-20001: APP-AR-294140: Please enter a receipt number.<br>
<br>
These BR processes are similar to the automatic receipts functionality and requires document sequence to be set up the for the receipt method associated to the Bills Receivable remittance. If your receipt method has Receipts Inherit Transaction Number unchecked, then a document sequence will be required.<br>
<br>
Read [810465/KB260126] for more information.<br>',
      p_success_msg            => 'If you do not want the Receipts to inherit the BR transaction number, you need to setup a document sequence for the BR Receipt Method.<br>
<br>
See Document Sequence section of [1401636/KB163596], for your setup checks.<br>',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC7_RCT_INHERIT_INV_NUM_BR');



debug('begin add_signature: AR_DS_SEC7_RCT_RECEIPT_CLASS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '1660',
      p_sig_id                 => 'AR_DS_SEC7_RCT_RECEIPT_CLASS',
      p_sig_sql                => 'select
 a.NAME
FROM AR_RECEIPT_CLASSES   a,
     AR_RECEIPT_METHODS c
  WHERE c.name = ''##$$METHOD$$##''
   AND a.receipt_class_id = c.receipt_class_id
   AND a.creation_method_code = ''MANUAL''',
      p_title                  => 'Receipt Class',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Receipt Class details impacting Document Sequencing.',
      p_solution               => 'The <b>Receipt Method</b> is Automatic.<br>
Receipt numbers can be generated automatically only in the case of Automatic Receipts.<br>
<br>
The driving factor for this is the setup of Receipt Source.<br>
For Manual receipts the receipt number cannot be automatic, it has to be entered manually.<br>
<br>
Use the following steps to review:<br>
<ol>
<li><b>Responsibility:</b> Receivables</li>
<li><b>Navigation:</b> Setup > Receipts > Receipt Classes</li>
</ol>',
      p_success_msg            => 'The <b>Receipt Method</b> is Manual.<br>
Receipt numbers can be generated automatically only in the case of Automatic Receipts.<br>
<br>
The driving factor for this is the setup of Receipt Source.<br>
For Manual receipts the receipt number cannot be automatic, it has to be entered manually.<br>
...
<li><b>Navigation:</b> Setup > Receipts > Receipt Classes</li>
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC7_RCT_RECEIPT_CLASS');



debug('begin add_signature: AR_DS_SEC7_RCT_INHERIT_INV_NUM');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '2389',
      p_sig_id                 => 'AR_DS_SEC7_RCT_INHERIT_INV_NUM',
      p_sig_sql                => 'select nvl(RECEIPT_INHERIT_INV_NUM_FLAG,''N'')
from AR_RECEIPT_METHODS
where name = ''##$$METHOD$$##''
and RECEIPT_INHERIT_INV_NUM_FLAG = ''Y''',
      p_title                  => 'Receipts Inherit Invoice Numbers Setup',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'You can ensure that the automatic receipt''s transaction number is the same as the transaction (document sequence) to which it is applied.',
      p_solution               => 'You do not have <i>Receipts Inherit Invoice Numbers</i> checked.<br>
<br>
To help you track receipts that the Automatic Receipts program creates, you can ensure that the automatic receipt''s number is the same as the number of the transaction (the document sequence number) to which it is applied.<br>
<br>
To do this, set the Receipt Inherit Invoice Number option to Yes when defining the receipt class for your automatic receipt.<br>
<ol>
<li><b>Responsibility:</b> Receivables</li>
<li><b>Navigation:</b>  Setup > Receipts > Receipt Classes</li>
<li>If the Receipt Inherit Invoice Number option is set to No, the Automatic Receipts program automatically generates a unique automatic receipt number.</li>
</ol>
<b>NOTE:</b> The Receipt Inherit Invoice Number option affects only receipts created by the Automatic Receipts program.<br>
Read [1401636/KB163596#Inherit] for more information<br>',
      p_success_msg            => 'You have <i>Receipts Inherit Invoice Numbers</i> checked.<br>
<br>
The Automatic Receipts program creates the automatic receipt''s number to be the same as the number of the transaction (the document sequence number) to which it is applied.<br>',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_SEC7_RCT_INHERIT_INV_NUM');
...
debug('begin add_signature: AR_DS_TRX_DETAIL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12553',
      p_sig_id                 => 'AR_DS_TRX_DETAIL',
      p_sig_sql                => 'select trx.customer_trx_id, trx.trx_number,  trx.trx_date, trx.cust_trx_type_id, 
       typ.name, trx.doc_sequence_id, trx.doc_sequence_value, trx.complete_flag
from ra_customer_trx trx, ra_cust_trx_types typ
where trx.customer_trx_id = ##$$TRXID$$##
and trx.cust_trx_type_id = typ.cust_trx_type_id',
      p_title                  => 'Transaction Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate transaction.',
      p_solution               => 'The transaction you provided was not found in the RA_CUSTOMER_TRX table.
Please check that you provided a correct value.

For more information on the parameter values required, please Read [2021711/KB627733] for more information.',
      p_success_msg            => 'Displaying transaction details.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_TRX_DETAIL');



debug('begin add_signature: AR_DS_TRX_TYPE_DETAIL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12555',
      p_sig_id                 => 'AR_DS_TRX_TYPE_DETAIL',
      p_sig_sql                => 'select cust_trx_type_id, name, description, creation_date, type, start_date, end_date
from ra_cust_trx_types 
where cust_trx_type_id = ##$$TRXTYPEID$$##',
      p_title                  => 'Transaction Type Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate transaction type.',
      p_solution               => 'The transaction type you provided was not found in the RA_CUST_TRX_TYPES table.
Please check that you provided a correct value.

For more information on the parameter values required, please Read [2021711/KB627733] for more information.',
      p_success_msg            => 'Displaying transaction type details.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_TRX_TYPE_DETAIL');



debug('begin add_signature: AR_DS_RCT_DETAIL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12554',
      p_sig_id                 => 'AR_DS_RCT_DETAIL',
      p_sig_sql                => 'select cr.cash_receipt_id, cr.receipt_number, to_char(cr.receipt_date,''DD-MON-YYYY''), cr.receipt_method_id,
       method.name, cr.doc_sequence_id, cr.doc_sequence_value
from AR_CASH_RECEIPTS cr, AR_RECEIPT_METHODS method
where method.receipt_method_id = cr.receipt_method_id
and cash_receipt_id = ##$$CASHRECEIPTID$$##',
      p_title                  => 'Receipt Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate receipt.
',
      p_solution               => 'The receipt you provided was not found in the AR_CASH_RECEIPTS table.
Please check that you provided a correct value.

For more information on the parameter values required, please Read [2021711/KB627733] for more information.',
      p_success_msg            => 'Displaying receipt details.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_RCT_DETAIL');



debug('begin add_signature: AR_DS_RCT_METHOD_DETAIL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12556',
      p_sig_id                 => 'AR_DS_RCT_METHOD_DETAIL',
      p_sig_sql                => 'select rm.receipt_method_id, rm.name, rc.receipt_class_id, rc.name, rm.creation_date, rm.start_date, rm.end_date
from ar_receipt_methods rm, ar_receipt_classes rc
where rm.receipt_method_id = ##$$METHODID$$##
and rc.receipt_class_id = rm.receipt_class_id',
      p_title                  => 'Receipt Method Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate receipt method.',
      p_solution               => 'The receipt method you provided was not found in the AR_RECEIPT_METHODS table.
Please check that you provided a correct value.

For more information on the parameter values required, please Read [2021711/KB627733] for more information.',
      p_success_msg            => 'Displaying receipt method details.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_DS_RCT_METHOD_DETAIL');
...
BEGIN
  ...
  analyzer_title := 'Receivables Document Sequence Analyzer';
...
debug('begin section: SYSTEM_OPTIONS');
start_section('System Options Settings', 'SYSTEM_OPTIONS');
   IF l_gen_lev is null THEN
      set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_NULL'));
   END IF;
   IF l_gen_lev is not null THEN
      set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_SAVED'));
      set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_COMPLETED'));
   END IF;
   IF g_trx_type_class = 'BR' THEN
      set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_DOC_SEQ_GEN_LEVEL_BR'));
   END IF;
   set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_CHANGE_PRINTED_INVOICE'));
   set_item_result(run_stored_sig('AR_DS_SEC4_SYSTEM_OPTIONS_DELETE_INVOICE'));
end_section;
debug('end section: SYSTEM_OPTIONS');

debug('begin section: ID_DETAILS');
IF g_customer_trx_id is not null OR g_cash_receipt_id is not null OR
g_cust_trx_type_id is not null OR g_receipt_method_id is not null THEN
   start_section('Details for ' || g_doc_string || '', 'ID_DETAILS');
      IF g_customer_trx_id is not null THEN
         set_item_result(run_stored_sig('AR_DS_TRX_DETAIL'));
      END IF;
      IF g_cust_trx_type_id is not null THEN
         set_item_result(run_stored_sig('AR_DS_TRX_TYPE_DETAIL'));
      END IF;
      IF g_cash_receipt_id is not null THEN
         set_item_result(run_stored_sig('AR_DS_RCT_DETAIL'));
      END IF;
      IF g_receipt_method_id is not null THEN
         set_item_result(run_stored_sig('AR_DS_RCT_METHOD_DETAIL'));
      END IF;
   end_section;
END IF;
debug('end section: ID_DETAILS');

debug('begin section: TRX_OR_RECEIPT');
start_section('Specific Document Sequence Setup for ' || g_doc_string || '', 'TRX_OR_RECEIPT');
   set_item_result(run_stored_sig('AR_DS_SEC5_FDS_SEQ'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDSA_SOBID'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDS_APPID'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDS_CACHE'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDSC_CATEGORY'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDSA_ASSIGNMENT'));
   set_item_result(run_stored_sig('AR_DS_SEC5_FDSA_APPID'));
   IF g_customer_trx_id is NOT NULL THEN
      set_item_result(run_stored_sig('AR_DS_SEC5_FDSA_TRX_DATES'));
      set_item_result(run_stored_sig('AR_DS_SEC5_FDS_DATES'));
      set_item_result(run_stored_sig('AR_DS_SEC6_TRX_BATCH_SOURCE'));
   END IF;
   IF g_cash_receipt_id is not null THEN
      set_item_result(run_stored_sig('AR_DS_SEC5_FDSA_RCT_DATES'));
      set_item_result(run_stored_sig('AR_DS_SEC5_FDS_DATES'));
      IF g_creation_method_code = 'BR_REMIT'  THEN
         set_item_result(run_stored_sig('AR_DS_SEC7_RCT_INHERIT_INV_NUM_BR'));
      END IF;
      IF g_creation_method_code <> 'BR_REMIT' THEN
         set_item_result(run_stored_sig('AR_DS_SEC7_RCT_RECEIPT_CLASS'));
         set_item_result(run_stored_sig('AR_DS_SEC7_RCT_INHERIT_INV_NUM'));
      END IF;
   END IF;
end_section;
debug('end section: TRX_OR_RECEIPT');

debug('begin section: GENERAL_DOC_SEQ_INFO');
start_section('General Document Sequence Setup', 'GENERAL_DOC_SEQ_INFO');
   set_item_result(run_stored_sig('AR_DS_FND_DOC_SEQUENCE_CATEGORIES'));
   set_item_result(run_stored_sig('AR_DS_FND_DOC_SEQUENCE_ASSIGNMENTS'));
   set_item_result(run_stored_sig('AR_DS_FND_DOC_SEQUENCE_USERS'));
   set_item_result(run_stored_sig('AR_DS_FND_DOCUMENT_SEQUENCES'));
   set_item_result(run_stored_sig('EBS_FIN_AR_ARDOCSEQ_AR_DS_FND_DOCUMENT_SEQUENCES_GAPLESS'));
   set_item_result(run_stored_sig('AR_DS_DBA_SEQUENCES'));
   set_item_result(run_stored_sig('AR_DS_CHECK_MISSING_CATEGORY'));
   set_item_result(run_stored_sig('AR_DS_SEC6_CATEG_SIZE_CHECK'));
end_section;
debug('end section: GENERAL_DOC_SEQ_INFO');

debug('begin section: PROFILE_OPTIONS_SETUP');
start_section('Profile Options Setup', 'PROFILE_OPTIONS_SETUP');
   IF g_prof_resp = 'Y' THEN
      set_item_result(run_stored_sig('AR_DS_SEC3_RESP_DOC_PROF_NOT_SET'));
      set_item_result(run_stored_sig('AR_DS_SEC3_RESP_DOC_PROF_PARTIAL'));
      set_item_result(run_stored_sig('AR_DS_SEC3_RESP_DOC_PROF_ALWAYS'));
   END IF;
   IF g_prof_app = 'Y' THEN
      set_item_result(run_stored_sig('AR_DS_SEC3_APP_DOC_PROF_NOT_SET'));
      set_item_result(run_stored_sig('AR_DS_SEC3_APP_DOC_PROF_PARTIAL'));
      set_item_result(run_stored_sig('AR_DS_SEC3_APP_DOC_PROF_ALWAYS'));
   END IF;
   IF g_prof_site = 'Y' THEN
      set_item_result(run_stored_sig('AR_DS_SEC3_SITE_DOC_PROF_NOT_SET'));
      set_item_result(run_stored_sig('AR_DS_SEC3_SITE_DOC_PROF_PARTIAL'));
      set_item_result(run_stored_sig('AR_DS_SEC3_SITE_DOC_PROF_ALWAYS'));
      set_item_result(run_stored_sig('AR_DS_SEC3_SITE_JOURNAL_ENTRIES_SETUP'));
   END IF;
end_section;
debug('end section: PROFILE_OPTIONS_SETUP');

debug('begin section: AR_INVALID_OBJECTS');
start_section('AR Invalid Objects', 'AR_INVALID_OBJECTS');
   set_item_result(run_stored_sig('AR_DS_SEC1_AR_INVALIDS'));
end_section;
debug('end section: AR_INVALID_OBJECTS');

debug('begin section: RECOMMENDED_PATCHES');
IF substr(g_apps_version,1,4) = '12.1' THEN
   start_section('AR Recommended Patches', 'RECOMMENDED_PATCHES');
      IF g_bug_number > 0 and substr(g_apps_version,1,4) = '12.1' THEN
         set_item_result(check_rec_patches_1); /* Signature AR_DS_SEC2_PATCH_REC_ACCT-RECON */
         set_item_result(check_rec_patches_2); /* Signature AR_DS_SEC2_PATCH_REC_FUNDS_CAPTURE */
         set_item_result(check_rec_patches_3); /* Signature AR_DS_SEC2_PATCH_REC_NO_RPC */
         set_item_result(check_rec_patches_4); /* Signature AR_DS_SEC2_PATCH_REC_RECEIPTS */
         set_item_result(check_rec_patches_5); /* Signature AR_DS_SEC2_PATCH_REC_TRANSACTIONS */
      END IF;
      set_item_result(run_stored_sig('AR_DS_SEC2_PATCH_REC_RPC_APPLIED'));
   end_section;
END IF;
...
IF p_apps_check = 'Y' THEN
   start_section('Apps Check Information', 'APPS_CHECK');
