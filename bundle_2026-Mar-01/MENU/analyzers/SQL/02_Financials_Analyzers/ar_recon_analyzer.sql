...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Receivables Reconciliation Analyzer    |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE ar_recon_analyzer_pkg AUTHID DEFINER AS
...
(            p_user_name                    IN VARCHAR2    DEFAULT NULL
           ...
           ,p_gl_end_date                  IN DATE        DEFAULT NULL
           ,p_pot_recon_items              IN VARCHAR2    DEFAULT 'Y'
           ,p_xla_gl                       IN VARCHAR2    DEFAULT 'N'
...
PROCEDURE main_cp (
           ...
           ,p_resp_id                      IN NUMBER      DEFAULT NULL
           ,p_org_id                       IN VARCHAR2    DEFAULT NULL
           ,p_gl_start_date                IN VARCHAR2    DEFAULT NULL
           ,p_gl_end_date                  IN VARCHAR2    DEFAULT NULL
           ,p_pot_recon_items              IN VARCHAR2    DEFAULT 'Y'
           ,p_xla_gl                       IN VARCHAR2    DEFAULT 'N'
           ,p_apps_check                   IN VARCHAR2    DEFAULT 'N'
           ,p_max_output_rows              IN NUMBER      DEFAULT 5000
...
-- Reconciliation by GL Date
---------------------------------

procedure recon_by_gl_date  (gldate in date);
function get_status (i in number) return varchar2;
function get_gl_date (i in number) return date;
function get_begin_age_acctd_amt (i in number) return number;
function get_trx_reg_acctd_amt (i in number) return number;
function get_unapp_reg_acctd_amt (i in number) return number;
function get_app_reg_acctd_amt (i in number) return number;
function get_adj_reg_acctd_amt (i in number) return number;
function get_cm_gain_loss (i in number) return number;
function get_inv_exp_acctd_amt (i in number) return number;
function get_rounding_diff (i in number) return number;
function get_end_age_acctd_amt (i in number) return number;
function get_difference (i in number) return number;

-- Reconciliation by Transaction
---------------------------------
procedure reset_trx_rec_t;
function calc_amount_due_original_inv (l_cust_trx_id in NUMBER) return number;
function calc_amount_due_rem_inv (l_cust_trx_id in NUMBER) return number;
function calc_amount_app_adj_inv (l_cust_trx_id in NUMBER) return number;
function get_cust_trx_id (j in number) return number;
function get_ado (j in number) return number;
function get_adr (j in number) return number;
function get_app_adj (j in number) return number;
function get_trx_diff (j in number) return number;


-- Reconciliation by Receipt
---------------------------------
procedure reset_rct_rec_t;
function get_cr_id (i in number) return number;
function get_rec_id (i in number) return number;
function get_rev_rec_id (i in number) return number;


END ar_recon_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY ar_recon_analyzer_pkg AS
-- $Id: ar_recon_analyzer.sql, 200.26 2025/12/08 13:47:00 kgnanase Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2255895.1));
...
----------------------------------------------------------------
  ...
  g_user_name_temp              VARCHAR2(100) := 0;
  ...
  TYPE daily_calc_tab IS TABLE OF daily_calc_rec INDEX BY BINARY_INTEGER;

  daily_calc_t daily_calc_tab;
  ...
  cursor inv_cur(l_gl_date date) is 
    ...
    where  gl_dist.gl_date = l_gl_date
  ...
  cursor get_cr_ids(l_gl_date date) is
    select distinct cash_receipt_id
    from ar_receivable_applications
    where gl_date = l_gl_date;
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'AR_RECON_ANALYZER_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'AR_RECON_ANALYZER_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>AR_RECON_ANALYZER Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2255895.1#feedback");
...
-- page title
    print_out('        <div class="header_title">'||p_analyzer_title|| ' Report <br>
        ...
        <img class="header_version" src="https://www.oracle.com/webfolder/s/analyzer/ar_recon_latest_version.gif" title="Click here to download the latest version of Analyzer" alt="Latest Version Icon"></a>');
    print_cloud_image;
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1093,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_up_lg" title="Analyzer Feedback: Thumbs Up"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1093,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_dn_lg" title="Analyzer Feedback: Thumbs Down"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1093,EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="af_idea_lg" title="Analyzer Idea"></span></a>');
    print_out('        </div>
    </div>');
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/4040068" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">1093</span>
...
<p>What&apos;s new:</p><ol><li>Modified below signature<ul><li>Transactions with Same CCID for REC and REV Acounts</li>
<li>Transactions with REV Account same as REC account.</li>
<li>&nbsp;Potential Reconciling Items</li>
<li>Applied GL_DATE &lt; Transactions GL_DATE</li>
<li>Mismatched GL_DATE in RA_CUST_TRX_LINE_GL_DIST_ALL and EVENT_DATE in XE_EVENTS</li>
<li>Transactions for GL Date ##$$LSTRDATE$$##</li>
<li>Reconciling Issues Identified</li>
<li>Receipts with Incorrect Reversal GL Date ##$$LSTRDATE$$##</li>
<li>AR_PAYMENT_SCHEDULES_ALL Data Exists for Transactions with ACCOUNTING_AFFECT_FLAG = N</li>
<li>Adjustments in Functional Currency with &nbsp;AMOUNT &lt;&gt; ACCTD_AMOUNT</li>
</ul>
</li>
</ol>
<p>&nbsp;</p>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1093,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1093,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1093,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Receivables" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
----------------------------------------------------------------
-- Procedures and Functions to reconcile by GL Date
------------------------------------------------------
procedure recon_by_gl_date (gldate in date) is

begin

      l_str_date := to_char(gldate,'DD-MON-YYYY');
      debug('l_str_date = ' || l_str_date);

      ar_calc_aging.aging_as_of (gldate-1 ,
                                 gldate,
                                 '3000',
                                 g_org_id_tmp,
                                 NULL,
      ...
      ar_calc_aging.transaction_register(gldate ,
                                         gldate ,
                                         '3000',
                                         g_org_id_tmp,
                                         NULL,
      ...
      ar_calc_aging.cash_receipts_register(gldate,
                                           gldate,
                                           '3000',
                                           g_org_id_tmp,
                                           NULL,
         ...
         l_unapp_reg_amt  := nvl(l_unapp_amt,0) + 
                             nvl(l_acc_amt,0) + 
                             nvl(l_claim_amt,0) + 
                             nvl(l_prepay_amt,0);

         l_unapp_reg_acctd_amt := nvl(l_unapp_acctd_amt,0) + 
                                  nvl(l_acc_acctd_amt,0) + 
                                  nvl(l_claim_acctd_amt,0) + 
                                  nvl(l_prepay_acctd_amt,0);

         l_app_reg_amt  := nvl(l_app_amt,0) + 
                           nvl(l_edisc_amt,0) +
                           nvl(l_unedisc_amt,0) + 
                           nvl(l_on_acc_cm_ref_amt,0);

         l_app_reg_acctd_amt := nvl(l_app_acctd_amt,0) + 
                                nvl(l_edisc_acctd_amt,0) +
                                nvl(l_unedisc_acctd_amt,0) -
                                nvl(l_on_acc_cm_ref_acctd_amt,0);

         ar_calc_aging.adjustment_register(gldate,
                                           gldate,
                                           '3000',
                                           g_org_id_tmp,
                                           NULL,
         ...
         ar_calc_aging.invoice_exceptions(gldate,
                                          gldate,
                                          '3000',
                                          g_org_id_tmp,
                                          NULL,
        ...
        l_period_total_acctd_amt := (l_begin_age_acctd_amt + 
                                     l_trx_reg_acctd_amt -
        ...
        IF l_period_total_acctd_amt =  l_end_age_acctd_amt THEN

           debug('Figures Match for '||to_char(gldate));
           debug('---------------------------');
           daily_calc_t(i).l_status := 'Figures Match';
           daily_calc_t(i).l_gl_date := gldate;

        ELSE
           daily_calc_t(i).l_status := 'Figures Do Not Match';
           daily_calc_t(i).l_gl_date := gldate;
           daily_calc_t(i).l_begin_age_acctd_amt := l_begin_age_acctd_amt;
           daily_calc_t(i).l_trx_reg_acctd_amt := l_trx_reg_acctd_amt;
           daily_calc_t(i).l_unapp_reg_acctd_amt := l_unapp_reg_acctd_amt;
           daily_calc_t(i).l_app_reg_acctd_amt := l_app_reg_acctd_amt;
           daily_calc_t(i).l_adj_reg_acctd_amt := l_adj_reg_acctd_amt;
           daily_calc_t(i).l_cm_gain_loss := l_cm_gain_loss;
           daily_calc_t(i).l_inv_exp_acctd_amt := l_inv_exp_acctd_amt;
           daily_calc_t(i).l_rounding_diff := l_rounding_diff;
           daily_calc_t(i).l_end_age_acctd_amt := l_end_age_acctd_amt; 
           daily_calc_t(i).l_difference := l_period_total_acctd_amt-l_end_age_acctd_amt;

           debug('Figures do not match for '||to_char(gldate));
           ...
           debug('Difference = '||to_char(l_period_total_acctd_amt-l_end_age_acctd_amt));  

        END IF;
end;

function get_status (i in number) return varchar2 is
retvalue varchar2(20);
begin
  retvalue := daily_calc_t(i).l_status;

  return(retvalue);
end;

function get_gl_date (i in number) return date is
retvalue date;
begin
  retvalue := daily_calc_t(i).l_gl_date;

  return(retvalue);
end;

function get_begin_age_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_begin_age_acctd_amt;

  return(retvalue);
end;

function get_trx_reg_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_trx_reg_acctd_amt;

  return(retvalue);
end;

function get_unapp_reg_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_unapp_reg_acctd_amt;

  return(retvalue);
end;

function get_app_reg_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_app_reg_acctd_amt;

  return(retvalue);
end;

function get_adj_reg_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_adj_reg_acctd_amt;

  return(retvalue);
end;

function get_cm_gain_loss (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_cm_gain_loss;

  return(retvalue);
end;

function get_inv_exp_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_inv_exp_acctd_amt;

  return(retvalue);
end;

function get_rounding_diff (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_rounding_diff;

  return(retvalue);
end;

function get_end_age_acctd_amt (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_end_age_acctd_amt;

  return(retvalue);
end;

function get_difference (i in number) return number is
retvalue number;
begin
  retvalue := daily_calc_t(i).l_difference;

  return(retvalue);
end;

-- Procedures and Functions to reconcile Transactions
------------------------------------------------------

function calc_amount_due_original_inv (l_cust_trx_id in NUMBER) return number is
begin

   select sum(decode(account_class,'REC', decode(latest_rec_flag,'Y',acctd_amount,0),0)) 
   into l_amount_due_original_inv
   from ra_cust_trx_line_gl_dist
   where customer_trx_id = l_cust_trx_id;

   g_sql_tokens('##$$TRX_ADO$$##') := nvl(l_amount_due_original_inv,0);

   return nvl(l_amount_due_original_inv,0);
end;

function calc_amount_due_rem_inv (l_cust_trx_id in NUMBER) return number is
begin
   select sum(acctd_amount_due_remaining)
   into l_amount_due_rem_inv
   from ar_payment_schedules
   where customer_trx_id = l_cust_trx_id;

   g_sql_tokens('##$$TRX_REM$$##') := nvl(l_amount_due_rem_inv,0);

   return nvl(l_amount_due_rem_inv,0);
end;

function calc_amount_app_adj_inv (l_cust_trx_id in NUMBER) return number is
begin
   ...
   for ps_rec in ps_cur(l_cust_trx_id) loop
       ...
       select sum(acctd_amount_applied_from + nvl(acctd_earned_discount_taken,0) + 
       ...
       select sum(-(acctd_amount_applied_to + nvl(acctd_earned_discount_taken,0) +
              nvl(acctd_unearned_discount_taken,0))) 
       ...
       l_amount_app_adj_inv := nvl(l_amount_app_adj_inv,0) + nvl(l_amount_applied_from,0) + 
                               nvl(l_amount_applied_to,0) + nvl(l_amount_adjusted,0) + 
                               nvl(l_cm_refund,0);
   end loop;


   g_sql_tokens('##$$TRX_APP$$##') := nvl(l_amount_app_adj_inv,0);


   return nvl(l_amount_app_adj_inv,0);
end;


-- Procedures and Functions for reconciling Receipts
----------------------------------------------------

procedure reset_trx_rec_t is
begin

   for k in 1..30 loop
      trx_rec_t(k).l_cust_trx_id := null;
      trx_rec_t(k).l_amount_due_original_inv := null;
      trx_rec_t(k).l_amount_due_rem_inv := null;
      trx_rec_t(k).l_amount_app_adj_inv := null;
      trx_rec_t(k).l_trx_diff := null;
   end loop;
end;

function get_cust_trx_id (j in number) return number is
retvalue number;
begin
  retvalue := trx_rec_t(j).l_cust_trx_id;

  return(retvalue);
end;

function get_ado (j in number) return number is
retvalue number;
begin
  retvalue := trx_rec_t(j).l_amount_due_original_inv;

  return(retvalue);
end;


function get_adr (j in number) return number is
retvalue number;
begin
  retvalue := trx_rec_t(j).l_amount_due_rem_inv;

  return(retvalue);
end;

function get_app_adj (j in number) return number is
retvalue number;
begin
  retvalue := trx_rec_t(j).l_amount_app_adj_inv;

  return(retvalue);
end;

function get_trx_diff (j in number) return number is
retvalue number;
begin
  retvalue := trx_rec_t(j).l_trx_diff;
...
procedure reset_rct_rec_t is
begin

   for k in 1..30 loop
      rct_rec_t(k).l_cr_id := null;
      rct_rec_t(k).l_rec_id := null;
      rct_rec_t(k).l_rev_rec_id := null;
   end loop;
end;
  

function get_cr_id (i in number) return number is
retvalue number;
begin
  retvalue := rct_rec_t(i).l_cr_id;

  return(retvalue);
end;

function get_rec_id (i in number) return number is
retvalue number;
begin
  retvalue := rct_rec_t(i).l_rec_id;

  return(retvalue);
end;

function get_rev_rec_id (i in number) return number is
retvalue number;
begin
  retvalue := rct_rec_t(i).l_rev_rec_id;
...
FUNCTION val_p_gl_end_date
   return VARCHAR2 IS
BEGIN
debug('begin parameter validation function: p_gl_end_date');
IF (p_gl_start_date <= p_gl_end_date) THEN
return 'Y';
ELSE
return 'N';
END IF;
debug('end parameter validation function: p_gl_end_date');
END val_p_gl_end_date;
...
debug('begin populate parameters hash table');
   ...
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Start GL Date';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_gl_start_date,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'End GL Date';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_gl_end_date,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Scan for Potential Reconciling Items';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_pot_recon_items,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Scan for XLA to GL Integrity';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_xla_gl,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2025/12/08 13:46:59 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'ar_recon_analyzer.sql';
...
INTO l_exists_val
FROM HR_OPERATING_UNITS O, AR_SYSTEM_PARAMETERS_ALL AR
WHERE O.ORGANIZATION_ID = p_org_id
 AND ar.org_id = o.organization_id
and (mo_global.is_mo_init_done = 'N' or mo_global.check_access(o.organization_id) = 'Y')

 and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Error: Invalid Organization.');
   raise invalid_parameters;
   WHEN OTHERS THEN
...
debug('begin parameter validation: p_gl_start_date');
IF p_gl_start_date IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Start GL Date is required.');
   raise invalid_parameters;
END IF;
debug('end parameter validation: p_gl_start_date');

debug('begin parameter validation: p_gl_end_date');
IF p_gl_end_date IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter End GL Date is required.');
   raise invalid_parameters;
END IF;
IF p_gl_end_date IS NOT NULL AND val_p_gl_end_date()='N' THEN
   print_error('INVALID ARGUMENT: Error: Please enter a valid gl_date range.
');
   raise invalid_parameters;
END IF;
l_system_function_var := null;
debug('end parameter validation: p_gl_end_date');

debug('begin parameter validation: p_pot_recon_items');
IF p_pot_recon_items IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Scan for Potential Reconciling Items is required.');
   raise invalid_parameters;
END IF;
IF p_pot_recon_items IS NOT NULL AND p_pot_recon_items NOT IN ( 'N','Y') THEN
   print_error('INVALID ARGUMENT: Invalid value for Scan for Potential Reconciling Items, valid values are Y or N.');
   raise invalid_parameters;
END IF;
debug('end parameter validation: p_pot_recon_items');

debug('begin parameter validation: p_xla_gl');
IF p_xla_gl IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Scan for XLA to GL Integrity is required.');
   raise invalid_parameters;
END IF;
IF p_xla_gl IS NOT NULL AND p_xla_gl NOT IN ( 'N','Y') THEN
   print_error('INVALID ARGUMENT: Invalid value for Scan for XLA to GL Integrity value, valid values are Y or N.');
   raise invalid_parameters;
END IF;
debug('end parameter validation: p_xla_gl');
...
begin
   ...
   -- we need to define separate _cp parameters to do a different kind of validation when
   -- the analyzers are run as a concurrent program

   g_org_id_tmp := p_org_id;
   g_user_name_temp := p_user_name;
   g_resp_id_tmp := p_resp_id;

   print_log('Setting Org Context to ' || g_org_id_tmp);

   mo_global.init('AR');
   mo_global.set_policy_context('S', g_org_id_tmp);


   -- initialize everything to N
   g_mfar := 'N';
   g_batch_line_level := 'N';
   g_cash_basis := 'N';
   g_create_detailed := 'N';
   g_show_detaildist_sig := 'Y';
  ...
  exception
   ...
   g_apps_ver := substr(g_apps_ver,1,4);


   select ar.org_id, ar.set_of_books_id, nvl(ar.create_detailed_dist_flag,'Y'), hr.name
   into g_org_id, g_ledger_id, g_create_detailed, g_ou_name
   ...
   -- where  start_date = g_start_date_tmp
   -- and    end_date = g_end_date_tmp
   -- and    set_of_books_id = g_ledger_id 
   -- and    application_id = 222;

   -- review settings associated to Create Detailed Distributions

   begin

      -- Check #1: is MFAR used?
      select nvl(FND_PROFILE.value('AR_MFAR_ACTIVATED'),'N')
      into   g_mfar
      from   dual;

      -- Check #2: Any batch sources use Line Level Balances
      select 'Y'
      into   g_batch_line_level
      from   dual
      where  exists (select 'Y' from ra_batch_sources_all
                     where  org_id = g_org_id
                     and    nvl(gen_line_level_bal_flag,'N') = 'Y');

      -- Check #3: is Accounting Method = Cash Basis?
      select 'Y'
      into   g_cash_basis
      from   gl_ledgers 
      where  nvl(SLA_LEDGER_CASH_BASIS_FLAG,'N') = 'Y'
      and    ledger_id = g_ledger_id;

      -- If ALL of the above are still N at this point, then we want to display
      -- AR_PC_CREATE_DETAILED_DIST
      print_log('g_mfar = ' || g_mfar || ' g_batch_line_level = ' || g_batch_line_level || 
                ' g_cash_basis = ' || g_cash_basis || 
                ' g_create_detailed = ' || g_create_detailed);

      IF g_mfar = 'N' and g_batch_line_level = 'N' and g_cash_basis = 'N' 
         and g_create_detailed = 'Y' THEN
         g_show_detaildist_sig := 'Y';
      END IF;
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$CASHBASIS$$##') := g_cash_basis;
   g_masked_tokens('##$$CASHBASIS$$##') := mask_text( g_sql_tokens('##$$CASHBASIS$$##'), 'NO_MASK');
   g_sql_tokens('##$$LINELEV$$##') := g_batch_line_level;
   g_masked_tokens('##$$LINELEV$$##') := mask_text( g_sql_tokens('##$$LINELEV$$##'), 'NO_MASK');
   g_sql_tokens('##$$MFAR$$##') := g_mfar;
   g_masked_tokens('##$$MFAR$$##') := mask_text( g_sql_tokens('##$$MFAR$$##'), 'NO_MASK');
   g_sql_tokens('##$$RESP_ID$$##') := g_resp_id_tmp;
   g_masked_tokens('##$$RESP_ID$$##') := mask_text( g_sql_tokens('##$$RESP_ID$$##'), 'NO_MASK');
   g_sql_tokens('##$$USERNAME$$##') := g_user_name_temp;
   g_masked_tokens('##$$USERNAME$$##') := mask_text( g_sql_tokens('##$$USERNAME$$##'), 'DISPLAY_BOTH_25_PCNT_WORD');
   g_sql_tokens('##$$ENDDATE$$##') := p_gl_end_date;
   g_masked_tokens('##$$ENDDATE$$##') := mask_text( g_sql_tokens('##$$ENDDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$STARTDATE$$##') := p_gl_start_date;
   g_masked_tokens('##$$STARTDATE$$##') := mask_text( g_sql_tokens('##$$STARTDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$i$$##') := i;
   g_masked_tokens('##$$i$$##') := mask_text( g_sql_tokens('##$$i$$##'), 'NO_MASK');
   g_sql_tokens('##$$GLDATE$$##') := l_start_gl_date;
   g_masked_tokens('##$$GLDATE$$##') := mask_text( g_sql_tokens('##$$GLDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$LSTRDATE$$##') := p_gl_start_date;
   g_masked_tokens('##$$LSTRDATE$$##') := mask_text( g_sql_tokens('##$$LSTRDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$APPSCHECK$$##') := p_apps_check;
   g_masked_tokens('##$$APPSCHECK$$##') := mask_text( g_sql_tokens('##$$APPSCHECK$$##'), 'NO_MASK');
   g_sql_tokens('##$$A$$##') := 'AR_RECON_ANALYZER_PKG';
...
debug('begin add_signature: AR_RECON_BY_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7133',
      p_sig_id                 => 'AR_RECON_BY_TRX',
      p_sig_sql                => 'select
ar_recon_analyzer_pkg.get_cust_trx_id(1) "Customer Trx ID",
ar_recon_analyzer_pkg.get_ado(1) "Amount Due Original",
ar_recon_analyzer_pkg.get_app_adj(1) "Amount Applied or Adjusted",
ar_recon_analyzer_pkg.get_adr(1) "Amount Due Remaining",
ar_recon_analyzer_pkg.get_trx_diff(1) "Difference"
from dual 
where ar_recon_analyzer_pkg.get_cust_trx_id(1) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(2),
ar_recon_analyzer_pkg.get_ado(2),
ar_recon_analyzer_pkg.get_app_adj(2),
ar_recon_analyzer_pkg.get_adr(2),
ar_recon_analyzer_pkg.get_trx_diff(2)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(2) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(3),
ar_recon_analyzer_pkg.get_ado(3),
ar_recon_analyzer_pkg.get_app_adj(3),
ar_recon_analyzer_pkg.get_adr(3),
ar_recon_analyzer_pkg.get_trx_diff(3)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(3) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(4),
ar_recon_analyzer_pkg.get_ado(4),
ar_recon_analyzer_pkg.get_app_adj(4),
ar_recon_analyzer_pkg.get_adr(4),
ar_recon_analyzer_pkg.get_trx_diff(4)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(4) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(5),
ar_recon_analyzer_pkg.get_ado(5),
ar_recon_analyzer_pkg.get_app_adj(5),
ar_recon_analyzer_pkg.get_adr(5),
ar_recon_analyzer_pkg.get_trx_diff(5)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(5) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(6),
ar_recon_analyzer_pkg.get_ado(6),
ar_recon_analyzer_pkg.get_app_adj(6),
ar_recon_analyzer_pkg.get_adr(6),
ar_recon_analyzer_pkg.get_trx_diff(6)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(6) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(7),
ar_recon_analyzer_pkg.get_ado(7),
ar_recon_analyzer_pkg.get_app_adj(7),
ar_recon_analyzer_pkg.get_adr(7),
ar_recon_analyzer_pkg.get_trx_diff(7)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(7) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(8),
ar_recon_analyzer_pkg.get_ado(8),
ar_recon_analyzer_pkg.get_app_adj(8),
ar_recon_analyzer_pkg.get_adr(8),
ar_recon_analyzer_pkg.get_trx_diff(8)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(8) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(9),
ar_recon_analyzer_pkg.get_ado(9),
ar_recon_analyzer_pkg.get_app_adj(9),
ar_recon_analyzer_pkg.get_adr(9),
ar_recon_analyzer_pkg.get_trx_diff(9)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(9) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(10),
ar_recon_analyzer_pkg.get_ado(10),
ar_recon_analyzer_pkg.get_app_adj(10),
ar_recon_analyzer_pkg.get_adr(10),
ar_recon_analyzer_pkg.get_trx_diff(10)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(10) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(11),
ar_recon_analyzer_pkg.get_ado(11),
ar_recon_analyzer_pkg.get_app_adj(11),
ar_recon_analyzer_pkg.get_adr(11),
ar_recon_analyzer_pkg.get_trx_diff(11)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(11) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(12),
ar_recon_analyzer_pkg.get_ado(12),
ar_recon_analyzer_pkg.get_app_adj(12),
ar_recon_analyzer_pkg.get_adr(12),
ar_recon_analyzer_pkg.get_trx_diff(12)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(12) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(13),
ar_recon_analyzer_pkg.get_ado(13),
ar_recon_analyzer_pkg.get_app_adj(13),
ar_recon_analyzer_pkg.get_adr(13),
ar_recon_analyzer_pkg.get_trx_diff(13)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(13) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(14),
ar_recon_analyzer_pkg.get_ado(14),
ar_recon_analyzer_pkg.get_app_adj(14),
ar_recon_analyzer_pkg.get_adr(14),
ar_recon_analyzer_pkg.get_trx_diff(14)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(14) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(15),
ar_recon_analyzer_pkg.get_ado(15),
ar_recon_analyzer_pkg.get_app_adj(15),
ar_recon_analyzer_pkg.get_adr(15),
ar_recon_analyzer_pkg.get_trx_diff(15)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(15) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(16),
ar_recon_analyzer_pkg.get_ado(16),
ar_recon_analyzer_pkg.get_app_adj(16),
ar_recon_analyzer_pkg.get_adr(16),
ar_recon_analyzer_pkg.get_trx_diff(16)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(16) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(17),
ar_recon_analyzer_pkg.get_ado(17),
ar_recon_analyzer_pkg.get_app_adj(17),
ar_recon_analyzer_pkg.get_adr(17),
ar_recon_analyzer_pkg.get_trx_diff(17)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(17) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(18),
ar_recon_analyzer_pkg.get_ado(18),
ar_recon_analyzer_pkg.get_app_adj(18),
ar_recon_analyzer_pkg.get_adr(18),
ar_recon_analyzer_pkg.get_trx_diff(18)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(18) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(19),
ar_recon_analyzer_pkg.get_ado(19),
ar_recon_analyzer_pkg.get_app_adj(19),
ar_recon_analyzer_pkg.get_adr(19),
ar_recon_analyzer_pkg.get_trx_diff(19)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(19) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(20),
ar_recon_analyzer_pkg.get_ado(20),
ar_recon_analyzer_pkg.get_app_adj(20),
ar_recon_analyzer_pkg.get_adr(20),
ar_recon_analyzer_pkg.get_trx_diff(20)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(20) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(21),
ar_recon_analyzer_pkg.get_ado(21),
ar_recon_analyzer_pkg.get_app_adj(21),
ar_recon_analyzer_pkg.get_adr(21),
ar_recon_analyzer_pkg.get_trx_diff(21)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(21) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(22),
ar_recon_analyzer_pkg.get_ado(22),
ar_recon_analyzer_pkg.get_app_adj(22),
ar_recon_analyzer_pkg.get_adr(22),
ar_recon_analyzer_pkg.get_trx_diff(22)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(22) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(23),
ar_recon_analyzer_pkg.get_ado(23),
ar_recon_analyzer_pkg.get_app_adj(23),
ar_recon_analyzer_pkg.get_adr(23),
ar_recon_analyzer_pkg.get_trx_diff(23)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(23) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(24),
ar_recon_analyzer_pkg.get_ado(24),
ar_recon_analyzer_pkg.get_app_adj(24),
ar_recon_analyzer_pkg.get_adr(24),
ar_recon_analyzer_pkg.get_trx_diff(24)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(24) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(25),
ar_recon_analyzer_pkg.get_ado(25),
ar_recon_analyzer_pkg.get_app_adj(25),
ar_recon_analyzer_pkg.get_adr(25),
ar_recon_analyzer_pkg.get_trx_diff(25)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(25) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(26),
ar_recon_analyzer_pkg.get_ado(26),
ar_recon_analyzer_pkg.get_app_adj(26),
ar_recon_analyzer_pkg.get_adr(26),
ar_recon_analyzer_pkg.get_trx_diff(26)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(26) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(27),
ar_recon_analyzer_pkg.get_ado(27),
ar_recon_analyzer_pkg.get_app_adj(27),
ar_recon_analyzer_pkg.get_adr(27),
ar_recon_analyzer_pkg.get_trx_diff(27)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(27) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(28),
ar_recon_analyzer_pkg.get_ado(28),
ar_recon_analyzer_pkg.get_app_adj(28),
ar_recon_analyzer_pkg.get_adr(28),
ar_recon_analyzer_pkg.get_trx_diff(28)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(28) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(29),
ar_recon_analyzer_pkg.get_ado(29),
ar_recon_analyzer_pkg.get_app_adj(29),
ar_recon_analyzer_pkg.get_adr(29),
ar_recon_analyzer_pkg.get_trx_diff(29)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(29) is not null
union
select
ar_recon_analyzer_pkg.get_cust_trx_id(30),
ar_recon_analyzer_pkg.get_ado(30),
ar_recon_analyzer_pkg.get_app_adj(30),
ar_recon_analyzer_pkg.get_adr(30),
ar_recon_analyzer_pkg.get_trx_diff(30)
from dual
where ar_recon_analyzer_pkg.get_cust_trx_id(30) is not null',
      p_title                  => 'Transactions for GL Date ##$$LSTRDATE$$##',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the first 30 transactions for ##$$LSTRDATE$$## that have potential reconciling issues.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID values listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Period Close Analyzer output.</li> 
</ul>
',
      p_success_msg            => 'No transactions were identified as having potential reconciling issues.',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_BY_TRX');



debug('begin add_signature: AR_RECON_BY_RCT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7168',
      p_sig_id                 => 'AR_RECON_BY_RCT',
      p_sig_sql                => 'select
ar_recon_analyzer_pkg.get_cr_id(1) "Cash Receipt ID",
ar_recon_analyzer_pkg.get_rec_id(1) "Amount Due Original",
ar_recon_analyzer_pkg.get_rev_rec_id(1) "Amount Applied or Adjusted"
from dual 
where ar_recon_analyzer_pkg.get_cr_id(1) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(2),
ar_recon_analyzer_pkg.get_rec_id(2),
ar_recon_analyzer_pkg.get_rev_rec_id(2)
from dual
where ar_recon_analyzer_pkg.get_cr_id(2) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(3),
ar_recon_analyzer_pkg.get_rec_id(3),
ar_recon_analyzer_pkg.get_rev_rec_id(3)
from dual
where ar_recon_analyzer_pkg.get_cr_id(3) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(4),
ar_recon_analyzer_pkg.get_rec_id(4),
ar_recon_analyzer_pkg.get_rev_rec_id(4)
from dual
where ar_recon_analyzer_pkg.get_cr_id(4) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(5),
ar_recon_analyzer_pkg.get_rec_id(5),
ar_recon_analyzer_pkg.get_rev_rec_id(5)
from dual
where ar_recon_analyzer_pkg.get_cr_id(5) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(6),
ar_recon_analyzer_pkg.get_rec_id(6),
ar_recon_analyzer_pkg.get_rev_rec_id(6)
from dual
where ar_recon_analyzer_pkg.get_cr_id(6) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(7),
ar_recon_analyzer_pkg.get_rec_id(7),
ar_recon_analyzer_pkg.get_rev_rec_id(7)
from dual
where ar_recon_analyzer_pkg.get_cr_id(7) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(8),
ar_recon_analyzer_pkg.get_rec_id(8),
ar_recon_analyzer_pkg.get_rev_rec_id(8)
from dual
where ar_recon_analyzer_pkg.get_cr_id(8) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(9),
ar_recon_analyzer_pkg.get_rec_id(9),
ar_recon_analyzer_pkg.get_rev_rec_id(9)
from dual
where ar_recon_analyzer_pkg.get_cr_id(9) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(10),
ar_recon_analyzer_pkg.get_rec_id(10),
ar_recon_analyzer_pkg.get_rev_rec_id(10)
from dual
where ar_recon_analyzer_pkg.get_cr_id(10) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(11),
ar_recon_analyzer_pkg.get_rec_id(11),
ar_recon_analyzer_pkg.get_rev_rec_id(11)
from dual
where ar_recon_analyzer_pkg.get_cr_id(11) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(12),
ar_recon_analyzer_pkg.get_rec_id(12),
ar_recon_analyzer_pkg.get_rev_rec_id(12)
from dual
where ar_recon_analyzer_pkg.get_cr_id(12) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(13),
ar_recon_analyzer_pkg.get_rec_id(13),
ar_recon_analyzer_pkg.get_rev_rec_id(13)
from dual
where ar_recon_analyzer_pkg.get_cr_id(13) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(14),
ar_recon_analyzer_pkg.get_rec_id(14),
ar_recon_analyzer_pkg.get_rev_rec_id(14)
from dual
where ar_recon_analyzer_pkg.get_cr_id(14) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(15),
ar_recon_analyzer_pkg.get_rec_id(15),
ar_recon_analyzer_pkg.get_rev_rec_id(15)
from dual
where ar_recon_analyzer_pkg.get_cr_id(15) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(16),
ar_recon_analyzer_pkg.get_rec_id(16),
ar_recon_analyzer_pkg.get_rev_rec_id(16)
from dual
where ar_recon_analyzer_pkg.get_cr_id(16) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(17),
ar_recon_analyzer_pkg.get_rec_id(17),
ar_recon_analyzer_pkg.get_rev_rec_id(17)
from dual
where ar_recon_analyzer_pkg.get_cr_id(17) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(18),
ar_recon_analyzer_pkg.get_rec_id(18),
ar_recon_analyzer_pkg.get_rev_rec_id(18)
from dual
where ar_recon_analyzer_pkg.get_cr_id(18) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(19),
ar_recon_analyzer_pkg.get_rec_id(19),
ar_recon_analyzer_pkg.get_rev_rec_id(19)
from dual
where ar_recon_analyzer_pkg.get_cr_id(19) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(20),
ar_recon_analyzer_pkg.get_rec_id(20),
ar_recon_analyzer_pkg.get_rev_rec_id(20)
from dual
where ar_recon_analyzer_pkg.get_cr_id(20) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(21),
ar_recon_analyzer_pkg.get_rec_id(21),
ar_recon_analyzer_pkg.get_rev_rec_id(21)
from dual
where ar_recon_analyzer_pkg.get_cr_id(21) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(22),
ar_recon_analyzer_pkg.get_rec_id(22),
ar_recon_analyzer_pkg.get_rev_rec_id(22)
from dual
where ar_recon_analyzer_pkg.get_cr_id(22) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(23),
ar_recon_analyzer_pkg.get_rec_id(23),
ar_recon_analyzer_pkg.get_rev_rec_id(23)
from dual
where ar_recon_analyzer_pkg.get_cr_id(23) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(24),
ar_recon_analyzer_pkg.get_rec_id(24),
ar_recon_analyzer_pkg.get_rev_rec_id(24)
from dual
where ar_recon_analyzer_pkg.get_cr_id(24) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(25),
ar_recon_analyzer_pkg.get_rec_id(25),
ar_recon_analyzer_pkg.get_rev_rec_id(25)
from dual
where ar_recon_analyzer_pkg.get_cr_id(25) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(26),
ar_recon_analyzer_pkg.get_rec_id(26),
ar_recon_analyzer_pkg.get_rev_rec_id(26)
from dual
where ar_recon_analyzer_pkg.get_cr_id(26) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(27),
ar_recon_analyzer_pkg.get_rec_id(27),
ar_recon_analyzer_pkg.get_rev_rec_id(27)
from dual
where ar_recon_analyzer_pkg.get_cr_id(27) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(28),
ar_recon_analyzer_pkg.get_rec_id(28),
ar_recon_analyzer_pkg.get_rev_rec_id(28)
from dual
where ar_recon_analyzer_pkg.get_cr_id(28) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(29),
ar_recon_analyzer_pkg.get_rec_id(29),
ar_recon_analyzer_pkg.get_rev_rec_id(29)
from dual
where ar_recon_analyzer_pkg.get_cr_id(29) is not null
union
select
ar_recon_analyzer_pkg.get_cr_id(30),
ar_recon_analyzer_pkg.get_rec_id(30),
ar_recon_analyzer_pkg.get_rev_rec_id(30)
from dual
where ar_recon_analyzer_pkg.get_cr_id(30) is not null',
      p_title                  => 'Receipts with Incorrect Reversal GL Date ##$$LSTRDATE$$##',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the first 30 receipts for ##$$LSTRDATE$$## that have potential reconciling issues.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Receipt Analyzer in [2089720.1] for the CASH_RECEIPT_ID values listed in the table above.</li>
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => 'No receipts were identified as having potential reconciling issues.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_BY_RCT');



debug('begin add_signature: AR_RECON_BY_GLDATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7142',
      p_sig_id                 => 'AR_RECON_BY_GLDATE',
      p_sig_sql                => 'select
ar_recon_analyzer_pkg.get_gl_date(##$$i$$##) "GL_Date",
ar_recon_analyzer_pkg.get_status(##$$i$$##) "Status",
ar_recon_analyzer_pkg.get_begin_age_acctd_amt(##$$i$$##) "Beginning Accounted Amount",
ar_recon_analyzer_pkg.get_trx_reg_acctd_amt(##$$i$$##) "Transaction Register",
ar_recon_analyzer_pkg.get_unapp_reg_acctd_amt(##$$i$$##) "Unapplied Register",
ar_recon_analyzer_pkg.get_app_reg_acctd_amt(##$$i$$##) "Applied Register",
ar_recon_analyzer_pkg.get_adj_reg_acctd_amt(##$$i$$##) "Adjustment Register",
ar_recon_analyzer_pkg.get_cm_gain_loss(##$$i$$##) "CM Gain/Loss",
ar_recon_analyzer_pkg.get_inv_exp_acctd_amt(##$$i$$##) "Invoice Exceptions",
ar_recon_analyzer_pkg.get_rounding_diff(##$$i$$##) "Rounding Difference",
ar_recon_analyzer_pkg.get_end_age_acctd_amt(##$$i$$##) "Ending Accounted Amount",
ar_recon_analyzer_pkg.get_difference(##$$i$$##) "Difference" 
from dual',
      p_title                  => 'Reconciling Issues Identified',
      p_fail_condition         => '[GL_Date] > [''01-Jan-1800'']',
      p_problem_descr          => 'Identified potential reconciling issues for ##$$LSTRDATE$$##.',
      p_solution               => 'Please review the details above, you may have to provide output for the Transaction Analyzer [2058269.1] or Receipt Analyzer [2089720.1] as requested.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('AR_RECON_BY_TRX','AR_RECON_BY_RCT'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '67'
      );
   l_info.delete;
debug('end add_signature: AR_RECON_BY_GLDATE');



debug('begin add_signature: AR_RECON_POT_RECON_ITEMS');
   l_info('##MASK##CUSTOMER'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7264',
      p_sig_id                 => 'AR_RECON_POT_RECON_ITEMS',
      p_sig_sql                => 'select          rep.ref10 category,
                rep.ref29 category_code,  
                rep.account  account,
                decode(cat.asset_type,''Y'',''Asset'',
                     decode( cat.revenue_type, ''Y'',''Revenue'',
                         decode( cat.expense_type, ''Y'',''Expense'',
                            decode(cat.liability_type, ''Y'',''Liablity'',''Unknown'')))) "EXPECTED ACCOUNT TYPE",
				decode(c.gl_account_type,''L'',''Liability'',
				       decode(c.gl_account_type,''A'',''ASSET'',
				       decode(c.gl_account_type,''O'',''Owners Equity'', 
					   decode(c.gl_account_type,''R'', ''Revenue'', ''Unkown'')))) "ACTUAL ACCOUNT TYPE",
                rep.ref26 customer ,
                rep.ref24 payment_number,
                rep.ref25 transaction_number,
                rep.trx_date,
                rep.gl_date gl_date,
                rep.entered_dr  entered_dr ,
                rep.entered_cr  entered_cr,
                rep.acctd_dr accounted_dr,
                rep.acctd_cr accounted_cr,
 ...
 (SELECT
        ...
        glc.concatenated_segments                               account,
...
WHERE   adj.org_id = ##$$ORGID$$##       
        ...
        and adj.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
UNION ALL
SELECT      
	...
	gldist.posting_control_id					   pc_id,
             ARPT_SQL_FUNC_UTIL.get_lookup_meaning(''INV/CM'',ctt.type) class_dbc
FROM 
	  ar_xla_ctlgd_lines_v gldist,  
    ra_customer_trx_all ct, 
    ra_cust_trx_types_all ctt, 
    hz_cust_accounts_all acct,
    gl_code_combinations_kfv glc
WHERE     ct.org_id = ##$$ORGID$$##
          and gldist.account_set_flag = ''N''   
          ...
          and nvl(ct.org_id, -99) = nvl(ctt.org_id, -99)
          and gldist.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
UNION ALL
SELECT
...
WHERE   crh.org_id = ##$$ORGID$$##
        ...
        and ard.code_combination_id = glc.code_combination_id
        and crh.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
UNION ALL
...
WHERE   ard.source_table = ''RA''
        ...
        and ard.code_combination_id = glc.code_combination_id
        and ra.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
...
SELECT  mcd.apply_date                                          trx_date,
 ...
 WHERE  mcd.cash_receipt_id = cr.cash_receipt_id
        ...
        and mcd.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
        and ard.code_combination_id = glc.code_combination_id
UNION ALL
SELECT
...
WHERE   th.transaction_history_id = ard.source_id
        ...
        and ctt.cust_trx_type_id = ct.cust_trx_type_id 
        and th.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
        and nvl(ct.org_id,-99) = nvl(ctt.org_id,-99)
and ard.code_combination_id = glc.code_combination_id) rep,
(Select ''ADJ_ADJ'' JOURNAL_CATEGORY ,''N'' ASSET_TYPE ,''Y'' LIABILITY_TYPE ,''Y'' EXPENSE_TYPE ,''Y'' REVENUE_TYPE from dual 
union all
...
union all
Select ''CB_REC'' JOURNAL_CATEGORY ,''Y'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''N'' REVENUE_TYPE from dual
union all
...
union all
Select ''CM_CHARGES'' JOURNAL_CATEGORY ,''N'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''Y'' REVENUE_TYPE from dual
union all
...
union all
Select ''DM_CHARGES'' JOURNAL_CATEGORY ,''N'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''Y'' REVENUE_TYPE from dual
union all
...
union all
Select ''INV_CHARGES'' JOURNAL_CATEGORY ,''N'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''Y'' REVENUE_TYPE from dual
union all
...
union all
Select ''MISC_CASH'' JOURNAL_CATEGORY ,''Y'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''N'' REVENUE_TYPE from dual
union all
...
union all
Select ''TRADE_ACC'' JOURNAL_CATEGORY ,''Y'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''N'' EXPENSE_TYPE ,''N'' REVENUE_TYPE from dual
union all
...
union all
Select ''TRADE_UNEDISC_NON_REC_TAX'' JOURNAL_CATEGORY ,''N'' ASSET_TYPE ,''N'' LIABILITY_TYPE ,''Y'' EXPENSE_TYPE ,''N'' REVENUE_TYPE from dual) cat,
GL_CODE_COMBINATIONS_KFV c
WHERE   rep.account = c.concatenated_segments(+)
and rep.ref29 = cat.journal_category
and nvl(decode(c.gl_account_type, ''A'', cat.asset_type,''R'', cat.revenue_type, ''E'',cat.expense_type, ''L'', cat.liability_type, ''N''), ''N'') = ''N''
order by rep.account',
      p_title                  => 'Potential Reconciling Items',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Identified records where the account type is not set up properly. These can prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => 'Review the EXPECTED ACCOUNT TYPE and ACTUAL ACCOUNT TYPE fields above. This shows what the actual account type is versus what it should be.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_POT_RECON_ITEMS');



debug('begin add_signature: AR_RECON_APP_GL_DATE_LESS_TRX_GL_DATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7051',
      p_sig_id                 => 'AR_RECON_APP_GL_DATE_LESS_TRX_GL_DATE',
      p_sig_sql                => 'select trx.trx_number "Transaction Number",
       trx.customer_trx_id "Invoice Customer Trx ID",
	   gld.gl_date "Invoice GL Date",
	   ra.gl_date "Application GL Date",
       ra.receivable_application_id "Receivable Application ID",
       ra.customer_trx_id "CM Customer Trx ID",
       ra.cash_receipt_id "Receipt Cash Receipt ID",
	   ra.amount_applied "Amount Applied",
	   ra.acctd_amount_applied_from "Acctd Amount Applied From",
	   ra.acctd_amount_applied_to "Acctd Amount Applied To",
	   ra.application_type "Type"
...
and    trunc(gld.GL_DATE) > trunc(ra.GL_DATE)
and    trx.ORG_ID = ##$$ORGID$$##
and    gld.gl_date < to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')',
      p_title                  => 'Applied GL_DATE < Transactions GL_DATE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are applications with GL_DATE earlier than the GL_date of the transaction it is applying from/to.',
      p_solution               => '<ul>
<li>The analyzer has identified data corruption described in [1638780.1]</li>
<li>Please run the Transaction Analyzer in [2058269.1] for the Trx_Number values listed.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_APP_GL_DATE_LESS_TRX_GL_DATE');



debug('begin add_signature: AR_RECON_APP_REVERSE_GL_DATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7066',
      p_sig_id                 => 'AR_RECON_APP_REVERSE_GL_DATE',
      p_sig_sql                => 'select cr.receipt_number Receipt_number, ra.cash_receipt_id Cash_Receipt_id,ra.receivable_application_id rec_app_id,
       ra_rev.receivable_application_id rev_rec_id, 
       ra.amount_applied+nvl(ra.earned_discount_taken,0)+nvl(ra.unearned_discount_taken,0) amount_applied
from   ar_cash_receipts_all cr,
       ar_receivable_applications_all ra, 
       ar_distributions_all ard,
       ar_receivable_applications_all ra_rev  
where  ra.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
...
and    ra.status =''APP''
and    ra_rev.status = ''APP''
and    nvl(ra.amount_applied_from, ra.amount_applied) = -nvl(ra_rev.amount_applied_from,ra_rev.amount_applied)
and    ra.display =''N''
and    ra_rev.display = ra.display
and    ra.gl_date > ra_rev.gl_date
and    ra.org_id = ##$$ORGID$$##',
      p_title                  => 'Unapplied GL_DATE < Applied GL_DATE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found records in AR_RECEIVABLE_APPLICATIONS_ALL whose unapplication GL_DATE is less than the original application GL_DATE.  These records will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Receipt Analyzer in [2089720.1] for the Receipt_Number values listed.</li>
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_APP_REVERSE_GL_DATE');



debug('begin add_signature: AR_RECON_RCPT_GL_DATE_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7070',
      p_sig_id                 => 'AR_RECON_RCPT_GL_DATE_CLOSED',
      p_sig_sql                => 'select ps.cash_receipt_id Cash_Receipt_Id, ps.payment_schedule_id Payment_Schedule_Id,
       ps.trx_number Receipt_Number, ps.gl_date GL_DATE,
       ps.amount_due_original AMOUNT_DUE_ORIGINAL, ps.gl_date_closed GL_DATE_CLOSED
from   ar_payment_schedules_all ps
where  gl_date_closed >= to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
and    trunc(ps.gl_date_Closed) <> to_date(''31-DEC-4712'',''dd-MON-YYYY'')
and    ps.class = ''PMT''
and    sign(ps.payment_schedule_id) <> -1
and    exists (select ''1'' from ar_receivable_applications_all ra
               where ra.payment_schedule_id = ps.payment_schedule_id
               and ra.gl_date > ps.gl_date_closed
		       and status in (''APP'', ''ACTIVITY'')
               union
               select ''1'' from ar_receivable_applications_all ra
               where ra.applied_payment_schedule_id = ps.payment_schedule_id
               and ra.gl_date > ps.gl_date_closed)
and    org_id = ##$$ORGID$$##',
      p_title                  => 'Receipts with Incorrect GL_DATE_CLOSED',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found receipts whose GL_DATE_CLOSED are wrongly populated in AR_PAYMENT_SCHEDULES_ALL.  These receipts will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.
',
      p_solution               => '<ul>
...
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_RCPT_GL_DATE_CLOSED');



debug('begin add_signature: AR_RECON_TRX_GL_DATE_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7061',
      p_sig_id                 => 'AR_RECON_TRX_GL_DATE_CLOSED',
      p_sig_sql                => 'select ps.trx_number Trx_Number, ps.customer_trx_id Customer_Trx_ID, 
       ps.gl_date GL_DATE, ps.amount_due_original AMOUNT_DUE_ORIGINAL,
...
and    exists (select ''1'' from ar_receivable_applications_all ra
               ...
               and ra.gl_date > ps.gl_date_closed
               union
               select ''1'' from ar_adjustments_all adj
               where adj.payment_schedule_id = ps.payment_schedule_id
               and    adj.status = ''A'' 
               and    adj.postable = ''Y'' 
               and adj.gl_date > ps.gl_date_closed)
and    org_id = ##$$ORGID$$##',
      p_title                  => 'Transaction with Incorrect GL_DATE_CLOSED',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions that have an incorrect GL_DATE_CLOSED value in AR_PAYMENT_SCHEDULES_ALL.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID for each value listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_TRX_GL_DATE_CLOSED');



debug('begin add_signature: AR_RECON_ACC_AFFECT_FLAG_N_PS_RECORD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7283',
      p_sig_id                 => 'AR_RECON_ACC_AFFECT_FLAG_N_PS_RECORD',
      p_sig_sql                => 'select ct.trx_number Trx_Number,
       ct.customer_trx_id Customer_Trx_Id
from   ra_customer_trx_all ct,
       ra_cust_trx_types_all ctt
where  ct.set_of_books_id = (select set_of_books_id
...
and exists (select ''1'' 
      ...
      p_title                  => 'AR_PAYMENT_SCHEDULES_ALL Data Exists for Transactions with ACCOUNTING_AFFECT_FLAG = N',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions that do not affect customer balances; however, table AR_PAYMENT_SCHEDULES_ALL has data.  These records will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the customer_trx_id values listed.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ACC_AFFECT_FLAG_N_PS_RECORD');



debug('begin add_signature: AR_RECON_DUP_REC_ROWS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7284',
      p_sig_id                 => 'AR_RECON_DUP_REC_ROWS',
      p_sig_sql                => 'select ct.trx_number TRX_NUMBER,
       gld.customer_trx_id CUSTOMER_TRX_ID, count(*) 
from   ra_cust_trx_line_gl_dist_all gld,
       ra_customer_trx_all ct
where  ct.customer_trx_id = gld.customer_trx_id
and    latest_rec_flag = ''Y'' 
and    account_class = ''REC'' 
and    gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
       and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and    ct.org_id = ##$$ORGID$$##
and    ct.org_id = gld.org_id
group  by TRX_NUMBER, gld.CUSTOMER_TRX_ID
having count(*) > 1',
      p_title                  => 'Transactions With Duplicate REC Rows',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions that have duplicate REC rows.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the TRX_NUMBER values listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_DUP_REC_ROWS');



debug('begin add_signature: AR_RECON_APP_UNAP_GLDATE_MISMATCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7065',
      p_sig_id                 => 'AR_RECON_APP_UNAP_GLDATE_MISMATCH',
      p_sig_sql                => 'select cr.receipt_number Receipt_Number, ra.cash_receipt_id Cash_Receipt_Id, 
       ra.receivable_application_id rec_id
...
and    nvl(ra.confirmed_flag,''Y'') = ''Y''
and    ra.gl_date  BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
       AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
and cr.org_id = ra.org_id',
      p_title                  => 'Inconsistent Value in SOURCE_ID_SECONDARY',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found UNAPP records in AR_DISTIRBUTIONS_ALL with SOURCE_ID_SECONDARY different from the AR_RECEIVABLE_APPLICATIONS.RECEIVABLE_APPLICATION_ID of their corresponding APP.  These records will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<li>Please run the Receipt Analyzer in [2089720.1] for the Receipt_Number values listed.</li>
<li>Please log a Service Request and upload the Receipt Analyzer output and this Period Close Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_APP_UNAP_GLDATE_MISMATCH');



debug('begin add_signature: AR_RECON_ADJ_AMOUNT_ACCTD_AMOUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7062',
      p_sig_id                 => 'AR_RECON_ADJ_AMOUNT_ACCTD_AMOUNT',
      p_sig_sql                => 'Select adj.adjustment_id adjustment_id,
...
Where  ps.gl_date_Closed >= to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
...
and    adj.org_id = trx.org_id',
      p_title                  => 'Adjustments in Functional Currency with  AMOUNT <> ACCTD_AMOUNT',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found adjustments in functional currency whose amount does not match the accounted amount. These adjustments will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Adjustment Analyzer in [2069916.1] for the adjustment_id values listed.</li>
<li>Please log a Service Request and upload the Adjustment Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ADJ_AMOUNT_ACCTD_AMOUNT');



debug('begin add_signature: AR_RECON_APPLIED_PS_ID_MISMATCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7071',
      p_sig_id                 => 'AR_RECON_APPLIED_PS_ID_MISMATCH',
      p_sig_sql                => 'select  ps.customer_trx_id inv_id, ra.cash_receipt_id, ra.payment_schedule_id,
        ra.receivable_application_id, ra.customer_trx_id cm_id, ra.gl_date
from    ar_payment_schedules_all ps, 
        ar_receivable_applications_all ra
...
and     ps.org_id = ra.org_id',
      ...
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records in AR_RECEIVABLE_APPLICATIONS_ALL with APPLIED_PAYMENT_SCHEDULE_ID and/or APPLIED_CUSTOMER_TRX_ID mismatch.  These records will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Receipt Analyzer in [2089720.1] for the cash_receipt_id values listed.</li>
...
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_APPLIED_PS_ID_MISMATCH');



debug('begin add_signature: AR_RECON_INCORRECT_PS_STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7072',
      p_sig_id                 => 'AR_RECON_INCORRECT_PS_STATUS',
      p_sig_sql                => 'select ps.payment_schedule_id, 
       ps.class, 
       ps.trx_number,
...
and    ps.org_id = ##$$ORGID$$##',
      p_title                  => 'STATUS is CLOSED but ACCTD_AMOUNT_DUE_REMAINING <> 0',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are record(s) in AR_PAYMENT_SCHEDULES where STATUS = CLOSED but ACCTD_AMOUNT_DUE_REMAINING <> 0.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => 'For Invoices, Credit Memos, Debit Memos, Guarantees, Deposits:
<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID listed the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>

For Receipts (CLASS = PMT)
<ul>
<li>Please run the Receipt Analyzer in [2089720.1] for the CASH_RECEIPT_ID values listed in the table above.</li>
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>
',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_INCORRECT_PS_STATUS');



debug('begin add_signature: AR_RECON_INCORRECT_OP_PS_STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7285',
      p_sig_id                 => 'AR_RECON_INCORRECT_OP_PS_STATUS',
      p_sig_sql                => 'select payment_schedule_id, customer_trx_id, cash_receipt_id, trx_number,
       decode(class, ''INV'', ''Invoice'',
       decode(class, ''DEP'', ''Deposit'', 
       decode(class, ''GUAR'', ''Guarantee'', 
       decode(class, ''CM'', ''Credit Memo'', 
       decode(class, ''DM'', ''Debit Memo'', 
       decode(class, ''CB'', ''Chargeback'',
       decode(class, ''PMT'', ''Receipt''))))))) CLASS
...
and    gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
       and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and    org_id = ##$$ORGID$$##',
      p_title                  => 'STATUS is OPEN but AMOUNT_DUE_REMAINING = 0',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are record(s) in AR_PAYMENT_SCHEDULES with STATUS = OPEN but AMOUNT_DUE_REMAINING = 0.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => 'For Invoices, Credit Memos, Debit Memos, Guarantees, Deposits:
<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li>
</ul>
...
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li>
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_INCORRECT_OP_PS_STATUS');



debug('begin add_signature: AR_RECON_CLAIM_CB_DRILLDOWN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7282',
      p_sig_id                 => 'AR_RECON_CLAIM_CB_DRILLDOWN',
      p_sig_sql                => 'select
   Receipt_Number,
   cash_receipt_id,
   type,
...
SELECT /*+ index(ra AR_RECEIVABLE_APPLICATIONS_N6) */
       ...
       ''Unapplied_Receipts_Register'' type,
       ''UNAPP_REC'' class,
       nvl(SUM(decode(ra.application_type,   ''CASH'',   decode(ra.status,   ''ACTIVITY'',   ra.amount_applied,   0),   0)),   0)  invoice_currency,
       nvl(SUM(decode(ra.application_type,   ''CASH'',   decode(ra.status,   ''ACTIVITY'',   ra.acctd_amount_applied_from,   0),   0)),   0) functional_currency
FROM   ar_receivable_applications_all ra,
       ar_cash_receipts_all cr,
       gl_code_combinations gc
WHERE nvl(ra.confirmed_flag,   ''Y'') = ''Y''
 AND ra.gl_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
 AND ra.cash_receipt_id = cr.cash_receipt_id
 AND  ra.APPLICATION_REF_TYPE = ''CHARGEBACK''
 AND  ra.status = ''ACTIVITY''
 AND(ra.org_id IS NULL OR EXISTS
  (SELECT /*  */ 1
   FROM hr_organization_information org_info
   WHERE ra.org_id = org_info.organization_id
   AND org_info.org_information_context = ''Operating Unit Information''
   AND to_number(org_info.org_information3) = (select set_of_books_id
                                               from   ar_system_parameters_all
                                               where  org_id = ##$$ORGID$$##)))
 AND gc.code_combination_id = ra.code_combination_id
GROUP BY ra.cash_receipt_id, cr.receipt_number
)',
      p_title                  => 'Claims Settled as Chargeback',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are claims settled as chargebacks which resulted in the activity getting accounted as claims in SLA.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>The analyzer has identified data corruption issue described in [2141351.1].</li>
<li>Please log a Service Request and upload the Receipt Analyzer for the CASH_RECEIPT_ID (DOCUMENT_ID) listed in the table above and this Reconciliation Analyzer output.</li> 
</ul>
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_CLAIM_CB_DRILLDOWN');



debug('begin add_signature: AR_RECON_OPHAN_CM_APPS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7064',
      p_sig_id                 => 'AR_RECON_OPHAN_CM_APPS',
      p_sig_sql                => 'select ra.payment_schedule_id, ra.customer_trx_id,ra.gl_date, ra.posting_control_id,ra.event_id
from   ar_receivable_applications_all ra
...
and not exists (select gl_dist.customer_trx_id
                ...
                AND    pay.org_id = ##$$ORGID$$##
                AND    gl_dist.org_id = pay.org_id)
and  ra.org_id = ##$$ORGID$$##',
      p_title                  => 'Orphan Credit Memo Applications',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records with an orphan credit memo applications.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID values listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_OPHAN_CM_APPS');



debug('begin add_signature: AR_RECON_ORPHAN_CASH_APPS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7181',
      p_sig_id                 => 'AR_RECON_ORPHAN_CASH_APPS',
      p_sig_sql                => 'select ra.payment_schedule_id, ra.cash_receipt_id, ra.applied_customer_trx_id,ra.gl_date,ra.apply_date, ra.posting_control_id,ra.event_id
from   ar_receivable_applications_all ra
where  ra.application_type = ''CASH''
and    nvl(ra.postable,''Y'') = ''Y''
and    status = ''APP''
and    ra.receivable_application_id > 0
and    not exists (select trx.customer_trx_id
                   from   ra_customer_trx_all trx,
                          ar_payment_schedules_all pay
                   WHERE  pay.payment_schedule_id = ra.applied_payment_schedule_id
                   AND    pay.customer_trx_id = ra.applied_customer_trx_id
                   AND    pay.customer_trx_id = trx.customer_trx_id
                   AND    pay.org_id = ##$$ORGID$$##
                   AND    trx.org_id = pay.org_id)
and ra.org_id = ##$$ORGID$$##',
      p_title                  => 'Orphan Cash Applications',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are record(s) with orphan application records.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Receipt Analyzer in [2089720.1] for the CASH_RECEIPT_ID for each value listed in the table above.</li>
<li>Please run the Transaction Analyzer in [2058269.1] for the APPLIED_CUSTOMER_TRX_ID values listed.</li>
<li>Please log a Service Request and upload the Receipt Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ORPHAN_CASH_APPS');



debug('begin add_signature: AR_RECON_ORPHAN_PS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7063',
      p_sig_id                 => 'AR_RECON_ORPHAN_PS',
      p_sig_sql                => 'select pay.trx_number Trx_Number,
       pay.customer_trx_id Customer_Trx_Id,
       pay.payment_schedule_id Payment_Schedule_Id,
       pay.gl_date GL_Date,
       pay.status,
	   decode(pay.class, ''INV'', ''Invoice'',
              decode(pay.class, ''DEP'', ''Deposit'',
			  decode(pay.class, ''GUAR'', ''Guarantee'',
			  decode(pay.class, ''CM'', ''Credit Memo'',
			  decode(pay.class, ''DM'', ''Debit Memo'',
			  decode(pay.class, ''CB'', ''Chargeback'')))))) CLASS
from   ar_payment_schedules pay
where  not exists (select ''x''
                   from ra_customer_trx trx
                   where pay.customer_trx_id = trx.customer_trx_id)
and    pay.class in (''INV'' , ''CM'' , ''DM'' , ''DEP'' , ''GUAR'' , ''CB'')
and    pay.gl_date <= to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and    pay.customer_trx_id > 0
and    pay.org_id = ##$$ORGID$$##',
      p_title                  => 'Orphan Payment Schedule Records',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions with orphan payment schedule records.  This will prevent you from reconciling ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_solution               => '<ul>
<li>Please run the Transaction Analyzer in [2058269.1] for the CUSTOMER_TRX_ID values listed in the table above.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li> 
</ul>',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ORPHAN_PS');



debug('begin add_signature: AR_RECON_AR_S_00033_OVERLAP_DISPUTE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11595',
      p_sig_id                 => 'AR_RECON_AR_S_00033_OVERLAP_DISPUTE',
      p_sig_sql                => 'SELECT 	p.customer_trx_id, 
        p.trx_number,
        p.class,
        dh.payment_schedule_id ps_id,
       	dh.dispute_history_id  dh_id,
       	dh.start_date,
       	nvl(dh.end_date,sysdate) end_date
FROM  	ar_dispute_history dh,
        ar_payment_schedules_all p
WHERE 	dh.start_date > nvl(dh.end_date,sysdate)
AND     dh.payment_schedule_id = p.payment_schedule_id
AND     p.class not in (''PMT'')
AND   	dh.payment_schedule_id in (select p2.payment_schedule_id
                                   from ar_payment_schedules_all p2
                                   where p2.org_id = ##$$ORGID$$##
                                   and p2.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
                                        and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
                                  )',
      p_title                  => 'Overlapping Dates on Transaction Dispute',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Transactions are showing overlapping on disputes table for start and end dates.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [2615433.1]</li>
<li>Please run the Transaction Analyzer in [2058269.1] for the customer_trx_id values listed.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li>
</ul>',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_AR_S_00033_OVERLAP_DISPUTE');



debug('begin add_signature: AR_RECON_CM_MISSING_PS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12793',
      p_sig_id                 => 'AR_RECON_CM_MISSING_PS',
      p_sig_sql                => 'select gld.customer_trx_id customer_trx_id, ct.trx_number trx_number, ctt.type type
from ra_customer_trx_all ct , ra_cust_trx_types_all ctt, ra_cust_trx_line_gl_dist_all gld
where ct.cust_trx_type_id = ctt.cust_trx_type_id
and ctt.type = ''CM'' and ctt.accounting_affect_flag = ''Y''
...
and gld.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and not exists (Select ''x'' from ar_payment_schedules_all ps
where ps.customer_trx_id = ct.customer_trx_id )
and ct.org_id = ##$$ORGID$$##',
      p_title                  => 'Credit Memos Missing Payment Schedule Records',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Credit Memos with missing payment schedule and application records.',
      p_solution               => '<ul>
<li>The Analyzer has identified the data corruption reported in [2044211.1].</li>
<li>Please run the Transaction Analyzer in [2058269.1] for the customer_trx_id values listed.</li>
<li>Please log a Service Request and upload the Transaction Analyzer output and this Reconciliation Analyzer output.</li>
</ul>',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_CM_MISSING_PS');



debug('begin add_signature: AR_RECON_ADJ_GL_DATE_LESS_TRX_GL_DATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '14371',
      p_sig_id                 => 'AR_RECON_ADJ_GL_DATE_LESS_TRX_GL_DATE',
      p_sig_sql                => 'select  b.TRX_NUMBER "Transaction_number",
        a.CUSTOMER_TRX_ID  "Invoice Customer Trx ID",
        a.GL_DATE "Invoice GL Date",
        c.GL_DATE "Adjustment GL Date",
        c.CUSTOMER_TRX_ID "Adjustment GL Date",
        c.AMOUNT "Adjustment Amount",
        c.ADJUSTMENT_ID "Adjustment_id",
        c.EVENT_ID "Event Id"
...
where   a.ACCOUNT_CLASS =''REC'' 
 ...
 and    trunc(a.GL_DATE) > trunc(c.GL_DATE)
 and    a.ORG_ID = ##$$ORGID$$##
 and     a.gl_date < to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')',
      p_title                  => 'Adjustment GL Date < Transaction GL Date',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are adjustments with GL Date earlier than GL date of the transaction.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [2652158.1]</li>
<li>Please run the Adjustment Analyzer in [2069916.1] for the Adjustment Number values listed.</li>
<li>Please log a Service Request and upload the Adjustment Analyzer output and this Reconciliation Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no adjustments with GL Date earlier than GL date of the transaction.',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ADJ_GL_DATE_LESS_TRX_GL_DATE');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00147_REV_AND_REC_WITH_SAME_CCID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30255',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00147_REV_AND_REC_WITH_SAME_CCID',
      p_sig_sql                => 'select gld_rec.customer_trx_id, 
gld_rec.cust_trx_line_gl_dist_id gld_rec_gl_dist_id,
gld_rev.cust_trx_line_gl_dist_id gld_rev_gl_dist_id, 
gld_rev.Amount,
gld_rev.code_combination_id REV_CCID, 
gld_rec.code_combination_id REC_CCID,
gld_rev.gl_date,gld_rev.Account_class, gld_rec.Account_class
from ra_cust_trx_line_gl_dist_all gld_rec,
ra_cust_trx_line_gl_dist_all gld_rev
where gld_rec.Customer_trx_id =gld_rev.Customer_trx_id
and gld_rec.org_id = ##$$ORGID$$##
and gld_rec.org_id = gld_rev.org_id
and gld_rec.account_class = (''REC'')
and gld_rev.account_class = (''REV'')
and gld_rec.account_set_flag = ''N''
and gld_rev.account_set_flag = ''N''
and gld_rec.code_combination_id = gld_rev.code_combination_id
   ',
      p_title                  => 'Transactions with Same CCID for REC and REV Accounts',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Transaction with Same CCID for REC and REV Accounts',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [3000272.1]</li>
<li>Please log a Service Request and upload this analyzer output.</li>
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00147_REV_AND_REC_WITH_SAME_CCID');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00148_REV_WITH_REC_CCID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30256',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00148_REV_WITH_REC_CCID',
      p_sig_sql                => 'select distinct gld_rev.customer_trx_id, 
gld_rev.code_combination_id REV_CCID,  
gld_rev.gl_date REV_GL_DATE
from ra_cust_trx_line_gl_dist_all gld_rev
where gld_rev.account_class = (''REV'') 
and gld_rev.account_set_flag = ''N''
and gld_rev.org_id = ##$$ORGID$$##
and gld_rev.code_combination_id in (Select distinct code_combination_id from  ra_cust_trx_line_gl_dist_all gld_rec
                                    where gld_rec.account_class = (''REC'')
                                    and gld_rec.account_set_flag = ''N'')
order by gld_rev.gl_date desc',
      p_title                  => 'Transactions with REV Account same as REC account.',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Transaction with REV CCID same as REC.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [3000272.1]</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>',
      p_success_msg            => ' ',
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00148_REV_WITH_REC_CCID');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_MISMATCH_GL_DATE_IN_GLD_XE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '31913',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_MISMATCH_GL_DATE_IN_GLD_XE',
      p_sig_sql                => 'SELECT  gld.customer_trx_id customer_trx_id, 
        xte.transaction_number trx_number,
        gld.gl_date gl_dist_gl_date,
         xe.event_id event_id,
        xe.event_date xe_event_date,
        gld.account_class account_class,
...
WHERE   gld.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MM-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MM-YYYY'')
AND     nvl(xte.source_id_int_1,-99)  = gld.customer_trx_id
AND     xte.entity_code in (''TRANSACTIONS'',''MANUAL'') 
AND     xe.event_id = gld.event_id
AND     gld.account_set_flag = ''N''
AND     trunc(gld.gl_date) <> trunc(xe.event_date)
AND     gld.org_id = ##$$ORGID$$##',
      p_title                  => 'Mismatched GL_DATE in RA_CUST_TRX_LINE_GL_DIST_ALL and EVENT_DATE in XE_EVENTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The Analyzer has identified the data corruption described in [2847027.1]',
      p_solution               => 'The analyzer has identified data corruption described in [2847027.1]
Please log a Service Request and upload this analyzer output.',
      p_success_msg            => 'There are no records with mismatched GL_DATE in RA_CUST_TRX_LINE_GL_DIST_ALL and EVENT_DATE in XE_EVENTS',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_MISMATCH_GL_DATE_IN_GLD_XE');



debug('begin add_signature: AR_PC_XLA_GL_NEG_LEDGER_ID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5120',
      p_sig_id                 => 'AR_PC_XLA_GL_NEG_LEDGER_ID',
      p_sig_sql                => 'select h.period_name as PERIOD_NAME,gjb.name as BATCH_NAME,gjb.je_batch_id AS BATCH_ID,
       gjb.group_id AS GROUP_ID,
       l.code_combination_id AS CCID,
       decode(h.status,''P'',''Posted'',''U'',''Unposted'',h.status) AS STATUS,
       sum(nvl(l.accounted_cr, 0)) ACCOUNTED_CREDIT , 
       sum(nvl(l.accounted_dr,0)) ACCOUNTED_DEBIT
from   gl.gl_je_headers h, 
       gl.gl_je_lines l,
       gl_import_references ir,
       gl_je_batches gjb
where  ir.gl_sl_link_table = ''XLAJEL''
and    ir.je_header_id = h.je_header_id
and    ir.je_batch_id = h.je_batch_id
and    h.je_batch_id = gjb.je_batch_id
and    h.ledger_id = -1 * ##$$LEDGERID$$##            
and    h.je_header_id = l.je_header_id
and    l.ledger_id = h.ledger_id 
and    UPPER(h.je_source) = ''RECEIVABLES''
and    h.period_name in (select distinct period_name 
                         from   gl_period_statuses
                         where  application_id = 101
                         and    start_date >= to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')
                         and    end_date <= to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
                         and    ledger_id = ##$$LEDGERID$$##)
group by h.period_name, gjb.name, gjb.je_batch_id,
         gjb.group_id, l.code_combination_id, h.status
order by gjb.name,gjb.je_batch_id,gjb.group_id,ccid',
      p_title                  => 'Negative Ledger Id',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) where ledger ID is negative.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], see section: Corruption Type 1.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
...
',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_NEG_LEDGER_ID');



debug('begin add_signature: AR_PC_XLA_GL_EXISTS_IN_GL_NOT_IN_XLA');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5124',
      p_sig_id                 => 'AR_PC_XLA_GL_EXISTS_IN_GL_NOT_IN_XLA',
      p_sig_sql                => 'select  /*+ parallel (gh,gl,imp) leading (gl) */ 
        gh.period_name,gjb.name,
        gjb.group_id, 
        gl.code_combination_id ccid,
        imp.reference_5 Ref_entity_id, 
        imp.reference_6 Ref_event_id, 
        imp.reference_7 Ref_Ae_header_id, 
        imp.reference_8 Ref_Line_num, 
        imp.reference_9 Ref_Accounted_Dr, 
        imp.reference_10 Ref_Accounted_cr
...
and     imp.je_header_id = gh.je_header_id
and     imp.je_batch_id = gh.je_batch_id
and     imp.gl_sl_link_table = ''XLAJEL''
and     gjb.je_batch_id = gh.je_batch_id
and     UPPER(gh.je_source) = ''RECEIVABLES''
and     gh.ledger_id = ##$$LEDGERID$$## 
and     nvl(gh.accrual_rev_je_header_id,0) = 0 
and     gh.period_name in (select distinct period_name 
                           from   gl_period_statuses
                           where  application_id = 101
                           and    start_date >= to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
                           and    end_date <= to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
and not exists (select 1 
                ...
                and    gl_sl_link_table = imp.gl_sl_link_table
                and    ledger_id = gh.ledger_id)
order by gh.period_name, gjb.name, gjb.group_id, ccid',
      p_title                  => 'Exists in GL but Missing in XLA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which exist in GL but are missing in XLA.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], see section: Corruption Type 5.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_EXISTS_IN_GL_NOT_IN_XLA');



debug('begin add_signature: AR_PC_XLA_GL_MULTIPLE_POST_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5126',
      p_sig_id                 => 'AR_PC_XLA_GL_MULTIPLE_POST_IN_GL',
      p_sig_sql                => 'select  distinct gjh.period_name, gjb.name, gjb.group_id 
from    gl_je_batches gjb, 
       ...
       (select gir.gl_sl_link_id, 
        ...
        and    exists
             (select /*+ parallel(xah,xal) leading(xah) */  ''found_dup''
              ...
              and      xah.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
                                           and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
        ...
        and  UPPER( gh.je_source ) = ''RECEIVABLES''
        and  nvl(gh.accrual_rev_je_header_id,0) = 0  
        group by gir.gl_sl_link_id,gir.gl_sl_link_table,gh.ledger_id  having count(1) > 1) dup
...
and     UPPER(gjh.je_source) = ''RECEIVABLES''',
      p_title                  => 'Posted Multiple Times in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which are posted multiple times in GL.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], see section: Corruption Type 4.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_MULTIPLE_POST_IN_GL');



debug('begin add_signature: AR_PC_XLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5125',
      p_sig_id                 => 'AR_PC_XLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL',
      p_sig_sql                => 'SELECT  /*+ parallel (h,l) leading (h) */
        gh.period_name,gjb.name, 
        gjb.group_id,
...
AND     nvl(gh.accrual_rev_je_header_id,0) = 0
AND     ir.je_batch_id = gjb.je_batch_id 
AND     gh.je_batch_id = gjb.je_batch_id
group by gh.period_name, gjb.name, gjb.group_id, l.code_combination_id, gh.je_from_sla_flag
order by gh.period_name, gjb.name, gjb.group_id, ccid',
      p_title                  => 'Not Marked as Transferred in XLA but Exists in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which are not marked as transferred in XLA but exist in GL.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], see section: Corruption Type 3.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL');



debug('begin add_signature: AR_PC_XLA_GL_NULL_GL_SL_LINK_ID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5121',
      p_sig_id                 => 'AR_PC_XLA_GL_NULL_GL_SL_LINK_ID',
      p_sig_sql                => 'select DISTINCT h.ae_header_id, h.accounting_date, h.event_type_code, h.gl_transfer_status_code
...
and    h.gl_transfer_date is not null
and    h.ae_header_id = l.ae_header_id
and    h.ledger_id = l.ledger_id
and    h.application_id = l.application_id
and    l.gl_sl_link_id is null',
      p_title                  => 'Transferred to GL but GL_SL_LINK_ID is Null',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred to GL but GL_SL_LINK_ID is null.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], see section: Corruption Type 7.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_NULL_GL_SL_LINK_ID');



debug('begin add_signature: AR_PC_XLA_GL_XFR_IN_XLA_NOT_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5122',
      p_sig_id                 => 'AR_PC_XLA_GL_XFR_IN_XLA_NOT_IN_GL',
      p_sig_sql                => 'SELECT /*+ parallel (h,l) leading (h) */
       h.period_name as PERIOD_NAME,
       h.group_id AS GROUP_ID, 
       l.code_combination_id AS CCID,
       sum(nvl(l.accounted_cr, 0)) AS ACCOUNTED_CREDIT, 
       sum(nvl(l.accounted_dr,0)) AS ACCOUNTED_DEBIT
FROM   xla_ae_lines l, 
       xla_ae_headers h 
WHERE  l.application_id = h.application_id 
AND    l.ae_header_id = h.ae_header_id 
...
AND NOT EXISTS (SELECT 1 
                ...
                AND    ir.je_batch_id = gh.je_batch_id 
                AND    gh.ledger_id = h.ledger_id)
group by h.period_name,h.group_id, l.code_combination_id
order by h.period_name,h.group_id, ccid',
      p_title                  => 'Marked as Transferred in XLA but not in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred in XLA but do not exist in GL.',
      p_solution               => '<ul>
<li>The analyzer has identified data corruption described in [3071537.1]</li> <li>Please follow the steps provided in the document in a TEST instance with the same data as PROD. If the issue is resolved, please migrate the solution to the other environments; otherwise, please log a Service Request and upload this Analyzer output</li>
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_XFR_IN_XLA_NOT_IN_GL');



debug('begin add_signature: AR_PC_XLA_GL_XFR_IN_XLA_REV_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5123',
      p_sig_id                 => 'AR_PC_XLA_GL_XFR_IN_XLA_REV_IN_GL',
      p_sig_sql                => 'SELECT /*+ parallel (h) parallel (l) leading (h)*/
        h.period_name as PERIOD_NAME,
	h.group_id AS GROUP_ID, 
	l.code_combination_id AS CCID,
	sum(nvl(l.accounted_cr, 0)) AS ACCOUNTED_CREDIT, 
...
group by h.period_name,h.group_id, l.code_combination_id           
order by h.period_name,h.group_id, ccid',
      p_title                  => 'Marked as Transferred in XLA but Reversed in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred in XLA but are reversed in GL.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557.1], Corruption Type 2.</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: AR_PC_XLA_GL_XFR_IN_XLA_REV_IN_GL');



debug('begin add_signature: EBS_FIN_AR_PC_GL_DATE_MISMATCH_BETWEEN_XLA_AND_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '27621',
      p_sig_id                 => 'EBS_FIN_AR_PC_GL_DATE_MISMATCH_BETWEEN_XLA_AND_GL',
      p_sig_sql                => 'SELECT distinct xte.entity_code, xte.source_id_int_1, xal.ae_header_id,
xal.ae_line_num, xal.gl_sl_link_id, xal.accounting_date,
gjl.je_header_id, gjl.je_line_num, gjl.effective_date,
xah.period_name AR_XLA_PERIOD_NAME, gjl.period_name GL_PERIOD_NAME,
xal.ENTERED_DR xal_amt_dr, xal.ENTERED_CR xal_amt_cr, xal.ACCOUNTED_DR
xal_aamt_dr, xal.ACCOUNTED_CR xal_aamt_cr, gjl.ENTERED_DR,
...
WHERE xte.application_id = 222
 AND xte.entity_code NOT IN (''MANUAL'')
 ...
 AND gir.gl_sl_link_id = xal.gl_sl_link_id
 AND gir.gl_sl_link_table = xal.gl_sl_link_table
 AND gjl.ledger_id = xah.ledger_id
 AND xah.period_name <> gjl.period_name
ORDER BY xal.accounting_date, xte.entity_code, xte.source_id_int_1',
      p_title                  => 'AR documents with accounting period/Date mismatch between in AR/XLA and GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found AR documents with accounting period/Date mismatch between in AR/XLA and GL',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [1630310.1].</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_PC_GL_DATE_MISMATCH_BETWEEN_XLA_AND_GL');



debug('begin add_signature: EBS_FIN_AR_PC_DUP_JOURNALS_POSTED_TO_XLA_AND_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '29531',
      p_sig_id                 => 'EBS_FIN_AR_PC_DUP_JOURNALS_POSTED_TO_XLA_AND_GL',
      p_sig_sql                => 'SELECT  et.SECURITY_ID_INT_1 org_id, et.source_id_int_1, et.entity_code,  xah.*
FROM
  apps.xla_ae_headers xah,
  xla.xla_transaction_entities et
WHERE  xah.application_id = 222
  AND et.application_id = xah.application_id
  AND et.entity_id = xah.entity_id
  AND xah.ledger_id = ##$$LEDGERID$$##
  AND xah.accounting_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
        and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
  AND ( xah.ledger_id, xah.event_id ) IN (  SELECT  aeh.ledger_id, aeh.event_id
                                              FROM
                                              apps.xla_ae_headers aeh
                                              WHERE  aeh.application_id = 222
                                              AND aeh.event_id = xah.event_id
                                              AND aeh.event_type_code <> ''MANUAL''
                                              GROUP BY  aeh.ledger_id, aeh.event_id
                                              HAVING  COUNT(*) > 1  )
ORDER BY event_id',
      p_title                  => 'Duplicate Journals posted to XLA and GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found AR documents with Duplicate Accounting',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [3025510.1].</li>
<li>Please log a Service Request and upload this analyzer output.</li>
</ul>
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_PC_DUP_JOURNALS_POSTED_TO_XLA_AND_GL');



debug('begin add_signature: AR_RECON_LEDGER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7029',
      p_sig_id                 => 'AR_RECON_LEDGER',
      p_sig_sql                => 'select a.target_ledger_id ledger_id, a.target_ledger_name name, 
...
order by a.target_ledger_category_code',
      ...
      p_problem_descr          => 'Primary and Reporting ledger information.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_LEDGER');



debug('begin add_signature: AR_RECON_ORGS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7033',
      p_sig_id                 => 'AR_RECON_ORGS',
...
order by organization_id ASC',
      ...
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Operating Unit(s) Under Primary Ledger',
      p_solution               => 'No Other Operating Units Have Been Identified.',
      p_success_msg            => 'This is a data collection of Orgs under Primary ledger.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_ORGS');



debug('begin add_signature: AR_RECON_LEDGER_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7026',
      p_sig_id                 => 'AR_RECON_LEDGER_INFO',
      p_sig_sql                => 'select '' '' "Ledger and Operating Unit Info"
from dual',
      p_title                  => 'Ledger and Operating Unit Information',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Could not locate ledger and org information',
      p_solution               => 'This section is data collection of Ledger and Operating Unit setup.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('AR_RECON_LEDGER','AR_RECON_ORGS'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '10'
      );
   l_info.delete;
debug('end add_signature: AR_RECON_LEDGER_INFO');



debug('begin add_signature: AR_RECON_AR_SYSTEM_PARAMETERS');
   l_info('##MASK##%ATTRIBUTE%'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7035',
      p_sig_id                 => 'AR_RECON_AR_SYSTEM_PARAMETERS',
      p_sig_sql                => 'select *
from   ar_system_parameters_all
where  org_id = ##$$ORGID$$##',
      p_title                  => 'AR System Parameters',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Unable to locate AR System Parameter record.
',
      p_solution               => 'Please verify that you have properly set-up the System Parameter record for ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_success_msg            => 'This is a data collection of AR System Parameter information for ORG_ID = '||mask_text(g_org_id_tmp, nvl( upper(''), 'NO_MASK') )||'.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_AR_SYSTEM_PARAMETERS');



debug('begin add_signature: AR_RECON_FORMS_PERSONALIZATION_RULES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7036',
      p_sig_id                 => 'AR_RECON_FORMS_PERSONALIZATION_RULES',
      p_sig_sql                => 'SELECT id, function_name, description, trigger_event, trigger_object
...
OR     form_name like ''ARBRMAIN%''',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_FORMS_PERSONALIZATION_RULES');



debug('begin add_signature: AR_RECON_AR_SETUP_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7034',
      p_sig_id                 => 'AR_RECON_AR_SETUP_INFO',
...
and    AR.ORG_ID = ##$$ORGID$$##',
      ...
      p_solution               => 'This section is a data collection of AR setup information.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('AR_RECON_AR_SYSTEM_PARAMETERS','AR_RECON_FORMS_PERSONALIZATION_RULES'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '4'
      );
   l_info.delete;
debug('end add_signature: AR_RECON_AR_SETUP_INFO');



debug('begin add_signature: AR_RECON_SLAM_PRIMARY_LEDGER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7038',
      p_sig_id                 => 'AR_RECON_SLAM_PRIMARY_LEDGER',
...
and    accounting_method_code IN (select sla_accounting_method_code 
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_SLAM_PRIMARY_LEDGER');



debug('begin add_signature: AR_RECON_AAD_PRIMARY_LEDGER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7039',
      p_sig_id                 => 'AR_RECON_AAD_PRIMARY_LEDGER',
...
where  application_id = 222
and   (PRODUCT_RULE_CODE, amb_context_code) in (SELECT PRODUCT_RULE_CODE, amb_context_code
                                                FROM   xla_acctg_method_rules_fvl
...
',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_AAD_PRIMARY_LEDGER');



debug('begin add_signature: AR_RECON_SLA_SETUP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7037',
      p_sig_id                 => 'AR_RECON_SLA_SETUP',
      p_sig_sql                => 'select ''Displaying SLA Setup information for '' || a.target_ledger_name "SLA Setup Information"
from   gl_ledger_relationships a
where  primary_ledger_id = (select set_of_books_id
                            from   ar_system_parameters_all
                            where  org_id = ##$$ORGID$$##)
and    a.target_ledger_category_code = ''PRIMARY''',
      ...
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('AR_RECON_SLAM_PRIMARY_LEDGER','AR_RECON_AAD_PRIMARY_LEDGER'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '6'
      );
   l_info.delete;
debug('end add_signature: AR_RECON_SLA_SETUP');



debug('begin add_signature: AR_RECON_MFAR_NO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7040',
      p_sig_id                 => 'AR_RECON_MFAR_NO',
...
where  nvl(FND_PROFILE.value(''AR_MFAR_ACTIVATED''),''N'') = ''N''',
      p_title                  => 'MFAR Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Your system is NOT setup to use Multi-Fund Accounts Receivable (MFAR).',
...
Review [1503451.1] What is the Purpose of Profile Option AR: Activate Multi Fund Accounting Extract',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_MFAR_NO');



debug('begin add_signature: AR_RECON_MFAR_YES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7041',
      p_sig_id                 => 'AR_RECON_MFAR_YES',
      p_sig_sql                => 'SELECT decode(FND_PROFILE.value(''AR_MFAR_ACTIVATED''),
       ''Y'',''YES'',
       ''NULL'') MFAR_Enabled
from   dual
where  nvl(FND_PROFILE.value(''AR_MFAR_ACTIVATED''),''XX'') = ''Y''',
      p_title                  => 'MFAR Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Your system is setup to use Multi-Fund Accounts Receivable (MFAR).',
...
Review [1503451.1] What is the Purpose of Profile Option AR: Activate Multi Fund Accounting Extract',
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_MFAR_YES');
...
debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_AR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30251',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_AR',
...
and    a.primary_ledger_id = b.ledger_id
AND    gps.period_year in  (select distinct period_year
  from   gl_period_statuses gps2
    where ( gps2.start_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')  and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
            or
            gps2.end_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')  and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'') )
...
order by 1 desc, 2 asc, 6 asc',
      ...
      p_success_msg            => 'This is a data collection of period status information for Oracle Receivables for the year associated to the date range passed',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_AR');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30252',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_GL',
      p_sig_sql                => 'SELECT application_id, set_of_books_id, closing_status, period_year, period_name, start_date, end_date, to_char(last_update_date,''DD-MON-YYYY HH24:MI:SS'') last_update_date
FROM gl_period_statuses gps
WHERE application_id = 101
AND nvl(adjustment_period_flag,''N'') = ''N'' 
...
order by 1 desc, 2 asc, 6 asc',
      ...
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Period status information for General Ledger periods.',
      p_solution               => 'Please check your calendar setup for General Ledger',
      p_success_msg            => 'This is a data collection of period status information for General Ledger for the year associated to the date range passed',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_GL');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_COMPARE_GL_PERIOD_STATUSES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30253',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_COMPARE_GL_PERIOD_STATUSES',
    ...
    WHERE  gps.period_year in (select distinct period_year
  from   gl_period_statuses gps2
    where ( gps2.start_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')  and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
            or
            gps2.end_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'')  and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'') )
...
',
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_COMPARE_GL_PERIOD_STATUSES');



debug('begin add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_PERIOD_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30250',
      p_sig_id                 => 'EBS_FIN_AR_AR_RECON_ANALYZER_PERIOD_INFO',
...
from dual',
      ...
      p_solution               => 'This section is a data collection of period information for this transaction for Oracle Receivables (AR) and General Ledger (GL).',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_AR','EBS_FIN_AR_AR_RECON_ANALYZER_GL_PERIOD_GL','EBS_FIN_AR_AR_RECON_ANALYZER_COMPARE_GL_PERIOD_STATUSES'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '1'
      );
   l_info.delete;
debug('end add_signature: EBS_FIN_AR_AR_RECON_ANALYZER_PERIOD_INFO');



debug('begin add_signature: AR_RECON_CREATE_DETAILED_DIST_OK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7042',
      p_sig_id                 => 'AR_RECON_CREATE_DETAILED_DIST_OK',
      p_sig_sql                => 'select nvl(create_detailed_dist_flag,''Y'') "Create Detailed Distributions",
      ''##$$LINELEV$$##'' "Line Level Cash Applications",
      ''##$$MFAR$$##'' "Multi-Fund Accounts Receivable",
      ''##$$CASHBASIS$$##'' "Cash Basis Accounting"
from  ar_system_parameters_all
where org_id = ##$$ORGID$$##
and nvl(create_detailed_dist_flag,''Y'') = ''Y''
and (''##$$MFAR$$##'' = ''Y'' OR 
     ''##$$LINELEV$$##'' = ''Y'' OR
     ''##$$CASHBASIS$$##'' = ''Y'')',
      p_title                  => 'Create Detailed Distributions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Create Detailed Distributions in AR_SYSTEM_PARAMETERS_ALL is set to Yes.',
      p_solution               => 'A review of your setup shows that you are using one or more of the following features:
<ol>
<li> Line Level Cash Applications</li> 
<li> Multi-Fund Accounts Receivable</li> 
<li> Cash-Basis Accounting</li> 
</ol>

It is <b>correct</b> to have the <em>Create Detailed Distributions</em> checkbox checked.

You can read the following to learn more about this setting:<br>
<ul>
<li>[1081657.1] System Options Form: What is the Checkbox ''Create Detailed Distributions'' for?</li>
<li>[1374115.1] Overview of Receivables Merged Distributions Feature</li>
<li>[1537608.1] R12 SLA : How to Improve Create Accounting Performance For Receivables?</li>
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_CREATE_DETAILED_DIST_OK');



debug('begin add_signature: AR_RECON_CREATE_DETAILED_DIST_NOT_OK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7043',
      p_sig_id                 => 'AR_RECON_CREATE_DETAILED_DIST_NOT_OK',
      p_sig_sql                => 'select nvl(create_detailed_dist_flag,''Y'') "Create Detailed Distributions",
       ''##$$LINELEV$$##'' "Line Level Cash Applications",
       ''##$$MFAR$$##'' "Multi-Fund Accounts Receivable",
       ''##$$CASHBASIS$$##'' "Cash Basis Accounting"
...
and    ''##$$LINELEV$$##'' = ''N''
and    ''##$$CASHBASIS$$##'' = ''N''',
      p_title                  => 'Create Detailed Distributions Setting',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Create Detailed Distributions in AR_SYSTEM_PARAMETERS_ALL is unnecessarily set to Yes.',
      p_solution               => '<b>POTENTIAL IMPACT TO CREATE ACCOUNTING PERFORMANCE:</b><br>

You only need to use <em>Detailed Distributions</em> if you are using one of the following features:
<ol>
<li> Batch Sources that generate Line Level Balances</li> 
<li> Multi-Fund Accounts Receivable</li> 
<li> Cash-Basis Accounting</li> 
</ol>

The analyzer has checked your instance and you do not use these features. We recommended you do the following:<br>
<b>Navigation:</b> Setup > System > System Options <br>
Uncheck the checkbox for <em>Create Detailed Distributions</em>. <br><br>

This will minimize the number of distributions created during receipt applications which could drastically improve the performance of Create Accounting.<br> 

Read the following to learn more about this setting:<br>
<ul>
<li>[1081657.1] System Options Form: What is the Checkbox ''Create Detailed Distributions'' for?</li>
<li>[1374115.1] Overview of Receivables Merged Distributions Feature</li>
<li>[1537608.1] R12 SLA : How to Improve Create Accounting Performance For Receivables?</li>
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_CREATE_DETAILED_DIST_NOT_OK');



debug('begin add_signature: AR_RECON_PERF_PROFILES');
   l_info('##MASK##LEVEL VALUE'):= 'NO_MASK';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7044',
      p_sig_id                 => 'AR_RECON_PERF_PROFILES',
...
WHERE e.profile_option_name in (''AFLOG_ENABLED'', ''AR_ENABLE_DEBUG_OUTPUT'', ''XLA_DIAGNOSTIC_MODE'')
...
a.level_id DESC',
      p_title                  => 'Profiles Impacting Create Accounting Performance',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This is a data collection of debugging profile option settings.
',
      p_solution               => 'Only the system profile options with values defined for the provided responsibility and user are displayed.

<ul>
<li>FND: Debug Log Enabled</li>
<li>AR: Enable Debug Message Output</li>
<li>SLA: Enable Diagnostics</li>
</ul>

The above profiles should be set to Yes only when you are troubleshooting an issue. If any of them are set to Yes, and you are not debugging or troubleshooting an issue, please evaluate setting the values to No.
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_PERF_PROFILES');
...
debug('begin add_signature: AR_RECON_COUNT_DEBUG');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7045',
      p_sig_id                 => 'AR_RECON_COUNT_DEBUG',
      p_sig_sql                => 'select count(*) from FND_LOG_MESSAGES',
      p_title                  => 'Count of Debug Messages',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This is a data collection of the number of records in FND_LOG_MESSAGES. ',
      p_solution               => 'The number of records in the table FND_LOG_MESSAGES can potentially impact performance. Please perform the following:
<ol>
<li>Ensure the profile option FND: Debug Log Enabled is set to No at all levels</li>
<li>Truncate the FND_LOG_MESSAGES table</li>
</ol>
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: AR_RECON_COUNT_DEBUG');
...
BEGIN
  ...
  analyzer_title := 'Receivables Reconciliation Analyzer';
...
debug('begin section: RECON_COMPLEX');
start_section('Reconciliation Issues');
begin
   mo_global.init('AR');
   mo_global.set_policy_context('S',g_org_id_tmp);

   l_start_gl_date := p_gl_start_date;
   l_end_gl_date := p_gl_end_date;
   ...
   while l_start_gl_date <= l_end_gl_date loop
      ...
      -- this procedure fills in the daily_cal_t table one record at a time
      -- from start to end gl date
      ar_recon_analyzer_pkg.recon_by_gl_date(l_start_gl_date);

      if daily_calc_t(i).l_status <> 'Figures Match' then

         g_sql_tokens('##$$LSTRDATE$$##') := l_start_gl_date;

         -- now analyze transactions for this GL date
         ar_recon_analyzer_pkg.reset_trx_rec_t;

         j := 1;
         

         for inv_rec in inv_cur(l_start_gl_date) loop
             ...
             EXCEPTION
                  when no_data_found
                  then 
                  debug('Could not retrieve accounting_affect_flag for customer_trx_id: ' 
                         ||l_cust_trx_id);
             ...
             IF l_account_affect_flag = 'Y' THEN
                ...
                l_amount_due_original_inv := ar_recon_analyzer_pkg.calc_amount_due_original_inv(l_cust_trx_id);
                l_amount_due_rem_inv := 
ar_recon_analyzer_pkg.calc_amount_due_rem_inv(l_cust_trx_id);
                l_amount_app_adj_inv := ar_recon_analyzer_pkg.calc_amount_app_adj_inv(l_cust_trx_id);
                l_trx_diff := nvl(l_amount_due_rem_inv,0) - nvl(l_amount_app_adj_inv,0) - nvl(l_amount_due_original_inv,0);
            
                if l_trx_diff <> 0 then
                  ...
                  debug('Difference = ' || to_char(l_trx_diff));

                  trx_rec_t(j).l_cust_trx_id := l_cust_trx_id;
                  trx_rec_t(j).l_amount_due_original_inv := nvl(l_amount_due_original_inv,0);
                  trx_rec_t(j).l_amount_due_rem_inv := nvl(l_amount_due_rem_inv,0);
                  trx_rec_t(j).l_amount_app_adj_inv := nvl(l_amount_app_adj_inv,0);
                  trx_rec_t(j).l_trx_diff := nvl(l_trx_diff,0);
         ...
         -- now analyze receipt reversals
         ar_recon_analyzer_pkg.reset_rct_rec_t;

         k := 1;

         for get_cr_rec in get_cr_ids(l_start_gl_date) loop
             for ra_rev_gl_rec in ra_rev_gl_cur(get_cr_rec.cash_receipt_id) loop
                IF ra_rev_gl_rec.amount_applied <> 0 THEN
                   ...
                   rct_rec_t(k).l_cr_id := ra_rev_gl_rec.cr_id;
                   rct_rec_t(k).l_rec_id := ra_rev_gl_rec.rec_id;
                   rct_rec_t(k).l_rev_rec_id := ra_rev_gl_rec.rev_rec_id;
         ...
         set_item_result(run_stored_sig('AR_RECON_BY_GLDATE'));
      end if;
...
debug('end section: RECON_COMPLEX');

debug('begin section: POT_RECON_ITEMS');
IF p_pot_recon_items = 'Y' THEN
   start_section('Potential Reconciling Items', 'POT_RECON_ITEMS');
      set_item_result(run_stored_sig('AR_RECON_POT_RECON_ITEMS'));
   end_section;
END IF;
debug('end section: POT_RECON_ITEMS');

debug('begin section: DATA_INTEGRITY');
start_section('Data Integrity', 'DATA_INTEGRITY');
   set_item_result(run_stored_sig('AR_RECON_APP_GL_DATE_LESS_TRX_GL_DATE'));
   set_item_result(run_stored_sig('AR_RECON_APP_REVERSE_GL_DATE'));
   set_item_result(run_stored_sig('AR_RECON_RCPT_GL_DATE_CLOSED'));
   set_item_result(run_stored_sig('AR_RECON_TRX_GL_DATE_CLOSED'));
   set_item_result(run_stored_sig('AR_RECON_ACC_AFFECT_FLAG_N_PS_RECORD'));
   set_item_result(run_stored_sig('AR_RECON_DUP_REC_ROWS'));
   set_item_result(run_stored_sig('AR_RECON_APP_UNAP_GLDATE_MISMATCH'));
   set_item_result(run_stored_sig('AR_RECON_ADJ_AMOUNT_ACCTD_AMOUNT'));
   set_item_result(run_stored_sig('AR_RECON_APPLIED_PS_ID_MISMATCH'));
   set_item_result(run_stored_sig('AR_RECON_INCORRECT_PS_STATUS'));
   set_item_result(run_stored_sig('AR_RECON_INCORRECT_OP_PS_STATUS'));
   set_item_result(run_stored_sig('AR_RECON_CLAIM_CB_DRILLDOWN'));
   set_item_result(run_stored_sig('AR_RECON_OPHAN_CM_APPS'));
   set_item_result(run_stored_sig('AR_RECON_ORPHAN_CASH_APPS'));
   set_item_result(run_stored_sig('AR_RECON_ORPHAN_PS'));
   set_item_result(run_stored_sig('AR_RECON_AR_S_00033_OVERLAP_DISPUTE'));
   set_item_result(run_stored_sig('AR_RECON_CM_MISSING_PS'));
   set_item_result(run_stored_sig('AR_RECON_ADJ_GL_DATE_LESS_TRX_GL_DATE'));
   set_item_result(run_stored_sig('EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00147_REV_AND_REC_WITH_SAME_CCID'));
   set_item_result(run_stored_sig('EBS_FIN_AR_AR_RECON_ANALYZER_AR_S_00148_REV_WITH_REC_CCID'));
   set_item_result(run_stored_sig('EBS_FIN_AR_AR_RECON_ANALYZER_MISMATCH_GL_DATE_IN_GLD_XE'));
end_section;
debug('end section: DATA_INTEGRITY');

debug('begin section: XLA_GL_INTEGRITY');
IF p_xla_gl = 'Y' THEN
   start_section('XLA-GL Integrity', 'XLA_GL_INTEGRITY');
      set_item_result(run_stored_sig('AR_PC_XLA_GL_NEG_LEDGER_ID'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_EXISTS_IN_GL_NOT_IN_XLA'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_MULTIPLE_POST_IN_GL'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_NULL_GL_SL_LINK_ID'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_XFR_IN_XLA_NOT_IN_GL'));
      set_item_result(run_stored_sig('AR_PC_XLA_GL_XFR_IN_XLA_REV_IN_GL'));
      set_item_result(run_stored_sig('EBS_FIN_AR_PC_GL_DATE_MISMATCH_BETWEEN_XLA_AND_GL'));
      set_item_result(run_stored_sig('EBS_FIN_AR_PC_DUP_JOURNALS_POSTED_TO_XLA_AND_GL'));
   end_section;
END IF;
debug('end section: XLA_GL_INTEGRITY');

debug('begin section: CONSOLIDATED_SETUP');
start_section('Setup and Environment Information', 'CONSOLIDATED_SETUP');
   set_item_result(run_stored_sig('AR_RECON_LEDGER_INFO'));
   set_item_result(run_stored_sig('AR_RECON_AR_SETUP_INFO'));
   set_item_result(run_stored_sig('AR_RECON_SLA_SETUP'));
   set_item_result(run_stored_sig('AR_RECON_MFAR_NO'));
   set_item_result(run_stored_sig('AR_RECON_MFAR_YES'));
   set_item_result(run_stored_sig('AR_GEN_AAD_VERSIONS'));
   set_item_result(run_stored_sig('AR_GEN_XLA_TRANSFER_PKG_VERSION'));
   set_item_result(run_stored_sig('EBS_FIN_AR_AR_RECON_ANALYZER_PERIOD_INFO'));
...
start_section('Potential Performance Issues', 'PERFORMANCE');
   set_item_result(run_stored_sig('AR_RECON_CREATE_DETAILED_DIST_OK'));
   set_item_result(run_stored_sig('AR_RECON_CREATE_DETAILED_DIST_NOT_OK'));
   set_item_result(run_stored_sig('AR_RECON_PERF_PROFILES'));
   IF g_count_dbg_msg > 10000 THEN
      set_item_result(run_stored_sig('AR_RECON_COUNT_DEBUG'));
   END IF;
end_section;
...
BEGIN
  ...
  g_errbuf := null;
   ...
   main(
    ...
    ,p_resp_id                      => p_resp_id
    ,p_org_id                       => p_org_id
    ,p_gl_start_date                => FND_DATE.canonical_to_date(p_gl_start_date)
    ,p_gl_end_date                  => FND_DATE.canonical_to_date(p_gl_end_date)
    ,p_pot_recon_items              => p_pot_recon_items
    ,p_xla_gl                       => upper(p_xla_gl)
    ,p_apps_check                   => upper(p_apps_check)
    ,p_max_output_rows              => p_max_output_rows
