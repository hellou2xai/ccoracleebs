...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for EBS Period Close Process Analyzer      |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE ebs_pcp_analyzer_pkg AUTHID DEFINER AS
...
(            p_ledger_id                    IN NUMBER      DEFAULT NULL
           ...
           ,p_max_output_rows              IN NUMBER      DEFAULT 1000
...
END ebs_pcp_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY ebs_pcp_analyzer_pkg AS
-- $Id: ebs_pcp_analyzer.sql, 200.22 2025/12/08 13:35:26 kobeid Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2468739.1));
...
----------------------------------------------------------------
  cursor get_orgs_using_sob (orgid IN NUMBER, sobid IN NUMBER) is
  select org_id
  from ar_system_parameters_all
  where set_of_books_id = sobid
  and org_id <> orgid;

  cursor get_orgs (sobid IN NUMBER) is
  select organization_id, 
	 name
  from  hr_operating_units
  where set_of_books_id = sobid;

  g_organization_id             NUMBER := 0;
  g_organization_name           VARCHAR2(240) := null;

  g_count_dbg_msg               NUMBER := 0;
  g_org_id                      NUMBER := 0;
  g_ledger_id                   NUMBER := 0;
  g_org_id_tmp                  NUMBER := 0;
  g_period_tmp                  VARCHAR2(15) := null;
  g_user_name_tmp               VARCHAR2(100) := 0;
  g_resp_id_tmp                 NUMBER := 0;
  g_resp_name                   VARCHAR2(100) := null;
  g_apps_ver                    FND_PRODUCT_GROUPS.RELEASE_NAME%TYPE;
  g_ou_name                     VARCHAR2(240);
  g_pa_count                    NUMBER := 0;  
  l_str_date                    VARCHAR2(25);



  -- product error flags
  g_AR_chk1 VARCHAR2(1) := 'N'; -- unprocessed events
  g_AR_chk2 VARCHAR2(1) := 'N'; -- unposted items
  g_AR_chk3 VARCHAR2(1) := 'N'; -- unposted quickcash
  g_AR_chk4 VARCHAR2(1) := 'N'; -- rev rec not run
  g_AR_chk5 VARCHAR2(1) := 'N'; -- incomplete trx
  g_AR_chk6 VARCHAR2(1) := 'N'; -- adj pending approval

  l_start_date VARCHAR2(25);
  l_end_date   VARCHAR2(25);
  l_org_id     NUMBER;
  l_org_string VARCHAR2(250) := ' ';
  l_org        NUMBER;
  l_oth_orgs   NUMBER := 2; -- used for rownum comparison in AR_PC_UNPOSTED_OTHER_ORG

  -- define globals to use for processing Create Detailed Distributions flag
  g_mfar             VARCHAR2(1) := 'N';
  g_batch_line_level VARCHAR2(1) := 'N';
  g_cash_basis       VARCHAR2(1) := 'N';
  g_create_detailed  VARCHAR2(1) := 'N';
  g_show_detaildist_sig VARCHAR2(1) := 'N';

  -- min/max IDs to improve performance
  g_min_entity_id   NUMBER := 0;
  g_max_entity_id   NUMBER := 0;
  g_min_event_id    NUMBER := 0;
  g_max_event_id    NUMBER := 0;
  g_min_header_id   NUMBER := 0;
  g_max_header_id   NUMBER := 0;
  
  g_status_CRM VARCHAR2(100) := null;
  g_status_OKL VARCHAR2(100) := null;
  g_status_AR VARCHAR2(100) := null;
  g_status_AP VARCHAR2(100) := null;
  g_status_GL VARCHAR2(100) := null;
  g_status_INV VARCHAR2(100) := null;
  g_status_PO VARCHAR2(100) := null;
  g_status_FA VARCHAR2(100) := null;
  g_status_PA VARCHAR2(100) := null;


-- PA period check
g_pa_enh_period_profile_option VARCHAR2(1) := 'N';
g_pa_maintain_common_period VARCHAR2(1) := 'N';
g_pa_NN VARCHAR2(1) := ' ';
g_sol_msg VARCHAR2(2000) := ' ';
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'EBS_PCP_ANALYZER_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'EBS_PCP_ANALYZER_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>EBS_PCP_ANALYZER Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2468739.1#feedback");
...
-- page title
    print_out('        <div class="header_title">'||p_analyzer_title|| ' Report <br>
        ...
        <img class="header_version" src="https://www.oracle.com/webfolder/s/analyzer/ebs_pcp_analyzer_latest_version.gif" title="Click here to download the latest version of Analyzer" alt="Latest Version Icon"></a>');
    print_cloud_image;
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1654,EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="thumb_up_lg" title="Analyzer Feedback: Thumbs Up"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1654,EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="thumb_dn_lg" title="Analyzer Feedback: Thumbs Down"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1654,EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="af_idea_lg" title="Analyzer Idea"></span></a>');
    print_out('        </div>
    </div>');
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/4186072" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">1654</span>
...
<p>What&apos;s new:</p><ul><li>Replaced "From Projects: Control &gt; Request &gt; Run: PRC: Generate Cost Accounting Events." with Run the appropriate cost distribution process.</li>
</ul>
<p>&nbsp;</p>
            <br><br>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1654,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1654,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1654,'||p_sig.sigrepo_id||',EBS,EBS Financials,EBS - Financial Control" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
debug('begin populate parameters hash table');
   ...
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'GL Period Name';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_period_name,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  l_revision := rtrim(replace('$Revision: 200.22  $','$',''));
  l_revision := ltrim(replace(l_revision,'Revision:',''));
  l_date_char := rtrim(replace('$Date: 2025/12/08 13:35:24 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'ebs_pcp_analyzer.sql';
...
INTO l_exists_val
FROM GL_LEDGERS GL
WHERE GL.LEDGER_ID = p_ledger_id and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Invalid Ledger.');
   raise invalid_parameters;
   WHEN OTHERS THEN
...
debug('begin parameter validation: p_period_name');
IF p_period_name IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter GL Period Name is required.');
   raise invalid_parameters;
END IF;
...
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Invalid period name.');
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_period_name failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
debug('end parameter validation: p_period_name');
...
begin
   ...
   g_period_tmp := p_period_name;
   g_ledger_id := p_ledger_id;

/*
begin
   -- only run PA section when count > 1
   select count(*)
   into g_pa_count
   from PA_RCV_EXCEPTIONS_SUM_V
   where GL_PERIOD_NAME = g_period_tmp
...
begin
   -- get start and end date from period name
   print_log('select 1.');
   select to_char(start_date,'DD-MON-YYYY'), 
          to_char(end_date,'DD-MON-YYYY'),
          '<em>' || decode(closing_status,'O','Open',
                          'N','Not Opened','C','Closed','F','Future','W','Close Pending',null) || '</em>'						  
  into l_start_date, 
        l_end_date, 
        g_status_AR
   from gl_period_statuses
   where period_name = g_period_tmp
   and set_of_books_id = g_ledger_id 
   and application_id = 222;

exception
when no_Data_found then
   g_status_AR := '<em> Not Defined </em>';
end;

begin
   print_log('select 2.');
   select '<em>' || decode(closing_status,'O','Open',
                          'N','Not Opened','C','Closed','F','Future','W','Close Pending',null) || '</em>'						  
   into g_status_AP
   from gl_period_statuses
   where period_name = g_period_tmp
...
exception
when no_Data_found then
   g_status_PO := '<em> Not Defined </em>';
end;

begin
      print_log('select 5.');
      select 'GL Period is <em>' || decode(closing_status,'O','Open',
                             'N','Not Opened','C','Closed','F','Future','W','Close Pending',null) || '</em>',
                   'Displaying General Ledger period status for the subledger applications for the '|| g_period_tmp || ' period.'
      into g_status_PA,  g_sol_msg
      from gl_period_statuses
      where period_name = g_period_tmp
      and set_of_books_id = g_ledger_id 
      and application_id = 275;

      -- check profile and system settings for how PA/GL periods are handled

      begin
         print_log('select 6.');
         select profile_option_value
            into g_pa_enh_period_profile_option
     ...
     exception
     when no_data_found then
         g_pa_enh_period_profile_option := 'N';
     end;

    begin
        print_log('select 7.');
        select distinct pi.SAME_PA_GL_PERIOD
           into g_pa_maintain_common_period 
         from pa_implementations_all pi,
                    hr_operating_units org
          where pi.org_id = org.organization_id
         and org.set_of_books_id = g_ledger_id;

         null;
     exception
     when no_data_found then
         g_pa_maintain_common_period := 'N';
     end;

    if g_pa_enh_period_profile_option = 'N' and 
       g_pa_maintain_common_period  = 'N' then
        g_pa_NN := '*';
        g_status_PA := ' ';
        g_sol_msg := '<ul>
<li>Displaying General Ledger period status for the subledger applications for the '|| g_period_tmp || ' period.</li>
<li>*For more information on GL period status in PA see [2738998.1]</li>
</ul>';
   end if;

exception
when no_Data_found then
    g_status_PA := '<em> Not Defined </em>';
end;  
...
exception
when no_Data_found then
   g_status_OKL := '<em> Not Defined </em>';
end;
...
exception
when no_Data_found then
   g_status_CRM := '<em> Not Defined </em>';
end;
...
exception
when no_Data_found then
   g_status_INV := '<em> Not Defined </em>';
end;


   select '<em>' || decode(period_close_date,NULL, 'OPEN','CLOSED') || '</em>'

   select  max(decode(fdp.period_close_date,NULL, 'OPEN','CLOSED')) "PERIOD STATUS" 
   into    g_status_FA
   from    FA_BOOK_CONTROLS fbc,
           FA_DEPRN_PERIODS fdp
   where   fbc.book_type_code = fdp.book_type_code
   and     fbc.book_class <> 'BUDGET'
   and     fbc.org_id is not null
   and     fbc.set_of_books_id =  g_ledger_id
   and     fdp.calendar_period_open_date = l_start_date
   and     fdp.calendar_period_close_date = l_end_date
   order      by 1

exception
when no_Data_found then
   g_status_FA := '<em> Not Defined </em>';
end;
...
*/
  ...
  exception
  when no_data_found then
     print_log('Invalid Ledger or Period Name values.');
     raise invalid_parameters;
  end;
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$ENDDATE$$##') := l_end_date;
   g_masked_tokens('##$$ENDDATE$$##') := mask_text( g_sql_tokens('##$$ENDDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$MAXENTITYID$$##') := g_max_entity_id;
   g_masked_tokens('##$$MAXENTITYID$$##') := mask_text( g_sql_tokens('##$$MAXENTITYID$$##'), 'NO_MASK');
   g_sql_tokens('##$$MINEVENTID$$##') := g_min_event_id;
   g_masked_tokens('##$$MINEVENTID$$##') := mask_text( g_sql_tokens('##$$MINEVENTID$$##'), 'NO_MASK');
   g_sql_tokens('##$$MAXEVENTID$$##') := g_max_event_id;
   g_masked_tokens('##$$MAXEVENTID$$##') := mask_text( g_sql_tokens('##$$MAXEVENTID$$##'), 'NO_MASK');
   g_sql_tokens('##$$ORGID$$##') := g_organization_id;
   g_masked_tokens('##$$ORGID$$##') := mask_text( g_sql_tokens('##$$ORGID$$##'), 'NO_MASK');
   g_sql_tokens('##$$PERIODNAME$$##') := g_period_tmp;
   g_masked_tokens('##$$PERIODNAME$$##') := mask_text( g_sql_tokens('##$$PERIODNAME$$##'), 'NO_MASK');
   g_sql_tokens('##$$STARTDATE$$##') := l_start_date;
   g_masked_tokens('##$$STARTDATE$$##') := mask_text( g_sql_tokens('##$$STARTDATE$$##'), 'NO_MASK');
   g_sql_tokens('##$$ROWLIMIT$$##') := '25';
   g_masked_tokens('##$$ROWLIMIT$$##') := mask_text( g_sql_tokens('##$$ROWLIMIT$$##'), 'NO_MASK');
   g_sql_tokens('##$$MINENTITYID$$##') := g_min_entity_id;
   g_masked_tokens('##$$MINENTITYID$$##') := mask_text( g_sql_tokens('##$$MINENTITYID$$##'), 'NO_MASK');
   g_sql_tokens('##$$MINHEADERID$$##') := g_min_header_id;
   g_masked_tokens('##$$MINHEADERID$$##') := mask_text( g_sql_tokens('##$$MINHEADERID$$##'), 'NO_MASK');
   g_sql_tokens('##$$MAXHEADERID$$##') := g_max_header_id;
   g_masked_tokens('##$$MAXHEADERID$$##') := mask_text( g_sql_tokens('##$$MAXHEADERID$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK1$$##') := g_AR_chk1;
   g_masked_tokens('##$$AR_CHK1$$##') := mask_text( g_sql_tokens('##$$AR_CHK1$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK2$$##') := g_AR_chk2;
   g_masked_tokens('##$$AR_CHK2$$##') := mask_text( g_sql_tokens('##$$AR_CHK2$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK3$$##') := g_AR_chk3;
   g_masked_tokens('##$$AR_CHK3$$##') := mask_text( g_sql_tokens('##$$AR_CHK3$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK4$$##') := g_AR_chk4;
   g_masked_tokens('##$$AR_CHK4$$##') := mask_text( g_sql_tokens('##$$AR_CHK4$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK5$$##') := g_AR_chk5;
   g_masked_tokens('##$$AR_CHK5$$##') := mask_text( g_sql_tokens('##$$AR_CHK5$$##'), 'NO_MASK');
   g_sql_tokens('##$$AR_CHK6$$##') := g_AR_chk6;
   g_masked_tokens('##$$AR_CHK6$$##') := mask_text( g_sql_tokens('##$$AR_CHK6$$##'), 'NO_MASK');
   g_sql_tokens('##$$ORGNAME$$##') := g_organization_name;
   g_masked_tokens('##$$ORGNAME$$##') := mask_text( g_sql_tokens('##$$ORGNAME$$##'), 'NO_MASK');
   g_sql_tokens('##$$PA_NN$$##') := g_pa_nn;
   g_masked_tokens('##$$PA_NN$$##') := mask_text( g_sql_tokens('##$$PA_NN$$##'), 'NO_MASK');
...
debug('begin add_signature: PCA_GL_LEDGER_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11543',
      p_sig_id                 => 'PCA_GL_LEDGER_INFO',
      p_sig_sql                => 'select   ledger_id, name, short_name, description
from     gl_ledgers
where    ledger_id = ##$$LEDGERID$$##',
      p_title                  => 'Ledger Information',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Ledger Information.',
      p_solution               => 'No ledger has been defined.',
      p_success_msg            => 'Displaying ledger information.',
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_LEDGER_INFO');



debug('begin add_signature: PCA_HR_ORG_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11544',
      p_sig_id                 => 'PCA_HR_ORG_INFO',
      p_sig_sql                => 'SELECT organization_id org_id, name, date_from, date_to
from   HR_OPERATING_UNITS
where  SET_OF_BOOKS_ID = ##$$LEDGERID$$##
order by organization_id ASC',
      p_title                  => 'Operating Units Under Ledger',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Operating unit information under ledger.',
      p_solution               => 'There are no operating units defined under ledger.',
      p_success_msg            => 'Displaying operating units under ledger.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_HR_ORG_INFO');



debug('begin add_signature: PCA_INV_ORG_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11545',
      p_sig_id                 => 'PCA_INV_ORG_INFO',
      p_sig_sql                => 'select organization_id inv_org, organization_name name, organization_code, operating_unit org_id
from  org_organization_definitions
where  set_of_books_id = ##$$LEDGERID$$##
order by organization_id ASC',
      p_title                  => 'Inventory Organization Information Under Ledger',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Inventory organization information under Ledger.',
      p_solution               => 'There are no Inventory organizations under ledger.',
      p_success_msg            => 'Displaying Inventory organization information under Ledger.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_ORG_INFO');



debug('begin add_signature: PCA_GL_PERIOD_STATUS_FOR_APPS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10625',
      p_sig_id                 => 'PCA_GL_PERIOD_STATUS_FOR_APPS',
      p_sig_sql                => 'SELECT apps.application_name Application,
decode(ps.closing_status , ''N'', ''Never Opened'', ''O'', ''Open'', ''C'', ''Closed'', ''F'', ''Future Entry'', ''P'', ''Permanently Closed'',ps.closing_status) || decode(apps.application_id, 275,''##$$PA_NN$$##'','' '') "PERIOD STATUS",
decode(i.status,''I'',''Installed'',''S'',''Shared'') "PRODUCT STATUS"
FROM gl_period_statuses ps,
fnd_application_vl apps, 
fnd_product_installations i
WHERE apps.application_id = ps.application_id
AND ps.application_id = i.application_id
and i.application_id in (540,682,660,260,185,200,222,201,275,140,101)
AND i.status in (''I'',''S'')
AND ps.period_name = ''##$$PERIODNAME$$##''
AND ps.ledger_id = ##$$LEDGERID$$##
order by decode(apps.application_id,540,1, 682,2, 660,3, 260,4, 200,5, 222,6, 201,7, 275,8, 140,9, 101,10)',
      p_title                  => 'General Ledger Period Status for Subledgers for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' Period',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No General Ledger periods have been defined for the subledger applications for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => 'No General Ledger periods have been defined for the subledger applications for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_success_msg            => ''||mask_text(g_sol_msg, nvl( upper(''), 'NO_MASK') )||'',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_PERIOD_STATUS_FOR_APPS');



debug('begin add_signature: PCA_INV_PERIOD_STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11179',
      p_sig_id                 => 'PCA_INV_PERIOD_STATUS',
      p_sig_sql                => 'select per.organization_id Organization_id, 
       org.organization_code Code, 
       org.organization_name Name, 
       per.Period_name, 
       per.open_flag Open, 
       per.period_close_date, 
       mtl.WMS_Enabled_Flag,
       look.meaning ||''( ''|| mtl.PRIMARY_COST_METHOD ||'' )'' Cost_Method
from  org_acct_periods per, org_organization_definitions org, mtl_parameters mtl,  mfg_lookups look 
where per.organization_id = org.organization_id
and   mtl.organization_id = org.organization_id
and   org.set_of_books_id = ##$$LEDGERID$$##
and per.period_name = ''##$$PERIODNAME$$##''
and   look.lookup_type = ''MTL_PRIMARY_COST''
and mtl.primary_cost_method = look.lookup_code',
      p_title                  => 'Inventory Organization Accounting Period Status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' Period',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Inventory organization accounting period status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => 'No Inventory organization accounting periods have been defined in Inventory for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_success_msg            => 'Displaying Inventory organization accounting periods information in Inventory Org for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PERIOD_STATUS');



debug('begin add_signature: PCA_PA_PERIOD_STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11207',
      p_sig_id                 => 'PCA_PA_PERIOD_STATUS',
      p_sig_sql                => 'select 	pap.status, pap.gl_period_name, pap.period_name PA_PERIOD_NAME, pap.start_date, pap.end_date, pap.org_id 
from   	pa_periods_all pap,
		hr_operating_units org		
where   pap.org_id = org.organization_id	
and    	org.set_of_books_id = ##$$LEDGERID$$##
and     gl_period_name = ''##$$PERIODNAME$$##''
order by pap.org_id, pap.start_date',
      p_title                  => 'Project Accounting Period Status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' Period',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Project Accounting period status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => 'No Project Accounting periods have been defined in for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_success_msg            => 'Displaying Project Accounting period status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_PERIOD_STATUS');



debug('begin add_signature: PCA_FA_BOOKS_STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11180',
      p_sig_id                 => 'PCA_FA_BOOKS_STATUS',
      p_sig_sql                => 'select  fbc.book_type_name,  
	fbc.org_id,fdp.period_name, 
	calendar_period_open_date GL_START_DATE, 
	calendar_period_close_date  GL_END_DATE,
        decode(fbc.deprn_status,''C'',''Completed'',''E'',''Error'',''R'',''Running'',''S'',''Submitted'') "DEPRECIATION STATUS",
        decode(period_close_date,NULL, ''OPEN'',''CLOSED'') "PERIOD STATUS" 
from    FA_BOOK_CONTROLS fbc,
        FA_DEPRN_PERIODS fdp
where   fbc.book_type_code = fdp.book_type_code
and     fbc.book_class <> ''BUDGET''
and     fbc.org_id is not null
and     fbc.set_of_books_id =  ##$$LEDGERID$$##
and     fdp.calendar_period_open_date = to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') 
and     fdp.calendar_period_close_date = to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
order      by 1',
      p_title                  => 'Asset book status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' Period',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Asset books status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => 'No Asset books periods have been defined for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_success_msg            => 'Displaying Asset book period status for the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_BOOKS_STATUS');



debug('begin add_signature: PCA_OKL_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11572',
      p_sig_id                 => 'PCA_OKL_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
UNACC AS
( select /*+ materialize */
xte.security_id_int_1 org_id, count(xle.event_id) count_rows
FROM xla_events xle,
xla_transaction_entities_upg xte,
xla_ledger_options xlo
WHERE NVL(xte.ledger_id, -99) = ##$$LEDGERID$$##
AND xle.event_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
AND glr2.target_ledger_id = ##$$LEDGERID$$##
...
AND xte.application_id = 540
group by xte.security_id_int_1
having count(xle.event_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unacc.count_rows || '' unprocessed events.'' Status
FROM
ORGS,
UNACC
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Events in OKL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in OKL. Unprocessed events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Lease Management: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please review Lease Accounting in R12.1 Oracle Lease and Finance Management [833044.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Lease Management.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed events in OKL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OKL_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_OKL_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11580',
      p_sig_id                 => 'PCA_OKL_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
INVAL_EVENTS AS
( select /*+ materialize */
xlt.security_id_int_1 org_id,count(*) count_rows
from xla_accounting_errors xae, xla_transaction_entities_upg xlt, xla_events xe
where xe.application_id = 540
and xe.process_status_code = ''I''
and xe.event_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and xlt.ledger_id = ##$$LEDGERID$$##
and xae.event_id = xe.event_id
and xlt.entity_id = xe.entity_id
group by xlt.security_id_int_1
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || inval_events.count_rows || '' invalid events.'' Status
FROM
ORGS,
INVAL_EVENTS
where
orgs.organization_id = inval_events.org_id',
      p_title                  => 'Invalid Events in OKL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in OKL. Invalid events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Lease Management: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please review Lease Accounting in R12.1 Oracle Lease and Finance Management [833044.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Lease Management.</li>
</ul>',
      p_success_msg            => 'There are no invalid SLA events in OKL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OKL_INVALID_EVENTS');



debug('begin add_signature: PCA_OKL_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11581',
      p_sig_id                 => 'PCA_OKL_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
NONXFR AS
( select /*+ materialize */
xte.security_id_int_1 org_id,count(*) count_rows
FROM XLA_AE_HEADERs XAH,
XLA_TRANSACTION_ENTITIES_UPG XTE
WHERE xah.application_id = XTE.APPLICATION_ID
AND xah.ledger_id = XTE.ledger_id
AND XAH.ENTITY_ID = XTE.ENTITY_ID
AND xah.LEDGER_ID = ##$$LEDGERID$$##
AND xah.application_id = 540
AND xah.gl_transfer_status_code = ''N''
AND xah.accounting_entry_status_code = ''F''
AND xah.accounting_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY xte.security_id_int_1
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || nonxfr.count_rows || '' untransferred events.'' Status
FROM
ORGS,
NONXFR
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in OKL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in OKL. Untransferred events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.
',
      p_solution               => '<ul>
<li>From Lease Management: Control > Request > Run: Transfer Journal Entries to GL</li>
<li>To gather more details regarding this issue, please review Lease Accounting in R12.1 Oracle Lease and Finance Management [833044.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Lease Management.</li>
</ul>
',
      p_success_msg            => 'There are no untransferred events in OKL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OKL_NONXFR_EVENTS');



debug('begin add_signature: PCA_OKL_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10234',
      p_sig_id                 => 'PCA_OKL_SUMMARY',
      p_sig_sql                => 'select ''Lease and Finance Management'' application_name, ''This section checks for Lease and Finance Management (OKL) issues.'' "Description"
from dual',
      p_title                  => 'Lease Management (OKL) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Lease Management (OKL) issues identified.',
      p_solution               => 'Please open this section to view Lease Management (OKL) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_OKL_UNPROCESSED_EVENTS','PCA_OKL_INVALID_EVENTS','PCA_OKL_NONXFR_EVENTS'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '40'
      );
   l_info.delete;
debug('end add_signature: PCA_OKL_SUMMARY');



debug('begin add_signature: PCA_CRM_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11567',
      p_sig_id                 => 'PCA_CRM_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Events in ChRM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in ChRM. Unprocessed events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Trade Management Administrator: Run Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Channel Revenue Management (ChRM) SLA Unprocessed Transactions Analyzer [2058083.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Channel Revenue Management and upload the Oracle Channel Revenue Management Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed events in ChRM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CRM_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_CRM_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11571',
      p_sig_id                 => 'PCA_CRM_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = inval_events.org_id',
      p_title                  => 'Invalid Events in ChRM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in ChRM. Invalid events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Trade Management: Control > Request > Run: Create Accounting</li>
<li>To gather more details regarding this issue, please run the Oracle Channel Revenue Management (ChRM) SLA Unprocessed Transactions Analyzer [2058083.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Channel Revenue Management and upload the Oracle Channel Revenue Management Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no invalid events in ChRM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CRM_INVALID_EVENTS');



debug('begin add_signature: PCA_CRM_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11562',
      p_sig_id                 => 'PCA_CRM_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in ChRM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred  events in ChRM. Untransferred events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Trade Management: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Oracle Channel Revenue Management (ChRM) SLA Unprocessed Transactions Analyzer [2058083.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Channel Revenue Management and upload the Oracle Channel Revenue Management Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in ChRM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CRM_NONXFR_EVENTS');



debug('begin add_signature: PCA_CRM_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11561',
      p_sig_id                 => 'PCA_CRM_SUMMARY',
      p_sig_sql                => 'select ''Channel Revenue Management'' application_name, ''This section checks for Trade Mgmt / Channel Revenue Mgmt (CRM) issues.'' "Description"
from dual',
      p_title                  => 'Trade Mgmt / Channel Revenue Mgmt (ChRM)',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Trade Mgmt / Channel Revenue Mgmt (ChRM) issues identified.',
      p_solution               => 'Please open this section to view Trade Mgmt / Channel Revenue Mgmt (ChRM) issues identified. ',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_CRM_UNPROCESSED_EVENTS','PCA_CRM_INVALID_EVENTS','PCA_CRM_NONXFR_EVENTS'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '41'
      );
   l_info.delete;
debug('end add_signature: PCA_CRM_SUMMARY');



debug('begin add_signature: PCA_OM_SALES_ORDER_LINES_NO_INVOICE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11395',
      p_sig_id                 => 'PCA_OM_SALES_ORDER_LINES_NO_INVOICE',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
INT AS
( select /*+ materialize */
oel.org_id org_id, count(oel.line_number) count_rows
...
and oel.org_ID = org.organization_id
and NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
and oel.fulfillment_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
group by oel.org_id
having count(oel.line_number) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || int.count_rows || '' OM Sales order lines not invoiced to AR.'' Status
FROM
ORGS,
INT
where
orgs.organization_id = int.org_id',
      p_title                  => 'Sales Order Lines Not Invoices to Receivables from OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found Sales Order lines not interfaced or invoiced to Receivables from OM.   Sales Order lines not interfaced to receivables will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period. ',
      p_solution               => '<ul>
<li>From Order Management: Orders, Returns > Sales Orders.</li>
<li><li>To gather more details regarding this issue, please run EBS Order Management Sales Order Analyzer [2038363.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no Sales Order lines not interfaced to Receivables from OM.',
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_SALES_ORDER_LINES_NO_INVOICE');



debug('begin add_signature: PCA_OM_SALES_ORDER_RA_INTERFACE_ERRORS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11394',
      p_sig_id                 => 'PCA_OM_SALES_ORDER_RA_INTERFACE_ERRORS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
INT AS
( select /*+ materialize */
ril.org_id  org_id, count(ril.interface_line_attribute6) count_rows
...
and nvl(ril.set_of_books_id, -99) = ##$$LEDGERID$$##
and ril.creation_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
group by ril.org_id
having count(ril.interface_line_attribute6) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || int.count_rows || '' OM Orders in error to AR.'' Status
FROM
ORGS,
INT
where
orgs.organization_id = int.org_id',
      p_title                  => 'Orders Interfaced From OM to AR in Error',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found orders interfaces from OM to AR in error. Orders from OM not interfaced to AR will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Receivables: Control > Request > Run: Oracle Receivables AutoInvoice Analyzer in [1523525.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables AutoInvoice Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no orders interfaces from OM to AR in error.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_SALES_ORDER_RA_INTERFACE_ERRORS');



debug('begin add_signature: PCA_OM_PRE_BILLING_IMPLICIT_ACCEPTANCE2');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13336',
      p_sig_id                 => 'PCA_OM_PRE_BILLING_IMPLICIT_ACCEPTANCE2',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
from org_organization_definitions
where set_of_books_id = ##$$LEDGERID$$##),
ACC AS
( select /*+ materialize */
lin.org_id, count(ord.order_number) count_rows
from OE_ORDER_LINES_ALL LIN,
OE_ORDER_HEADERS_ALL ORD,
MTL_SYSTEM_ITEMS ITM,
org_organization_definitions org
where NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
and ord.ORDERED_DATE between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
and LIN.ORG_ID = org.organization_id
and LIN.REVREC_EVENT_CODE = ''CUSTOMER_ACCEPTANCE''
and LIN.REVREC_IMPLICIT_FLAG = ''Y''
group by lin.org_id
having count(ord.order_number) > 0 )
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || acc.count_rows || '' pre-billing acceptance records missing acceptance.'' Status
FROM
INVORGS,
ACC
where
invorgs.organization_id = acc.org_id',
      p_title                  => 'Pre-Billing With Implicit Acceptance in OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pre-billing acceptance records which have not been accepted in OM.  Non acceptance records will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Order Management: Control > Request > Run: Implicit Acceptance Request Set.</li>  </li>  
<li>For more information regarding this issue, review [863454.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no customer acceptance records which have not been formally accepted in OM. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_PRE_BILLING_IMPLICIT_ACCEPTANCE2');



debug('begin add_signature: PCA_OM_OPEN_LINES_OPEN_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11390',
      p_sig_id                 => 'PCA_OM_OPEN_LINES_OPEN_ERROR',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
EXP AS
( select /*+ materialize */
del.organization_id org_id, count(exp.delivery_detail_id) count_rows
FROM wsh_exceptions exp,
     wsh_delivery_details del,
	 org_organization_definitions org
WHERE NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
AND exp.creation_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND exp.status IN (''OPEN'', ''ERROR'', ''IN_PROGRESS'')
AND exp.severity = ''ERROR''
AND exp.delivery_detail_id = del.delivery_detail_id
AND del.released_status <> ''C''
AND del.ORGANIZATION_ID =  org.organization_id
group by del.ORGANIZATION_ID
having count(exp.delivery_detail_id) > 0 )
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || exp.count_rows || '' open error exceptions.'' Status
FROM
INVORGS,
EXP
where
invorgs.organization_id = exp.org_id',
      p_title                  => 'Open Lines With Error Exceptions in OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found open lines with open exceptions in OM.  Open error exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Order Management: Control > Request > Run: Resolve/Purge Exceptions.</li>  
<li>For more information regarding this issue, review [1576170.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no open lines with error exceptions in OM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_OPEN_LINES_OPEN_ERROR');



debug('begin add_signature: PCA_OM_SHIPPED_LINES_NOT_INV_INTERFACED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11392',
      p_sig_id                 => 'PCA_OM_SHIPPED_LINES_NOT_INV_INTERFACED',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
INT AS
( select /*+ materialize */
wdd.ORGANIZATION_ID org_id, count(wdd.source_header_number) count_rows
FROM wsh_delivery_details wdd,
	 ...
	 org_organization_definitions org
WHERE NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
AND wts.actual_departure_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
having count(wdd.source_header_number) > 0 )
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || int.count_rows || '' shipped lines not interfaced to inventory.'' Status
FROM
INVORGS,
INT
where
invorgs.organization_id = int.org_id',
      p_title                  => 'Shipped Lines Not Interfaced to Inventory From OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found shipped lines not interfaced to Inventory from OM.  Shipped lines not interfaced to inventory will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period. However these can prevent you from closing Inventory Period.',
      p_solution               => '<ul>
<li>From Order Management: Shipping > Interfaces > Run: Interface Trip Stop.</li>  
<li>For more information regarding this issue, review [885949.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no shipped lines not interfaced to inventory from OM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_SHIPPED_LINES_NOT_INV_INTERFACED');



debug('begin add_signature: PCA_OM_WSH_INV_PENDING_INTERFACE_MTL_TXN_DONE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11388',
      p_sig_id                 => 'PCA_OM_WSH_INV_PENDING_INTERFACE_MTL_TXN_DONE',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
org_organization_definitions org
WHERE NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
AND wdd.creation_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND wdd.ORGANIZATION_ID = org.organization_id
...
having count(wdd.source_header_number) > 0 )
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || int.count_rows || '' delivery lines not interfaced to inventory.'' Status
FROM
INVORGS,
INT
where
invorgs.organization_id = int.org_id',
      p_title                  => 'Delivery Lines Not Interfaced to Inventory From OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found delivery lines already interfaced to Inventory from OM but not marked as INV interfaced. Delivery lines not marked as interfaced to inventory will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period. However these can prevent you from closing Inventory Period.',
      p_solution               => '<ul>
<li>From Order Management: Shipping > Interfaces > Run: Interface Trip Stop.</li>
<li>For more information regarding this issue, review [2226194.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no delivery lines not interfaced to Inventory from OM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_WSH_INV_PENDING_INTERFACE_MTL_TXN_DONE');



debug('begin add_signature: PCA_OM_INTERFACED_LINES_TRIP_NOT_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11393',
      p_sig_id                 => 'PCA_OM_INTERFACED_LINES_TRIP_NOT_CLOSED',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
INT AS
( select /*+ materialize */
wdd.ORGANIZATION_ID org_id, count(wt.trip_id) count_rows
FROM wsh_delivery_details wdd,
wsh_delivery_assignments wda,
wsh_new_deliveries wnd,
wsh_delivery_legs wdl,
...
AND ( ( wts.actual_arrival_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
    OR (wts.actual_arrival_date is null
        AND ( wts.planned_arrival_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'') ) ) ) )
group by wdd.ORGANIZATION_ID
having count(wt.trip_id) > 0 )
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || int.count_rows || ''  trips interfaced to OM showing as open.'' Status
FROM
INVORGS,
INT
where
invorgs.organization_id = int.org_id',
      p_title                  => 'Trips are interfaced to Order Management, but remain open',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found trips interfaced to Order Management; however, they remain open.  Once a Trip is interfaced to Order Management it should progress to a status of Closed. Trips not closed  will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.

',
      p_solution               => '<ul>
<li>From Order Management: Shipping > Transactions: query trip number and close.</li>
<li>For more information regarding this issue, review [1509294.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>

',
      p_success_msg            => 'There are no trips interfaced to OM showing as open.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_INTERFACED_LINES_TRIP_NOT_CLOSED');



debug('begin add_signature: PCA_OM_WSH_ITS_OM_NO_INV_YES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11387',
      p_sig_id                 => 'PCA_OM_WSH_ITS_OM_NO_INV_YES',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
INT AS
( select /*+ materialize */
ol.org_id org_id, count(oh.order_number) count_rows
from wsh_delivery_details wdd,
...
where ol.line_id = wdd.source_line_id
   ...
   and ol.org_id = org.organization_id
   and NVL(org.set_of_books_id, -99) = ##$$LEDGERID$$##
   and wdd.creation_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
   and wdd.delivery_detail_id in
       (select wdd1.delivery_detail_id
        from wsh_delivery_details wdd1, oe_order_lines_all ol
        where wdd.delivery_detail_id = wdd1.delivery_detail_id
        and wdd1.source_line_id = ol.line_id
        and wdd1.released_status = ''C''
        and wdd1.inv_interfaced_flag = ''Y''
        and wdd1.oe_interfaced_flag = ''N''
        and ol.open_flag = ''N''
        and ol.cancelled_flag = ''N'')
group by ol.org_id
having count(oh.order_number) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || int.count_rows || '' delivery lines with OM interfaced flag = N but INV interfaced flag = Y.'' Status
FROM
ORGS,
INT
where
orgs.organization_id = int.org_id',
      p_title                  => 'Delivery Lines with OM Interfaced Flag = N but INV Interfaced Flag = Y',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found delivery lines already interfaced to Inventory from OM but with Order Management Interfaced flag = N (no). 
Delivery lines not marked as interfaced to inventory will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period. However these can prevent you from closing Inventory Period.',
      p_solution               => '<ul>
<li>For more information regarding this issue, review [1537132.1] for instructions and details.</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no delivery lines with OM interfaced flag = N but INV interfaced flag = Y.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_WSH_ITS_OM_NO_INV_YES');



debug('begin add_signature: PCA_OM_WSH_DD_MTI_EXIST_NO_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11389',
      p_sig_id                 => 'PCA_OM_WSH_DD_MTI_EXIST_NO_ERROR',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
INT AS
( select /*+ materialize */
wdd.ORGANIZATION_ID org_id, count(wdd.delivery_detail_id) count_rows
FROM wsh_delivery_details wdd,
wsh_delivery_assignments wda,
wsh_delivery_legs wdl,
wsh_trip_stops wts,
...
and mti.trx_source_line_id = wdd.source_line_id
)
and not exists ( select 1
from mtl_material_transactions mmt
where mmt.picking_line_id = wdd.delivery_detail_id
...
group by wdd.ORGANIZATION_ID
having count(wdd.delivery_detail_id) > 0 
)
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || int.count_rows || '' trip stop/delivery appearing interfaced to OM but stick in inteface table.'' Status
FROM
INVORGS,
INT
where
invorgs.organization_id = int.org_id',
      p_title                  => 'Trip Stop/Delivery Appear Interfaced to OM but Remain in MTL_TRANSACTIONS_INTERFACE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found Trip Stop/Delivery that appear to be interfaced to OM but are stuck in MTL_TRANSACTIONS_INTERFACE and SO not interfaced.
Trips not closed will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>To gather more details regarding this issue, please run EBS Order Management Sales Order Analyzer [1994535.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>',
      p_success_msg            => 'There are no Trip Stop/Delivery that appear to be interfaced to OM but are stuck in MTL_TRANSACTIONS_INTERFACE.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_WSH_DD_MTI_EXIST_NO_ERROR');



debug('begin add_signature: PCA_OM_SHIPPED_LINES_NOT_OM_INTERFACED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11391',
      p_sig_id                 => 'PCA_OM_SHIPPED_LINES_NOT_OM_INTERFACED',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code
...
SELECT
''Inventory Org = <b>'' || invorgs.name || ''</b>: Found '' || int.count_rows || '' shipped lines not interfaced to OM.'' Status
FROM
INVORGS,
INT
where
invorgs.organization_id = int.org_id',
      p_title                  => 'Shipped Lines not Interfaced to OM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found shipped lines not interfaced to OM.
Shipped lines not interfaced to OM will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>To gather more details regarding this issue, please run Shipping Execution Interface Trip Stop Analyzer [2380643.1].</li>
<li>If you need assistance, please log a Service Request with Oracle Order Management.</li>
</ul>
',
      p_success_msg            => 'Found no shipped lines not interfaced to OM.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_OM_SHIPPED_LINES_NOT_OM_INTERFACED');



debug('begin add_signature: PCA_OM_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '17503',
      p_sig_id                 => 'PCA_OM_SUMMARY',
      p_sig_sql                => 'select ''Order Management'' application_name, ''This section checks for Order Management (OM) issues.'' "Description"
from dual',
      p_title                  => 'Order Management (OM) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Order Management (OM) issues identified.',
      p_solution               => 'Please open this section to view Order Management (OM) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_OM_SALES_ORDER_LINES_NO_INVOICE','PCA_OM_SALES_ORDER_RA_INTERFACE_ERRORS','PCA_OM_PRE_BILLING_IMPLICIT_ACCEPTANCE2','PCA_OM_OPEN_LINES_OPEN_ERROR','PCA_OM_SHIPPED_LINES_NOT_INV_INTERFACED','PCA_OM_WSH_INV_PENDING_INTERFACE_MTL_TXN_DONE','PCA_OM_INTERFACED_LINES_TRIP_NOT_CLOSED','PCA_OM_WSH_ITS_OM_NO_INV_YES','PCA_OM_WSH_DD_MTI_EXIST_NO_ERROR','PCA_OM_SHIPPED_LINES_NOT_OM_INTERFACED'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '5'
      );
   l_info.delete;
debug('end add_signature: PCA_OM_SUMMARY');



debug('begin add_signature: PCA_CE_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11303',
      p_sig_id                 => 'PCA_CE_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Bank Account Transfer or Bank Statement Cash Flow Events in CE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed bank account transfer or bank statement cash flow events in CE. Unprocessed events in CE will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cash Management: Control > Request > Run: Create Accounting.</li>
<li>To gather more details on Create Accounting please see [1476284.2].</li>
<li>If you need assistance, please log a Service Request with Oracle Cash Management and upload the Period Close Advisor Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed bank account transfer or bank statement cash flow events in CE.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CE_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_CE_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11304',
      p_sig_id                 => 'PCA_CE_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = inval_events.org_id',
      p_title                  => 'Invalid Bank Account Transfer or Bank Statement Cash Flow Events in CE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid bank account transfer or bank statement cash flow events in CE. Invalid events in CE will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cash Management: Control > Request > Run: Create Accounting.</li>
<li>To gather more details on Create Accounting please see [1476284.2].</li>
<li>If you need assistance, please log a Service Request with Oracle Cash Management and upload the Period Close Advisor Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no invalid bank account transfer or bank statement cash flow events in CE.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CE_INVALID_EVENTS');



debug('begin add_signature: PCA_CE_FINAL_BUT_NOT_TRANSFERRED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11305',
      p_sig_id                 => 'PCA_CE_FINAL_BUT_NOT_TRANSFERRED',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
AND glr2.source_ledger_id = glr1.source_ledger_id
AND glr2.application_id = glr1.application_id
AND glr1.target_ledger_id = ##$$LEDGERID$$##
...
AND aeh.accounting_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and
to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND xte.application_id = 206
group by xte.security_id_int_1
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || transfer_no.count_rows || '' untransferred events.'' Status
FROM
ORGS,
TRANSFER_NO
where
orgs.organization_id = transfer_no.org_id',
      p_title                  => 'Untransferred Bank Account Transfer Or Bank Statement Cash Flow Events in CE',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred bank account transfer or bank statement cash flow events in CE. Untransferred events in CE will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cash Management: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details on Create Accounting please see [1476284.2].</li>
<li>If you need assistance, please log a Service Request with Oracle Cash Management and upload the Period Close Advisor Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred bank account transfer or bank statement cash flow events in CE.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_CE_FINAL_BUT_NOT_TRANSFERRED');



debug('begin add_signature: PCA_CE_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10215',
      p_sig_id                 => 'PCA_CE_SUMMARY',
      p_sig_sql                => 'select ''Cash Management'' application_name, ''This section checks for Cash Management (CE) issues.'' "Description"
from dual',
      p_title                  => 'Cash Management (CE) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Cash Management (CE) issues identified.',
      p_solution               => 'Please open this section to view Cash Management (CE) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_CE_UNPROCESSED_EVENTS','PCA_CE_INVALID_EVENTS','PCA_CE_FINAL_BUT_NOT_TRANSFERRED'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '38'
      );
   l_info.delete;
debug('end add_signature: PCA_CE_SUMMARY');



debug('begin add_signature: PCA_AP_UNACC_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10447',
      p_sig_id                 => 'PCA_AP_UNACC_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Events in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in AP. Unprocessed events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed events in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNACC_EVENTS');



debug('begin add_signature: PCA_AP_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10449',
      p_sig_id                 => 'PCA_AP_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
having count(xle.event_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unacc.count_rows || '' invalid events.'' Status
FROM
ORGS,
UNACC
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Invalid Events in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in AP. Invalid events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no invalid events in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_INVALID_EVENTS');



debug('begin add_signature: PCA_AP_NONXFR_HEADERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10448',
      p_sig_id                 => 'PCA_AP_NONXFR_HEADERS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
xte.security_id_int_1 org_id, 
                 count(aeh.ae_header_id) count_rows
FROM  xla_ae_headers aeh
     ,xla_transaction_entities_upg xte
     ,gl_ledger_relationships glr1
     ,gl_ledger_relationships glr2
...
AND  xte.entity_id = aeh.entity_id
AND  xte.application_id = aeh.application_id
AND  aeh.gl_transfer_status_code   IN (''N'',''E'',''S'')
AND  aeh.accounting_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND  xte.application_id = 200
group by xte.security_id_int_1
having count(aeh.ae_header_id) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || nonxfr.count_rows || '' untransferred events.'' Status
FROM
ORGS,
...
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in AP. Untransferred events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_NONXFR_HEADERS');



debug('begin add_signature: PCA_AP_UNPOSTED_INV_DIST');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10436',
      p_sig_id                 => 'PCA_AP_UNPOSTED_INV_DIST',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
UNPOST AS
( select /*+ materialize */
aid.org_id, count(ai.invoice_id) count_rows
from
ap_invoices_all ai,
ap_invoice_distributions_all aid
WHERE NVL(ai.set_of_books_id, -99) = ##$$LEDGERID$$##
AND aid.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND ai.invoice_id = aid.invoice_id
AND ai.approval_ready_flag <> ''S''
AND aid.posted_flag in (''N'' , ''S'', ''P'')
group by aid.org_id
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted invoice distributions.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Invoice Distributions in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted invoice distributions in AP.  Unposted invoice distributions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted invoice distributions in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_INV_DIST');



debug('begin add_signature: PCA_AP_UNPOSTED_INV_DIST_ENC');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10446',
      p_sig_id                 => 'PCA_AP_UNPOSTED_INV_DIST_ENC',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted invoice distributions - Encumbrance.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Invoice Distributions - Encumbrance in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted invoice distributions - Encumbrance in AP. Unposted invoice distributions - Encumbrance will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted invoice distributions - Encumbrance in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_INV_DIST_ENC');



debug('begin add_signature: PCA_AP_UNPOSTED_SELF_TAX_DIST');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10450',
      p_sig_id                 => 'PCA_AP_UNPOSTED_SELF_TAX_DIST',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
UNPOST AS
( select /*+ materialize */
ai.org_id, count(ai.invoice_id) count_rows
FROM
ap_invoices_all ai,
ap_self_assessed_tax_dist_all astd
WHERE NVL(ai.set_of_books_id, -99) = ##$$LEDGERID$$##
AND astd.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted self assessed tax distributions.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Self Assessed Tax Distributions in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted self assessed tax distributions in AP.  Unposted self assessed tax distributions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no unposted self assessed tax distributions in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_SELF_TAX_DIST');



debug('begin add_signature: PCA_AP_UNPOSTED_SELF_TAX_DIST_ENC');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10451',
      p_sig_id                 => 'PCA_AP_UNPOSTED_SELF_TAX_DIST_ENC',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted self assessed tax distributions - Encumbrance.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Self Assessed Tax Distributions - Encumbrance in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted self assessed tax distributions - Encumbrance in AP. Unposted self assessed tax distributions - Encumbrance will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted self assessed tax distributions - Encumbrance in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_SELF_TAX_DIST_ENC');



debug('begin add_signature: PCA_AP_UNPOSTED_PAY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10456',
      p_sig_id                 => 'PCA_AP_UNPOSTED_PAY',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
UNPOST AS
( select /*+ materialize */
aip.org_id, count(aip.invoice_payment_id) count_rows
FROM ap_invoice_payments_all aip,
ap_checks_All ac
WHERE NVL(aip.set_of_books_id, -99) = ##$$LEDGERID$$##
AND aip.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND aip.posted_flag IN (''N'',''S'')
AND ac.check_id = aip.check_id
group by aip.org_id
having count(aip.invoice_payment_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted payments.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Payments in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted payments in AP. Unposted payments will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted payments in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_PAY');



debug('begin add_signature: PCA_AP_UNPOSTED_PAY_HIST');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10457',
      p_sig_id                 => 'PCA_AP_UNPOSTED_PAY_HIST',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
UNPOST AS
( select /*+ materialize */
aph.org_id, count(aph.payment_history_id) count_rows
FROM ap_payment_history_all aph,
ap_checks_all ac,
ap_system_parameters_all aps
WHERE NVL(aps.set_of_books_id, -99) = ##$$LEDGERID$$##
AND aph.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND aph.posted_flag IN (''N'',''S'')
AND ac.check_id = aph.check_id
AND aph.org_id =  aps.org_id 
group by aph.org_id
having count(aph.payment_history_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted payment history.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Payment History in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted payment history in AP. Unposted payment history will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no unposted payment history in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_PAY_HIST');



debug('begin add_signature: PCA_AP_INVOICE_LINES_ALL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10452',
      p_sig_id                 => 'PCA_AP_INVOICE_LINES_ALL',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
NODIST AS
( select /*+ materialize */
ail.org_id, count(ai.invoice_id) count_rows
FROM ap_invoices_all ai,
ap_invoice_lines_all ail
WHERE NVL(ai.set_of_books_id, -99) = ##$$LEDGERID$$##
AND ail.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND ai.invoice_id = ail.invoice_id
AND NOT EXISTS ( --> lines without distributions
...
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || nodist.count_rows || '' invoice lines without distributions.'' Status
FROM
ORGS,
NODIST
where
orgs.organization_id = nodist.org_id',
      p_title                  => 'Invoice Lines Without Distributions in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invoice lines without distributions in AP. Invoice lines without distributions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Validate the invoice to generate invoice distributions or enter the distribution lines manually.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no invoice lines without distributions in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_INVOICE_LINES_ALL');



debug('begin add_signature: PCA_AP_UNPOSTED_PREPAY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10453',
      p_sig_id                 => 'PCA_AP_UNPOSTED_PREPAY',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
WHERE NVL(ai.set_of_books_id, -99) = ##$$LEDGERID$$##
AND apph.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
...
having count(ai.invoice_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted prepayment invoices.'' Status
FROM
ORGS,
UNPOST
where
orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted Prepayment Invoices in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted prepayment invoices in AP.  Unposted prepayment invoices will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no unposted prepayment invoices in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_UNPOSTED_PREPAY');



debug('begin add_signature: PCA_AP_PEND_PAY_BATCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10454',
      p_sig_id                 => 'PCA_AP_PEND_PAY_BATCH',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
PENDPAY AS
( select /*+ materialize */
asica.org_id, count(asica.checkrun_name) count_rows
FROM ap_inv_selection_criteria_all asica,
ap_system_parameters_all aps
WHERE NVL(aps.set_of_books_id, -99) = ##$$LEDGERID$$##
AND asica.check_date  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND asica.org_id = aps.org_id
AND asica.status NOT IN (''CONFIRMED'' , ''CANCELED'' , ''QUICKCHECK'' )
AND EXISTS (
SELECT 1 FROM ap_selected_invoices_all asia
WHERE asica.checkrun_name = asia.checkrun_name)
group by asica.org_id
having count(asica.checkrun_name) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || pendpay.count_rows || '' outstanding payment batches.'' Status
FROM
ORGS,
PENDPAY
where
orgs.organization_id = pendpay.org_id',
      p_title                  => 'Outstanding Payment Batches in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found outstanding payment batches in AP. Outstanding payment batches will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Confirm or cancel the payment process requests (PPR).</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no outstanding payment batches in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_PEND_PAY_BATCH');



debug('begin add_signature: PCA_AP_CHECK_MAT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10455',
      p_sig_id                 => 'PCA_AP_CHECK_MAT',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
APCHECK AS
( select /*+ materialize */
apc.org_id, count(apc.check_id) count_rows
from AP_CHECKS_ALL apc,
ap_system_parameters_all aps
WHERE NVL(aps.set_of_books_id, -99) = ##$$LEDGERID$$##
AND apc.FUTURE_PAY_DUE_DATE  between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'') 
AND aps.org_id = aps.org_id
AND apc.FUTURE_PAY_DUE_DATE IS NOT NULL
AND apc.STATUS_LOOKUP_CODE = ''ISSUED''
group by apc.org_id
having count(apc.check_id) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || apcheck.count_rows || '' checks not matured.'' Status
FROM
ORGS,
APCHECK
where
orgs.organization_id = apcheck.org_id',
      p_title                  => 'Checks Not Matured in AP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found checks not matured in AP. Checks not matured will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Payables: Submit Update Matured Bills Payables Status process.</li>
<li>To gather more details regarding this issue, please run the Oracle Payables Period Close Analyzer in [1489381.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Payables and upload the Payables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no checks not matured in AP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AP_CHECK_MAT');



debug('begin add_signature: PCA_AP_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10208',
      p_sig_id                 => 'PCA_AP_SUMMARY',
      p_sig_sql                => 'select ''Payables'' application_name, ''This section checks for Payables (AP) issues.'' "Description"
from dual',
      p_title                  => 'Payables (AP) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Payables (AP) issues identified.',
      p_solution               => 'Please open this section to view Payables (AP) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_AP_UNACC_EVENTS','PCA_AP_INVALID_EVENTS','PCA_AP_NONXFR_HEADERS','PCA_AP_UNPOSTED_INV_DIST','PCA_AP_UNPOSTED_INV_DIST_ENC','PCA_AP_UNPOSTED_SELF_TAX_DIST','PCA_AP_UNPOSTED_SELF_TAX_DIST_ENC','PCA_AP_UNPOSTED_PAY','PCA_AP_UNPOSTED_PAY_HIST','PCA_AP_INVOICE_LINES_ALL','PCA_AP_UNPOSTED_PREPAY','PCA_AP_PEND_PAY_BATCH','PCA_AP_CHECK_MAT'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '127'
      );
   l_info.delete;
debug('end add_signature: PCA_AP_SUMMARY');



debug('begin add_signature: PCA_AR_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10197',
      p_sig_id                 => 'PCA_AR_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Events in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in AR. Unprocessed events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Receivables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no unprocessed events in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_AR_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10199',
      p_sig_id                 => 'PCA_AR_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
    organization_id, 
...
INVAL_EVENTS AS
  ( select /*+ materialize */
     ...
     where xe.application_id = 222
       ...
       and xae.event_id = xe.event_id
       and xlt.entity_id = xe.entity_id
       and xlt.entity_code in (''ADJUSTMENTS'', ''TRANSACTIONS'', ''BILLS_RECEIVABLE'', ''RECEIPTS'')
       group by xlt.security_id_int_1
       having count(*) > 0 )
SELECT
   ''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || inval_events.count_rows || '' invalid events.'' Status
...
where
   orgs.organization_id = inval_events.org_id',
      p_title                  => 'Invalid Events in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in AR. Invalid events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Receivables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no invalid events in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_INVALID_EVENTS');



debug('begin add_signature: PCA_AR_FINAL_BUT_NOT_TRANSFERRED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10202',
      p_sig_id                 => 'PCA_AR_FINAL_BUT_NOT_TRANSFERRED',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
    organization_id, 
...
where
   orgs.organization_id = transfer_no.org_id',
      p_title                  => 'Untransferred Events in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in AR. Untransferred events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Receivables: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_FINAL_BUT_NOT_TRANSFERRED');



debug('begin add_signature: PCA_AR_UNPOSTED_ALL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10211',
      p_sig_id                 => 'PCA_AR_UNPOSTED_ALL',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
           organization_id, 
...
UNPOST AS
  ( select /*+ materialize */
           org_id org_id, count(*) || '' unposted <em>adjustment(s)</em>.'' count_rows
      FROM AR_ADJUSTMENTS_ALL
     WHERE posting_control_id = -3 
       AND gl_date between  to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
       AND NVL(set_of_books_id, -99) = ##$$LEDGERID$$##
       AND NVL(postable, ''Y'') = ''Y''
     group by org_id
    having count(*) > 0
   union all
   select b.org_id, count(distinct b.customer_trx_id) || '' unposted <em>transaction(s)</em>.'' 
     FROM RA_CUST_TRX_LINE_GL_DIST_ALL a, RA_CUSTOMER_TRX_ALL b 
    WHERE a.posting_control_id = -3 
      AND a.gl_date between  to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
      AND NVL(a.set_of_books_id, -99) = ##$$LEDGERID$$##
      AND b.complete_flag = ''Y'' 
      AND a.account_set_flag = ''N'' 
      AND b.customer_trx_id = a.customer_trx_id      
    group by b.org_id 
   having count(b.customer_trx_id) > 0
   union all
   SELECT org_id, count(receivable_application_id) || '' unposted <em>credit memo application(s)</em>.''
    ...
    WHERE posting_control_id = -3
      ...
      AND NVL(postable, ''Y'') = ''Y''
    group by org_id 
   having count(customer_trx_id) > 0
   union all
   SELECT a.org_id, count(distinct a.cash_receipt_id) || '' unposted <em>cash receipt(s)</em>.''
     FROM AR_CASH_RECEIPT_HISTORY_ALL a, AR_CASH_RECEIPTS_ALL b
    WHERE a.posting_control_id = -3
      AND a.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
      AND NVL(b.set_of_books_id, -99) = ##$$LEDGERID$$##
      AND a.postable_flag = ''Y''
      AND b.cash_receipt_id(+) = a.cash_receipt_id
    group by a.org_id 
   having count(a.cash_receipt_id) > 0
   union all
   SELECT org_id, count(receivable_application_id) || '' unposted <em>cash receipt application(s)</em>. ''
    FROM AR_RECEIVABLE_APPLICATIONS_ALL
   WHERE posting_control_id = -3
     ...
     AND NVL(postable, ''Y'') = ''Y''
    group by org_id 
   having count(receivable_application_id) > 0
   union all
   select org_id, count(cash_receipt_id) || '' unposted <em>miscellaneous cash receipt(s)</em>.''
     from ar_misc_cash_distributions_all
    where posting_control_id = -3
      AND gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
      AND NVL(set_of_books_id, -99) = ##$$LEDGERID$$##
    group by org_id 
   having count(cash_receipt_id) > 0
   union all 
   SELECT a.org_id, count(distinct b.customer_trx_id) || '' unposted <em>bill(s) receivable</em>.''
    ...
    WHERE a.posting_control_id = -3
      ...
      AND b.customer_trx_id = a.customer_trx_id
    group by a.org_id 
   having count(a.customer_trx_id) > 0
 )
SELECT
   ''Operating Unit <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows Status
FROM
   ORGS,
   UNPOST
where
   orgs.organization_id = unpost.org_id
order by 1',
      p_title                  => 'Unposted Items in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted items in AR. Unposted items will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Receivables: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted items in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_UNPOSTED_ALL');



debug('begin add_signature: PCA_AR_UNPOSTED_QUICKCASH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10209',
      p_sig_id                 => 'PCA_AR_UNPOSTED_QUICKCASH',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
           organization_id, 
...
UNPOST AS
  ( select /*+ materialize */
           a.org_id, count(a.receipt_number) count_rows
      FROM AR_INTERIM_CASH_RECEIPTS_ALL a, AR_BATCHES_ALL b
     WHERE a.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
       AND NVL(b.set_of_books_id, -99) = ##$$LEDGERID$$##
       AND b.batch_id(+) = a.batch_id
     group by a.org_id
    having count(a.receipt_number) > 0 )
SELECT
   ''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unpost.count_rows || '' unposted quickcash receipts.'' Status
FROM
   ORGS,
   UNPOST
where
   orgs.organization_id = unpost.org_id',
      p_title                  => 'Unposted QuickCash Receipts in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unposted quickcash receipts in AR.  Unposted quickcash will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Receivables: Run Post QuickCash and Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unposted quickcash receipts in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_UNPOSTED_QUICKCASH');



debug('begin add_signature: PCA_AR_REVREC_NOT_RUN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10210',
      p_sig_id                 => 'PCA_AR_REVREC_NOT_RUN',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
           organization_id, 
...
REVREC AS
  ( select /*+ materialize */
           ct.org_id, count(ct.customer_trx_id) count_rows
     ...
     WHERE gld.account_class = ''REC''
       ...
       AND gld.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and
                               to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
       AND NVL(gld.set_of_books_id, -99) = ##$$LEDGERID$$##
  group by ct.org_id
    having count(ct.customer_trx_id) > 0 )
SELECT
   ''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || revrec.count_rows || '' transactions with revenue recognition not run.'' Status
FROM
   ORGS,
   REVREC
where
   orgs.organization_id = revrec.org_id',
      p_title                  => 'Transactions with Revenue Recognition Not Run in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found transactions with revenue recognition not run in AR. Transactions without revenue recognition will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Receivables: Control > Request > Run: Submit Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no transactions without revenue recognition in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_REVREC_NOT_RUN');



debug('begin add_signature: PCA_AR_INCOMPLETE_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10212',
      p_sig_id                 => 'PCA_AR_INCOMPLETE_TRX',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
           organization_id, 
...
INCOMP AS
  ...
  group by ct.org_id
    having count(ct.customer_trx_id) > 0 )
SELECT
   ''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || incomp.count_rows || '' incomplete transactions.'' Status
FROM
   ORGS,
   INCOMP
where
   orgs.organization_id = incomp.org_id',
      p_title                  => 'Incomplete Transactions in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found incomplete transactions in AR. Incomplete transactions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<p>If you need these transactions to be included in the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period you should process them before closing the period.</p>
<ul>
<li>From Receivables: Control > Request > Run: Incomplete Invoices Report, complete transactions and run Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no incomplete transactions in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_INCOMPLETE_TRX');



debug('begin add_signature: PCA_AR_ADJ_PENDING_APPROVAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10213',
      p_sig_id                 => 'PCA_AR_ADJ_PENDING_APPROVAL',
      p_sig_sql                => 'WITH
ORGS AS
  ( select /*+ materialize */  
           organization_id, 
...
ADJAPPR AS
  ( select /*+ materialize */
           adj.org_id, count(adj.adjustment_id) count_rows
      from ar_adjustments_all adj
     where status not in (''A'',''R'')
       and adj.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and
                               to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
       AND NVL(set_of_books_id, -99) = ##$$LEDGERID$$##
     group by adj.org_id
    having count(adj.adjustment_id) > 0 )
SELECT
   ''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || adjappr.count_rows || '' adjustments pending approval.'' Status
FROM
   ORGS,
   ADJAPPR
where
   orgs.organization_id = adjappr.org_id',
      p_title                  => 'Adjustments Pending Approval in AR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found adjustments pending approval in AR. Adjustments pending approval will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<p>If you need these adjustments to be included in the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period you should process them before closing the period.</p>
<ul>
<li>From Receivables: Control > Adjustment > Approve: <ul><li>Approve the adjustment and run Create Accounting, or</li>
<li>Reject the adjustment.</li></ul>
<li>To gather more details regarding this issue, please run the Oracle Receivables Period Close Analyzer in [2019636.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Receivables and upload the Receivables Period Close Analyzer output.</li>
</ul>
</ul>',
      p_success_msg            => 'There are no adjustments pending approval in AR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_AR_ADJ_PENDING_APPROVAL');



debug('begin add_signature: PCA_AR_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10205',
      p_sig_id                 => 'PCA_AR_SUMMARY',
      p_sig_sql                => 'select ''Receivables'' application_name, ''This section checks for Receivables (AR) issues.'' "Description"
from dual',
      p_title                  => 'Receivables (AR) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Receivables (AR) issues identified.',
      p_solution               => 'Please open this section to view Receivables (AR) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_AR_UNPROCESSED_EVENTS','PCA_AR_INVALID_EVENTS','PCA_AR_FINAL_BUT_NOT_TRANSFERRED','PCA_AR_UNPOSTED_ALL','PCA_AR_UNPOSTED_QUICKCASH','PCA_AR_REVREC_NOT_RUN','PCA_AR_INCOMPLETE_TRX','PCA_AR_ADJ_PENDING_APPROVAL'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '176'
      );
   l_info.delete;
debug('end add_signature: PCA_AR_SUMMARY');



debug('begin add_signature: PCA_PO_ENC_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10662',
      p_sig_id                 => 'PCA_PO_ENC_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
WHERE NVL(xte.ledger_id, -99) = ##$$LEDGERID$$##
AND xle.event_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND NVL(xle.budgetary_control_flag, ''N'') =''Y''
AND xte.entity_code IN (''REQUISITION'',''PURCHASE_ORDER'',''RELEASE'')
AND xle.entity_id = xte.entity_id
AND xle.application_id = xte.application_id
AND xle.application_id = 201
AND xle.process_status_code <> ''P''
AND xle.process_status_code <> ''I''
AND xle.event_status_code <> ''P''
AND xle.event_status_code <> ''I''
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed, Draft, Invalid Budgetary Control Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed, draft or invalid budgetary control events in PO. Unprocessed budgetary control events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Purchasing: Control > Request > Run: Create Accounting.</li>
<li>To gather more details regarding this issue, please run the Subledger Period Close Exception Report in [1264605.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed, draft or invalid budgetary control events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_ENC_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_PO_ENC_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10663',
      p_sig_id                 => 'PCA_PO_ENC_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unproc.count_rows || '' untransferred budgetary control events.'' Status
FROM
ORGS,
UNPROC
where
orgs.organization_id = unproc.org_id',
      p_title                  => 'Untransferred Budgetary Contol Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred budgetary control events in PO. Untransferred budgetary control events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Purchasing: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Subledger Period Close Exception Report [1264605.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
</ul>
',
      p_success_msg            => 'There are no untransferred budgetary control events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_ENC_NONXFR_EVENTS');



debug('begin add_signature: PCA_PO_RCV_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10664',
      p_sig_id                 => 'PCA_PO_RCV_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
UNPROC AS
( select /*+ materialize */
xte.security_id_int_2 org_id,count(*) count_rows
...
GROUP BY xte.security_id_int_2
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unproc.count_rows || '' unprocessed receiving events.'' Status
FROM
ORGS,
UNPROC
where
orgs.organization_id = unproc.org_id',
      p_title                  => 'Unprocessed Receiving Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed receiving events in PO. Unprocessed events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting for Process Category = Receiving or</li>
<li>From Purchasing: Control > Request > Run: Create Accounting - Receiving.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
</ul>
',
      p_success_msg            => 'There are no unprocessed receiving events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_RCV_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_PO_RCV_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10665',
      p_sig_id                 => 'PCA_PO_RCV_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || invalid.count_rows || '' invalid receiving events.'' Status
FROM
ORGS,
INVALID
where
orgs.organization_id = invalid.org_id',
      p_title                  => 'Invalid Receiving Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid receiving events in PO. Invalid events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting for Process Category = Receiving or</li>
<li>From Purchasing: Control > Request > Run: Create Accounting - Receiving.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
      ...
      p_success_msg            => 'There are no invalid receiving events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_RCV_INVALID_EVENTS');



debug('begin add_signature: PCA_PO_RCV_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10666',
      p_sig_id                 => 'PCA_PO_RCV_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in PO. Untransferred events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_RCV_NONXFR_EVENTS');



debug('begin add_signature: PCA_PO_WO_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10670',
      p_sig_id                 => 'PCA_PO_WO_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
FROM xla_transaction_entities_upg xte,
  xla_events xe
WHERE xte.entity_code      IN (''WO_ACCOUNTING_EVENTS'')
AND xe.application_id       = xte.application_id
AND xte.entity_id           = xe.entity_id
AND xte.application_id      = 707
AND xe.process_status_code <> ''P''
...
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || unproc.count_rows || '' unprocessed write off events.'' Status
FROM
ORGS,
UNPROC
where
orgs.organization_id = unproc.org_id',
      p_title                  => 'Unprocessed Write Off Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed write off events in PO. Unprocessed events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting for Process Category = Receiving or</li>
<li>From Purchasing: Control > Request > Run: Create Accounting - Receiving.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
...
',
      p_success_msg            => 'There are no unprocessed write off events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_WO_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_PO_WO_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10671',
      p_sig_id                 => 'PCA_PO_WO_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || invalid.count_rows || '' invalid work order events.'' Status
FROM
ORGS,
INVALID
where
orgs.organization_id = invalid.org_id',
      p_title                  => 'Invalid Work Order Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid work order events in PO. Invalid events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting for Process Category = Receiving or</li>
<li>From Purchasing: Control > Request > Run: Create Accounting - Receiving.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
      ...
      p_success_msg            => 'There are no invalid work order events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_WO_INVALID_EVENTS');



debug('begin add_signature: PCA_PO_WO_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10672',
      p_sig_id                 => 'PCA_PO_WO_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
HAVING COUNT(*) > 0)
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || nonxfr.count_rows || '' untransferred work order events.'' Status
FROM
ORGS,
NONXFR
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Work Order Events in PO',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred work order events in PO. Untransferred events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Transfer Journal Entries to GL./li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Purchasing and upload the Subledger Period Close Exception Report output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred work order events in PO.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PO_WO_NONXFR_EVENTS');



debug('begin add_signature: PCA_PO_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10368',
      p_sig_id                 => 'PCA_PO_SUMMARY',
      p_sig_sql                => 'select ''Purchasing'' application_name, ''This section checks for Purchase Order (PO) issues.'' "Description"
from dual',
      p_title                  => 'Purchase Order (PO) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Purchase Order (PO) issues identified.',
      p_solution               => 'Please open this section to view Purchase Order (PO) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_PO_ENC_UNPROCESSED_EVENTS','PCA_PO_ENC_NONXFR_EVENTS','PCA_PO_RCV_UNPROCESSED_EVENTS','PCA_PO_RCV_INVALID_EVENTS','PCA_PO_RCV_NONXFR_EVENTS','PCA_PO_WO_UNPROCESSED_EVENTS','PCA_PO_WO_INVALID_EVENTS','PCA_PO_WO_NONXFR_EVENTS'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '82'
      );
   l_info.delete;
debug('end add_signature: PCA_PO_SUMMARY');



debug('begin add_signature: PCA_INV_UNPROCESSED_CST_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10772',
      p_sig_id                 => 'PCA_INV_UNPROCESSED_CST_EVENTS',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
from     org_organization_definitions
where set_of_books_id = ##$$LEDGERID$$##),
UNPROC AS
( select /*+ materialize */
xte.security_id_int_2 org_id, count(xle.event_id) count_rows
FROM xla_events xle,
xla_transaction_entities_upg xte,
xla_ledger_options xlo
WHERE NVL(xte.ledger_id, -99) = ##$$LEDGERID$$##
...
AND xte.entity_code in (''WIP_ACCOUNTING_EVENTS'',''RCV_ACCOUNTING_EVENTS'',''MTL_ACCOUNTING_EVENTS'')
AND xle.process_status_code <> ''P''
AND xle.process_status_code <> ''I''
AND xle.event_status_code <> ''P''
AND xle.event_status_code <> ''I''
...
having count(xle.event_id) > 0 )
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || unproc.count_rows || '' unprocessed costing events.'' Status
FROM
INVORGS,
UNPROC
where
invorgs.organization_id = unproc.org_id',
      p_title                  => 'Unprocessed Events in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in CST. Unprocessed events will note prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting - Cost Management.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Ledger.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Create Accounting-Cost Management Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed events in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_UNPROCESSED_CST_EVENTS');



debug('begin add_signature: PCA_INV_INVALID_CST_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10842',
      p_sig_id                 => 'PCA_INV_INVALID_CST_EVENTS',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
...
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || unproc.count_rows || '' invalid costing events.'' Status
FROM
INVORGS,
UNPROC
where
invorgs.organization_id = unproc.org_id',
      p_title                  => 'Invalid Events in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in CST. Invalid events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Create Accounting - Cost Management.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Ledger.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Create Accounting-Cost Management Analyzer output.</li>
...
',
      p_success_msg            => 'There are no invalid events in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_INVALID_CST_EVENTS');



debug('begin add_signature: PCA_INV_NONXFR_CST_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10844',
      p_sig_id                 => 'PCA_INV_NONXFR_CST_EVENTS',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
...
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || nonxfr.count_rows || '' untransferred costing events.'' Status
FROM
INVORGS,
NONXFR
where
invorgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in CST. Untransferred events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.
',
      p_solution               => '<ul>
<li>From Cost Management: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Create Accounting-Cost Management Analyzer in [1580316.1] for Ledger.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Create Accounting-Cost Management Analyzer output.</li>
      ...
      p_success_msg            => 'There are no untransferred events in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_NONXFR_CST_EVENTS');



debug('begin add_signature: PCA_INV_UNPROCESSED_MATERIAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11191',
      p_sig_id                 => 'PCA_INV_UNPROCESSED_MATERIAL',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
from org_organization_definitions
where set_of_books_id = 1--##$$LEDGERID$$##
),
UNPROC AS
( select /*+ materialize */
mtt.organization_id org_id, count(*) count_rows
FROM MTL_MATERIAL_TRANSACTIONS_TEMP MTT,
org_organization_definitions org
WHERE org.organization_id = mtt.organization_id
AND org.set_of_books_id = 1--##$$LEDGERID$$##
--AND mtt.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND NVL(mtt.TRANSACTION_STATUS,0) <> 2
GROUP BY mtt.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || unproc.count_rows || '' unprocessed material transactions.'' Status
FROM
INVORGS,
UNPROC
where
invorgs.organization_id = unproc.org_id',
      p_title                  => 'Unprocessed Material Transactions in Inventory Accounting Period in INV',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed material transactions in Inventory accounting period in INV. Unprocessed material transactions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Inventory: Transactions > Pending Transactions: Pending Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Inventory Transactions Analyzer in [1499475.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Inventory and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no unprocessed material transactions in Inventory accounting period in INV.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_UNPROCESSED_MATERIAL');



debug('begin add_signature: PCA_INV_UNCOSTED_MATERIAL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11192',
      p_sig_id                 => 'PCA_INV_UNCOSTED_MATERIAL',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */ 
organization_id, organization_name name, organization_code code
...
UNCOST AS
( select /*+ materialize */
mt.organization_id org_id, count(*) count_rows
FROM MTL_MATERIAL_TRANSACTIONS MT,
     org_organization_definitions org
WHERE org.organization_id = mt.organization_id
AND   org.set_of_books_id = ##$$LEDGERID$$##
AND   mt.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND   mt.COSTED_FLAG IS NOT NULL
GROUP BY mt.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || uncost.count_rows || '' uncosted material transactions.'' Status
FROM
INVORGS,
UNCOST
where
invorgs.organization_id = uncost.org_id',
      p_title                  => 'Uncosted Material Transactions in Inventory Accounting Period in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found uncosted material transactions in Inventory accounting period in CST. Uncosted material transactions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: View Transactions > Material Transactions: Find Material Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Inventory Transactions Analyzer in [1499475.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Inventory Transactions Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no uncosted material transactions in Inventory accounting period in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_UNCOSTED_MATERIAL');



debug('begin add_signature: PCA_INV_PEND_WIP_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11193',
      p_sig_id                 => 'PCA_INV_PEND_WIP_TRX',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */ 
organization_id, organization_name name, organization_code code
...
PEND AS
( select /*+ materialize */
wci.organization_id org_id, count(*) count_rows
FROM WIP_COST_TXN_INTERFACE wci,
     org_organization_definitions org
WHERE wci.organization_id = org.organization_id
AND   org.set_of_books_id = ##$$LEDGERID$$##
AND   wci.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY wci.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || pend.count_rows || '' pending WIP transactions.'' Status
FROM
INVORGS,
PEND
where
invorgs.organization_id = pend.org_id',
      p_title                  => 'Pending Work in Process Transactions in Inventory Accounting Period in WIP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending work in process transactions in Inventory accounting period in WIP. Pending transactions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Work In Process: Resource Transactions > Pending Resource Transactions: Pending Resource Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no pending work in process transactions in Inventory accounting period in WIP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PEND_WIP_TRX');



debug('begin add_signature: PCA_INV_PEND_LCM_INT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11194',
      p_sig_id                 => 'PCA_INV_PEND_LCM_INT',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */ 
organization_id, organization_name name, organization_code code
...
PEND AS
( select /*+ materialize */
cli.organization_id org_id, count(*) count_rows
FROM  CST_LC_ADJ_INTERFACE CLI,
      org_organization_definitions org
WHERE cli.organization_id = org.organization_id
AND   org.set_of_books_id = ##$$LEDGERID$$##
AND   CLI.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY cli.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || pend.count_rows || '' pending LCM interface transactions.'' Status
FROM
INVORGS,
PEND
where
invorgs.organization_id = pend.org_id',
      p_title                  => 'Pending LCM Interface Transactions in Inventory Accounting Period in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending LCM interface transactions in Inventory accounting period in CST. Pending LCM interface transactions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Cost Management: View Transactions > Pending Landed Cost Adjustments: Pending Landed Cost Adjustments Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Cost Management and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no pending LCM interface transactions in Inventory accounting period in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PEND_LCM_INT');



debug('begin add_signature: PCA_INV_PEND_RCV_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11195',
      p_sig_id                 => 'PCA_INV_PEND_RCV_TRX',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */ 
organization_id, organization_name name, organization_code code
...
PEND AS
( select /*+ materialize */
rct.to_organization_id org_id, count(*) count_rows
FROM  RCV_TRANSACTIONS_INTERFACE RCT,
      org_organization_definitions org
WHERE rct.to_organization_id = org.organization_id
AND   RCT.DESTINATION_TYPE_CODE = ''INVENTORY''
AND   org.set_of_books_id = ##$$LEDGERID$$##
AND   rct.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY rct.to_organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || pend.count_rows || '' pending receiving transactions.'' Status
FROM
INVORGS,
PEND
where
invorgs.organization_id = pend.org_id',
      p_title                  => 'Pending Receiving Transactions in Inventory Accounting Period in CST',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending receiving transactions in Inventory accounting period in CST. Pending pending receiving transactions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Purchasing: Receiving > Transactions Status Summary: Find Transaction Statuses [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Inventory and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no pending receiving transactions in Inventory accounting period in CST.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PEND_RCV_TRX');



debug('begin add_signature: PCA_INV_PEND_MTL_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11196',
      p_sig_id                 => 'PCA_INV_PEND_MTL_TRX',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
...
PEND AS
( select /*+ materialize */
mti.organization_id org_id, count(*) count_rows
FROM MTL_TRANSACTIONS_INTERFACE MTI,
org_organization_definitions org
WHERE mti.organization_id = org.organization_id
AND MTI.PROCESS_FLAG <> 9
AND org.set_of_books_id = ##$$LEDGERID$$##
AND mti.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY mti.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || pend.count_rows || '' pending material transactions.'' Status
FROM
INVORGS,
PEND
where
invorgs.organization_id = pend.org_id',
      p_title                  => 'Pending Material Transactions in Inventory Accounting Period in INV',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending material transactions in Inventory accounting period in INV. Pending transactions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Inventory: Transactions > Transaction Open Interface: Transaction Interface [1069492.1]</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Inventory and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no pending material transactions in Inventory accounting period in INV.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PEND_MTL_TRX');



debug('begin add_signature: PCA_INV_PEND_WIP_MV_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11197',
      p_sig_id                 => 'PCA_INV_PEND_WIP_MV_TRX',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
...
PEND AS
( select /*+ materialize */
wmti.organization_id org_id, count(*) count_rows
FROM WIP_MOVE_TXN_INTERFACE WMTI,
org_organization_definitions org
WHERE wmti.organization_id = org.organization_id
AND org.set_of_books_id = ##$$LEDGERID$$##
AND wmti.TRANSACTION_DATE BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
GROUP BY wmti.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || pend.count_rows || '' pending move transactions.'' Status
FROM
INVORGS,
PEND
where
invorgs.organization_id = pend.org_id',
      p_title                  => 'Pending Move Transactions in Inventory Accounting Period in WIP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending move transactions in Inventory accounting period in WIP. Pending shop move transactions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Work In Process: Move Transactions > Pending Move Transactions: Pending Move Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Work In Process and upload the Cost Management Period Close Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no pending move transactions in Inventory accounting period in WIP.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_PEND_WIP_MV_TRX');



debug('begin add_signature: PCA_INV_UNPROSS_SHIP_TRX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11198',
      p_sig_id                 => 'PCA_INV_UNPROSS_SHIP_TRX',
      p_sig_sql                => 'WITH
INVORGS AS
( select /*+ materialize */
organization_id, organization_name name, organization_code code
...
UNPROC AS
( select /*+ materialize */
wdd.organization_id org_id, count(*) count_rows
...
AND org.set_of_books_id = ##$$LEDGERID$$##
AND TRUNC(WTS.ACTUAL_DEPARTURE_DATE) BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND WDL.PICK_UP_STOP_ID = WTS.STOP_ID
GROUP BY wdd.organization_id
HAVING COUNT(*) > 0)
SELECT
''<b>Org Code = '' || invorgs.code || '', Inventory Org Name = '' || invorgs.name || ''</b>: Found '' || unproc.count_rows || '' unprocessed shipping transactions.'' Status
FROM
INVORGS,
UNPROC
where
invorgs.organization_id = unproc.org_id',
      p_title                  => 'Unprocessed Shipping Transactions in Inventory Accounting Period to INV',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed shipping transactions in Inventory accounting period to INV. Unprocessed shipping transactions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Order Management: Shipping > Transactions [1069492.1].</li>
<li>To gather more details regarding this issue, please run the Cost Management Period Close Analyzer in [2315206.1] for Operating Unit identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Inventory and upload the Cost Management Period Close Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no unprocessed shipping transactions in Inventory accounting period to INV.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_INV_UNPROSS_SHIP_TRX');



debug('begin add_signature: PCA_INV_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10369',
      p_sig_id                 => 'PCA_INV_SUMMARY',
      p_sig_sql                => 'select ''Inventory'' application_name, ''This section checks for Cost (CST) / Inventory (INV) issues.'' "Description"
from dual',
      p_title                  => 'Cost (CST) / Inventory (INV) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Cost (CST) / Inventory (INV) issues identified.',
      p_solution               => 'Please open this section to view Cost (CST) / Inventory (INV) issues identified. ',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_INV_PERIOD_STATUS','PCA_INV_UNPROCESSED_CST_EVENTS','PCA_INV_INVALID_CST_EVENTS','PCA_INV_NONXFR_CST_EVENTS','PCA_INV_UNPROCESSED_MATERIAL','PCA_INV_UNCOSTED_MATERIAL','PCA_INV_PEND_WIP_TRX','PCA_INV_PEND_LCM_INT','PCA_INV_PEND_RCV_TRX','PCA_INV_PEND_MTL_TRX','PCA_INV_PEND_WIP_MV_TRX','PCA_INV_UNPROSS_SHIP_TRX'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '198'
      );
   l_info.delete;
debug('end add_signature: PCA_INV_SUMMARY');



debug('begin add_signature: PCA_PA_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11208',
      p_sig_id                 => 'PCA_PA_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
AND xte.entity_code in (''REVENUE'',''EXPENDITURES'')
AND xle.process_status_code <> ''P''
AND xle.process_status_code <> ''I''
AND xle.event_status_code <> ''P''
AND xle.event_status_code <> ''I''
...
where
orgs.organization_id = unacc.org_id',
      p_title                  => 'Unprocessed Events in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in PA. Unprocessed events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Create Accounting.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>
',
      p_success_msg            => 'There are no unprocessed events in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_PA_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11209',
      p_sig_id                 => 'PCA_PA_INVALID_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */ 
organization_id, 
...
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || invalid.count_rows || '' invalid events.'' Status
FROM
ORGS,
INVALID
where
orgs.organization_id = invalid.org_id',
      p_title                  => 'Invalid Events in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events In PA. Invalid events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Create Accounting.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no invalid events in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_INVALID_EVENTS');



debug('begin add_signature: PCA_PA_NONXFR_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11212',
      p_sig_id                 => 'PCA_PA_NONXFR_EVENTS',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
where
orgs.organization_id = nonxfr.org_id',
      p_title                  => 'Untransferred Events in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in PA. Untransferred events will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Transfer Journal Entries to GL.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_NONXFR_EVENTS');



debug('begin add_signature: PCA_PA_COST_EXCP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11273',
      p_sig_id                 => 'PCA_PA_COST_EXCP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
EXCP AS
(select /*+ materialize */
pa_view.org_id, count(*) count_rows
from
(SELECT imp.org_id, cdl.gl_period_name
FROM PA_COST_DISTRIBUTION_LINES_ALL CDL, PA_EXPENDITURE_ITEMS_ALL EI, PA_IMPLEMENTATIONS_ALL IMP
WHERE CDL.TRANSFER_STATUS_CODE IN (''P'',''R'',''X'')
...
AND ((EI.SYSTEM_LINKAGE_FUNCTION IN (''ST'', ''OT'')
         ...
         OR (EI.SYSTEM_LINKAGE_FUNCTION IN (''USG'',''PJ'',''BTC'',''INV'',''WIP'')
          AND IMP.INTERFACE_USAGE_TO_GL_FLAG = ''Y'')
         OR (EI.SYSTEM_LINKAGE_FUNCTION IN (''ER'',''VI'')))
AND EI.ORG_ID = IMP.ORG_ID
AND CDL.ORG_ID = IMP.ORG_ID
UNION ALL
SELECT imp.org_id, cdl.gl_period_name
FROM PA_COST_DISTRIBUTION_LINES_ALL CDL, PA_EXPENDITURE_ITEMS_ALL EI, PA_IMPLEMENTATIONS_ALL IMP, XLA_EVENTS XE
WHERE CDL.TRANSFER_STATUS_CODE =''A''
AND CDL.ACCT_EVENT_ID(+) = XE.EVENT_ID
AND XE.APPLICATION_ID = 275
...
AND CDL.ORG_ID = IMP.ORG_ID
UNION ALL
SELECT ei.org_id, cdl.gl_period_name
FROM PA_COST_DISTRIBUTION_LINES_ALL CDL, PA_EXPENDITURE_ITEMS_ALL EI, PA_IMPLEMENTATIONS_ALL IMP , PA_PROJECT_TYPES_ALL PPTA, PA_PROJECTS_ALL PPA
WHERE CDL.BURDEN_SUM_SOURCE_RUN_ID = -9999
AND CDL.LINE_TYPE = ''R'' AND NVL(CDL.AMOUNT,0) <> 0
AND CDL.IND_COMPILED_SET_ID <> -1
AND EI.EXPENDITURE_ITEM_ID = CDL.EXPENDITURE_ITEM_ID
AND EI.ORG_ID = CDL.ORG_ID
AND CDL.PA_DATE is NOT NULL
AND PPTA.ORG_ID = PPA.ORG_ID
AND PPA.ORG_ID = EI.ORG_ID
AND EI.PROJECT_ID = PPA.PROJECT_ID
AND PPA.PROJECT_TYPE = PPTA.PROJECT_TYPE
AND upper(PPTA.BURDEN_COST_FLAG) = ''Y''
AND upper(PPTA.BURDEN_AMT_DISPLAY_METHOD) in (''D'',''S'')
AND upper(PPTA.BURDEN_ACCOUNT_FLAG) = ''Y''
AND PA_PROJECT_UTILS.CHECK_PRJ_STUS_ACTION_ALLOWED(PPA.PROJECT_STATUS_CODE,''GENERATE_BURDEN'') = ''Y''
AND EI.ORG_ID = IMP.ORG_ID
AND CDL.ORG_ID = IMP.ORG_ID
) pa_view, hr_operating_units hou
where pa_view.GL_PERIOD_NAME = ''##$$PERIODNAME$$##''
and pa_view.org_id = hou.organization_id
and hou.set_of_books_id = ##$$LEDGERID$$##
group by pa_view.org_id, pa_view.gl_period_name
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' costing exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'Costing Exceptions in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found costing exceptions in PA. Costing exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Generate Cost Accounting Events.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no costing exceptions in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_COST_EXCP');



debug('begin add_signature: PCA_PA_GL_INT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11479',
      p_sig_id                 => 'PCA_PA_GL_INT',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
pa_view.org_id, count(*) count_rows
from
(SELECT   imp.org_id, prd.gl_period_name
FROM
    ...
    pa_periods_all prd
WHERE
    imp.interface_revenue_to_gl_flag = ''Y''
    AND dr.transfer_status_code IN (''P'',''R'',''X'',''T'')
    AND dr.draft_revenue_num = dri.draft_revenue_num
    AND dr.project_id = dri.project_id
    AND dr.pa_date BETWEEN prd.start_date AND prd.end_date
...
WHERE
    ...
    AND ( xe.event_status_code <> ''P''
          OR xe.process_status_code <> ''P'' )
    AND dr.draft_revenue_num = dri.draft_revenue_num
    AND dr.project_id = dri.project_id
    AND dr.pa_date BETWEEN prd.start_date AND prd.end_date
    AND dr.generation_error_flag != ''Y''
UNION
SELECT dr.org_id, prd.gl_period_name
FROM
    pa_draft_revenues_all dr,
    pa_draft_revenue_items dri,
    pa_periods_all prd
WHERE
    dr.draft_revenue_num = dri.draft_revenue_num
    AND dr.project_id = dri.project_id
    AND dr.pa_date BETWEEN prd.start_date AND prd.end_date
    AND EXISTS (
        SELECT
...
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || int.count_rows || '' revenue lines not transferred to GL.'' Status
FROM
ORGS,
INT
where
orgs.organization_id = int.org_id',
      p_title                  => 'Revenue Not Transferred to GL From PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found revenue not transferred to GL from PA. Revenue not transferred will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run:  EXC: Transaction Exceptions reports to identify the transactions</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no revenue lines not transferred to GL from PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_GL_INT');



debug('begin add_signature: PCA_PA_AP_INV_EXCP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11427',
      p_sig_id                 => 'PCA_PA_AP_INV_EXCP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
pa_view.org_id, count(*) count_rows
from
(SELECT IMP.org_id, invdist.accounting_date
FROM
    gl_ledgers glsob,
    ap_suppliers vend,
...
WHERE
    ...
    AND DECODE(inv.invoice_type_lookup_code, ''EXPENSE REPORT'', vend.employee_id, - 99) IS NOT NULL
    AND invdist.project_id > 0
    AND invdist.pa_addition_flag IN (''A'',''B'',''C'',''D'',''I'',''J'',''K'',''M'',''N'',''P'',''Q'',''S'',''V'',''X'',''O'')
    ...
    AND nvl(invdist.tax_recoverable_flag, ''N'') = ''N''
    AND ( ( ( inv.invoice_type_lookup_code = ''EXPENSE REPORT''
              AND inv.source IN (''XpenseXpress'',''Manual Invoice Entry'',''SelfService'')
              AND invdist.line_type_lookup_code <> ''PREPAY''
              AND ( vend.employee_id IS NOT NULL
                    OR nvl(inv.paid_on_behalf_employee_id, 0) > 0 ) )
            OR ( inv.invoice_type_lookup_code IN (''STANDARD'',''CREDIT'',''MIXED'')
        ...
        WHERE
            inv.paid_on_behalf_employee_id IS NULL
            ...
            AND inv.invoice_type_lookup_code NOT IN (''PREPAYMENT'',''EXPENSE REPORT'')
            AND invdist.line_type_lookup_code <> ''PREPAY''
            AND ( nvl(inv.source, ''xx'') NOT IN (''Oracle Project Accounting'',''PA_IC_INVOICES'',''PA_COST_ADJUSTMENTS'')
                  OR invdist.line_type_lookup_code = ''TAX'' )
    ) )
    AND ( pa_nl_installed.is_nl_installed = ''N''
        ...
        FROM
            ...
            po_lines_all pol
        WHERE
            pod.po_distribution_id = invdist.po_distribution_id
            AND pod.po_line_id = pol.po_line_id
            AND si.inventory_item_id = pol.item_id
            AND si.comms_nl_trackable_flag = ''Y''
            AND ( si.organization_id = pod.org_id
                  OR si.organization_id IN (
                SELECT
                    nvl(ship_to_organization_id, - 9999)
                FROM
                    po_line_locations_all pll
                WHERE
                    pll.po_header_id = pod.po_header_id
        ...
        FROM
            ...
            po_lines_all pol
        WHERE
            invdist.charge_applicable_to_dist_id = apdist.invoice_distribution_id
...
FROM
    ...
    ap_invoices_all inv,
    pa_implementations_all imp,
    ap_system_parameters_all appar
WHERE
    vend.vendor_id = inv.vendor_id
    AND inv.invoice_id = invdist.invoice_id
    AND inv.invoice_id = invline.invoice_id
    ...
    AND DECODE(inv.invoice_type_lookup_code, ''EXPENSE REPORT'', vend.employee_id, - 99) IS NOT NULL
    AND invdist.project_id > 0
    AND paydist.pa_addition_flag IN (''A'',''B'',''C'',''D'',''I'',''J'',''K'',''M'',''N'',''P'',''Q'',''S'',''V'',''X'',''O'')
    ...
    AND imp.org_id = appar.org_id
    AND nvl(glsob.sla_ledger_cash_basis_flag, ''N'') IN (''N'',''Y'')
    AND ( ( invdist.expenditure_item_date >= TO_DATE(nvl(fnd_profile.value_specific(''PA_DISC_PULL_START_DATE''), ''2051/01/01''), ''yyyy/mm/dd'')
            AND nvl(glsob.sla_ledger_cash_basis_flag, ''N'') = ''N''
            AND appar.discount_distribution_method IN (''TAX'',''EXPENSE'') )
          OR ( ( ( appar.discount_distribution_method IN (''TAX'',''EXPENSE'')
                   AND invdist.expenditure_item_date < TO_DATE(nvl(fnd_profile.value_specific(''PA_DISC_PULL_START_DATE''), ''2051/01/01''), ''yyyy/mm/dd'')
                   AND nvl(invdist.pa_addition_flag, ''N'') <> ''Y'' )
                 OR ( appar.discount_distribution_method IN (''TAX'',''EXPENSE'')
                      AND invdist.expenditure_item_date >= TO_DATE(nvl(fnd_profile.value_specific(''PA_DISC_PULL_START_DATE''), ''2051/01/01''), ''yyyy/mm/dd'')
                      AND invdist.pa_addition_flag = ''Y'' )
                 OR ( appar.discount_distribution_method = ''TAX''
                      AND invdist.line_type_lookup_code <> ''NONREC_TAX''
                      AND invdist.expenditure_item_date >= TO_DATE(nvl(fnd_profile.value_specific(''PA_DISC_PULL_START_DATE''), ''2051/01/01''), ''yyyy/mm/dd'') )
                 OR appar.discount_distribution_method = ''SYSTEM'' )
               AND nvl(glsob.sla_ledger_cash_basis_flag, ''N'') = ''Y'' ) )
    AND paydist.pay_dist_lookup_code = DECODE(nvl(glsob.sla_ledger_cash_basis_flag, ''N''), ''N'', ''DISCOUNT'', nvl(glsob.sla_ledger_cash_basis_flag, ''N''), ''Y'',''CASH'')
    AND inv.source <> ''Oracle Project Accounting''
    AND glsob.ledger_id = invdist.set_of_books_id
    AND invdist.org_id = imp.org_id
    AND nvl(invdist.tax_recoverable_flag, ''N'') = ''N''
...
WHERE
    ...
    AND NOT EXISTS (
        ...
        WHERE
            invdist.po_distribution_id = podist.po_distribution_id
            AND nvl(podist.destination_type_code, ''EXPENSE'') = ''SHOP FLOOR''
    )
    AND ( pa_nl_installed.is_nl_installed = ''N''
          OR ( pa_nl_installed.is_nl_installed = ''Y''
               AND NOT EXISTS (
...
where pa_view.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and pa_view.org_id = hou.organization_id
and hou.set_of_books_id = ##$$LEDGERID$$##
group by pa_view.org_id
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' AP invoice exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'AP Invoice Exceptions in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found AP invoice exceptions in PA. AP invoice exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Interface Supplier Costs.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no AP invoice exceptions in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_AP_INV_EXCP');



debug('begin add_signature: PCA_PA_UNCOST_TRX_EXCP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11922',
      p_sig_id                 => 'PCA_PA_UNCOST_TRX_EXCP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
pa_view.org_id, count(*) count_rows
from
(SELECT   imp.org_id, prd.period_name ,ei.expenditure_item_id 
FROM
...
WHERE
    ...
    AND ( ( ei.system_linkage_function <> ''PJ''
            AND ei.expenditure_item_date BETWEEN prd.start_date AND prd.end_date )
          OR ( ei.system_linkage_function = ''PJ''
               AND nvl(ei.prvdr_accrual_date, ei.expenditure_item_date) BETWEEN prd.start_date AND prd.end_date ) )
    AND prd.application_id = 8721
    AND prd.set_of_books_id = imp.set_of_books_id
    AND prd.adjustment_period_flag = ''N''
    AND nvl(ei.org_id, - 99) = imp.org_id
    AND ei.expenditure_item_date BETWEEN ppa.start_date AND ppa.end_date
...
WHERE
    ...
    AND ei.task_id = tt.task_id
    AND tt.project_id = pp.project_id
    AND nvl(exp.incurred_by_person_id, 0) = 0
    AND exp.expenditure_id = ei.expenditure_id
    AND ( ( ei.system_linkage_function <> ''PJ''
            AND ei.expenditure_item_date BETWEEN prd.start_date AND prd.end_date )
          OR ( ei.system_linkage_function = ''PJ''
    ...
    AND cdl.line_type = ''R''
    AND ei.transaction_source IN (''AP EXPENSE'',''AP INVOICE'')
...
WHERE
    ...
    AND ( ( ei.system_linkage_function NOT IN (''VI'',''ER'') )
        ...
        FROM
            pa_cost_distribution_lines_all cdl
        WHERE
            cdl.expenditure_item_id = ei.expenditure_item_id
            AND cdl.line_type = ''R''
            AND cdl.system_reference1 IS NOT NULL
    ) ) )
    AND ei.expenditure_item_date BETWEEN ppa.start_date AND ppa.end_date
...
where pa_view.period_name = ''##$$PERIODNAME$$##''
and hou.set_of_books_id = ##$$LEDGERID$$##
and pa_view.org_id = hou.organization_id
group by pa_view.org_id, pa_view.period_name
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' uncosted transaction exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'Uncosted Transaction Exceptions In PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found uncosted transaction exceptions in PA. Uncosted transaction exceptions will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>Run the appropriate cost distribution process.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>
',
      p_success_msg            => 'There are no uncosted transaction exceptions in PA. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_UNCOST_TRX_EXCP');



debug('begin add_signature: PCA_PA_RCV_ECP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11434',
      p_sig_id                 => 'PCA_PA_RCV_ECP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
EXCP AS
( select /*+ materialize */
imp.org_id, count(podist.project_id) count_rows
FROM
 po_distributions_all podist,
...
WHERE
 ...
 AND ( ( rcv.transaction_type = ''DELIVER''
       ...
       OR ( rcv.transaction_type = ''RETURN TO RECEIVING''
            AND rcv_sub.entered_cr IS NOT NULL )
       OR ( rcv.transaction_type = ''CORRECT''
            AND ( rcv_sub.entered_cr IS NOT NULL
                  OR rcv_sub.entered_dr IS NOT NULL )
            AND rcv_sub.accounting_line_type = ''Charge''
            AND DECODE(pol.order_type_lookup_code, ''RATE'', rcv.amount, ''FIXED PRICE'', rcv.amount, rcv.primary_quantity) < 0 )
       OR ( rcv.transaction_type = ''CORRECT''
            AND rcv_sub.entered_dr IS NOT NULL
            AND DECODE(pol.order_type_lookup_code, ''RATE'', rcv.amount, ''FIXED PRICE'', rcv.amount, rcv.primary_quantity) >= 0 )
            )
 AND nvl(podist.distribution_type, ''X'') <> ''PREPAYMENT''
...
AND rcv_sub.period_name = ''##$$PERIODNAME$$##''
AND imp.set_of_books_id = ##$$LEDGERID$$##
 AND podist.po_line_id = pol.po_line_id
 AND nvl(podist.org_id, - 99) = imp.org_id
 and imp.org_id = hou.organization_id   --> Moved inside
 AND rcv.po_distribution_id = podist.po_distribution_id
 AND rcv.shipment_header_id = rcv_hdr.shipment_header_id
 AND ( pa_pjc_cwk_utils.is_rate_based_line(NULL, podist.po_distribution_id) = ''N''
       OR pa_pjc_cwk_utils.is_cwk_tc_xface_allowed(podist.project_id) = ''N''
       OR ( pa_pjc_cwk_utils.is_rate_based_line(NULL, podist.po_distribution_id) = ''Y''
            AND nvl(rcv_sub.entered_nr_tax, 0) <> 0 ) )
group by imp.org_id
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' PO receipt accrual exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'PO Receipt Accrual Exceptions in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found PO receipt accrual exceptions in PA. PO receipt accrual exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Generate Cost Accounting Events.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no PO receipt accrual exceptions in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_RCV_ECP');



debug('begin add_signature: PCA_PA_PM_EXCP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11925',
      p_sig_id                 => 'PCA_PA_PM_EXCP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
EXCP AS
( select /*+ materialize */
cpp.organization_id org_id, count(cpp.project_id) count_rows
from    CST_PM_PENDING_TXN_V cpp,
        hr_operating_units hou
where   hou.set_of_books_id = ##$$LEDGERID$$##
and     cpp.transaction_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and     cpp.organization_id = hou.organization_id
group by cpp.organization_id
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' Project Manufacturing/Inventory Exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'Project Manufacturing/Inventory Exceptions in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found Project Manufacturing/Inventory Exceptions in PA. Project Manufacturing/Inventory Exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Inventory: Accounting Close Cycle > Project Cost Transfer: Cost Collection Manager.</li>
<li>and From Projects: Control > Request > Run: PRC: Transaction Import process.</li>
<li>For more details on the issues review [329701.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no Project Manufacturing/Inventory Exceptions in PA. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_PM_EXCP');



debug('begin add_signature: PCA_PA_CROSS_CHRG_EXCP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11924',
      p_sig_id                 => 'PCA_PA_CROSS_CHRG_EXCP',
      p_sig_sql                => 'WITH
ORGS AS
( select /*+ materialize */
organization_id,
...
pa_view.org_id, count(*) count_rows
from
(SELECT pp.org_id, cdl.gl_date, ei.expenditure_item_id 
FROM
...
WHERE
    ...
    AND DECODE(otl.organization_id, NULL, ''1'', otl.language) = DECODE(otl.organization_id, NULL, ''1'', userenv(''lang''))
...
where pa_view.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
and pa_view.org_id = hou.organization_id
and hou.set_of_books_id = ##$$LEDGERID$$##
group by pa_view.org_id, pa_view.gl_date
having count(*) > 0 )
SELECT
''Operating Unit = <b>'' || orgs.name || ''</b>: Found '' || excp.count_rows || '' cross charge exceptions.'' Status
FROM
ORGS,
EXCP
where
orgs.organization_id = excp.org_id',
      p_title                  => 'Cross Charge Exceptions in PA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found cross charge exceptions in PA. Cross charge exceptions will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Projects: Control > Request > Run: PRC: Generate Cross Charge Accounting Events.</li>
<li>For more details on issue run the EXC: Transaction Exception Reports [1336233.1].</li>
<li>If you need assistance, please log a Service Request with Project Support.</li>
</ul>',
      p_success_msg            => 'There are no cross charge exceptions in PA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_PA_CROSS_CHRG_EXCP');



debug('begin add_signature: PCA_PA_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10370',
      p_sig_id                 => 'PCA_PA_SUMMARY',
      p_sig_sql                => 'select ''Projects'' application_name, ''This section checks for Project Accounting (PA) issues.'' "Description"
from dual',
      p_title                  => 'Project Accounting (PA) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Project Accounting (PA) issues identified.',
      p_solution               => 'Please open this section to view Project Accounting (PA) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_PA_PERIOD_STATUS','PCA_PA_UNPROCESSED_EVENTS','PCA_PA_INVALID_EVENTS','PCA_PA_NONXFR_EVENTS','PCA_PA_COST_EXCP','PCA_PA_GL_INT','PCA_PA_AP_INV_EXCP','PCA_PA_UNCOST_TRX_EXCP','PCA_PA_RCV_ECP','PCA_PA_PM_EXCP','PCA_PA_CROSS_CHRG_EXCP'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '111'
      );
   l_info.delete;
debug('end add_signature: PCA_PA_SUMMARY');



debug('begin add_signature: PCA_FA_BOOKS_NOT_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11040',
      p_sig_id                 => 'PCA_FA_BOOKS_NOT_CLOSED',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */
book_type_code,
...
DEPR AS
( select /*+ materialize */
fbc.book_type_code, count(*) count_rows
from FA_BOOK_CONTROLS fbc, FA_DEPRN_PERIODS fdp
where fbc.book_type_code = fdp.book_type_code
and fbc.book_class <> ''BUDGET''
and fbc.date_ineffective is null
and fdp.PERIOD_CLOSE_DATE is null
and fbc.deprn_status = ''C''
and     fbc.set_of_books_id = ##$$LEDGERID$$##
and     fdp.period_name = ''##$$PERIODNAME$$##''
GROUP BY fbc.book_type_code
HAVING COUNT(*) > 0)
SELECT
''Asset Book = <b>'' || books.book_type_name || ''</b>: Found '' || depr.count_rows || '' asset book not closed.'' Status
FROM
BOOKS,
DEPR
where
books.book_type_code = depr.book_type_code',
      p_title                  => 'Asset Books Not Closed in FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found asset books not closed in FA. Asset books will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Fixed Assets: Depreciation > Run Depreciation: Deprecation and Close Asset Books</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'Asset books are closed in FA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_BOOKS_NOT_CLOSED');



debug('begin add_signature: PCA_FA_UNPROCESSED_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10967',
      p_sig_id                 => 'PCA_FA_UNPROCESSED_EVENTS',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */ 
book_type_code,
...
UNPROC AS
( select /*+ materialize */
xlt.VALUATION_METHOD, count(*) count_rows
from xla_events xe, 
...
AND xlt.entity_code in (''TRANSACTIONS'',''DEPRECIATION'')
AND xlt.ledger_id = ##$$LEDGERID$$##
AND xe.event_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND xe.process_status_code <> ''P''
AND xe.process_status_code <> ''I''
AND xe.event_status_code <> ''P''
AND xe.event_status_code <> ''I''
group by xlt.ledger_id, xlt.VALUATION_METHOD, xe.EVENT_STATUS_CODE
HAVING COUNT(*) > 0)
SELECT
''Asset Book = <b>'' || books.book_type_name || ''</b>: Found '' || unproc.count_rows || '' unprocessed events.'' Status
FROM
BOOKS,
UNPROC
where
books.book_type_code = unproc.VALUATION_METHOD',
      p_title                  => 'Unprocessed Events in  FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found unprocessed events in FA. Unprocessed events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Fixed Assets: Depreciation > Run Depreciation: Depreciation.</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'Found no unprocessed events in FA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_UNPROCESSED_EVENTS');



debug('begin add_signature: PCA_FA_INVALID_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11027',
      p_sig_id                 => 'PCA_FA_INVALID_EVENTS',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */ 
book_type_code,
...
where
books.book_type_code = unproc.VALUATION_METHOD',
      p_title                  => 'Invalid Events in  FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found invalid events in FA. Invalid events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Fixed Assets: Depreciation > Run Depreciation: Depreciation.</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no invalid events in FA. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_INVALID_EVENTS');



debug('begin add_signature: PCA_FA_NONXF_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11036',
      p_sig_id                 => 'PCA_FA_NONXF_EVENTS',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */ 
book_type_code,
...
FROM XLA_AE_HEADERs XAH,
XLA_TRANSACTION_ENTITIES_UPG xlt
WHERE xah.application_id = xlt.APPLICATION_ID
AND xah.ledger_id = xlt.ledger_id
AND XAH.ENTITY_ID = xlt.ENTITY_ID
AND xah.LEDGER_ID = ##$$LEDGERID$$##
AND xah.application_id = 140
AND xah.gl_transfer_status_code = ''N''
AND xah.accounting_entry_status_code = ''F''
AND xah.accounting_date BETWEEN to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') AND to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
group by xlt.ledger_id, xlt.VALUATION_METHOD
HAVING COUNT(*) > 0)
SELECT
''Asset Book = <b>'' || books.book_type_name || ''</b>: Found '' || nonxfr.count_rows || '' untransferred events.'' Status
FROM
BOOKS,
NONXFR
where
books.book_type_code = nonxfr.VALUATION_METHOD',
      p_title                  => 'Untransferred Events in FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found untransferred events in FA. Untransferred events will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => 'ul>
<li>From Fixed Assets: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no untransferred events in FA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_NONXF_EVENTS');



debug('begin add_signature: PCA_FA_DEPR_INCOMPLETE_BOOKS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10953',
      p_sig_id                 => 'PCA_FA_DEPR_INCOMPLETE_BOOKS',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */ 
book_type_code,
...
HAVING COUNT(*) > 0)
SELECT
''Asset Book = <b>'' || books.book_type_name || ''</b>: Found '' || depr.count_rows || '' incomplete deprecation book.'' Status
FROM
BOOKS,
DEPR
where
books.book_type_code = depr.book_type_code',
      p_title                  => 'Incomplete Depreciation Books in FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found incomplete depreciation books in FA. Incomplete depreciation books will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Fixed Assets: Depreciation > Run Depreciation: Depreciation.</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no incomplete depreciation books in FA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_DEPR_INCOMPLETE_BOOKS');



debug('begin add_signature: PCA_FA_MRC_DEPR_INCOMPLETE_BOOKS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10959',
      p_sig_id                 => 'PCA_FA_MRC_DEPR_INCOMPLETE_BOOKS',
      p_sig_sql                => 'WITH
BOOKS AS
( select /*+ materialize */
book_type_code,
...
( select /*+ materialize */
fbc.book_type_code, count(*) count_rows
FROM  	fa_mc_book_controls fmc, fa_book_controls fbc, fa_deprn_periods fdp
where 	fmc.book_type_code = fbc.book_type_code
and 	fmc.book_type_code = fdp.book_type_code
and 	fmc.deprn_status <> ''C''
and 	fbc.deprn_status <> ''C''
and     fbc.set_of_books_id = ##$$LEDGERID$$##
and     fdp.period_name = ''##$$PERIODNAME$$##''
GROUP BY fbc.book_type_code
HAVING COUNT(*) > 0)
SELECT
''Asset Book = <b>'' || books.book_type_name || ''</b>: Found '' || depr.count_rows || '' incomplete depreciation MRC books.'' Status
FROM
BOOKS,
DEPR
where
books.book_type_code = depr.book_type_code',
      p_title                  => 'Incomplete Depreciation MRC Books in FA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found incomplete depreciation MRC books in FA. Incomplete depreciation MRC books will prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From Fixed Assets Run Depreciation.</li>
<li>To gather more details regarding this issue, please run the Fixed Assets Analyzer in [2061234.1] for Posting Book identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle Fixed Assets and upload the Fixed Assets Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no incomplete depreciation MRC books in FA.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_FA_MRC_DEPR_INCOMPLETE_BOOKS');



debug('begin add_signature: PCA_FA_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10214',
      p_sig_id                 => 'PCA_FA_SUMMARY',
      p_sig_sql                => 'select ''Assets'' application_name, ''This section checks for Fixed Assets (FA) issues.'' "Description"
from dual',
      p_title                  => 'Fixed Assets (FA) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view Fixed Assets (FA) issues identified.',
      p_solution               => 'Please open this section to view Fixed Assets (FA) issues identified. ',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_FA_BOOKS_STATUS','PCA_FA_BOOKS_NOT_CLOSED','PCA_FA_UNPROCESSED_EVENTS','PCA_FA_INVALID_EVENTS','PCA_FA_NONXF_EVENTS','PCA_FA_DEPR_INCOMPLETE_BOOKS','PCA_FA_MRC_DEPR_INCOMPLETE_BOOKS'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '94'
      );
   l_info.delete;
debug('end add_signature: PCA_FA_SUMMARY');



debug('begin add_signature: PCA_GL_JOURNAL_AWAITING_IMP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10532',
      p_sig_id                 => 'PCA_GL_JOURNAL_AWAITING_IMP',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
JOURN AS
( select /*+ materialize */
gli.ledger_id ledger_id, gli.user_je_source_name source, count(gli.group_id) count_rows
FROM GL_INTERFACE gli, 
     GL_PERIOD_STATUSES ps
WHERE NVL(gli.ledger_id, -99) = ##$$LEDGERID$$##
AND gli.accounting_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND gli.ledger_id = ps.ledger_id
and gli.period_name = ps.period_name
AND ps.application_id = 101
AND gli.accounting_date < (ps.end_date+1)
AND gli.actual_flag in(''A'',''E'')
AND upper(gli.status) != ''PROCESSED'' 
GROUP BY gli.ledger_id, gli.user_je_source_name
having count(gli.group_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || journ.count_rows || '' journal(s) in '' || journ.source || '' journals awaiting import.'' Status
FROM
SOB,
JOURN
where
sob.ledger_id = journ.ledger_id',
      p_title                  => 'Journals Awaiting Import in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found journals awaiting import in GL.  Journals awaiting import will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From General Ledger: Journals > Import > Run: Journal Import.</li>
<li>To gather more details regarding this issue, please run the Oracle General Ledger Analyzer in [2117528.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the General Ledger Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no journals awaiting import in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_JOURNAL_AWAITING_IMP');



debug('begin add_signature: PCA_GL_JOURNAL_AWAITING_POST');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10533',
      p_sig_id                 => 'PCA_GL_JOURNAL_AWAITING_POST',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
JOURN AS
( select /*+ materialize */
jh.ledger_id ledger_id, count(jb.je_batch_id) count_rows
FROM gl_je_batches jb,
     gl_je_headers jh, 
	 gl_lookups lookupBatch, 
	 gl_lookups lookupApprove
WHERE NVL(jh.ledger_id, -99) = ##$$LEDGERID$$##
AND jb.default_effective_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND jb.je_batch_id = jh.je_batch_id
AND lookupBatch.lookup_type = ''BATCH_STATUS''
AND jb.status = lookupBatch.lookup_code
AND lookupApprove.lookup_type = ''JE_BATCH_APPROVAL_STATUS''
AND jb.approval_status_code = lookupApprove.lookup_code
AND (jb.status != ''P''
OR jh.status != ''P'')
group by jh.ledger_id
having count(jb.je_batch_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || journ.count_rows || '' journals awaiting posting.'' Status
FROM
SOB,
JOURN
where
sob.ledger_id = journ.ledger_id',
      p_title                  => 'Journals Awaiting Posting in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found journals awaiting posting in GL. Journals awaiting posting will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From General Ledger: Journals > Post: Post Journal.</li>
<li>To gather more details regarding this issue, please run the Oracle General Ledger Analyzer in [2117528.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the General Ledger Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no journals awaiting posting in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_JOURNAL_AWAITING_POST');



debug('begin add_signature: PCA_GL_RECURR_JOURN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10535',
      p_sig_id                 => 'PCA_GL_RECURR_JOURN',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
RECURR AS
( select /*+ materialize */
rb.ledger_id ledger_id, count(rb.name) count_rows
FROM gl_recurring_batches rb 
WHERE NVL(rb.ledger_id, -99) = ##$$LEDGERID$$##
AND rb.last_executed_period_name in 
(SELECT p.period_name 
FROM gl_periods p, gl_ledgers gll 
WHERE p.period_set_name = gll.period_set_name 
AND p.period_type = gll.accounted_period_type 
AND gll.ledger_id = ##$$LEDGERID$$## 
AND p.end_date < (select g.end_date
from gl_period_statuses g
where g.application_id = 101
and g.ledger_id = ##$$LEDGERID$$##
and g.period_name = ''##$$PERIODNAME$$##'') )
AND rb.last_executed_period_name is not null
group by rb.ledger_id
having count(rb.name) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || recurr.count_rows || '' recurring journal batches.'' Status
FROM
SOB,
RECURR
where
sob.ledger_id = recurr.ledger_id',
      p_title                  => 'Recurring Journal Batches in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found recurring journal batches in GL.  Recurring journal batches will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From General Ledger: Journals > Generate > Recurring: Recurring Batch.</li>
<li>To gather more details regarding this issue, please run the Oracle General Ledger Analyzer in [2117528.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the General Ledger Analyzer output.</li>
</ul>

',
      p_success_msg            => 'There are no recurring journal batches in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_RECURR_JOURN');



debug('begin add_signature: PCA_GL_BC_PACK_PEND');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10540',
      p_sig_id                 => 'PCA_GL_BC_PACK_PEND',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
BC AS
( select /*+ materialize */
ledger_id ledger_id, count(packet_id) count_rows
FROM gl_bc_packets 
WHERE NVL(ledger_id, -99) = ##$$LEDGERID$$##
AND period_name = ''##$$PERIODNAME$$##''
AND status_code in (''P'',''C'') 
group by ledger_id
having count(packet_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || bc.count_rows || '' pending balance control Packets.'' Status
FROM
SOB,
BC
where
sob.ledger_id = bc.ledger_id',
      p_title                  => 'Pending Balance Control Packets in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found pending balance control packets in GL. Pending balance control packets will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Purchasing, Cost management and Payables: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>From General Ledger: Control > Request > Run: Program - Create Journals.</li>
<li>From General Ledger: Control > Request > Run: Budgetary Control Optimizer (BCOPT).</li>
<li>To gather more details regarding this issue, please run the Oracle General Ledger Analyzer in [2117528.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the General Ledger Analyzer output.</li>
</ul>
',
      p_success_msg            => 'There are no pending balance control packets in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_BC_PACK_PEND');



debug('begin add_signature: PCA_GL_BC_PACK_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10541',
      p_sig_id                 => 'PCA_GL_BC_PACK_ERROR',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
AND ae_header_id is not null 
group by ledger_id
having count(packet_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || bc.count_rows || '' balance control packets in error.'' Status
FROM
SOB,
BC
where
sob.ledger_id = bc.ledger_id',
      p_title                  => 'Balance Control Packets in Error in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found balance control packets in error in GL. Balance control packets in error will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul><li>From Purchasing, Cost management and Payables: Control > Request > Run: Transfer Journal Entries to GL.</li>
<li>From General Ledger: Control > Request > Run: Program - Create Journals.</li>
<li>From General Ledger: Control > Request > Run: Budgetary Control Optimizer (BCOPT).</li>
<li>To gather more details regarding this issue, please run the Oracle General Ledger Analyzer in [2117528.1] for Ledger identified above.</li>
...
',
      p_success_msg            => 'There are no balance control packets in error in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_BC_PACK_ERROR');



debug('begin add_signature: PCA_GL_AGIS_TRX_ERROR_B');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10615',
      p_sig_id                 => 'PCA_GL_AGIS_TRX_ERROR_B',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
AGIS AS
( select /*+ materialize */
b.from_ledger_id ledger_id, count(h.trx_id) count_rows
FROM fun_trx_batches b ,
     fun_trx_headers h
WHERE NVL(b.from_ledger_id, -99) = ##$$LEDGERID$$##
AND b.gl_date between to_date(''##$$STARTDATE$$##'',''DD-MON-YYYY'') and to_date(''##$$ENDDATE$$##'',''DD-MON-YYYY'')
AND b.batch_id = h.batch_id
AND h.status =''ERROR''
group by b.from_ledger_id
having count(h.trx_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || agis.count_rows || '' AGIS transactions in error.'' Status
FROM
SOB,
AGIS
where
sob.ledger_id = agis.ledger_id',
      p_title                  => 'AGIS Transactions in Error in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found AGIS transactions in error in GL. AGIS transactions in error will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From General Ledger: Journals > Import > Run: Journal Import for source "Global Intercompany".</li>
<li>To gather more details regarding this issue, please run the Oracle AGIS Analyzer in [2208479.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the AGIS Analyzer output.</li>
</ul>',
      p_success_msg            => 'There are no AGIS transactions in error in GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_AGIS_TRX_ERROR_B');



debug('begin add_signature: PCA_GL_AGIS_TRX_SENT_B');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10616',
      p_sig_id                 => 'PCA_GL_AGIS_TRX_SENT_B',
      p_sig_sql                => 'WITH
SOB AS
( select /*+ materialize */
ledger_id,
...
having count(h.trx_id) > 0 )
SELECT
''Ledger = <b>'' || sob.name || ''</b>: Found '' || agis.count_rows || '' AGIS transactions with sent status.'' Status
FROM
SOB,
AGIS
where
sob.ledger_id = agis.ledger_id',
      p_title                  => 'AGIS Transactions with Sent Status in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found AGIS transactions with sent status in GL. AGIS transactions in sent status will not prevent you from closing the '||mask_text(g_period_tmp, nvl( upper(''), 'NO_MASK') )||' period.',
      p_solution               => '<ul>
<li>From General Ledger: Journals > Import > Run: Journal Import for source "Global Intercompany".</li>
<li>To gather more details regarding this issue, please run the Oracle AGIS Analyzer in [2208479.1] for Ledger identified above.</li>
<li>If you need assistance, please log a Service Request with Oracle General Ledger and upload the AGIS Analyzer output.</li>
...
',
      p_success_msg            => 'There are no AGIS transactions in sent status with GL.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: PCA_GL_AGIS_TRX_SENT_B');



debug('begin add_signature: PCA_GL_SUMMARY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10371',
      p_sig_id                 => 'PCA_GL_SUMMARY',
      p_sig_sql                => 'select ''This section checks for General Ledger (GL) issues.'' "Description"
from dual',
      p_title                  => 'General Ledger (GL) Summary',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Please open this section to view General Ledger (GL) issues identified.',
      p_solution               => 'Please open this section to view General Ledger (GL) issues identified.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PCA_GL_JOURNAL_AWAITING_IMP','PCA_GL_JOURNAL_AWAITING_POST','PCA_GL_RECURR_JOURN','PCA_GL_BC_PACK_PEND','PCA_GL_BC_PACK_ERROR','PCA_GL_AGIS_TRX_ERROR_B','PCA_GL_AGIS_TRX_SENT_B'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '74'
      );
   l_info.delete;
debug('end add_signature: PCA_GL_SUMMARY');



debug('begin add_signature: EBS_FIN_CAL_PERIOD_GAP_AND_OVERLAP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '27823',
      p_sig_id                 => 'EBS_FIN_CAL_PERIOD_GAP_AND_OVERLAP',
      p_sig_sql                => 'SELECT  perset1.period_set_name "Accounting Calendar Name"
      ...
      , decode(sign(perset2.start_date - perset1.end_date - 1) 
               ,-1,''OVERLAP'',1,''GAP'',0,null) "Problem" 
FROM    (SELECT rownum rn
        ,       period_name 
        ,       period_set_name 
        ,       start_date, end_date, period_type
        FROM    (SELECT per.period_name 
                ,       per.period_set_name 
                ,       per.period_type 
                ,       per.start_date 
                ,       per.end_date 
                ...
                ,       GL_PERIODS per 
                WHERE   ledger.period_set_name = per.period_set_name 
                AND     per.adjustment_period_flag = ''N'' 
                AND     ledger.ledger_id = ##$$LEDGERID$$##
       ...
       (SELECT rownum rn
        ...
        FROM    (SELECT per.period_name
                ...
                AND     per.adjustment_period_flag = ''N'' 
                AND     ledger.ledger_id = ##$$LEDGERID$$## 
                order by per.period_type, per.start_date) ) perset2, 
       GL_PERIOD_TYPES pt
WHERE   perset2.rn = perset1.rn + 1
...
AND     trunc(to_char(perset1.end_date,''yyyy'')) >= trunc(to_char(sysdate,''yyyy'') - 1)',
      p_title                  => 'Calendar Gap or Overlapping Periods',
      p_fail_condition         => 'RS',
      p_problem_descr          => '<b>ALERT! </b> Overlapping periods or gaps between periods have been detected.',
      p_solution               => '<ul><li>Please fix the problems identified in this section as soon as possible. Run the General Ledger concurrent program "Other - Calendar Validation Report" for more information.</li>
<li> [1061798.1] R12: Calendar Changes that can be Supported in General Ledger. </li>
<li> [95447.1] Calendar Missing Date - Gap Datafix  </li></ul>',
      p_success_msg            => 'No overlapping periods or gaps in periods found for this Ledger for the current and/or previous year.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_FIN_CAL_PERIOD_GAP_AND_OVERLAP');
...
BEGIN
  ...
  analyzer_title := 'EBS Period Close Process Analyzer';
...
debug('begin section: PCA_PERIOD_INFO');
start_section('Ledger and Period Information', 'PCA_PERIOD_INFO');
   set_item_result(run_stored_sig('PCA_GL_LEDGER_INFO'));
   set_item_result(run_stored_sig('PCA_HR_ORG_INFO'));
   set_item_result(run_stored_sig('PCA_INV_ORG_INFO'));
   set_item_result(run_stored_sig('PCA_GL_PERIOD_STATUS_FOR_APPS'));
   set_item_result(run_stored_sig('PCA_INV_PERIOD_STATUS'));
   set_item_result(run_stored_sig('PCA_PA_PERIOD_STATUS'));
   set_item_result(run_stored_sig('PCA_FA_BOOKS_STATUS'));
end_section;
debug('end section: PCA_PERIOD_INFO');

debug('begin section: Lease_Management');
start_section('Lease Management (OKL) ' || '<br>' || 'GL Period is '|| g_status_OKL||'', 'Lease_Management');
   set_item_result(run_stored_sig('PCA_OKL_SUMMARY'));
end_section('There are no Lease Management (OKL) issues identified preventing period close.');
debug('end section: Lease_Management');

debug('begin section: Channel_Revenue_Mgmt');
start_section('Trade Mgmt / Channel Rev Mgmt (ChRM)' || '<br>' ||' GL Period is '|| g_status_CRM||'', 'Channel_Revenue_Mgmt');
   set_item_result(run_stored_sig('PCA_CRM_SUMMARY'));
end_section('There are no Trade Mgmt / Channel Rev Mgmt (ChRM) issues identified preventing period close.');
debug('end section: Channel_Revenue_Mgmt');

debug('begin section: Order_Management');
start_section('Order Management (OM) ', 'Order_Management');
   set_item_result(run_stored_sig('PCA_OM_SUMMARY'));
end_section('There are no Order Management (OM) issues identified preventing period close.');
debug('end section: Order_Management');

debug('begin section: Cash_Management');
start_section('Cash Management (CE)', 'Cash_Management');
   set_item_result(run_stored_sig('PCA_CE_SUMMARY'));
end_section('There are no Cash Management (CE) issues identified preventing period close.');
debug('end section: Cash_Management');

debug('begin section: Payables');
start_section('Payables (AP) ' || '<br>'||'GL Period is '||g_status_AP||'', 'Payables');
   set_item_result(run_stored_sig('PCA_AP_SUMMARY'));
end_section('There are no Payables (AP) issues identified preventing period close.');
debug('end section: Payables');

debug('begin section: Receivables');
start_section('Receivables (AR) ' || '<br>' || 'GL Period is '|| g_status_AR||'', 'Receivables');
   set_item_result(run_stored_sig('PCA_AR_SUMMARY'));
end_section('There are no Receivables (AR) issues identified preventing period close.');
debug('end section: Receivables');

debug('begin section: Purchase_Order');
start_section('Purchase Order (PO) ' || '<br>' || 'GL Period is '|| g_status_PO||'', 'Purchase_Order');
   set_item_result(run_stored_sig('PCA_PO_SUMMARY'));
end_section('There are no Purchase Order (PO) issues identified preventing period close.');
debug('end section: Purchase_Order');

debug('begin section: Cost_Inventory');
start_section('Cost (CST) / Inventory (INV)', 'Cost_Inventory');
   set_item_result(run_stored_sig('PCA_INV_SUMMARY'));
end_section('There are no Cost (CST) / Inventory (INV) issues identified preventing period close.');
debug('end section: Cost_Inventory');

debug('begin section: Project_Accounting');
start_section('Project Accounting (PA) ' || '<br>' ||g_status_PA||'', 'Project_Accounting');
   set_item_result(run_stored_sig('PCA_PA_SUMMARY'));
end_section('There are no Project Accounting (PA) issues identified preventing period close.');
debug('end section: Project_Accounting');

debug('begin section: Fixed_Assets');
start_section('Fixed Assets (FA) ', 'Fixed_Assets');
   set_item_result(run_stored_sig('PCA_FA_SUMMARY'));
end_section('There are no Fixed Assets (FA) issues identified preventing period close.');
debug('end section: Fixed_Assets');

debug('begin section: General_Ledger');
start_section('General Ledger (GL) '||'<br>'||'GL Period is '|| g_status_GL||'', 'General_Ledger');
   set_item_result(run_stored_sig('PCA_GL_SUMMARY'));
   set_item_result(run_stored_sig('EBS_FIN_CAL_PERIOD_GAP_AND_OVERLAP'));
end_section('There are no General Ledger (GL) issues identified preventing period close.');
debug('end section: General_Ledger');
