...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Order Management Credit Check Analyzer |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE om_credit_analyzer_pkg AUTHID DEFINER AS
...
END om_credit_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY om_credit_analyzer_pkg AS
-- $Id: om_credit_analyzer.sql, 200.34 2026/01/27 21:18:39 cschmehl Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2493364.1));
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'OMCREDITCK_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'OMCREDITCK_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>OMCREDITCK Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2493364.1#feedback");
...
-- page title
    print_out('        <div class="header_title">'||p_analyzer_title|| ' Report <br>
        ...
        <img class="header_version" src="https://www.oracle.com/webfolder/s/analyzer/om_credit_latest_version.gif" title="Click here to download the latest version of Analyzer" alt="Latest Version Icon"></a>');
    print_cloud_image;
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1694,EBS,EBS MFG,EBS - Order Management" target="_blank"><span class="thumb_up_lg" title="Analyzer Feedback: Thumbs Up"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1694,EBS,EBS MFG,EBS - Order Management" target="_blank"><span class="thumb_dn_lg" title="Analyzer Feedback: Thumbs Down"></span></a>');
    print_out('<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1694,EBS,EBS MFG,EBS - Order Management" target="_blank"><span class="af_idea_lg" title="Analyzer Idea"></span></a>');
    print_out('        </div>
    </div>');
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/4194504" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">1694</span>
...
Signature AC_TABLE_INDEXES has been changed.</p>
            <br><br>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1694,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Order Management" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1694,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Order Management" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
...
----------------------------------------------------------------
PROCEDURE get_cc_level IS 
BEGIN
  SELECT credit_check_level_code
  INTO g_cc_level
  FROM oe_credit_check_rules
  WHERE credit_check_rule_id = g_sql_tokens('##$$RULEID$$##');
END get_cc_level;
...
IS
  ...
  v_option_number    NUMBER; 
  v_component_number NUMBER; 
  v_service_number   NUMBER;
  p_concat_value     VARCHAR2(60);
BEGIN
  SELECT line_number, shipment_number, option_number, component_number, service_number
  INTO v_line_number, v_shipment_number, v_option_number, v_component_number, v_service_number       
  FROM oe_order_lines_all  
  WHERE line_id = v_line_id;
  
  -- Added for identifying Service Lines
  IF v_service_number is not null then
     IF v_option_number is not null then
        IF v_component_number is not null then
           p_concat_value := v_line_number||'.'||v_shipment_number||'.'||v_option_number||'.'||v_component_number||'.'||v_service_number;
        ELSE
           p_concat_value := v_line_number||'.'||v_shipment_number||'.'||v_option_number||'..'||v_service_number;
        END IF;
      --- if a option is not attached
      ELSE
        IF v_component_number is not null then
           p_concat_value := v_line_number||'.'||v_shipment_number||'..'||v_component_number||'.'||v_service_number;
        ELSE
           p_concat_value := v_line_number||'.'||v_shipment_number||'...'||v_service_number;
        END IF;
    ...
    ELSE
      IF v_option_number is not null then
         IF v_component_number is not null then
            p_concat_value := v_line_number||'.'||v_shipment_number||'.'||v_option_number||'.'||v_component_number;
        ELSE
            p_concat_value := v_line_number||'.'||v_shipment_number||'.'||v_option_number;
        END IF;
      --- if a option is not attached
      ELSE
        IF v_component_number is not null then
           p_concat_value := v_line_number||'.'||v_shipment_number||'..'||v_component_number;
        ELSE
           IF (v_line_number is NULL and v_shipment_number is NULL ) THEN
              p_concat_value := NULL;
           ELSE
              p_concat_value := v_line_number||'.'||v_shipment_number;
           END IF;
        END IF;
      END IF; /* if option number is not null */
...
FUNCTION get_RULEID
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: RULEID');
BEGIN
    SELECT
        DECODE(p_phase,
          'BOOKING',tt.entry_credit_check_rule_id,
          'PICKING',tt.picking_credit_check_rule_id,
          'PACKING',tt.packing_credit_check_rule_id,
          'SHIPPING',tt.shipping_credit_check_rule_id)
...
EXCEPTION
    WHEN no_data_found THEN
        RETURN NULL;
END;
debug('end sql token function: RULEID');
END get_RULEID;

FUNCTION get_CRPROFID
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: CRPROFID');
BEGIN 
      SELECT
          hcp.credit_profile_id
      INTO l_system_function_var
      FROM
          hz_credit_profiles_v hcp,
          oe_order_headers_all oh
      WHERE
          oh.header_id = p_header_id
AND ( hcp.organization_id = oh.org_id
      OR hcp.organization_id IS NULL );
...
EXCEPTION
    WHEN no_data_found THEN
        RETURN 9999999999;
END;
debug('end sql token function: CRPROFID');
END get_CRPROFID;
...
debug('begin populate parameters hash table');
   ...
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Credit Check Phase';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_phase,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/01/27 21:18:38 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'om_credit_analyzer.sql';
...
WHERE HEADER_ID = p_header_id
 AND (mo_global.is_mo_init_done = 'N' or mo_global.check_access(H.ORG_ID) = 'Y')
 and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Invalid Header ID parameter provided.  Process cannon continue.');
   raise invalid_parameters;
   WHEN OTHERS THEN
...
debug('begin parameter validation: p_phase');
IF p_phase IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Credit Check Phase is required.');
   raise invalid_parameters;
END IF;
IF p_phase IS NOT NULL AND p_phase NOT IN ( 'BOOKING','PACKING','PICKING','SHIPPING') THEN
   print_error('INVALID ARGUMENT: Invalid Credit Check Phase.  Process cannot continue.');
   raise invalid_parameters;
END IF;
debug('end parameter validation: p_phase');
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$PHASE$$##') := p_phase;
   g_masked_tokens('##$$PHASE$$##') := mask_text( g_sql_tokens('##$$PHASE$$##'), 'NO_MASK');
   g_sql_tokens('##$$RULEID$$##') := get_RULEID();
   l_system_function_var := null;
   g_masked_tokens('##$$RULEID$$##') := mask_text( g_sql_tokens('##$$RULEID$$##'), 'NO_MASK');
   g_sql_tokens('##$$CRPROFID$$##') := get_CRPROFID();
   l_system_function_var := null;
   g_masked_tokens('##$$CRPROFID$$##') := mask_text( g_sql_tokens('##$$CRPROFID$$##'), 'NO_MASK');
...
debug('begin add_signature: OMCREDIT_ORDER_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10266',
      p_sig_id                 => 'OMCREDIT_ORDER_DETAILS',
      p_sig_sql                => 'SELECT ord.order_number "Order Number",
       ord.header_id "Header ID",
       substr(cus.account_name, 1, 20) "Customer",
       ord.sold_to_org_id "Customer ID",
       cus.party_id "Party ID",
       ord.ordered_date "Order Date",
       ord.flow_status_code "Status",
       ord.ship_from_org_id "Warehouse",
       ord.transactional_curr_code "Currency",
       typ.name "Order Type",
       ord.order_type_id "Order Type ID",
       rt.name "Payment Term",
       ord.payment_term_id "Payment Term ID",
       rt.credit_check_flag "Credit Check Flag",
       ord.invoice_to_org_id "Bill To ID"
  FROM oe_order_headers_all ord,
       hz_cust_accounts cus,
       oe_transaction_types_tl typ,
       ra_terms rt
 WHERE ord.sold_to_org_id = cus.cust_account_id
   AND ord.header_id = ##$$HEADERID$$##
   AND ord.order_type_id = typ.transaction_type_id
   AND rt.term_id = ord.payment_term_id
   AND typ.language = ''US''',
      p_title                  => 'Order Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Order Details',
      p_solution               => '',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_ORDER_DETAILS');



debug('begin add_signature: OMCREDIT_LINES_ELIGIBLE_FOR_CREDIT_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10282',
      p_sig_id                 => 'OMCREDIT_LINES_ELIGIBLE_FOR_CREDIT_CHECK',
      p_sig_sql                => 'SELECT om_credit_analyzer_pkg.line_shipment_option(lin.line_id) "Line",
       lin.line_id "Line ID",
       itm.segment1 Item,
       itm.inventory_item_id "Item ID",
       lin.ship_from_org_id "Warehouse ID",
       mic.category_set_name "Category Set",
       mic.category_id "Category ID",
       mic.category_concat_segs "Category Name",
       lin.flow_status_code "Line Status",
       trm.term_id "Payment Term ID",
       trm.name "Payment Term",
       lin.invoice_to_org_id "Bill To ID",
       nvl(lin.ordered_quantity, 0) "Order Qty",
       lin.order_quantity_uom "UOM",
       lin.unit_selling_price "Unit Price",
 ...
 WHERE lin.header_id = ##$$HEADERID$$##
   AND lin.org_id = osp.org_id
   AND trm.term_id = lin.payment_term_id
   AND nvl(lin.ship_from_org_id, osp.master_organization_id) = itm.organization_id
   AND lin.inventory_item_id = itm.inventory_item_id (+)
   AND ( ''ORDER'' = (
  SELECT occr.credit_check_level_code
    FROM oe_credit_check_rules occr,
         oe_transaction_types_all otta,
         oe_order_headers_all ooh
   WHERE ooh.header_id = ##$$HEADERID$$##
     AND ooh.order_type_id = otta.transaction_type_id
     AND otta.entry_credit_check_rule_id = occr.credit_check_rule_id )
    OR ( ''LINE'' = (
    SELECT occr.credit_check_level_code
      FROM oe_credit_check_rules occr,
           oe_transaction_types_all otta,
     ...
     WHERE ooh.header_id = ##$$HEADERID$$##
       AND ooh.order_type_id = otta.transaction_type_id
       AND otta.entry_credit_check_rule_id = occr.credit_check_rule_id )
   AND nvl(trm.credit_check_flag, ''Y'') = ''Y'' ) )
   AND nvl(lin.open_flag, ''N'') = ''Y''
   AND nvl(lin.invoiced_quantity, 0) = 0
   AND mic.category_set_id = mdc.category_set_id
   AND mdc.functional_area_id = 7
   AND mic.inventory_item_id = lin.inventory_item_id
   AND mic.organization_id = nvl(lin.ship_from_org_id, osp.master_organization_id)
 ORDER BY lin.line_number,
      ...
      p_title                  => 'Lines Eligible for Credit Check',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Lines Eligible for Credit Check
',
      p_solution               => 'If lines should have been eligible, review setups detailed in [1639006/KB766813]. <br>
<br>
In order to allow credit checking to take place at the next level, the previous level must have the Credit Check flag ON. <br>
For example, if the credit checking is set up to take place at the Customer level, the credit check flag at Site level must be enabled.<br>
The Credit limit and Order Limit can be left as null.  Also verify the following are defined:
<ul>
  <li>Enabling credit check flag on payment terms used on the order or Line.</li>
  <li>Credit check rule attached to the Order Type Used on the Order.</li>
  <li>Credit Check Box checked at Customer Profiles (Site, Customer, Party, and Operating Unit).</li>
</ul>
',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_LINES_ELIGIBLE_FOR_CREDIT_CHECK');



debug('begin add_signature: SALES_ORDER_GENERIC_HOLD_DISPLAY');
   l_info('##MASK##APPLIED BY'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##MASK##RELEASED BY'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '700',
      p_sig_id                 => 'SALES_ORDER_GENERIC_HOLD_DISPLAY',
      p_sig_sql                => 'SELECT hld.order_hold_id       "Order Hold ID",
       hdf.name                "Hold Name",
       (select meaning from oe_lookups where lookup_type = ''HOLD_TYPE'' and lookup_code=hdf.type_code) "Hold Type",
       ( SELECT display_name FROM wf_item_types_vl WHERE name = hdf.item_type ) "Workflow Item",
       ( SELECT meaning FROM oe_lookups 
	     WHERE lookup_type = decode(hdf.item_type, ''OEOH'', ''HOLDABLE_HEADER_ACTIVITIES'', ''OEOL'', ''HOLDABLE_LINE_ACTIVITIES'')
           AND lookup_code = hdf.activity_name
           AND enabled_flag = ''Y''
           AND trunc(sysdate) BETWEEN nvl(start_date_active, trunc(sysdate)) AND nvl(end_date_active, trunc(sysdate)) ) "Workflow Activity",
       (select meaning from oe_lookups where lookup_type = ''HOLD_ENTITY_DESC'' and lookup_code=hsr.hold_entity_code) "Hold Entity",
       hsr.hold_entity_id      "Hold Entity ID",
       --(select meaning from oe_lookups where lookup_type = ''HOLD_ENTITY_DESC'' and lookup_code=hsr.hold_entity_code2) "Hold Entity 2",
       --hsr.hold_entity_id2     "Hold Entity ID 2",
       hsr.hold_until_date     "Hold Until",
       hld.creation_date       "Applied Date",
       (select user_name from fnd_user where user_id = hld.created_by) "Applied By",
       hsr.hold_comment        "Hold Comment",
       hld.released_flag       "Hold Released",       
       hsr.released_flag       "Hold Source Released",
       (select meaning from oe_lookups where lookup_type = ''RELEASE_REASON'' and lookup_code=hrl.release_reason_code) "Release Reason",
       hrl.creation_date       "Released Date",
       (select user_name from fnd_user where user_id = hrl.created_by) "Released By",
       hrl.release_comment     "Release Comment",
	   hdf.hold_included_items_flag "Hold Included Items",
       hdf.progress_wf_on_release_flag "Progress WF on Release"
...
ORDER BY hld.creation_date, hrl.creation_date',
      p_title                  => 'Sales Order Holds',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no sales order holds.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
...
debug('begin add_signature: OMCREDIT_LINE_HOLDS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11920',
      p_sig_id                 => 'OMCREDIT_LINE_HOLDS',
      p_sig_sql                => 'SELECT om_credit_analyzer_pkg.line_shipment_option(lin.line_id) "Line",
       hld.line_id "Line ID",
       hdf.hold_id "Hold ID",
       hdf.name "Hold Name",
       ...
       ( SELECT meaning FROM oe_lookups WHERE lookup_type = ''HOLD_ENTITY_DESC'' AND lookup_code=hsr.hold_entity_code2 ) "Hold Entity 2",
       hsr.hold_entity_id2 "Hold Entity ID 2",
       to_char(hsr.hold_until_date, ''DD-MON-RR HH24:MI:SS'') "Hold Until"
  FROM oe_order_holds_all hld,
       oe_hold_sources_all hsr,
       oe_hold_definitions hdf,
       oe_hold_releases hrl,
...
',
      ...
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Order Line specific holds.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_LINE_HOLDS');



debug('begin add_signature: OMCREDIT_ORDER_TRANSACTION_TYPE_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10283',
      p_sig_id                 => 'OMCREDIT_ORDER_TRANSACTION_TYPE_DETAILS',
      p_sig_sql                => 'SELECT ottt.name "Transaction Type Name",
       ( SELECT name FROM oe_credit_check_rules WHERE credit_check_rule_id = otta.entry_credit_check_rule_id ) "CC Rule: Ordering",
       ( SELECT name FROM oe_credit_check_rules WHERE credit_check_rule_id = otta.picking_credit_check_rule_id ) "CC Rule: Picking",
       ( SELECT name FROM oe_credit_check_rules WHERE credit_check_rule_id = otta.packing_credit_check_rule_id ) "CC Rule: Packing",
       ( SELECT name FROM oe_credit_check_rules WHERE credit_check_rule_id = otta.shipping_credit_check_rule_id ) "CC Rule: Shipping",
       otta.order_category_code "Order Category",
       otta.start_date_active "Start Date",
       otta.end_date_active "End Date",
       to_char(otta.last_update_date, ''DD-MON-RR HH:MM:SS'') "Last Updated"
  FROM oe_transaction_types_tl ottt,
       oe_transaction_types_all otta,
       oe_order_headers_all oh
 WHERE oh.header_id = ##$$HEADERID$$##
   ...
   AND oh.order_type_id = otta.transaction_type_id',
      ...
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Transaction Type Details',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_ORDER_TRANSACTION_TYPE_DETAILS');



debug('begin add_signature: OMCREDIT_CUSTOMER_PROFILE_INFORMATION');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10868',
      p_sig_id                 => 'OMCREDIT_CUSTOMER_PROFILE_INFORMATION',
      p_sig_sql                => 'SELECT os.organization_id "Customer ID",
       cpc.name "Profile Class",
       cp.profile_class_id "Profile Class ID",
       cp.credit_rating "Credit Rating",
       cp.credit_classification "Credit Classification",
       cp.account_status "Account Status",
       cp.risk_code "Risk Code",
       cp.tolerance "Tolerance",
       cp.credit_checking "Credit Check Flag",
       cp.credit_hold "Credit Hold",
       rt.name "Payment Term",
       cp.standard_terms "Term ID",
       cp.override_terms "Overrride Terms"
 ...
 WHERE cp.cust_account_id = os.customer_id
   AND os.organization_id = ord.sold_to_org_id
   AND cp.site_use_id IS NULL
   AND cp.standard_terms = rt.term_id (+)
   AND cp.profile_class_id = cpc.profile_class_id (+)
   AND ord.header_id = ##$$HEADERID$$##',
      p_title                  => 'Customer Profile',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Customer Profile.
',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CUSTOMER_PROFILE_INFORMATION');



debug('begin add_signature: OMCREDIT_CUSTOMER_PROFILE_LIMIT_INFORMATION');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10869',
      p_sig_id                 => 'OMCREDIT_CUSTOMER_PROFILE_LIMIT_INFORMATION',
      p_sig_sql                => 'SELECT cpa.currency_code "Currency",
       cpa.overall_credit_limit "Credit Limit",
       cpa.trx_credit_limit "Order Limit",
       cpa.cust_acct_profile_amt_id "Credit Profile Amount ID",
       cu.name "Rule Set Name",
       cu.credit_usage_id "Credit Usage ID",
       cu.credit_usage_rule_set_id "Set ID"
  FROM hz_customer_profiles cp,
       hz_cust_profile_amts cpa,
       oe_sold_to_orgs_v os,
       oe_order_headers_all ord,
       hz_credit_usages_v cu
 WHERE cp.cust_account_id = os.customer_id
   AND os.organization_id = ord.sold_to_org_id
   AND cp.site_use_id IS NULL
   AND cp.cust_account_profile_id = cpa.cust_account_profile_id
   AND ord.header_id = ##$$HEADERID$$##
   AND cu.credit_profile_amt_id (+) = - 1
   AND cu.profile_class_amount_id (+) = - 1
   AND cu.cust_acct_profile_amt_id (+) = cpa.cust_acct_profile_amt_id',
      p_title                  => 'Customer Profile Limit',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Customer Profile Limit.
',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CUSTOMER_PROFILE_LIMIT_INFORMATION');



debug('begin add_signature: OMCREDIT_FAILED_BILL_TO_SITE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11911',
      p_sig_id                 => 'OMCREDIT_FAILED_BILL_TO_SITE',
      p_sig_sql                => 'SELECT
    ohsa.hold_source_id   "Hold Source ID",
    ohsa.org_id   "Org ID",
    hca.account_number   "Customer Number",
    hcp.site_use_id   "Site Use ID"
FROM
...
WHERE
    hcp.credit_hold = ''N''
    AND hcp.site_use_id = ##$$FK1$$##
    AND hcp.cust_account_id = hca.cust_account_id
        AND ( ( hcp.site_use_id IS NULL
                AND hcp.cust_account_id = ohsa.hold_entity_id
                    AND ohsa.hold_entity_code = ''C'' )
              OR ( hcp.site_use_id IS NOT NULL
                   AND hcp.site_use_id = ohsa.hold_entity_id
                       AND ohsa.hold_entity_code = ''B'' ) )
            AND ohsa.hold_id = 1
                AND ohsa.released_flag = ''N''',
      p_title                  => 'Credit Check Failure Hold - Bill To Site',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There is data corruption in the credit hold setup on the Bill-To site of this customer form.<br>
This will cause the order to fall into Credit Check Failure Hold - Bill to Site when it was saved.  
',
      p_solution               => 'Review the <b>Credit Hold</b> setup at the Bill-To site in the customer form. <br>
You may need to fix the corrupted data. <br>
Refer to [1435944/KB795432] for more information.
',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_FAILED_BILL_TO_SITE');



debug('begin add_signature: OMCREDIT_CUSTOMER_SITE_INFORMATION');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10284',
      p_sig_id                 => 'OMCREDIT_CUSTOMER_SITE_INFORMATION',
      p_sig_sql                => 'SELECT ord.transactional_curr_code "Currency",
       cp.cust_account_profile_id "Profile ID",
       ord.invoice_to_org_id "Bill To Site Id",
       cpc.name "Profile Class",
       cp.profile_class_id "Profile Class Id",
       cp.credit_rating "Credit Rating",
       cp.credit_classification "Credit Classification",
       ...
       cp.standard_terms "Term ID",
       cp.override_terms "Overrride Terms",
       ord.invoice_to_org_id "##$$FK1$$##"
  FROM hz_customer_profiles cp,
       hz_cust_site_uses_all site,
       ra_terms rt,
       hz_cust_profile_classes cpc,
       oe_order_headers_all ord
 WHERE cp.site_use_id = site.site_use_id
   AND ord.header_id = ##$$HEADERID$$##
   AND site.site_use_id = ord.invoice_to_org_id
   AND cp.standard_terms = rt.term_id (+)
   AND cp.profile_class_id = cpc.profile_class_id (+)',
      p_title                  => 'Customer Site',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Customer Site',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      ...
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('OMCREDIT_FAILED_BILL_TO_SITE'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '13'
      );
   l_info.delete;
debug('end add_signature: OMCREDIT_CUSTOMER_SITE_INFORMATION');



debug('begin add_signature: OMCREDIT_CUSTOMER_SITE_LIMIT_INFORMATION');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10901',
      p_sig_id                 => 'OMCREDIT_CUSTOMER_SITE_LIMIT_INFORMATION',
      p_sig_sql                => 'SELECT cpa.currency_code "Currency",
       cpa.overall_credit_limit "Credit Limit",
       cpa.trx_credit_limit "Order Limit",
       cpa.cust_acct_profile_amt_id "Credit Profile Amount ID",
  ...
  FROM hz_customer_profiles cp,
       ...
       oe_order_headers_all ord,
       hz_credit_usages_v cu
 WHERE cp.site_use_id = ord.invoice_to_org_id
   AND ord.header_id = ##$$HEADERID$$##
   AND cp.cust_account_profile_id = cpa.cust_account_profile_id
   AND cu.credit_profile_amt_id (+) = - 1
   AND cu.profile_class_amount_id (+) = - 1
   AND cu.cust_acct_profile_amt_id (+) = cpa.cust_acct_profile_amt_id',
      p_title                  => 'Customer Site Limit',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Customer Site Limit.
',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CUSTOMER_SITE_LIMIT_INFORMATION');



debug('begin add_signature: OMCREDIT_CREDIT_PROFILE_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10338',
      p_sig_id                 => 'OMCREDIT_CREDIT_PROFILE_DETAILS',
      p_sig_sql                => 'SELECT
  hcp.profile_type "Profile Type",
  hcp.credit_profile_id "Profile ID",
  hcp.organization_id "Organization ID",
  hcp.item_category_id "Item Category ID",
  hcp.enable_flag "Enabled Flag",
  hcp.credit_checking "Credit Checking",
  hcp.effective_date_from "Effective Date From",
  hcp.effective_date_to "Effective Date Until",
  hcp.credit_hold "Credit Hold",
...
AND ( hcp.organization_id = oh.org_id 
      OR hcp.organization_id IS NULL )',
      p_title                  => 'Credit Profile Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Profile Details',
      p_solution               => 'Refer to [444837/KB625474] to verify your Credit Check Setup.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_PROFILE_DETAILS');



debug('begin add_signature: OMCREDIT_CREDIT_PROFILE_RULE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11775',
      p_sig_id                 => 'OMCREDIT_CREDIT_PROFILE_RULE',
      p_sig_sql                => 'SELECT
curs.name                   "Credit Usage Rule Set",
cu.credit_usage_rule_set_id "Credit Usage Rule Set ID",
cu.credit_usage_id          "Credit Usage ID"
 from HZ_CREDIT_USAGES_V cu,
      HZ_CREDIT_USAGE_RULE_SETS_VL curs,
      HZ_CREDIT_PROFILE_AMTS_V hcp
where cu.credit_profile_amt_id = hcp.credit_profile_amt_id
  and hcp.credit_profile_id = ##$$CRPROFID$$##
  and cu.credit_usage_rule_set_id = curs.credit_usage_rule_set_id',
      p_title                  => 'Credit Usage Rule Set',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Usage Rule Set.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_PROFILE_RULE');



debug('begin add_signature: OMCREDIT_CREDIT_PROFILE_AMOUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11774',
      p_sig_id                 => 'OMCREDIT_CREDIT_PROFILE_AMOUNT',
      p_sig_sql                => 'SELECT
    credit_profile_amt_id   "Credit Profile Amount ID",
    currency_code           "Currency",
    trx_credit_limit        "Transaction Credit Limit",
    overall_credit_limit    "Overall Limit"
FROM
    hz_credit_profile_amts_v
WHERE 
    credit_profile_id = ##$$CRPROFID$$##',
      p_title                  => 'Credit Profile Amount',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Profile Amount.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_PROFILE_AMOUNT');



debug('begin add_signature: OMCREDIT_CREDIT_USAGE_RULES_SETS_WITH_INCLUDED_CURRENCY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10340',
      p_sig_id                 => 'OMCREDIT_CREDIT_USAGE_RULES_SETS_WITH_INCLUDED_CURRENCY',
      p_sig_sql                => 'SELECT
     curs.name                      "Rule Set Name",
     cur.credit_usage_rule_set_id   "Usage Set ID",
     curs.global_exposure_flag      "Global Exposure Flag",
     cur.usage_type                 "Usage Type",
     cur.usage_type_meaning         "Usage Meaning",
     cur.user_code                  "Usage Rule User Code",
     cur.include_all_flag           "Include All Flag"
 FROM
     hz_credit_usage_rules_v cur,
     hz_credit_usage_rule_sets_vl curs,
     oe_order_headers_all oh
 WHERE
     oh.header_id = ##$$HEADERID$$##
 AND cur.credit_usage_rule_set_id = curs.credit_usage_rule_set_id 
 AND cur.usage_type = ''CURRENCY''
 AND cur.user_code = oh.transactional_curr_code
 AND cur.exclude_flag = ''N''',
      p_title                  => 'Credit Usage Rules Sets with Included Currency',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Usage Rules Sets with Included Currency.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_USAGE_RULES_SETS_WITH_INCLUDED_CURRENCY');



debug('begin add_signature: OMCREDIT_CREDIT_RULE_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11306',
      p_sig_id                 => 'OMCREDIT_CREDIT_RULE_DETAILS',
      p_sig_sql                => 'SELECT 
occr.credit_check_rule_id "Rule ID",
occr.name "Rule Name",
occr.start_date_active "Effective Date Active",
occr.end_date_active "Effective End Date",
nvl(occr.credit_check_level_code, ''ORDER'') "Credit Check Level",
nvl(occr.credit_hold_level_code, ''ORDER'') "Credit Hold Level",
DECODE(occr.days_honor_manual_release, NULL, ''N'', ''Y'') "Override Manual Release",
nvl(occr.days_honor_manual_release, 0) "Days to Honor Manual Release",
occr.tolerance_curr_code "Tolerance Currency",
occr.tolerance_amount "Tolerance Amount",
occr.tolerance_percentage "Tolerance Percentage",
occr.conversion_type "Conversion Type",
nvl(occr.check_item_categories_flag, ''N'') "Check Item Categories",
nvl(occr.send_hold_notifications_flag, ''N'') "Send Hold Notifications",
nvl(occr.quick_cr_check_flag, ''N'') "Use Pre-Calculated Exposure",
nvl(occr.include_external_exposure_flag, ''N'') "Incl External Credit Exposure",
nvl(occr.open_ar_balance_flag, ''N'') "Incl Open Receivables Balance",
nvl(occr.include_payments_at_risk_flag, ''N'') "Include Payment at Risk",
occr.open_ar_days "Open Receivables Days",
nvl(occr.uninvoiced_orders_flag,''N'') "Include Uninvoiced Orders",
nvl(occr.incl_freight_charges_flag,''N'') "Incl Freight and Spec Charges",
nvl(occr.include_tax_flag, ''N'') "Include Tax",
nvl(occr.orders_on_hold_flag, ''N'') "Incl Orders Currently on Hold",
nvl(occr.include_returns_flag, ''N'') "Include Returns",
occr.shipping_interval "Schedule Shipping Horizon Days",
occr.maximum_days_past_due "Maximum Days Past Due"
FROM
oe_credit_check_rules occr
where occr.credit_check_rule_id = ##$$RULEID$$##',
      p_title                  => 'Credit Rule Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Rule Details',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_RULE_DETAILS');



debug('begin add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11326',
      p_sig_id                 => 'OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK',
      p_sig_sql                => 'SELECT
     SUM( nvl(l.unit_selling_price,0) * nvl(l.ordered_quantity,0) ) "Order Amount",
     SUM( DECODE(occr.include_tax_flag, ''Y'', nvl(l.tax_value,0), 0) ) "Tax Amount"
 FROM
...
WHERE occr.credit_check_rule_id = ##$$RULEID$$##
  ...
  AND nvl(l.invoiced_quantity,0) = 0',
      p_title                  => 'Transaction Amounts for Credit Check',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Transaction Amounts for Credit Check.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK');



debug('begin add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11348',
      p_sig_id                 => 'OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRH',
      p_sig_sql                => 'SELECT
    nvl(SUM(DECODE(p.credit_or_charge_flag,''C'', (-1), (+1) ) * p.operand),0) "Order Level Freight"
FROM
...
WHERE
    ...
    AND nvl(p.invoiced_flag,''N'') = ''N''',
      p_title                  => 'Freight Amount on Header',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Freight Amounts on Header.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRH');



debug('begin add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11349',
      p_sig_id                 => 'OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRL',
      p_sig_sql                => 'SELECT
    nvl(SUM(DECODE(p.credit_or_charge_flag,''C'', (-1), (+1) ) * l.pricing_quantity * p.adjusted_amount),0) "Line Level Freight"
FROM
    oe_price_adjustments p,
    oe_order_lines l,
    oe_order_headers h
WHERE
    ...
    AND p.list_line_type_code = ''FREIGHT_CHARGE''
    AND nvl(p.invoiced_flag,''N'') = ''N''',
      p_title                  => 'Freight Amount on Order Lines',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Freight Amounts on any order lines',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRL');



debug('begin add_signature: OMCREDIT_CREDIT_MANAGEMENT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11350',
      p_sig_id                 => 'OMCREDIT_CREDIT_MANAGEMENT',
      p_sig_sql                => 'SELECT
     DECODE(COUNT(*),0,''No'',''Yes'') "Credit Management Installed"
 FROM
     ar_cmgt_setup_options',
      p_title                  => 'Credit Management Installation Status',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Management installation status.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_MANAGEMENT');



debug('begin add_signature: OMCREDIT_CONCURRENT_HISTORY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10912',
      p_sig_id                 => 'OMCREDIT_CONCURRENT_HISTORY',
      p_sig_sql                => 'SELECT fcr.request_id "Request ID",
       ...
       fcr.actual_start_date "Actual Start Date",
       fcr.actual_completion_date "Actual Completion Date",
       fcr.parent_request_id "Parent Request ID",
       fcr.phase_code "Phase Code",
       fcr.status_code "Status Code",
       fcr.completion_text "Completion Text",
       fcr.argument_text "Parameters"
  FROM fnd_concurrent_requests fcr,
       fnd_concurrent_programs fcp,
       fnd_concurrent_programs_tl fcpt 
 WHERE fcr.concurrent_program_id = fcp.concurrent_program_id
   AND fcp.concurrent_program_name IN ( ''OEXAUCRC'', ''OEXINICS'', ''OEXAUCRC_XML'' )
   AND fcp.application_id = fcpt.application_id
   AND fcp.concurrent_program_id = fcpt.concurrent_program_id
   AND fcpt.language=''US''
   AND fcr.request_date > trunc(sysdate) - 2
 ORDER BY fcr.request_id
',
      p_title                  => 'Credit Check Concurrent History in Past 2 Days',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no Credit Check Concurrent History in Past 2 Days.
...
',
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CONCURRENT_HISTORY');



debug('begin add_signature: OMCREDIT_RULE_ACTIVE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11919',
      p_sig_id                 => 'OMCREDIT_RULE_ACTIVE',
      p_sig_sql                => 'SELECT occr.credit_check_rule_id "Rule ID",
occr.name "Rule Name",
occr.start_date_active "Effective Date Active",
occr.end_date_active "Effective End Date"
FROM
oe_credit_check_rules occr
where occr.credit_check_rule_id = ##$$RULEID$$##
AND (nvl(occr.start_date_active, trunc(SYSDATE)) > trunc(SYSDATE)
    OR nvl(occr.end_date_active, trunc(SYSDATE)) < trunc(SYSDATE))',
      p_title                  => 'Credit Rule Active',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The credit rule being selected is not active.  In order to trigger automatic credit checking on sales orders it is necessary that the credit checking is enabled for the order type, customer profile and payment terms associated with the transaction.',
      p_solution               => 'Verify the Credit Rule Active date range is set properly. <br>
Refer to [744642/KB727766] for more details on the setup.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_RULE_ACTIVE');



debug('begin add_signature: OMCREDIT_PHASES_DEFINED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11239',
      p_sig_id                 => 'OMCREDIT_PHASES_DEFINED',
      p_sig_sql                => 'SELECT
     ''Ordering'' "Credit Check Phase"
 FROM
     oe_order_headers_all oh,
     oe_transaction_types_all tt,
     oe_credit_check_rules cr
WHERE oh.header_id = ##$$HEADERID$$##
    AND oh.order_type_id = tt.transaction_type_id
    AND tt.entry_credit_check_rule_id = cr.credit_check_rule_id
    AND ''##$$PHASE$$##'' = ''BOOKING''
UNION
SELECT
...
WHERE
    oh.header_id = ##$$HEADERID$$##
    AND oh.order_type_id = tt.transaction_type_id
    AND tt.picking_credit_check_rule_id = cr.credit_check_rule_id
    AND ''##$$PHASE$$##'' = ''PICKING'' 
UNION
SELECT
    ''Packing''
...
WHERE
    oh.header_id = ##$$HEADERID$$##
    AND oh.order_type_id = tt.transaction_type_id
    AND tt.packing_credit_check_rule_id = cr.credit_check_rule_id
    AND ''##$$PHASE$$##'' = ''PACKING'' 
UNION
SELECT
    ''Shipping''
...
WHERE
    oh.header_id = ##$$HEADERID$$##
    AND oh.order_type_id = tt.transaction_type_id
    AND tt.shipping_credit_check_rule_id = cr.credit_check_rule_id
    AND ''##$$PHASE$$##'' = ''SHIPPING''',
      p_title                  => 'Credit Check Phase Defined',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The Credit Check Phase you selected is not defined.  ',
      p_solution               => 'Verify your Order Type Credit Check Rules.  Refer to [744642/KB727766] for setup details.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_PHASES_DEFINED');



debug('begin add_signature: OMCREDIT_OCM_RULE_USED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11343',
      p_sig_id                 => 'OMCREDIT_OCM_RULE_USED',
      p_sig_sql                => 'SELECT occr.credit_check_rule_id "Rule ID",
    occr.name "Rule Name"
FROM
    oe_credit_check_rules occr
where occr.credit_check_rule_id = ##$$RULEID$$##
and occr.credit_check_rule_id = -999',
      p_title                  => 'Oracle Credit Management Usage',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Oracle Credit Management default credit check rule is being used.<br>
This is a seeded Credit Check Rule created for the Oracle Credit Management (OCM) Team.<br>
The rule is interpreted by the code as having a special meaning, and is not supposed to be adopted and used externally.',
      p_solution               => 'Please create your own credit check rule and assign the rule to the transaction type. <br>
Refer to [351307/KB736842] for details.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_OCM_RULE_USED');



debug('begin add_signature: OMCREDIT_CONCURRENT_CHECK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11917',
      p_sig_id                 => 'OMCREDIT_CONCURRENT_CHECK',
      p_sig_sql                => 'SELECT fcr.request_id "Request ID",
       fcp.concurrent_program_name "Concurrent Program Name",
       fcpt.user_concurrent_program_name "Program Name",
       fcr.request_date "Request Date",
  ...
  FROM fnd_concurrent_requests fcr,
       ...
       fnd_concurrent_programs_tl fcpt 
 WHERE fcr.concurrent_program_id = fcp.concurrent_program_id
   AND fcp.concurrent_program_name IN ( ''OEXINICS'' )
   AND fcp.application_id = fcpt.application_id
   AND fcp.concurrent_program_id = fcpt.concurrent_program_id
   AND fcpt.language=''US''
   AND fcr.request_date > trunc(sysdate) - 2
 ORDER BY fcr.request_id',
      p_title                  => 'Initialize Credit Summaries Table History in Past 2 Days',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Initialize Credit Summaries Table has not been submitted in past 2 days.

',
      p_solution               => 'Verify that you are running Concurrent Request <b>Initialize Credit Summaries</b> enough for your business.  Refer to [293409/KB774026].',
      p_success_msg            => 'Initialize Credit Summaries Table has been submitted in past 2 days.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CONCURRENT_CHECK');



debug('begin add_signature: OMCREDIT_PRECALC_EXPOSURE_USED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11918',
      p_sig_id                 => 'OMCREDIT_PRECALC_EXPOSURE_USED',
      p_sig_sql                => 'SELECT occr.credit_check_rule_id "Rule ID",
occr.name "Rule Name",
nvl(occr.credit_check_level_code, ''ORDER'') "Credit Check Level",
nvl(occr.credit_hold_level_code, ''ORDER'') "Credit Hold Level",
...
oe_credit_check_rules occr
where occr.credit_check_rule_id = ##$$RULEID$$##
and occr.quick_cr_check_flag = ''Y''',
      p_title                  => 'Pre-Calculated Exposure',
      p_fail_condition         => 'RS',
      p_problem_descr          => '<b>Use Pre-Calculated Exposure</b> option is being used in the defined Credit Check Rule.<br>
This option validates exposure against balance information stored in a summary table, instead of performing credit calculations real time.
',
      p_solution               => 'The use of this option was incorporated to improve performance, but you should review if this is beneficial to your business. <br>
Review [416249/KB752758] for more information.
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      ...
      p_limit_rows             => nvl('Y','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('OMCREDIT_CONCURRENT_CHECK'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '20'
      );
   l_info.delete;
debug('end add_signature: OMCREDIT_PRECALC_EXPOSURE_USED');



debug('begin add_signature: OMCREDIT_NO_CC_PAYMENT_TERMS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11612',
      p_sig_id                 => 'OMCREDIT_NO_CC_PAYMENT_TERMS',
      p_sig_sql                => 'SELECT
    LIN.LINE_ID "Line ID",
    RT.NAME "Payment Term",
    RT.TERM_ID "Payment Term ID",
...
FROM
    ...
    RA_TERMS RT
WHERE
    LIN.PAYMENT_TERM_ID = RT.TERM_ID
    AND RT.CREDIT_CHECK_FLAG = ''Y''
    AND LIN.HEADER_ID = ##$$HEADERID$$##',
      p_title                  => 'Check Payment Terms on Lines',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There is no credit check enabled on the payment terms for the order line(s).  This will cause orders to pass without a credit check hold.',
      p_solution               => 'You will need to setup the payment term with a credit check.  Review [444837/KB625474#payment_term] for more payment term setup information.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_NO_CC_PAYMENT_TERMS');



debug('begin add_signature: OMCREDIT_MANUAL_RELEASE_HONORED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11921',
      p_sig_id                 => 'OMCREDIT_MANUAL_RELEASE_HONORED',
      p_sig_sql                => '
SELECT om_credit_analyzer_pkg.line_shipment_option(lin.line_id) "Line",
       hld.line_id "Line ID",
       hdf.hold_id "Hold ID",
       hdf.name "Hold Name",
       hdf.type_code "Hold Type",
 ...
 WHERE hld.header_id = ##$$HEADERID$$##
   ...
   AND hsr.hold_id = hdf.hold_id
   AND hld.hold_release_id = hrl.hold_release_id (+)
   AND ( sysdate - hrl.creation_date ) < (
  SELECT nvl(occr.days_honor_manual_release, - 1)
    FROM oe_credit_check_rules occr
   WHERE credit_check_rule_id = ##$$RULEID$$## )
 ORDER BY lin.line_number,
          nvl(lin.shipment_number, - 1),
          nvl(lin.option_number, - 1),
          nvl(lin.component_number, - 1),
          nvl(lin.service_number, - 1)',
      p_title                  => 'Number of Days to Honor Manual Release - Line Level',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are a number of days that are setup to be honored before a manual release on credit holds.',
      p_solution               => 'If you are using the Override Manual Release, you should verify the number of days used is appropriate for your business.
Review [1543030/KB779337] for more information.
',
      p_success_msg            => 'Manual Hold Release is not being used and/or honored.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_MANUAL_RELEASE_HONORED');



debug('begin add_signature: OMCREDIT_MANUAL_RELEASE_HONORED_HEADER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12029',
      p_sig_id                 => 'OMCREDIT_MANUAL_RELEASE_HONORED_HEADER',
      p_sig_sql                => 'SELECT hdf.hold_id "Hold ID",
       hdf.name "Hold Name",
 ...
 WHERE hld.header_id = ##$$HEADERID$$##
  ...
  SELECT nvl(occr.days_honor_manual_release, - 1)
   ...
   WHERE credit_check_rule_id = ##$$RULEID$$## )
',
      p_title                  => 'Number of Days to Honor Manual Release - Header Level',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are a number of days that are setup to be honored before a manual release on credit holds.',
      p_solution               => 'If you are using the Override Manual Release, you should verify the number of days used is appropriate for your business.
Review [1543030/KB779337] for more information.
',
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_MANUAL_RELEASE_HONORED_HEADER');



debug('begin add_signature: OMCREDIT_CREDIT_HOLD_ZERO_VALUE_ORDER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11813',
      p_sig_id                 => 'OMCREDIT_CREDIT_HOLD_ZERO_VALUE_ORDER',
      p_sig_sql                => 'SELECT
    osp.org_id "Org ID",
    osp.parameter_value "Value"
FROM
    oe_sys_parameters_all osp,
    oe_order_headers_all oh
WHERE
    oh.header_id = ##$$HEADERID$$##
    AND oh.org_id = osp.org_id
    AND osp.parameter_code = ''CREDIT_HOLD_ZERO_VALUE_ORDER''
    AND nvl(osp.parameter_value, ''Y'') = ''N''',
      p_title                  => 'Credit Hold on Zero Value Order System Parameter',
      p_fail_condition         => 'RS',
      p_problem_descr          => '<b>Apply credit hold on zero value order<b> system parameter is not set to Yes.
',
      p_solution               => '<b>Apply credit hold on zero value order</b> System Parameter is an optional System Parameter.<br>
If you set No as the value of this system parameter, then credit checking does not happen on orders with zero amount. <br>
If you set Yes as the value, then credit checking happens on orders with zero amount. <br>
This is useful if you want credit checking to happen on orders with overdue invoices. <br>
If there is no value assigned, it will be considered as Yes. <br>
<br>
Refer to [1457757/KB740804] for additional details if needed.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_CREDIT_HOLD_ZERO_VALUE_ORDER');



debug('begin add_signature: OMCREDIT_RULE_NO_LIMIT_ON_SITE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '14673',
      p_sig_id                 => 'OMCREDIT_RULE_NO_LIMIT_ON_SITE',
      p_sig_sql                => 'SELECT CPA.CURRENCY_CODE "Currency",
  CP.CREDIT_CHECKING "Credit Check",
  CPA.OVERALL_CREDIT_LIMIT "Credit Limit",
  CPA.TRX_CREDIT_LIMIT "Order Limit",
  CPA.CUST_ACCT_PROFILE_AMT_ID "Credit Profile Amount ID",
  CU.NAME "Rule Set Name",
...
AND ORD.HEADER_ID                 = ##$$HEADERID$$##
AND CP.CUST_ACCOUNT_PROFILE_ID    = CPA.CUST_ACCOUNT_PROFILE_ID
AND CPA.CURRENCY_CODE             = ORD.TRANSACTIONAL_CURR_CODE
AND CPA.OVERALL_CREDIT_LIMIT      is null
AND CPA.TRX_CREDIT_LIMIT          is null
AND CP.CREDIT_CHECKING            = ''Y''
AND CU.CREDIT_PROFILE_AMT_ID(+)   =-1
AND CU.PROFILE_CLASS_AMOUNT_ID(+) =-1
AND CU.CUST_ACCT_PROFILE_AMT_ID(+)=cpa.cust_acct_profile_amt_id',
      p_title                  => 'Site Level Credit Check is Enabled without Credit Limits',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The Site level credit check is enabled and both credit limit and order credit limit are NULL/BLANK => No Credit Check.
In order to trigger credit check, change the credit check level or set credit limits for the customer site.',
      p_solution               => 'The Site level credit check is enabled and both credit limit and order credit limit are NULL/BLANK => No Credit Check.<br>
If Credit Check is enabled and both credit limit and order credit limit are NULL/BLANK then Credit check will be done based on Customer level credit limit and order credit limit.<br>
Verify credit check flag and credit limit setups. Refer to [365866/KB733646] for more details on the setup.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: OMCREDIT_RULE_NO_LIMIT_ON_SITE');
...
BEGIN
  ...
  IF (p_phase IS NOT NULL) THEN
    g_params_string := g_params_string || to_char(p_phase) || '_';
  END IF;
  ...
  analyzer_title := 'Order Management Credit Check Analyzer';
...
debug('begin section: ORDER_DETAILS');
start_section('Order Details', 'ORDER_DETAILS');
   set_item_result(run_stored_sig('OMCREDIT_ORDER_DETAILS'));
   set_item_result(run_stored_sig('OMCREDIT_LINES_ELIGIBLE_FOR_CREDIT_CHECK'));
   set_item_result(run_stored_sig('SALES_ORDER_GENERIC_HOLD_DISPLAY'));
   set_item_result(run_stored_sig('OMCREDIT_LINE_HOLDS'));
   set_item_result(run_stored_sig('OMCREDIT_ORDER_TRANSACTION_TYPE_DETAILS'));
end_section;
debug('end section: ORDER_DETAILS');

debug('begin section: CUSTOMER_DETAILS');
start_section('Customer Details', 'CUSTOMER_DETAILS');
   set_item_result(run_stored_sig('OMCREDIT_CUSTOMER_PROFILE_INFORMATION'));
   set_item_result(run_stored_sig('OMCREDIT_CUSTOMER_PROFILE_LIMIT_INFORMATION'));
   set_item_result(run_stored_sig('OMCREDIT_CUSTOMER_SITE_INFORMATION'));
   set_item_result(run_stored_sig('OMCREDIT_CUSTOMER_SITE_LIMIT_INFORMATION'));
end_section;
debug('end section: CUSTOMER_DETAILS');

debug('begin section: CREDIT_DETAILS');
start_section('Credit Details', 'CREDIT_DETAILS');
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_PROFILE_DETAILS'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_PROFILE_RULE'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_PROFILE_AMOUNT'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_USAGE_RULES_SETS_WITH_INCLUDED_CURRENCY'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_RULE_DETAILS'));
   set_item_result(run_stored_sig('OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK'));
   set_item_result(run_stored_sig('OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRH'));
   set_item_result(run_stored_sig('OMCREDIT_TRANSACTION_AMOUNTS_FOR_CREDIT_CHECK_FRL'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_MANAGEMENT'));
   set_item_result(run_stored_sig('OMCREDIT_CONCURRENT_HISTORY'));
end_section;
debug('end section: CREDIT_DETAILS');

debug('begin section: CREDIT_ANALYSIS');
start_section('Credit Check Data Analysis', 'CREDIT_ANALYSIS');
   set_item_result(run_stored_sig('OMCREDIT_RULE_ACTIVE'));
   set_item_result(run_stored_sig('OMCREDIT_PHASES_DEFINED'));
   set_item_result(run_stored_sig('OMCREDIT_OCM_RULE_USED'));
   set_item_result(run_stored_sig('OMCREDIT_PRECALC_EXPOSURE_USED'));
   set_item_result(run_stored_sig('OMCREDIT_NO_CC_PAYMENT_TERMS'));
   set_item_result(run_stored_sig('OMCREDIT_MANUAL_RELEASE_HONORED'));
   set_item_result(run_stored_sig('OMCREDIT_MANUAL_RELEASE_HONORED_HEADER'));
   set_item_result(run_stored_sig('OMCREDIT_CREDIT_HOLD_ZERO_VALUE_ORDER'));
   set_item_result(run_stored_sig('OMCREDIT_RULE_NO_LIMIT_ON_SITE'));
end_section;
debug('end section: CREDIT_ANALYSIS');
