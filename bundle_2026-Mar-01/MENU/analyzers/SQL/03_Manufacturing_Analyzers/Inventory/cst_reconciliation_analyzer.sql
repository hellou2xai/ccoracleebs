...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Inventory to General Ledger            |
...
CREATE OR REPLACE PACKAGE cst_recon_analyzer_pkg AUTHID DEFINER AS
...
(            p_from_date                    IN DATE        DEFAULT NULL
           ...
           ,p_ledger_id_sql                IN NUMBER      DEFAULT NULL
...
END cst_recon_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY cst_recon_analyzer_pkg AS
-- $Id: cst_reconciliation_analyzer.sql, 200.7 2026/01/28 12:20:30 sothman Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2660156.1));
...
g_gl_acct_bal                   NUMBER;
g_mtl_trans_sum                 NUMBER;
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'CSTRECON_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'CSTRECON_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>CSTRECON Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2660156.1#feedback");
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/4345277" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">2875</span>
...
<p>The Inventory to General Ledger Reconciliation Analyzer is a self-service health-check script that reviews Inventory, Costing, Sub-ledger and GL accounting reconciliation related data, analyzes issues, and provides solutions to known issues.</p><p>It collects data for a particular account for issues where there are discrepancies between Inventory, SLA and GL.</p><p>Use this analyzer on a weekly bases to check for reconciliation issues before closing the Inventory and GL Period.</p><p><strong>Version 200.6</strong> MOSFS links</p><p><strong>Version 200.5 </strong>08-MAY-2025&nbsp;</p><p><strong>Enhanced Row Selection and Scrolling Functionality</strong></p><p>We are pleased to announce the introduction of a new feature designed to improve the user experience when interacting with large datasets. This enhancement allows for seamless scrolling and highlighting of selected rows, even when dealing with tables containing many columns.</p><p><strong>Key Benefits</strong>:</p><ul><li>&nbsp; &nbsp;<strong>Improved readability</strong>: When selecting a row, the highlighted selection will remain visible as you scroll horizontally, enabling you to easily read and analyze the data.</li>
...
<li>&nbsp; &nbsp;<strong>Streamlined data analysis:</strong> By maintaining the highlighted selection as you scroll, you can effortlessly compare and analyze data across multiple columns. This new feature is designed to provide a more intuitive and user-friendly experience, enabling you to work more efficiently with your data.</li>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,2875,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,2875,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},2875,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
l_gl_acct_bal                   NUMBER;
l_mtl_trans_sum                 NUMBER;
...
IF (p_ledger_id_sql IS NOT NULL) THEN
   return p_ledger_id_sql;
...
debug('begin populate parameters hash table');
   ...
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_ledger_id_sql,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Ledger ID';
   ...
   END IF;
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'COST_GROUP_ID';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_COST_GROUP_ID,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Account ID code_combination_id';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_Account_ID,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/01/28 12:20:29 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'cst_reconciliation_analyzer.sql';
...
debug('begin parameter validation: p_ledger_id_sql');
IF NOT g_is_concurrent THEN
IF p_ledger_id_sql IS NULL THEN
...
INTO l_exists_val
FROM GL_LEDGERS L
WHERE L.LEDGER_ID = p_ledger_id_sql and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   ...
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_ledger_id_sql failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
END IF;
debug('end parameter validation: p_ledger_id_sql');

debug('begin parameter validation: p_COST_GROUP_ID');
IF p_COST_GROUP_ID IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter COST_GROUP_ID is required.');
   raise invalid_parameters;
END IF;
...
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Please provide a valid Cost Group ID.');
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_COST_GROUP_ID failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
debug('end parameter validation: p_COST_GROUP_ID');

debug('begin parameter validation: p_Account_ID');
IF p_Account_ID IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Account ID code_combination_id is required.');
   raise invalid_parameters;
END IF;
...
WHERE CODE_COMBINATION_ID = p_Account_ID and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Please enter a valid Account ID (gl_code_combinations_kfv.code_combination_id). ');
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_Account_ID failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
debug('end parameter validation: p_Account_ID');
...
WHERE PERIOD_NAME = p_Period_Name and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Please enter a valid Period_name (org_acct_periods.period_name). ');
   raise invalid_parameters;
   WHEN OTHERS THEN
...
--CST_RECON_MTL_TRANSACTION_ACCOUNTS variable
l_step := '10';
BEGIN
          SELECT sum(mta.base_transaction_value)
        INTO l_mtl_trans_sum
         FROM mtl_transaction_accounts mta, org_acct_periods oap
          WHERE mta.reference_account = p_Account_ID
...
--CST_RECON_GL_VALUE variable
l_step := '15';
     BEGIN
          SELECT SUM(NVL(gjl.accounted_dr, 0) - NVL(gjl.accounted_cr, 0)) GL_ACCT_BAL
          INTO l_gl_acct_bal
          FROM gl_je_lines gjl, gl_je_headers gjh
          WHERE gjl.je_header_id = gjh.je_header_id
           AND gjh.actual_flag = 'A'
           AND gjh.ledger_id = gjl.ledger_id
           AND gjl.code_combination_id = p_Account_ID
...
g_mtl_trans_sum := l_mtl_trans_sum;
g_gl_acct_bal := l_gl_acct_bal;

--Accommodations for 19 c database
...
--Accommodations for 19 c DBdebug end
...
print_log('L_MTL_TRANS_SUM	= '||l_mtl_trans_sum);
print_log('L_GL_ACCT_BAL	= '||l_gl_acct_bal);


print_log('G_MTL_TRANS_SUM	= '||g_mtl_trans_sum);
print_log('G_GL_ACCT_BAL	= '||g_gl_acct_bal);
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$from_date$$##') := p_from_date;
   g_masked_tokens('##$$from_date$$##') := mask_text( g_sql_tokens('##$$from_date$$##'), 'NO_MASK');
   g_sql_tokens('##$$to_date$$##') := p_to_date;
   g_masked_tokens('##$$to_date$$##') := mask_text( g_sql_tokens('##$$to_date$$##'), 'NO_MASK');
   ...
   g_sql_tokens('##$$GL_ACCT_BAL$$##') := g_gl_acct_bal;
   g_masked_tokens('##$$GL_ACCT_BAL$$##') := mask_text( g_sql_tokens('##$$GL_ACCT_BAL$$##'), 'NO_MASK');
   g_sql_tokens('##$$MTL_TRANS_SUM$$##') := g_mtl_trans_sum;
   g_masked_tokens('##$$MTL_TRANS_SUM$$##') := mask_text( g_sql_tokens('##$$MTL_TRANS_SUM$$##'), 'NO_MASK');
   g_sql_tokens('##$$acct_id$$##') := p_Account_ID;
   g_masked_tokens('##$$acct_id$$##') := mask_text( g_sql_tokens('##$$acct_id$$##'), 'NO_MASK');
   g_sql_tokens('##$$period_name$$##') := p_Period_Name;
   g_masked_tokens('##$$period_name$$##') := mask_text( g_sql_tokens('##$$period_name$$##'), 'NO_MASK');
...
debug('begin add_signature: CST_SLA_EVENT_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7208',
      p_sig_id                 => 'CST_SLA_EVENT_DETAILS',
      p_sig_sql                => 'SELECT
    COUNT(*),
    xte.ledger_id,
    xte.security_id_int_1   inv_org_id,
    xte.security_id_int_2   ou_id,
    xe.event_type_code,
   decode(xe.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
                           ''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'',''Z'') PROCESS_STATUS, 
decode(xe.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'',''Z'') EVENT_STATUS,
    xe.upg_batch_id,
    gps.period_name,
    gps.period_num,
    gps.period_year,
    decode(gps.closing_status,''C'',''Closed'',''F'',''Future Entry'',''N'',''Never Opened'',''O'',''Open'',''P'',''Permanently Closed'',''N/A'') CLOSING_STATUS
FROM
    xla_events                     xe,
    xla_transaction_entities_upg   xte,
    gl_period_statuses             gps
WHERE
    xe.application_id = 707
    AND xe.event_type_code <> ''P''
    ...
    AND gps.set_of_books_id = xte.ledger_id
...
GROUP BY
    ...
    gps.closing_status
ORDER BY
    gps.period_year,gps.period_num',
      p_title                  => 'Summary of Unprocessed Records From XLA_EVENTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed Records exist in XLA_EVENTS table. Displaying Events Summary.',
      p_solution               => 'Refer to [1457611/KB752540] to review the meaning of EVENT_STATUS_CODE and PROCESS_STATUS_CODE in XLA_EVENTS table.<br>

If the EVENT_STATUS_CODE is Unprocessed and PROCESS_STATUS_CODE is Unprocessed, then run Create Accounting - Cost Management Program.<br>
If the EVENT_STATUS_CODE is Unprocessed and PROCESS_STATUS_CODE is Invalid, then please run the Create Accounting - Cost Management Program with FND debug enabled and provide the accounting program FND debug log for further analysis.',
      p_success_msg            => 'No unprocessed Event detail records found.
xla_events.event_status_code = Processed
xla_events.process_status_code = Processed',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_EVENT_DETAILS');



debug('begin add_signature: CST_SLA_PENDING_TRANSFER_TO_G');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7000',
      p_sig_id                 => 'CST_SLA_PENDING_TRANSFER_TO_G',
      p_sig_sql                => 'SELECT
    xe.event_type_code,
    xte.entity_code,
    xah.ledger_id,
   decode(xah.GL_TRANSFER_STATUS_CODE,''Y'',''Yes'',''N'',''No'',''NT'',''Not Transferable'',''S'',''Incomplete'') GL_TRANSFER_STATUS,
decode(xah.accounting_entry_status_code,''F'',''Final Mode'',''I'',''Incomplete'') ACCOUNTING_STATUS,
  decode(xe.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
                           ''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'',''Z'') PROCESS_STATUS, 
decode(xe.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'',''Z'') EVENT_STATUS,
    gps.period_name,
...
ORDER BY
    ...
    xe.event_type_code,
    xah.ledger_id',
      p_title                  => 'Untransferred Transactions to GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Pending Transfer to GL records. Non Transferred to GL Records were Found.',
...
If the records are not getting processed, please refer to [951233/KB339228] Errors while running Journal Import from SLA/Create Accounting Or Transfer to GL does not pick rows/Fails<br>
    for specific error condition and follow the steps.',
      p_success_msg            => 'No records found pending for transfer to GL.
...
xla_ae_headers.gl_transfer_date is Not Null',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_PENDING_TRANSFER_TO_G');



debug('begin add_signature: CST_RECON_MTL_TRANSACTION_ACCOUNTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15109',
      p_sig_id                 => 'CST_RECON_MTL_TRANSACTION_ACCOUNTS',
      p_sig_sql                => 'SELECT mp.organization_code,
mta.organization_id organization_id,
mta.reference_account account_id,
mta.accounting_line_type alt,
ml.meaning,
oap.period_name period,
oap.period_year,
oap.period_num,
sum(mta.base_transaction_value)  Accounted_Value
...
and mta.transaction_date >=oap.period_start_date
and mta.transaction_date <=(oap.schedule_close_date+0.99999)
and oap.organization_id = mta.organization_id
and mp.organization_id=mta.organization_id
and ml.lookup_code = mta.accounting_line_type
and ml.lookup_type like ''CST_ACCOUNTING_LINE_TYPE''
GROUP BY 
   ...
   oap.period_year,
   oap.period_num
order by mta.organization_id,oap.period_year, oap.period_num',
      p_title                  => 'Period Summarization of Inventory Valuation Account per Organization per Period',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the accounting information from MTL_TRANSACTION_ACCOUNTS for a specific account and period.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_MTL_TRANSACTION_ACCOUNTS');



debug('begin add_signature: CST_RECON_WIP_TRANSACTION_ACCOUNTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15111',
      p_sig_id                 => 'CST_RECON_WIP_TRANSACTION_ACCOUNTS',
      p_sig_sql                => 'SELECT 
  mp.organization_code,
  mta.organization_id Organization_id,
  mta.reference_account Account_id,
...
GROUP BY mp.organization_code,
    ...
    oap.period_num 
      order by mta.organization_id,oap.period_year,  oap.period_num',
      p_title                  => 'Inventory Valuation Account Used in WIP Resource Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'WIP Transactions that are accounted with the Inventory Valuation Account will cause discrepancies. <BR>
A Resource or Overhead that is setup with an Absorption Account that is the Inventory Valuation account will cause discrepancies.<BR>
Displaying the accounting information from WIP_TRANSACTION_ACCOUNTS. 


',
      p_solution               => 'Review [2711893/KB712332#Issue_2] Reconciling Inventory with General Ledger (GL) - Common Issues - for solution steps.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_WIP_TRANSACTION_ACCOUNTS');



debug('begin add_signature: CST_RECON_RCV_RECEIVING_SUB_LEDGER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15114',
      p_sig_id                 => 'CST_RECON_RCV_RECEIVING_SUB_LEDGER',
      p_sig_sql                => 'select  gps.PERIOD_YEAR, 
gps.period_num,
        rrsl.period_name,
      ...
      rrsl.set_of_books_id,
       ...
       to_number(sum(nvl(rrsl.accounted_dr,0)), 9999999999999.9999) Accounted_DR,
        to_number(sum(nvl(rrsl.accounted_cr,0)), 9999999999999.9999) Accounted_CR,
       to_number(sum(nvl(rrsl.accounted_dr,0) - nvl(rrsl.accounted_cr,0)), 9999999999999.9999) Amount
...
WHERE  rrsl.code_combination_id =##$$acct_id$$##
       ...
       and gps.PERIOD_NAME = rrsl.PERIOD_NAME
       and gps.APPLICATION_ID = 201
...
gps.period_num,
   rrsl.period_name,
  pod.destination_organization_id',
      p_title                  => 'Inventory Valuation Account Used in Receiving Account Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Receiving Transactions that are accounted with the Inventory Valuation Account will cause discrepancies. <BR>
Displaying the accounting information from RCV_RECEIVING_SUB_LEDGER. <BR>
',
      p_solution               => 'Review [2711893/KB712332#Issue_3] Reconciling Inventory with General Ledger (GL) - Common Issues - for solution steps.
',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_RCV_RECEIVING_SUB_LEDGER');



debug('begin add_signature: CST_RECON_SLA VALUE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15116',
      p_sig_id                 => 'CST_RECON_SLA VALUE',
      p_sig_sql                => '     select gps.PERIOD_YEAR ,
  gps.period_num,
       h.period_name ,
       h.ledger_id,
    h.je_category_name,
     l.accounting_class_code,l.code_combination_id,
       to_number(sum(nvl(l.accounted_dr,0)), 9999999999999.9999)  Accounted_DR,
      to_number(sum(nvl(l.accounted_cr,0)), 9999999999999.9999) Accounted_CR,
       to_number(sum(nvl(l.accounted_dr,0) - nvl(l.accounted_cr,0)), 9999999999999.9999)   Amount ,
	   decode(h.gl_transfer_status_code,''E'',''Accounting Entry In Error'',''N'',''Not Transferred'',''Y'',''Transferred Succesfully'',''N/A'') GL_TRANSFER_STATUS,
       decode(h.accounting_entry_status_code,''I'',''Invalid'',''F'',''Final Mode'',''D'',''Draft'',''NT'',''No Action'',''N/A'') ACCOUNTING_ENTRY_STATUS
from gl_period_statuses gps,
     xla_ae_lines   l
  ,    xla_ae_headers h
where l.code_combination_id =##$$acct_id$$##
      and   l.ae_header_id = h.ae_header_id
      and gps.PERIOD_NAME = h.PERIOD_NAME
      and gps.APPLICATION_ID = 101
...
gps.period_num,
      ...
      h.je_category_name,
       h.ledger_id,
       h.gl_transfer_status_code,h.accounting_entry_status_code,l.accounting_class_code,l.code_combination_id
order by gps.PERIOD_YEAR,
gps.period_num,
      h.period_name',
      p_title                  => 'Period Summarization of Inventory Account in SLA (Subledger Accounting)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the accounting information from SLA for a specific account, period and ledger. 
',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_SLA VALUE');



debug('begin add_signature: CST_RECON_GL_VALUE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15115',
      p_sig_id                 => 'CST_RECON_GL_VALUE',
      p_sig_sql                => 'select
gjl.ledger_id,
        gjh.je_source,
        gjh.je_category,
        decode(gjl.status,''P'',''Posted'',''U'',''Unposted'',''Other'') GL_STATUS,
        gjh.period_name,
        gjl.code_combination_id ccid,
        NVL(sum(nvl(gjl.accounted_cr,0)),0) CR,
        NVL(sum(nvl(gjl.accounted_dr,0)),0) DR,
        NVL(sum(nvl(gjl.accounted_dr,0)),0)-NVL(sum(nvl(gjl.accounted_cr,0)),0)  GL_VALUE,
        gps.period_year,
        gps.period_num,
        gps.period_name
...
GROUP BY gjl.ledger_id, gjh.je_source,
gjh.je_category, gjl.status, gjh.period_name,
gjl.code_combination_id,
       gps.period_year,
        ...
        order by gps.period_year,
        gps.period_num',
      p_title                  => 'Period Summarization of Inventory Account in General Ledger',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the accounting information from GL for a specific account, period, and ledger.	  
',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_GL_VALUE');



debug('begin add_signature: CST_RECON_GL_INCORRECT_ACCOUNT_USAGE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15471',
      p_sig_id                 => 'CST_RECON_GL_INCORRECT_ACCOUNT_USAGE',
      p_sig_sql                => 'select
gjl.ledger_id,
        gjh.je_source,
        gjh.je_category,
...
gjl.code_combination_id,
       gps.period_year,
        ...
        order by gps.period_year,
        gps.period_num',
      p_title                  => 'Inventory Valuation Account Incorrectly Used in GL for Other Purposes',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Reconciliation issues found. The account has been used in GL for other sources than Cost Management.
',
      p_solution               => 'Review [2711893/KB712332#Issue_5] Reconciling Inventory with General Ledger (GL) - Common Issues - for solution steps.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_GL_INCORRECT_ACCOUNT_USAGE');



debug('begin add_signature: EBS_MFG_DM_CSTRECON_MTA_INVENTORY_VALUATION_AS_OFFSET_ACCOUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22842',
      p_sig_id                 => 'EBS_MFG_DM_CSTRECON_MTA_INVENTORY_VALUATION_AS_OFFSET_ACCOUNT',
      p_sig_sql                => 'SELECT * FROM mtl_material_transactions 
    WHERE organization_id = ##$$Organization_id$$##
        AND transaction_id IN ( 
         SELECT transaction_id FROM mtl_Transaction_accounts 
         WHERE organization_id = ##$$Organization_id$$##
              AND  reference_account = ##$$acct_id$$## 
              AND accounting_line_type = 2)',
      p_title                  => 'Inventory Valuation Account Used as Offset Account (accounting_line_type = 2)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Running the analyzer for an inventory valuation account, any output from the above scripts will cause reconciliation issue (On Hand Quantity will change but the Accounting will not change).

',
      p_solution               => 'Review [2711893/KB712332#Issue_1] Reconciling Inventory with General Ledger (GL) - Common Issues - for solution steps.
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_CSTRECON_MTA_INVENTORY_VALUATION_AS_OFFSET_ACCOUNT');



debug('begin add_signature: INV_RECON_IN_RCV_SUPPLY_NOT_IN_MTL_SUPPLY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13462',
      p_sig_id                 => 'INV_RECON_IN_RCV_SUPPLY_NOT_IN_MTL_SUPPLY',
      p_sig_sql                => 'SELECT rs.* FROM rcv_supply rs
WHERE rs.supply_type_code = ''RECEIVING''
AND rs.oe_order_line_id IS null
AND NOT EXISTS (SELECT ''1''
            FROM mtl_supply ms
            WHERE ms.supply_source_id = rs.supply_source_id
            AND ms.supply_type_code = ''RECEIVING'')
AND NOT EXISTS (SELECT ''1''
                FROM rcv_transactions rt
                WHERE rt.transaction_id = rs.supply_source_id
                AND rt.transaction_type = ''UNORDERED'')',
      p_title                  => 'Records Not Displayed in Receiving Value Report but Displayed in Account Analysis Report',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'In some instances there will be a balance in the Receiving Inspection Account but no record in mtl_supply, hence these records will not be displayed in the Receiving Value report but will be displayed in Account Analysis report. ',
      p_solution               => 'Review [1390813/KB566150#Recon_Issue] Receiving Inventory Account Reconciliation - Troubleshooting.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: INV_RECON_IN_RCV_SUPPLY_NOT_IN_MTL_SUPPLY');



debug('begin add_signature: EBS_MFG_DM_CSTRECON_GL_BALANCE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25297',
      p_sig_id                 => 'EBS_MFG_DM_CSTRECON_GL_BALANCE',
      p_sig_sql                => 'SELECT
    ...
    gb.period_net_dr,
    gb.period_net_cr,
    gb.period_net_dr-gb.period_net_cr  net_activity_by_period
   , sum(gb.period_net_dr-gb.period_net_cr) over (partition by gb.currency_code order by gb.currency_code, gb.period_year ,gb.period_num) "Running Total"

FROM
    gl_balances gb,
    gl_code_combinations_kfv gcck
WHERE
    gb.code_combination_id =##$$acct_id$$##
...
ORDER BY
    ...
    gb.period_year,
    gb.period_num',
      p_title                  => 'GL Balance for the Inventory Valuation Account per Period',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displaying the running balance of the account for each currency.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_CSTRECON_GL_BALANCE');



debug('begin add_signature: EBS_MFG_DM_CSTRECON_GL_NO_INVENTORY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25296',
      p_sig_id                 => 'EBS_MFG_DM_CSTRECON_GL_NO_INVENTORY',
      p_sig_sql                => 'select
gjl.ledger_id,
        gjh.je_source,
        gjh.je_category,
...
gjl.code_combination_id,
       gps.period_year,
        ...
        order by gps.period_year,
        gps.period_num',
      p_title                  => 'Entries in GL from Source Management but from Other Categories than Inventory',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Reconciliation issues identified. The inventory valuation account has been used in GL for other categories than Inventory.',
      p_solution               => 'Review [2711893/KB712332#Issue_5] Reconciling Inventory with General Ledger (GL) - Common Issues - for solution steps.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_CSTRECON_GL_NO_INVENTORY');



debug('begin add_signature: EBS_MFG_DM_CSTRECON_INVENTORY_NO_SLA');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25298',
      p_sig_id                 => 'EBS_MFG_DM_CSTRECON_INVENTORY_NO_SLA',
      p_sig_sql                => ' select  oap.period_year  ,
     oap.period_name ,
  oap.period_num,
     a.organization_id,
     a.reference_account,
 to_number(sum(a.base_transaction_value), 9999999999999.9999) Accounted_Value
FROM mtl_transaction_accounts a,
org_acct_periods oap
WHERE a.reference_account  =##$$acct_id$$##
    AND a.inv_sub_ledger_id > 0
    and a.transaction_date >=oap.period_start_date
    and a.transaction_date <(oap.schedule_close_date+1)
    and oap.organization_id = a.organization_id
    and oap.acct_period_id <=(select oap1.acct_period_id from org_acct_periods oap1 
     where oap.organization_id=oap1.organization_id and upper(period_name)=upper(''##$$period_name$$##''))
...
group by oap.period_year,
oap.period_num,
a.organization_id ,
oap.period_name,
a.reference_account
order by oap.period_year,
oap.period_name,
 a.organization_id',
      p_title                  => 'Inventory Valuation Account Unprocessed in SLA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Inventory accounting not processed by Create Accounting  Cost Management Program when reconciling  Inventory with GL.',
      p_solution               => 'Review [755943/KB720633] Transfer Inventory Transaction Into General Ledger through Subledger Accounting SLA Flow for solution steps.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_CSTRECON_INVENTORY_NO_SLA');



debug('begin add_signature: FILES USED BY  PERIOD CLOSE RECONCILIATION REPORT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '8899',
      p_sig_id                 => 'FILES USED BY  PERIOD CLOSE RECONCILIATION REPORT',
      p_sig_sql                => 'SELECT a.app_short_name, a.subdir, a.filename, b.version 
FROM   apps.ad_files a, 
       apps.ad_file_versions b 
WHERE  a.FILENAME in ( ''CSTVIVTB.pls'',''CSTVCPVB.pls'',''CSTPAPEB.pls'',''CSTACDPB.pls'',''CSTRPCRE.rdf'')
AND    a.file_id = b.file_id 
AND    a.app_short_name = ''BOM'' 
AND    b.creation_date = (SELECT MAX (creation_date) 
                          FROM apps.ad_file_versions ver 
                          WHERE ver.file_id = a.file_id)',
      p_title                  => 'Files Versions used by  Period Close Reconciliation Report',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Versions of files used by Period Close Reconciliation Report.',
      p_solution               => 'As a proactive approach you can consider upgrading to latest versions of files used by Period Close Reconciliation Program.<br>

Review the following documents:<br>
• Patch Wizard Utility [976188/KB739927]<br>
• Patch Wizard FAQ [10940175/KA580]<br>
• How to find the Latest, Recommended Patches for E-Business Suite R12 or 11i [1525452/KB455714]
',
      p_success_msg            => 'Versions of files used by Period Close Reconciliation Report.',
   ...
   l_info.delete;
debug('end add_signature: FILES USED BY  PERIOD CLOSE RECONCILIATION REPORT');



debug('begin add_signature: CST_RECON_PCRR_PERIODS_WITH_DISCREPANCIES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15182',
      p_sig_id                 => 'CST_RECON_PCRR_PERIODS_WITH_DISCREPANCIES',
      p_sig_sql                => 'SELECT mp.organization_code,cpcs.organization_id, cpcs.cost_group_id,cpcs.acct_period_id,oap.period_year, oap.period_name,
round(SUM(cpcs.rollback_value),5) rollback_value, round(sum(cpcs.accounted_value),5) accounted_value, round(SUM(cpcs.rollback_value)-sum(cpcs.accounted_value), 5) diff
FROM cst_period_close_summary cpcs, org_acct_periods oap, MTL_parameters mp
WHERE cpcs.organization_id=##$$Organization_id$$##
...
GROUP BY mp.organization_code,cpcs.organization_id, cpcs.cost_group_id,cpcs.acct_period_id,oap.period_year, oap.period_name
order by cpcs.acct_period_id',
      p_title                  => 'Accounted Value and Onhand Value Across All Periods',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Accounted values and onhand values across all periods (and possible discrepancies) in Period Close Reconciliation PCRR report.

',
      p_solution               => 'Review [331038/KB197814] How To Validate the Period Close Reconciliation Report and the Period Close Process.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_PCRR_PERIODS_WITH_DISCREPANCIES');



debug('begin add_signature: CST_RECON_PCRR_ITEMS_PERIODS_DISCREPANCIES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15181',
      p_sig_id                 => 'CST_RECON_PCRR_ITEMS_PERIODS_DISCREPANCIES',
      p_sig_sql                => 'SELECT cpcs.organization_id, mp.organization_code,cpcs.inventory_item_id,msi.concatenated_segments,
cpcs.cost_group_id,cpcs.acct_period_id, oap.period_year,oap.period_num,oap.period_name, SUM(cpcs.rollback_value) rollback_value, sum(cpcs.accounted_value) accounted_value,
SUM(cpcs.rollback_value)-sum(cpcs.accounted_value) discrepancy
FROM cst_period_close_summary cpcs , mtl_system_items_kfv msi, org_acct_periods oap, mtl_parameters mp
where  cpcs.organization_id =##$$Organization_id$$##
and msi.organization_id=cpcs.organization_id
and msi.inventory_item_id=cpcs.inventory_item_id
and cpcs.organization_id=mp.organization_id
and oap.organization_id=cpcs.organization_id
and oap.acct_period_id=cpcs.acct_period_id
GROUP BY cpcs.organization_id, mp.organization_code, cpcs.cost_group_id , cpcs.acct_period_id,oap.period_year,oap.period_num,oap.period_name ,cpcs.inventory_item_id, msi.concatenated_segments
HAVING ABS(SUM(cpcs.accounted_value) - SUM(cpcs.rollback_value)) > 1
order by cpcs.acct_period_id',
      p_title                  => 'Discrepancies between Accounted Value and Onhand Value across all Items and Periods ',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Discrepancies between accounted values and onhand values across all  items and periods.',
      p_solution               => 'Review [331038/KB197814] How To Validate the Period Close Reconciliation Report and the Period Close Process.
',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_PCRR_ITEMS_PERIODS_DISCREPANCIES');



debug('begin add_signature: CST_RECON_PCCR_ROUNDING');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15361',
      p_sig_id                 => 'CST_RECON_PCCR_ROUNDING',
      p_sig_sql                => 'select inventory_item_id, acct_period_id, cost_group_id, subinventory_code,organization_id,
sum(intransit_value_discrepancy),sum(onhand_value_discrepancy) from cst_period_close_summary 
group by inventory_item_id, acct_period_id, cost_group_id, subinventory_code,organization_id
having  (sum(onhand_value_discrepancy)<>0 and sum(onhand_value_discrepancy) between - 2 and 2
or sum(intransit_value_discrepancy)<>0 and  sum(intransit_value_discrepancy) between - 2 and 2)',
      p_title                  => 'Period Close Reconciliation Report  Rounding',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Rounding issue in Period Close Reconciliation Report. On Hand Quantity and Valuation Discrepancies Caused by Rounding. 
',
      p_solution               => 'Review [741195/KB515676] On Hand Qty. and Valuation Discrepancies Caused by Rounding.

',
      p_success_msg            => 'There is no rounding issue in Period Close Reconciliation Report',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_PCCR_ROUNDING');



debug('begin add_signature: CST_RECON_MMT_MOQ_MISMATCH_SUBINVENTORY_LEVEL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15373',
      p_sig_id                 => 'CST_RECON_MMT_MOQ_MISMATCH_SUBINVENTORY_LEVEL',
      p_sig_sql                => 'SELECT
    txn.organization_id,
    ...
    ( txn.mmt_qty - nvl(onhand.qty,0) ) qty_diff
FROM
    (
        SELECT
            mmt.organization_id,
        ...
        WHERE
            mse.organization_id = mmt.organization_id
        AND
            mse.secondary_inventory_name = mmt.subinventory_code
        AND
            mse.quantity_tracked = 1
        AND
            mmt.transaction_action_id NOT IN (
                5,6,24,30,50,51,52,55,26,7,11,17,10,9,13,14,15,22
            )
        AND
        ...
        AND
            mmt.lpn_id IS NULL
        AND
            mmt.transfer_lpn_id IS NULL
        AND
            mmt.content_lpn_id IS NULL
        GROUP BY
            mmt.organization_id,
            mmt.inventory_item_id,
            mmt.subinventory_code
    ...
    (
        ...
        FROM
            ...
            mtl_secondary_inventories mse
        WHERE
            moq.organization_id = mse.organization_id
        AND
            mse.secondary_inventory_name = moq.subinventory_code
        AND
            mse.quantity_tracked = 1
        AND
            nvl(moq.containerized_flag,2) = 2
        GROUP BY
            moq.organization_id,
            moq.inventory_item_id,
            moq.subinventory_code
...
AND
    txn.organization_id = onhand.organization_id (+)
AND
    txn.subinventory_code = onhand.subinventory_code (+)
AND
    abs(txn.mmt_qty - nvl(onhand.qty,0) ) > 0
AND
    EXISTS (
        SELECT
            1
        ...
        AND
            wms_enabled_flag = ''N'')',
      p_title                  => 'MTL_MATERIAL_TRANSACTIONS and MTL_ONHAND_QUANTITIES_DETAIL Tables  Quantity Mismatch at Subinventory Level',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There is a mismatch between the sum of the quantity from mtl_material_transactions and mtl_onhand_quantities_detail.<br>
Note: if your Organization(s) are WMS enabled or you have ever run the transactions purge process, you can ignore these results.
',
      p_solution               => 'Review [279205/KB157147] - Current Onhand Quantity and Total Transactions History Quantity Mismatch Diagnosis for solution steps.',
      p_success_msg            => 'A mismatch does not exist between the onhand quantity and the transactions table.<br>
There are no items affected by MMT-MOQ qty mismatch data corruption.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_MMT_MOQ_MISMATCH_SUBINVENTORY_LEVEL');



debug('begin add_signature: CST_RECON_MMT_MOQD_QUANTITY_MISMATCH_ORG');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '14110',
      p_sig_id                 => 'CST_RECON_MMT_MOQD_QUANTITY_MISMATCH_ORG',
      p_sig_sql                => 'SELECT
    txn.organization_id,
    txn.inventory_item_id,
    txn.mmt_qty,
...
WHERE
    ...
    AND nvl(mp.wms_enabled_flag, ''N'') = ''N''',
      p_title                  => 'MTL_MATERIAL_TRANSACTIONS and MTL_ONHAND_QUANTITIES_DETAIL Tables Quantity Mismatch per Organization',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There is a quantity mismatch between MTL_MATERIAL_TRANSACTIONS table and MTL_ONHAND_QUANTITIES_DETAIL table per Organization.',
      p_solution               => 'Review [279205/KB157147] Current Onhand Quantity and Total Transactions History Quantity Mismatch Diagnosis. 
',
      p_success_msg            => 'There are no MMT MOQD Quantity Mismatch per Organization.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_MMT_MOQD_QUANTITY_MISMATCH_ORG');



debug('begin add_signature: MMT-CQL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '14372',
      p_sig_id                 => 'MMT-CQL',
      p_sig_sql                => 'SELECT CQL.ORGANIZATION_ID,CQL.INVENTORY_ITEM_ID,CQL.COST_GROUP_ID,CQL.LAYER_QUANTITY,TXN.PRIMARY_QUANTITY,
       INTRANSIT.INTRANSIT_QTY,CQL.ITEM_COST
 FROM 
(SELECT ORGANIZATION_ID,INVENTORY_ITEM_ID,COST_GROUP_ID,ITEM_COST,
              NVL(LAYER_QUANTITY,0) LAYER_QUANTITY
          FROM CST_QUANTITY_LAYERS
        WHERE ORGANIZATION_ID = ##$$Organization_id$$##
     --AND COST_GROUP_ID = &&COST_GROUP_ID
        ) CQL,
(SELECT MMT.INVENTORY_ITEM_ID INVENTORY_ITEM_ID,SUM(MMT.PRIMARY_QUANTITY) PRIMARY_QUANTITY 
 FROM MTL_MATERIAL_TRANSACTIONS MMT, 
  ...
  WHERE MP.ORGANIZATION_ID = ##$$Organization_id$$##
    ...
    AND MSE.SECONDARY_INVENTORY_NAME=MMT.SUBINVENTORY_CODE
    AND MSE.ASSET_INVENTORY=1
    /* fixed for logical transactions corresponding to 
       drop shipments and global procurement transactions */
    AND NVL(MMT.logical_transaction,-1) <> 1
    AND NVL(MMT.OWNING_TP_TYPE,2)  =2
    AND NVL(MMT.OWNING_ORGANIZATION_ID,MMT.ORGANIZATION_ID)=MMT.ORGANIZATION_ID
    AND MSE.ORGANIZATION_ID=MP.ORGANIZATION_ID
    --AND NVL(MMT.COST_GROUP_ID, MP.DEFAULT_COST_GROUP_ID) = &COST_GROUP_ID
...
( SELECT NVL(SUM(DECODE(TO_ORGANIZATION_ID, 
      ...
      WHERE MS.INTRANSIT_OWNING_ORG_ID = ##$$Organization_id$$##
        AND MS.SUPPLY_TYPE_CODE IN (''SHIPMENT'', ''RECEIVING'') 
        AND RSH.SHIPMENT_HEADER_ID = MS.SHIPMENT_HEADER_ID           
        AND (MS.TO_ORGANIZATION_ID  = ##$$Organization_id$$## 
              OR MS.FROM_ORGANIZATION_ID = ##$$Organization_id$$##) 
        AND EXISTS 
            ( SELECT ''X'' 
                FROM MTL_MATERIAL_TRANSACTIONS MMT,
		     MTL_PARAMETERS MP
               WHERE MMT.INVENTORY_ITEM_ID = MS.ITEM_ID 
                 ...
                 AND ( 
                        ...
                        MMT.TRANSFER_ORGANIZATION_ID=MS.TO_ORGANIZATION_ID AND
                        MMT.TRANSFER_ORGANIZATION_ID=MS.INTRANSIT_OWNING_ORG_ID
                        --AND NVL(MMT.TRANSFER_COST_GROUP_ID, MP.DEFAULT_COST_GROUP_ID) = &COST_GROUP_ID                       
                     ) /* INTSIT SHIP, FOB: SHIP */ 
                     OR 
                     ( 
                        MMT.ORGANIZATION_ID=MS.FROM_ORGANIZATION_ID AND
                        MMT.TRANSFER_ORGANIZATION_ID=MS.TO_ORGANIZATION_ID   AND
                        MMT.ORGANIZATION_ID=MS.INTRANSIT_OWNING_ORG_ID
                        --AND NVL(MMT.COST_GROUP_ID, MP.DEFAULT_COST_GROUP_ID) = &COST_GROUP_ID                    
                     ) /* INTSIT SHIP, FOB: RCPT */
         )
     GROUP BY MS.ITEM_ID ) INTRANSIT
 WHERE CQL.INVENTORY_ITEM_ID = TXN.INVENTORY_ITEM_ID
   AND TXN.INVENTORY_ITEM_ID = INTRANSIT.ITEM_ID (+)
   AND CQL.LAYER_QUANTITY <>(TXN.PRIMARY_QUANTITY + NVL(INTRANSIT.INTRANSIT_QTY,0))',
      p_title                  => 'MTL_MATERIAL_TRANSACTIONS and CST_QUANTITY_LAYERS Tables Quantity Mismatch',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There is MTL_MATERIAL_TRANSACTIONS CST_QUANTITY_LAYERS corruption present in the system.',
      p_solution               => 'Review [2715695/KB163718] How to identify and fix MMT-CQL Mismatch for solution steps.
',
      p_success_msg            => 'There are no transactions that need review in Material Transactions table (MTL_MATERIAL_TRANSACTIONS MMT)
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: MMT-CQL');



debug('begin add_signature: CST_RECON_BACKDATED_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15183',
      p_sig_id                 => 'CST_RECON_BACKDATED_TRANSACTIONS',
      p_sig_sql                => 'SELECT DISTINCT
mmt1.organization_id,
mmt1.transaction_id Backdated_transaction_id,
to_char(mmt1.transaction_date,''DD-MON-YYYY HH24:MI:SS'') transaction_date,
to_char(mmt1.creation_Date,''DD-MON-YYYY HH24:MI:SS'') creation_date,
to_char(mcacd1.transaction_costed_date,''DD-MON-YYYY HH24:MI:SS'') transaction_costed_date,
mmt2.transaction_id,
to_char(mmt2.transaction_date,''DD-MON-YYYY HH24:MI:SS'') transaction_date_1,
to_char(mmt2.creation_Date,''DD-MON-YYYY HH24:MI:SS'') creation_date_1,
to_char(mcacd2.transaction_costed_date,''DD-MON-YYYY HH24:MI:SS'') transaction_costed_date_1, 
...
WHERE  mmt1.inventory_item_id = mmt2.inventory_item_id
and mmt1.transaction_date between to_date(''##$$from_date$$##'',''DD-MON-YYYY HH24:MI:SS'') and to_date(''##$$to_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mmt1.cost_group_id = mmt2.cost_group_id
AND mmt1.organization_id=mmt2.organization_id
and mmt1.organization_id=##$$Organization_id$$##
AND mmt1.transaction_date < mmt2.transaction_date
and mmt2.transaction_id < mmt1.transaction_id
AND mcacd1.transaction_costed_date > mcacd2.transaction_costed_date
AND mcacd1.transaction_id = mmt1.transaction_id
AND mcacd2.transaction_id = mmt2.transaction_id
and mmt1.acct_period_id<>mmt2.acct_period_id
and mmt1.organization_id=msi.organization_id
and mmt1.inventory_item_id=msi.inventory_item_id
order by  transaction_costed_date_1 asc',
      p_title                  => 'Backdated Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions which have a transaction date earlier than creation date. ',
      p_solution               => 'Review [762134/KB732879] How Does Backdated Transactions Affect Reconciliation? , to understand the impact of backdating transactions in Cost Management.',
...
',
   ...
   l_info.delete;
debug('end add_signature: CST_RECON_BACKDATED_TRANSACTIONS');
...
debug('begin add_signature: CST_SLA_GL_NEG_LEDGER_ID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7219',
      p_sig_id                 => 'CST_SLA_GL_NEG_LEDGER_ID',
...
and    h.ledger_id = -1 * ##$$ledger_id$$##             
and    h.je_header_id = l.je_header_id
and    l.ledger_id = h.ledger_id 
and    UPPER(h.je_source) = ''Cost Management''
and    h.period_name in (select distinct period_name 
                         from   gl_period_statuses
                         where  application_id = 707
                         and    start_date >= to_date(''##$$start_date$$##'',''DD-MON-YYYY'')
                         and    end_date <= to_date(''##$$end_date$$##'',''DD-MON-YYYY'')
...
order by gjb.name,gjb.je_batch_id,gjb.group_id,ccid',
      p_title                  => 'Records With Negative Ledger_ID in GL Tables',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) where Ledger_ID is Negative in GL.',
...
<li>Contact Support referring this note.</li>
</ul>

',
      p_success_msg            => 'No Negative Ledger_ID found.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_NEG_LEDGER_ID');



debug('begin add_signature: CST_SLA_GL_NULL_GL_SL_LINK_ID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7239',
      p_sig_id                 => 'CST_SLA_GL_NULL_GL_SL_LINK_ID',
...
and    h.ledger_id = ##$$ledger_id$$## 
and    h.accounting_date between to_date(''##$$start_date$$##'',''DD-MON-YYYY'') 
...
and    l.gl_sl_link_id is null',
      p_title                  => 'Records Transferred to GL but GL_SL_LINK_ID is NULL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred to GL but GL_SL_LINK_ID is NULL.',
      p_solution               => '<ul><li>The analyzer has identified data corruption described in [883557/KB627857], Corruption Type 8.</li>
<li> Please apply the generic data fix (GDF) Patch 13783741, following the instructions in the README file.</li>
</ul>
',
      p_success_msg            => 'No records found that are marked as transferred to GL and GL_SL_LINK_ID is NULL.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_NULL_GL_SL_LINK_ID');



debug('begin add_signature: CST_SLA_GL_XFR_IN_XLA_NOT_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7240',
      p_sig_id                 => 'CST_SLA_GL_XFR_IN_XLA_NOT_IN_GL',
...
order by h.period_name,h.group_id, ccid',
      p_title                  => 'Records Marked as Transferred to XLA but Do Not Exist in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred in XLA but do not exist in GL.',
      p_solution               => 'Review [883557/KB627857] for more information. Contact Support for a datafix, referring section: Corruption Type 2. of this note.',
      p_success_msg            => 'There are no record(s) marked as transferred in XLA but do not exist in GL.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_XFR_IN_XLA_NOT_IN_GL');



debug('begin add_signature: CST_SLA_GL_XFR_IN_XLA_REV_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7243',
      p_sig_id                 => 'CST_SLA_GL_XFR_IN_XLA_REV_IN_GL',
      p_sig_sql                => 'SELECT /*+ parallel (h,l) leading (h) */
       h.period_name as PERIOD_NAME,
       h.group_id AS GROUP_ID,
       l.code_combination_id AS CCID,
...
AND    h.accounting_date BETWEEN to_date(''##$$start_date$$##'',''DD-MON-YYYY'') 
       and to_date(''##$$end_date$$##'',''DD-MON-YYYY'')
AND EXISTS (SELECT /*+ leading (gh) USE_NL(ir) index(gh  GL_IMPORT_REFERENCES_N1 )  */ 1 
            FROM   gl_import_references ir, 
                   gl_je_headers gh 
            WHERE  gh.ledger_id = ##$$ledger_id$$##
            AND    ir.gl_sl_link_id = l.gl_sl_link_id 
...
group by h.period_name,h.group_id, l.code_combination_id
order by h.period_name,h.group_id, ccid',
      p_title                  => 'Records Marked as Transferred in XLA but Reversed in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) marked as transferred in XLA but are reversed in GL.',
      p_solution               => 'The analyzer has identified data corruption described in [883557/KB627857]. For datafix, please contact support mentioning Corruption Type 2 of this note. Reversal of a posted journal batch is not caused by system code bug, but by business decision to manually reverse the GL data.
',
      p_success_msg            => 'There are no record(s) marked as transferred in XLA but are reversed in GL.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_XFR_IN_XLA_REV_IN_GL');



debug('begin add_signature: CST_SLA_GL_EXISTS_IN_GL_NOT_IN_XLA');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7244',
      p_sig_id                 => 'CST_SLA_GL_EXISTS_IN_GL_NOT_IN_XLA',
...
and     UPPER(gh.je_source) = ''COST MANAGEMENT''
...
order by gh.period_name, gjb.name, gjb.group_id, ccid',
      p_title                  => 'Records Exist in GL but Missing in XLA',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which exist in GL but are missing in XLA.',
      p_solution               => 'The analyzer has identified data corruption described in [883557/KB627857]. For datafix, please contact support mentioning Corruption Type 5 of this note.
',
      p_success_msg            => 'There are no record(s) which exist in GL but are missing in XLA.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_EXISTS_IN_GL_NOT_IN_XLA');



debug('begin add_signature: CST_SLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7245',
      p_sig_id                 => 'CST_SLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL',
      p_sig_sql                => 'SELECT  /*+ parallel (h,l) leading (h) */
        gh.period_name,gjb.name, 
        gjb.group_id,
...
order by gh.period_name, gjb.name, gjb.group_id, ccid',
      p_title                  => 'Records Not Marked as Transferred in XLA but Exist in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which are not marked as transferred in XLA but exist in GL.',
      p_solution               => 'The analyzer has identified data corruption described in [883557/KB627857]. For datafix, please contact support mentioning Corruption Type 3 of this note.',
      p_success_msg            => 'There are no record(s) which are not marked as transferred in XLA but exist in GL.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL');



debug('begin add_signature: CST_SLA_GL_MULTIPLE_POST_IN_GL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7246',
      p_sig_id                 => 'CST_SLA_GL_MULTIPLE_POST_IN_GL',
       ...
       (select gir.gl_sl_link_id, 
        ...
        and    exists
             (select /*+ parallel(xah,xal) leading(xah) */  ''found_dup''
              ...
              and      xah.application_id = 707
              and      xah.accounting_entry_status_code = ''F''   
              and      xah.accounting_date between to_date(''##$$start_date$$##'',''DD-MON-YYYY'') 
                                           and to_date(''##$$end_date$$##'',''DD-MON-YYYY'')
...
and     UPPER(gjh.je_source) = ''RECEIVABLES''',
      p_title                  => 'Records Posted Multiple Times in GL',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Found record(s) which are posted multiple times in GL.',
      p_solution               => 'The analyzer has identified data corruption described in [883557/KB627857]. For datafix, please contact support mentioning Corruption Type 4 of this note.',
      p_success_msg            => 'There are no record(s) which are posted multiple times in GL.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_GL_MULTIPLE_POST_IN_GL');



debug('begin add_signature: EBS_MFG_DMEF_CSTRECON_CST_MTA_XE_DATE_MISMATCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '28643',
      p_sig_id                 => 'EBS_MFG_DMEF_CSTRECON_CST_MTA_XE_DATE_MISMATCH',
      p_sig_sql                => 'SELECT mta.reference_account, mta.accounting_line_type, SUM(mta.base_transaction_value), decode(xe.event_status_code, ''I'', ''Incomplete'', ''N'', ''No action'', ''P'', ''Processed'', ''U'', ''Unprocessed'') event_status_code,
( ''MTA Transaction Date not matching with event date'' ) comments
FROM mtl_transaction_accounts mta, xla_events xe, xla_distribution_links xdl
WHERE mta.organization_id =##$$Organization_id$$##
AND mta.transaction_date >=to_date(''##$$from_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.transaction_date <=to_date(''##$$to_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.reference_account = ##$$acct_id$$## AND xdl.application_id = 707 AND xe.event_id = xdl.event_id AND xe.application_id = xdl.application_id
AND xdl.source_distribution_type = ''MTL_TRANSACTION_ACCOUNTS'' AND xdl.application_id = 707 AND xdl.source_distribution_id_num_1 = mta.inv_sub_ledger_id
AND to_date(mta.transaction_date, ''DD-MON-YYYY'') <> to_date(xe.event_date, ''DD-MON-YYYY'')
GROUP BY mta.reference_account, mta.accounting_line_type, event_status_code
UNION
SELECT mta.reference_account, mta.accounting_line_type, SUM(mta.base_transaction_value), decode(xe.event_status_code, ''I'', ''Incomplete'', ''N'', ''No action'', ''P'', ''Processed'', ''U'', ''Unprocessed'') event_status_code,
( ''Event date not matching with MTA Transaction date'' ) comments
FROM mtl_transaction_accounts mta, xla_events xe, xla_distribution_links xdl
WHERE mta.organization_id =##$$Organization_id$$##
AND xe.event_date >=to_date(''##$$from_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND xe.event_date <=to_date(''##$$to_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.reference_account = ##$$acct_id$$## AND xdl.application_id = 707 AND xe.event_id = xdl.event_id AND xe.application_id = xdl.application_id
AND xdl.source_distribution_type = ''MTL_TRANSACTION_ACCOUNTS'' AND xdl.application_id = 707 AND xdl.source_distribution_id_num_1 = mta.inv_sub_ledger_id
AND to_date(mta.transaction_date, ''DD-MON-YYYY'') <> to_date(xe.event_date, ''DD-MON-YYYY'')
GROUP BY mta.reference_account, mta.accounting_line_type, event_status_code',
      p_title                  => 'MTA and XLA Date Mismatch',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions which have a transaction date in MTA (mtl_transaction_accounts) that does not match the event date in XE (xla_events). ',
      p_solution               => 'This is caused by sweeping the event_date in xla_events table. This usually happens when the GL period is closed before running Create Accounting.
Review [3007941/KB530516] Inventory and General Ledger Reconciliation For Discrete Costing - An Approach, Steps And Scripts, to understand the impact in Reconciliation between Inventory and GL when the date in MTA does not match the date in XE.',
      p_success_msg            => 'There are no mismatches in dates between MTA and XE for Cost Management source and Inventory category
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DMEF_CSTRECON_CST_MTA_XE_DATE_MISMATCH');



debug('begin add_signature: EBS_MFG_DMEF_CSTRECON_CST_MTA_DATE_DIFF_PERIOD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '28644',
      p_sig_id                 => 'EBS_MFG_DMEF_CSTRECON_CST_MTA_DATE_DIFF_PERIOD',
      p_sig_sql                => 'SELECT mta.* FROM mtl_transaction_accounts mta, xla_events xe, xla_distribution_links xdl
WHERE mta.organization_id =##$$Organization_id$$##
AND mta.transaction_date >=to_date(''##$$from_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.transaction_date <=to_date(''##$$to_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.reference_account =##$$acct_id$$## AND xdl.application_id = 707 AND xe.event_id = xdl.event_id AND xe.application_id = xdl.application_id AND xdl.source_distribution_type = ''MTL_TRANSACTION_ACCOUNTS''
AND xdl.application_id = 707 AND xdl.source_distribution_id_num_1 = mta.inv_sub_ledger_id AND to_date(mta.transaction_date, ''DD-MON-YYYY'') <> to_date(xe.event_date, ''DD-MON-YYYY'')',
      p_title                  => 'MTA Date is in a Different Period while XE Date falls in Reconciling Period',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions which have a transaction date in MTA (mtl_transaction_accounts) that are in a different period while the event date in XE (xla_events) is in the period which is being reconciled ',
      p_solution               => 'This is caused by sweeping the event_date in xla_events table. This usually happens when the GL period is closed before running Create Accounting.
Review [3007941/KB530516] Inventory and General Ledger Reconciliation For Discrete Costing - An Approach, Steps And Scripts, to understand the impact in Reconciliation between Inventory and GL when the date in MTA does not match the date in XE.',
      p_success_msg            => 'There are no mismatches in dates between MTA and XE for Cost Management source and Inventory category
',
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DMEF_CSTRECON_CST_MTA_DATE_DIFF_PERIOD');



debug('begin add_signature: EBS_MFG_DMEF_CSTRECON_CST_XE_DATE_DIFF_PERIOD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '28645',
      p_sig_id                 => 'EBS_MFG_DMEF_CSTRECON_CST_XE_DATE_DIFF_PERIOD',
      p_sig_sql                => 'SELECT xe.* FROM mtl_transaction_accounts mta, xla_events xe, xla_distribution_links xdl
WHERE mta.organization_id =##$$Organization_id$$##
AND xe.event_date >=to_date(''##$$from_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND xe.event_date <=to_date(''##$$to_date$$##'',''DD-MON-YYYY HH24:MI:SS'')
AND mta.reference_account =##$$acct_id$$## AND xdl.application_id = 707 AND xe.event_id = xdl.event_id AND xe.application_id = xdl.application_id AND xdl.source_distribution_type = ''MTL_TRANSACTION_ACCOUNTS''
AND xdl.application_id = 707 AND xdl.source_distribution_id_num_1 = mta.inv_sub_ledger_id AND to_date(mta.transaction_date, ''DD-MON-YYYY'') <> to_date(xe.event_date, ''DD-MON-YYYY'')',
      p_title                  => 'XE Date is in a Different Period while MTA Date falls in Reconciling Period',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions which have an event date in XE (xla_events) that is in a different period while the transaction date in MTA (mtl_transaction_accounts) is in the period which is being reconciled.',
      p_solution               => 'This is caused by sweeping the event_date in xla_events table. This usually happens when the GL period is closed before running Create Accounting.
Review [3007941/KB530516] Inventory and General Ledger Reconciliation For Discrete Costing - An Approach, Steps And Scripts, to understand the impact in Reconciliation between Inventory and GL when the date in MTA does not match the date in XE.',
      p_success_msg            => 'There are no mismatches in dates between MTA and XE for Cost Management source and Inventory category
',
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DMEF_CSTRECON_CST_XE_DATE_DIFF_PERIOD');
...
BEGIN
  ...
  IF (p_Period_Name IS NOT NULL) THEN
    g_params_string := g_params_string || to_char(p_Period_Name) || '_';
  END IF;
  ...
  analyzer_title := 'Inventory to General Ledger Reconciliation Analyzer';
...
debug('begin section: Discrete_Manufacturing');
start_section('Discrepancy Information', 'Discrete_Manufacturing');
   set_item_result(run_stored_sig('UNCOSTED_TXNS_COUNT'));
   set_item_result(run_stored_sig('CST_SLA_EVENT_DETAILS'));
   set_item_result(run_stored_sig('CST_SLA_PENDING_TRANSFER_TO_G'));
   set_item_result(run_stored_sig('CST_RECON_MTL_TRANSACTION_ACCOUNTS'));
   set_item_result(run_stored_sig('CST_RECON_WIP_TRANSACTION_ACCOUNTS'));
   set_item_result(run_stored_sig('CST_RECON_RCV_RECEIVING_SUB_LEDGER'));
   set_item_result(run_stored_sig('CST_RECON_SLA VALUE'));
   set_item_result(run_stored_sig('CST_RECON_GL_VALUE'));
   set_item_result(run_stored_sig('CST_RECON_GL_INCORRECT_ACCOUNT_USAGE'));
   set_item_result(run_stored_sig('EBS_MFG_DM_CSTRECON_MTA_INVENTORY_VALUATION_AS_OFFSET_ACCOUNT'));
   set_item_result(run_stored_sig('INV_RECON_IN_RCV_SUPPLY_NOT_IN_MTL_SUPPLY'));
   set_item_result(run_stored_sig('EBS_MFG_DM_CSTRECON_GL_BALANCE'));
   set_item_result(run_stored_sig('EBS_MFG_DM_CSTRECON_GL_NO_INVENTORY'));
   set_item_result(run_stored_sig('EBS_MFG_DM_CSTRECON_INVENTORY_NO_SLA'));
end_section;
debug('end section: Discrete_Manufacturing');

debug('begin section: Period_Close_Reconciliation_Report');
start_section('Period Close Reconciliation Report', 'Period_Close_Reconciliation_Report');
   set_item_result(run_stored_sig('FILES USED BY  PERIOD CLOSE RECONCILIATION REPORT'));
   set_item_result(run_stored_sig('CST_RECON_PCRR_PERIODS_WITH_DISCREPANCIES'));
   set_item_result(run_stored_sig('CST_RECON_PCRR_ITEMS_PERIODS_DISCREPANCIES'));
   set_item_result(run_stored_sig('CST_RECON_PCCR_ROUNDING'));
   set_item_result(run_stored_sig('CST_RECON_MMT_MOQ_MISMATCH_SUBINVENTORY_LEVEL'));
   set_item_result(run_stored_sig('CST_RECON_MMT_MOQD_QUANTITY_MISMATCH_ORG'));
   set_item_result(run_stored_sig('MMT-CQL'));
   set_item_result(run_stored_sig('CST_RECON_BACKDATED_TRANSACTIONS'));
   set_item_result(run_stored_sig('CST_BACKDATED_TRANS_DETIALS_MTL_TRANS_ACCT'));
end_section;
debug('end section: Period_Close_Reconciliation_Report');

debug('begin section: XLA_GL_Integrity');
start_section('XLA GL Integrity', 'XLA_GL_Integrity');
   set_item_result(run_stored_sig('CST_SLA_GL_NEG_LEDGER_ID'));
   set_item_result(run_stored_sig('CST_SLA_GL_NULL_GL_SL_LINK_ID'));
   set_item_result(run_stored_sig('CST_SLA_GL_XFR_IN_XLA_NOT_IN_GL'));
   set_item_result(run_stored_sig('CST_SLA_GL_XFR_IN_XLA_REV_IN_GL'));
   set_item_result(run_stored_sig('CST_SLA_GL_EXISTS_IN_GL_NOT_IN_XLA'));
   set_item_result(run_stored_sig('CST_SLA_GL_NOT_XFR_IN_XLA_BUT_IN_GL'));
   set_item_result(run_stored_sig('CST_SLA_GL_MULTIPLE_POST_IN_GL'));
   set_item_result(run_stored_sig('EBS_MFG_DMEF_CSTRECON_CST_MTA_XE_DATE_MISMATCH'));
   set_item_result(run_stored_sig('EBS_MFG_DMEF_CSTRECON_CST_MTA_DATE_DIFF_PERIOD'));
   set_item_result(run_stored_sig('EBS_MFG_DMEF_CSTRECON_CST_XE_DATE_DIFF_PERIOD'));
end_section;
