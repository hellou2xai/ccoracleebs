...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Create Accounting-Cost Management      |
REM | Analyzer                                                                  |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE cst_sla_analyzer_pkg AUTHID DEFINER AS
...
(            ledger_id                      IN NUMBER      DEFAULT NULL
           ,ledger_id_sql                  IN NUMBER      DEFAULT NULL
...
END cst_sla_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY cst_sla_analyzer_pkg AS
-- $Id: cst_sla_unprocessed_trx_analyzer.sql, 200.30 2026/01/28 12:01:59 sothman Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(1580316.1));
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'CSTSUTA_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'CSTSUTA_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>CSTSUTA Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=1580316.1#feedback");
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/3727654" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">1073</span>
...
<p>Version 200.28 28-APR-2025&nbsp;</p><p>Enhanced Row Selection and Scrolling Functionality<br>
...
This new feature is designed to provide a more intuitive and user-friendly experience, enabling you to work more efficiently with your data.<br>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1073,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1073,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1073,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
IF (ledger_id_sql IS NOT NULL) THEN
   return ledger_id_sql;
...
FUNCTION get_RESPID
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: RESPID');
...
END IF;
debug('end sql token function: RESPID');
END get_RESPID;

FUNCTION get_USERNAME
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: USERNAME');
...
END IF;
debug('end sql token function: USERNAME');
END get_USERNAME;
...
debug('begin populate parameters hash table');
   ...
   g_parameters(g_parameters.LAST).pvalue := mask_text(ledger_id,'NO_MASK');
   END IF;
   IF NOT g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Ledger ID:';
   g_parameters(g_parameters.LAST).pvalue := mask_text(ledger_id_sql,'NO_MASK');
   END IF;
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Enter Start Date:';
   g_parameters(g_parameters.LAST).pvalue := mask_text(to_char(p_from_date,'DD-MON-YYYY'),'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Enter End Date:';
   ...
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_responsibility_id,'NO_MASK');
   END IF;
   IF g_is_concurrent THEN 
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'User Name';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_user_name,'DISPLAY_NONE');
   END IF;
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  l_revision := rtrim(replace('$Revision: 200.30  $','$',''));
  l_revision := ltrim(replace(l_revision,'Revision:',''));
  l_date_char := rtrim(replace('$Date: 2026/01/28 12:01:58 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'cst_sla_unprocessed_trx_analyzer.sql';
...
debug('begin parameter validation: ledger_id_sql');
IF NOT g_is_concurrent THEN
IF ledger_id_sql IS NULL THEN
   print_error('INVALID ARGUMENT: Parameter Ledger ID: is required.');
   raise invalid_parameters;
END IF;
END IF;
debug('end parameter validation: ledger_id_sql');
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$RESPID$$##') := get_RESPID();
   l_system_function_var := null;
   g_masked_tokens('##$$RESPID$$##') := mask_text( g_sql_tokens('##$$RESPID$$##'), 'NO_MASK');
   g_sql_tokens('##$$USERNAME$$##') := get_USERNAME();
   l_system_function_var := null;
...
debug('begin add_signature: CST_SLA_ERROR_DETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7105',
      p_sig_id                 => 'CST_SLA_ERROR_DETAILS',
      p_sig_sql                => 'select xe.event_type_code Event_Type,xae.ledger_id,
decode(xe.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(xe.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS
, xae.message_number, xae.encoded_msg Error_Msg
,xae.EVENT_ID,xae.ENTITY_ID,xae.AE_HEADER_ID,xae.AE_LINE_NUM,xae.CREATION_DATE
from xla_accounting_errors xae, xla_events xe
...
group by xe.event_type_code, xae.message_number, 
xe.PROCESS_STATUS_CODE,xe.EVENT_STATUS_CODE,
xae.message_number, xae.encoded_msg,xae.EVENT_ID,xae.ENTITY_ID,xae.AE_HEADER_ID,xae.AE_LINE_NUM,xae.CREATION_DATE,xae.ledger_id',
      p_title                  => 'Detail Error Records From XLA_ACCOUNTING_ERRORS , XLA_EVENTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records which did not process.',
      p_solution               => '<ul><li>  
   Non Processed Records were found. Please process these records.<br>
   Review Create Accounting report output and Subledger Period Close Exceptions Report output for more detail.<br>
   Refer to [1457611/KB752540] R12: What is the Meaning of EVENT_STATUS_CODE and PROCESS_STATUS_CODE in XLA_EVENTS Table?
</li></ul>
',
      p_success_msg            => 'No Error records found in XLA tables and hence no action is required.',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_DETAILS');
...
debug('begin add_signature: CST_SLA_TRANSACTION_ENTITIES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7003',
      p_sig_id                 => 'CST_SLA_TRANSACTION_ENTITIES',
      p_sig_sql                => 'SELECT ENTITY_ID,APPLICATION_ID,ENTITY_CODE,CREATION_DATE,SOURCE_ID_INT_1,
...
AND event_status_code <> ''P''
AND process_status_code <> ''P'')',
      p_title                  => 'Display  Data From XLA_TRANSACTION_ENTITIES',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Display Data from XLA_TRANSACTION_ENTITIES table.',
      p_solution               => '<ul><li>
    Non Processed records found in the XLA_TRANSACTION_ENTITIES table.<br>
    These records need to be processed based on the error table details. Review Create Accounting report output and Subledger Period Close Exceptions Report output.<br>
    Review [2005838/KB763584]  SLA Create Accounting -Cost Management Common Error Code/Number.
</li></ul>
',
      p_success_msg            => 'No unprocessed records found in the XLA_TRANSACTION_ENTITIES table.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_TRANSACTION_ENTITIES');



debug('begin add_signature: CST_SLA_XLA_AE_HEADERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7002',
      p_sig_id                 => 'CST_SLA_XLA_AE_HEADERS',
      p_sig_sql                => 'SELECT AE_HEADER_ID,APPLICATION_ID,LEDGER_ID,ENTITY_ID,EVENT_ID,EVENT_TYPE_CODE,
       ACCOUNTING_DATE,decode(GL_TRANSFER_STATUS_CODE,''Y'',''Yes'',''N'',''No'',''NT'',''Not Transferable'') GL_TRANSFER_STATUS,
	   decode(accounting_entry_status_code,''F'',''Final Mode'',''I'',''Incomplete'') ACCOUNTING_STATUS
FROM XLA_ae_headers
WHERE application_id = 707
...
order by je_category_name',
      p_title                  => 'Data From XLA_AE_HEADERS Eligible For GL Transfer',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Data From XLA_AE_HEADERS table eligible for GL transfer.',
      p_solution               => '<ul><li>
 Non Processed records found in the XLA_AE_HEADERS table which are not transferred to GL.<br>
 These records need to be processed based on the error table details.Review Create Accounting report output and Subledger Period Close Exceptions Report output.<br>
 Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number  
</li></ul>
',
      p_success_msg            => 'No unprocessed records pending transfer to GL found in the XLA_AE_HEADERS table.
xla_ae_headers.gl_transfer_status_code = ''Y''
xla_ae_headers.gl_transfer_date is Not Null',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_XLA_AE_HEADERS');



debug('begin add_signature: CST_SLA_ERROR_0_WIP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7197',
      p_sig_id                 => 'CST_SLA_ERROR_0_WIP',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
and xe.event_type_code like ''%WIP%''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''0'')',
      p_title                  => 'Detail Error Records From SLA Error Number 0 WIP',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 for WIP classes',
      p_solution               => '<ul><li>  
Follow solution from: [726864/KB158997] Create Accounting - Cost Management WIP Subledger Journal Entries Do Not Balance , Subledger Journal Entry does not balance in the Entered Currency. 
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_WIP');



debug('begin add_signature: CST_SLA_ERROR_95277');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7195',
      p_sig_id                 => 'CST_SLA_ERROR_95277',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95277'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95277',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95277',
      p_solution               => '<ul><li>  
Follow solution from: [1364958/KB738040] Create Accounting - Cost Management with Error 95277 The Subledger Journal Entry does not Balance by Balancing Segment. 
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95277');



debug('begin add_signature: CST_SLA_ERROR_95283');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7193',
      p_sig_id                 => 'CST_SLA_ERROR_95283',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95283'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95283',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records which did not process due to error 95283',
      p_solution               => '<ul><li>  
Refer to:[1293178/KB749129] XLAACCUP Create Accounting Subledger Period Close Exception Report Invalid COGS Recognition Event Err 95283.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95283');



debug('begin add_signature: CST_SLA_ERROR_95288');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7194',
      p_sig_id                 => 'CST_SLA_ERROR_95288',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95288'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95288',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95288',
      p_solution               => '<ul><li>  
Follow solution from: [1529194/KB745055] Create Accounting Cost Management Completes With Warning: Error is 95288 Based upon your access set setup, You do not have write access to the balancing segment X01X51 used by line 2.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95288');



debug('begin add_signature: CST_SLA_ERROR_95301');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7196',
      p_sig_id                 => 'CST_SLA_ERROR_95301',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95301'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95301',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95301',
      p_solution               => '<ul><li>  
Follow solution from: [1081277/KB733435] Create Accounting Request Ends In Warning With Error 95301.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95301');



debug('begin add_signature: CST_SLA_ERROR_95318');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7376',
      p_sig_id                 => 'CST_SLA_ERROR_95318',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95318'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95318',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records which did not process due to error 95318',
      p_solution               => '<ul><li>  
Refer to:[1367223/KB718106] Create Accounting - Cost Management with Error 95318 The Account Code Combination ID () specified for line () does not exist, Incorrect Account Code Combination. </li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95318');



debug('begin add_signature: CST_SLA_ERROR_95325');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7381',
      p_sig_id                 => 'CST_SLA_ERROR_95325',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95325'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95325',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to Error 95325.',
      p_solution               => '<ul><li>  
Follow solution from: [1317253/KB738688] Create Accounting - Cost Management with Error 95325 The GL Date DD-MON-YYYY is not in an Open or a Future Enterable Period. Please select a valid Period.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95325');



debug('begin add_signature: CST_SLA_ERROR_95333');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7382',
      p_sig_id                 => 'CST_SLA_ERROR_95333',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95333'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95333',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95333',
      p_solution               => '<ul><li>  
Follow solution from: [1153024/KB783811] Create Accounting - Cost Management with Error 95333 A Conversion Rate does not exist to Convert YYY to ZZZ for the Conversion Type AAA and conversion date DD-MMM-YY for line.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95333');



debug('begin add_signature: CST_SLA_ERROR_95349');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7383',
      p_sig_id                 => 'CST_SLA_ERROR_95349',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95349'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95349',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95349',
      p_solution               => '<ul><li>  
Follow solution from: [871362/KB775729] Create Accounting - Cost Management with Error 95349 There Are no Eligible Transactions to Account.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95349');



debug('begin add_signature: CST_SLA_ERROR_95353');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7384',
      p_sig_id                 => 'CST_SLA_ERROR_95353',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95353'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95353',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95353.',
      p_solution               => '<ul><li>  
Follow solution from: [1158183/KB755169] Create Accounting - Cost Management with Error 95353 Subledger Accounting was unable to derive an accounting code combination using the account derivation rule Cost Management Default Account owned by Oracle.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95353');



debug('begin add_signature: CST_SLA_ERROR_95937');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7385',
      p_sig_id                 => 'CST_SLA_ERROR_95937',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95937'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95937',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95937',
      p_solution               => '<ul><li>  
Follow solution from: [846119/KB197604] Create Accounting - Cost Management with Error 95937 There are Events which could not be Processed.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95937');



debug('begin add_signature: CST_SLA_ERROR_0_95314_THE_ACCOUNTED_AMOUNT_HAS_DIFFERENT_SIGN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7232',
      p_sig_id                 => 'CST_SLA_ERROR_0_95314_THE_ACCOUNTED_AMOUNT_HAS_DIFFERENT_SIGN',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (0, 95314)',
      p_title                  => 'Detail Error Records From SLA Error Number 0 or 95314 When THE_ACCOUNTED_AMOUNT_HAS_DIFFERENT_SIGN',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 or 95314 when Accounted and Entered Amount for Subledger Journal Entry Line have Different Sign Subledger Journal Entry does not balance ',
      p_solution               => '<ul><li>  
Follow solution from:  [1169373/KB159006] Err 0 or 95314 Accounted and Entered Amount for Subledger Journal Entry Line have Different Sign Subledger Journal Entry does not balance in the Entered Currency.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_95314_THE_ACCOUNTED_AMOUNT_HAS_DIFFERENT_SIGN');



debug('begin add_signature: CST_SLA_ERROR_0_95937');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7393',
      p_sig_id                 => 'CST_SLA_ERROR_0_95937',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''0'',''95937'')',
      p_title                  => 'Detail Error Records From SLA Error Number 0 or 95937',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 or 95937',
      p_solution               => '<ul><li>  
Follow solution from:  [1380873/KB159923] Create Accounting Cost Management Raises Error 0 Journal entry is unbalanced, and 95937 There are either non-accountable events existing in the system.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_95937');



debug('begin add_signature: CST_SLA_ERROR_0_EVENT_TYPE_ASSOCIATED_NOT_INCLUDED_INALL_AAD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7394',
      p_sig_id                 => 'CST_SLA_ERROR_0_EVENT_TYPE_ASSOCIATED_NOT_INCLUDED_INALL_AAD',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
and  encoded_msg like ''%The event type associated with the event is not included in all the application accounting definitions used by the ledgers in the Accounting Setup%''',
      p_title                  => 'Detail Error Records From SLA Error Number 0 in Case The Event Type Associated With The Event is not Included in All Application Accounting Definitions Used by The Ledgers in The Accounting Setup',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 or 95937',
      p_solution               => '<ul><li>  
Follow solution from:  [1100983/KB745583] Create Accounting Error 0 Event Type Not Included in Application Accounting Definition.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_EVENT_TYPE_ASSOCIATED_NOT_INCLUDED_INALL_AAD');



debug('begin add_signature: CST_SLA_ERROR_0_LAYER_COST_UPDATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7377',
      p_sig_id                 => 'CST_SLA_ERROR_0_LAYER_COST_UPDATE',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xe.event_id=xah.event_id
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_type_code LIKE ''%LAYER_COST_UPD%''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number = ''0''',
      p_title                  => 'Detail Error Records From SLA Error Number 0 for  Layer Cost Update',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 for Layer Cost Update transactions',
      p_solution               => '<ul><li>  
Follow solution from:  [1527157/KB788600] Create Accounting Error The Subledger Journal Entry Does Not Balance In The Entered Currency for Layer Cost Update source type.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_LAYER_COST_UPDATE');



debug('begin add_signature: CST_SLA_ERROR_0_WIP_PERIOD_CLOSE_VARIANCE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7260',
      p_sig_id                 => 'CST_SLA_ERROR_0_WIP_PERIOD_CLOSE_VARIANCE',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xe.event_id=xah.event_id
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xal.accounting_class_code like ''WIP VARIANCE''
and xe.event_type_code LIKE ''PERIOD_CLOSE_VARIANCE'' 
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number = ''0''',
      p_title                  => 'Detail Error Records From SLA Error Number 0 for  Period Close Variance',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 for Period Close Variance.',
      p_solution               => '<ul><li>  
Follow solution from: [1317791/KB766804] Create Accounting - Cost Management Warning for Event Class ''WIP Variance'' Subledger Journal Entry does not balance in the Entered Currency.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_WIP_PERIOD_CLOSE_VARIANCE');



debug('begin add_signature: CST_SLA_ERROR_95311_CYCLE_COUNT_ADJUSTEMENT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7391',
      p_sig_id                 => 'CST_SLA_ERROR_95311_CYCLE_COUNT_ADJUSTEMENT',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xe.event_id=xah.event_id
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_type_code LIKE ''cycle_count_adj''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number = ''95311''',
      p_title                  => 'Detail Error Records From SLA Error Number 95311 for  Cycle Count Adjustement Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 95311 for  Cycle Count Adjustment transactions.',
      p_solution               => '<ul><li>  
Follow solution from:  [1532500/KB742835] Create Accounting Error 95311 Cycle Count Adjustment Event Company Segment Value for the Account Code Combination not assigned to Ledger.
</li></ul>
',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95311_CYCLE_COUNT_ADJUSTEMENT');



debug('begin add_signature: CST_SLA_ERROR_95311_MISC_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7392',
      p_sig_id                 => 'CST_SLA_ERROR_95311_MISC_TRANSACTIONS',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xe.event_id=xah.event_id
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_type_code LIKE ''%MISC%''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number = ''95311''',
      p_title                  => 'Detail Error Records From SLA Error Number 95311 For  MISC Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are miscellaneous issue transactions which did not process due to error 95311.',
      p_solution               => '<ul><li>  
Follow solution note:  [781480/KB724016] Create Accounting Error 95311 Legal Entity/Company Segment Value for the Account Code Combination not Assigned to the Ledger.
</li></ul>
',
      p_success_msg            => '
',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95311_MISC_TRANSACTIONS');



debug('begin add_signature: CST_SLA_NOT_IN_GL_IMPORT_REFERENCES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '17860',
      p_sig_id                 => 'CST_SLA_NOT_IN_GL_IMPORT_REFERENCES',
      p_sig_sql                => 'SELECT h.group_id, l.ae_header_id, l.gl_sl_link_id, l.gl_sl_link_table
FROM xla_ae_lines l, xla_ae_headers h WHERE
l.application_id=h.application_id AND
l.ae_header_id=h.ae_header_id AND
h.application_id=200 AND
h.ledger_id= ##$$ledger_id$$## AND
h.upg_batch_id IS NULL AND
h.gl_transfer_status_code=''Y'' AND
h.accounting_entry_status_code=''F'' AND
h.accounting_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##'' AND
NOT EXISTS
(SELECT 1 FROM gl_import_references ir
WHERE ir.gl_sl_link_id=l.gl_sl_link_id AND
ir.gl_sl_link_table=l.gl_sl_link_table)',
      p_title                  => 'Inventory Transactions Not Posted To GL - Cannot Be Seen in GL_IMPORT_REFERENCES',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions that show as transferred to the general ledger from any sub ledger (AP,AR,..Inventory), but cannot be seen in the gl_import_references table.',
      p_solution               => 'Follow solution from: [818931/KB795425] Inventory Transactions Not Posted To GL - Cannot Be Seen in GL_IMPORT_REFERENCES',
      p_success_msg            => 'Inventory Transactions  posted to GL and can be seen in GL_IMPORT_REFERENCES table.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_NOT_IN_GL_IMPORT_REFERENCES');



debug('begin add_signature: CST_SLA_ERRED_CREATE_ACCOUNTING');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18701',
      p_sig_id                 => 'CST_SLA_ERRED_CREATE_ACCOUNTING',
      p_sig_sql                => 'SELECT eve.event_type_code,
ent.entity_code,
err.*
...
AND eve.entity_id = ent.entity_id',
      p_title                  => 'Create Accounting process ends in Error',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Create Accounting process ends in Error for the following rows.',
      p_solution               => '<ul><li>
These records need to be processed based on the error table details.<br>
Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number<br>
If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.
</li></ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERRED_CREATE_ACCOUNTING');



debug('begin add_signature: CST_PURGE_SLA_DIAG_HIGH_COUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19973',
      p_sig_id                 => 'CST_PURGE_SLA_DIAG_HIGH_COUNT',
      p_sig_sql                => 'select ''XLA_DIAG_SOURCES'' TABLE_NAME, count(*) RECORD_COUNT
from XLA_DIAG_SOURCES
union
select ''XLA_DIAG_EVENTS'', count(*)
from XLA_DIAG_EVENTS
group by 1',
      p_title                  => 'SLA Diagnostic Tables have High Record Count',
      p_fail_condition         => '[RECORD_COUNT] > [1000]',
      p_problem_descr          => 'The tables used by SLA diagnostics have high record counts. Keeping records in these tables may cause performance issues.',
      p_solution               => '<ul><li>The analyzer has identified the issue described in [1516562/KB533504].</li>
<li>Please review note for steps to purge these tables.</li></ul>
<li>If you need more assistance, please log a Service Request and upload this Create Accounting Analyzer output.</li>
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_PURGE_SLA_DIAG_HIGH_COUNT');



debug('begin add_signature: CST_SLA_ERROR_95261');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22300',
      p_sig_id                 => 'CST_SLA_ERROR_95261',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95261'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95261',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to Error 95261.',
      p_solution               => '<ul><li>  
Follow solution from: [1099995/KB783815] Create Accounting failed with Error 95261 - The account code combination was invalid
.</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_95261');



debug('begin add_signature: CST_SLA_AADS_NOT_VALIDATED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22450',
      p_sig_id                 => 'CST_SLA_AADS_NOT_VALIDATED',
      p_sig_sql                => 'select ''XLA_''||LPAD(application_id ,5 ,0)||
                  ''_AAD_''||product_rule_type_code||
                  ''_''||LPAD(product_rule_hash_id,6  ,0)||
                  ''_PKG'' "OBJECT_NAME" , PRODUCT_RULE_CODE,PRODUCT_RULE_TYPE_CODE,CREATION_DATE ,ENABLED_FLAG,COMPILE_STATUS_CODE,COMPILE_STATUS_DSP,NAME,DESCRIPTION,PRODUCT_RULE_HASH_ID
from XLA_PRODUCT_RULES_fvl
where APPLICATION_ID = 707
and COMPILE_STATUS_CODE=''N''',
      p_title                  => 'Application Accounting Definitions Not Validated',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Not Validated Application Accounting Definitions for Cost Management.
The child request Accounting Program (XLAACCUP) of main process Create Accounting -Cost Management errors when not validated AADS found:
...
<li>If you use listed AAD, review solution in [2153997/KB722830]</li>
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_AADS_NOT_VALIDATED');



debug('begin add_signature: EBS_MFG_CSTSUTA_CST_SLA_ERROR_95308');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22540',
      p_sig_id                 => 'EBS_MFG_CSTSUTA_CST_SLA_ERROR_95308',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
AND xah.GL_TRANSFER_STATUS_CODE=''N''
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''95308'')',
      p_title                  => 'Detail Error Records From SLA Error Number 95308',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to Error 95308.',
      p_solution               => '<ul><li>  
Follow solution from: [1999532/KB772030] Create Accounting - Cost Management Ends With Error 95308.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_CSTSUTA_CST_SLA_ERROR_95308');



debug('begin add_signature: CST_SLA_ERROR_0_XLA_INTER_BSV_NOT_ASSIGNED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12094',
      p_sig_id                 => 'CST_SLA_ERROR_0_XLA_INTER_BSV_NOT_ASSIGNED',
      p_sig_sql                => 'SELECT XE.event_type_code Event_Code,decode(XE.PROCESS_STATUS_CODE,''D'',''Draft'',''E'',''Error'',''I'',''Invalid'',''P'',''Processed'',
''R'',''Related event in error'',''U'',''Unprocessed'',''F'',''Final Mode'') PROCESS_STATUS,
decode(XE.EVENT_STATUS_CODE,''I'',''Incomplete'',''N'',''No action'',''P'',''Processed'',''U'',''Unprocessed'') EVENT_STATUS,
XAE.EVENT_ID,XAE.ENTITY_ID,XAE.LEDGER_ID,XAE.MESSAGE_NUMBER,XAE.ENCODED_MSG,XAE.AE_HEADER_ID,XAE.AE_LINE_NUM,XAE.APPLICATION_ID,XAE.CREATION_DATE
...
and xe.event_date BETWEEN ''##$$start_date$$##'' and ''##$$end_date$$##''
and xae.message_number in (''0'')
and xae.encoded_msg like ''%The subledger journal entry contains lines with balancing segment values that are not assigned to any legal entity%''',
      p_title                  => 'Detail Error Records From SLA Error Number 0 With The subledger journal entry contains lines with balancing segment values that are not assigned to any legal entity',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are SLA records which did not process due to error 0 "The subledger journal entry contains lines with balancing segment values that are not assigned to any legal entity."
',
      p_solution               => '<ul><li>  
Follow solution from:  [2504350/KB764594] Create Accounting - Cost Management Ends With Error "The subledger journal entry contains lines with balancing segment values that are not assigned to any legal entity.
</li></ul>',
      p_success_msg            => 'No Error records found in SLA tables and hence no action is required.
',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_ERROR_0_XLA_INTER_BSV_NOT_ASSIGNED');



debug('begin add_signature: CST_SLA_MTL_TRANSACTION_ACCOUNTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7172',
      p_sig_id                 => 'CST_SLA_MTL_TRANSACTION_ACCOUNTS',
      p_sig_sql                => 'SELECT TRANSACTION_ID,REFERENCE_ACCOUNT,CREATION_DATE,INVENTORY_ITEM_ID,ORGANIZATION_ID,TRANSACTION_DATE,
       TRANSACTION_SOURCE_TYPE_ID,ACCOUNTING_LINE_TYPE,BASE_TRANSACTION_VALUE,INV_SUB_LEDGER_ID
          ...
          (SELECT ae_header_id
           ...
           and event_id IN
              (SELECT event_id
               ...
               AND event_status_code   <> ''P''
               AND process_status_code <> ''P'')))',
      p_title                  => 'Display  Related Data From MTL_TRANSACTION_ACCOUNTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the MTL_TRANSACTION_ACCOUNTS table. Display related Data from MTL_TRANSACTION_ACCOUNTS table.',
      p_solution               => '<ul><li>
    These records need to be processed based on the error table details.<br>
    Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number.<br>
    If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.
...
',
      p_success_msg            => 'No unprocessed records found in the MTL_TRANSACTION_ACCOUNTS table.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_MTL_TRANSACTION_ACCOUNTS');



debug('begin add_signature: CST_SLA_MTL_MATERIAL_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7173',
      p_sig_id                 => 'CST_SLA_MTL_MATERIAL_TRANSACTIONS',
...
FROM MTL_MATERIAL_TRANSACTIONS mmt , MTL_TRANSACTION_ACCOUNTS mta, xla_events xe ,xla_ae_headers xah, 
...
and xdl.source_distribution_id_num_1 = mta.inv_sub_ledger_id',
      p_title                  => 'Display  Related Data From MTL_MATERIAL_TRANSACTIONS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the MTL_MATERIAL_TRANSACTIONS table. Display related Data from MTL_MATERIAL_TRANSACTIONS table.',
      p_solution               => '<ul><li>
    These records need to be processed based on the error table details.<br>
    Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number<br>
    If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.
...
',
      p_success_msg            => 'No unprocessed records found in the MTL_MATERIAL_TRANSACTIONS table. There are no transactions that need review in Material Transactions (MTL_MATERIAL_TRANSACTIONS MMT table).
mtl_material_transactions.costed_flag is Null ',
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_MTL_MATERIAL_TRANSACTIONS');



debug('begin add_signature: CST_SLA_WIP_TRANSACTION_ACCOUNTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7174',
      p_sig_id                 => 'CST_SLA_WIP_TRANSACTION_ACCOUNTS',
      p_sig_sql                => 'SELECT TRANSACTION_ID,REFERENCE_ACCOUNT,CREATION_DATE,ORGANIZATION_ID,TRANSACTION_DATE,
       WIP_ENTITY_ID,ACCOUNTING_LINE_TYPE,BASE_TRANSACTION_VALUE,WIP_SUB_LEDGER_ID
FROM WIP_TRANSACTION_ACCOUNTS
WHERE wip_sub_ledger_id IN
(SELECT source_distribution_id_num_1
FROM XLA_DISTRIBUTION_LINKS
WHERE application_id = 707
...
AND event_status_code <> ''P''
AND process_status_code <> ''P'')))',
      p_title                  => 'Display  Related Data From WIP_TRANSACTION_ACCOUNTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the WIP_TRANSACTION_ACCOUNTS table. Display related Data from WIP_TRANSACTION_ACCOUNTS  table.',
      p_solution               => '<ul><li>
    These records need to be processed based on the error table details.<br>
    Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number.<br>
    If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.
...
',
      p_success_msg            => 'No unprocessed records found in the WIP_TRANSACTION_ACCOUNTS table. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_WIP_TRANSACTION_ACCOUNTS');



debug('begin add_signature: CST_SLA_WIP_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7175',
      p_sig_id                 => 'CST_SLA_WIP_TRANSACTIONS',
      p_sig_sql                => 'SELECT TRANSACTION_ID,CREATION_DATE,ORGANIZATION_ID,WIP_ENTITY_ID,TRANSACTION_TYPE,TRANSACTION_DATE
               ...
               (SELECT ae_header_id
                ...
                AND event_id IN
                    (SELECT event_id
                     ...
                     AND event_status_code   <> ''P''
                     AND process_status_code <> ''P''))))',
      p_title                  => 'Display  Related Data From WIP_TRANSACTIONS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed relatedrecords found in the WIP_TRANSACTIONS table. Display related Data from WIP_TRANSACTIONS table.',
      p_solution               => '<ul><li>
    These records need to be processed based on the error table details.<br>
    Review [2005838/KB763584] SLA Create Accounting -Cost Management Common Error Code/Number.<br>
    If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.
...
',
      p_success_msg            => 'No unprocessed records found in the WIP_TRANSACTIONS table.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_WIP_TRANSACTIONS');



debug('begin add_signature: CST_SLA_RCV_RECEIVING_SUB_LEDGER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7177',
      p_sig_id                 => 'CST_SLA_RCV_RECEIVING_SUB_LEDGER',
      p_sig_sql                => 'SELECT CREATION_DATE,RCV_TRANSACTION_ID,JE_SOURCE_NAME,JE_CATEGORY_NAME,SET_OF_BOOKS_ID,
 	ACCOUNTING_DATE,CODE_COMBINATION_ID,ACCOUNTED_DR,ACCOUNTED_CR,TRANSACTION_DATE,PERIOD_NAME
      ...
      (SELECT source_distribution_id_num_1
       ...
       AND ae_header_id IN
           (SELECT ae_header_id
            ...
            WHERE application_id = 707
            and ledger_id = ##$$ledger_id$$##
            and je_category_name = ''Receiving''
            AND event_id IN
               (SELECT event_id
                FROM xla_events
                WHERE application_id = 707
                ...
                AND event_status_code   <> ''P''
                AND process_status_code <> ''P'')))',
      p_title                  => 'Display  Related Data From RCV_RECEIVING_SUB_LEDGER',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the RCV_RECEIVING_SUB_LEDGER table. Display related Data from RCV_RECEIVING_SUB_LEDGER table.',
      p_solution               => '<ul><li>
These records need to be processed based on the error table details. If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.<br>
Review [802966/KB626348] SLA Data Flow and Table Links.
</li></ul>
',
      p_success_msg            => 'No unprocessed records found in the RCV_RECEIVING_SUB_LEDGER table.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_RCV_RECEIVING_SUB_LEDGER');



debug('begin add_signature: CST_SLA_RCV_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7178',
      p_sig_id                 => 'CST_SLA_RCV_TRANSACTIONS',
      p_sig_sql                => 'SELECT TRANSACTION_ID,TRANSACTION_TYPE,TRANSACTION_DATE,SHIPMENT_HEADER_ID,SHIPMENT_LINE_ID,INV_TRANSACTION_ID,
       PO_HEADER_ID,PO_DISTRIBUTION_ID,WIP_ENTITY_ID
       FROM RCV_TRANSACTIONS
       WHERE transaction_id
       IN(SELECT rcv_transaction_id
       FROM RCV_RECEIVING_SUB_LEDGER
       WHERE rcv_sub_ledger_id IN
            (SELECT source_distribution_id_num_1
             FROM XLA_DISTRIBUTION_LINKS
               ...
               (SELECT ae_header_id
                ...
                AND event_id IN
                  (SELECT event_id
                   ...
                   AND event_status_code   <> ''P''
                   AND process_status_code <> ''P''))))',
      p_title                  => 'Display  Related Data From RCV_TRANSACTIONS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the RCV_TRANSACTIONS table. Display related Data from RCV_TRANSACTIONS table.',
      p_solution               => '<ul><li>
These records need to be processed based on the error table details. If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.<br>
...
',
      p_success_msg            => 'No unprocessed records found in the RCV_TRANSACTIONS table.
',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_RCV_TRANSACTIONS');



debug('begin add_signature: CST_SLA_RCV_ACCOUNTING_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7179',
      p_sig_id                 => 'CST_SLA_RCV_ACCOUNTING_EVENTS',
      p_sig_sql                => 'SELECT ACCOUNTING_EVENT_ID,LAST_UPDATE_DATE,CREATION_DATE,RCV_TRANSACTION_ID,EVENT_TYPE_ID,EVENT_SOURCE,
       EVENT_SOURCE_ID,SET_OF_BOOKS_ID,ORG_ID,TRANSFER_ORG_ID,ORGANIZATION_ID,TRANSFER_ORGANIZATION_ID,
	   DEBIT_ACCOUNT_ID,CREDIT_ACCOUNT_ID,TRANSACTION_DATE	
             ...
             (SELECT ae_header_id
              ...
              AND event_id IN
                 (SELECT event_id
                  ...
                  AND event_status_code   <> ''P''
                  AND process_status_code <> ''P''))))',
      p_title                  => 'Display  Related Data From RCV_ACCOUNTING_EVENTS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Non Processed related records found in the RCV_ACCOUNTING_EVENTS table. Display related Data from RCV_ACCOUNTING_EVENTS table.',
      p_solution               => '<ul><li>
These records need to be processed based on the error table details. If you still need assistance, please log a Service Request and upload this Create Accounting-Cost Management Analyzer output.<br>
Review [802966/KB626348] SLA Data Flow and Table Links.
</li></ul>
',
      p_success_msg            => 'No unprocessed records found in the RCV_ACCOUNTING_EVENTS table.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_SLA_RCV_ACCOUNTING_EVENTS');
...
debug('begin add_signature: CST_XLA_POLICY_FUNCTION_XLA_TRANSACTION_ENTITIES AND XLA_EVENTS');
   l_info('##MASK##1'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15260',
      p_sig_id                 => 'CST_XLA_POLICY_FUNCTION_XLA_TRANSACTION_ENTITIES AND XLA_EVENTS',
      p_sig_sql                => 'select OBJECT_OWNER,OBJECT_NAME,POLICY_GROUP,POLICY_NAME,PACKAGE,FUNCTION FUNCTION,SEL,INS,UPD,DEL,IDX,CHK_OPTION,ENABLE,STATIC_POLICY,POLICY_TYPE,LONG_PREDICATE
from dba_policies 
where object_name IN ( ''XLA_TRANSACTION_ENTITIES'',''XLA_EVENTS'' )',
      p_title                  => 'Function Policy for XLA_TRANSACTION_ENTITIES and XLA_EVENTS',
      p_fail_condition         => '[FUNCTION] not in [MO_POLICY, XLA_STANDARD_POLICY]',
      p_problem_descr          => '''Policy function'' setup in Subledger Application should be finally same as ''Security function''',
      p_solution               => 'See [1612567/KB727691] for details.',
      p_success_msg            => 'Function Policies are correctly defined.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: CST_XLA_POLICY_FUNCTION_XLA_TRANSACTION_ENTITIES AND XLA_EVENTS');
...
BEGIN
  ...
  analyzer_title := 'Create Accounting-Cost Management Analyzer';
...
debug('begin section: XLA_Accounting_Details');
start_section('XLA Accounting Details', 'XLA_Accounting_Details');
   set_item_result(run_stored_sig('CST_SLA_EVENT_DETAILS'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_DETAILS'));
   set_item_result(run_stored_sig('CST_SLA_PENDING_TRANSFER_TO_G'));
   set_item_result(run_stored_sig('CST_SLA_TRANSACTION_ENTITIES'));
   set_item_result(run_stored_sig('CST_SLA_XLA_AE_HEADERS'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_WIP'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95277'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95283'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95288'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95301'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95318'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95325'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95333'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95349'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95353'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95937'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_95314_THE_ACCOUNTED_AMOUNT_HAS_DIFFERENT_SIGN'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_95937'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_EVENT_TYPE_ASSOCIATED_NOT_INCLUDED_INALL_AAD'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_LAYER_COST_UPDATE'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_WIP_PERIOD_CLOSE_VARIANCE'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95311_CYCLE_COUNT_ADJUSTEMENT'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95311_MISC_TRANSACTIONS'));
   set_item_result(run_stored_sig('CST_SLA_NOT_IN_GL_IMPORT_REFERENCES'));
   set_item_result(run_stored_sig('CST_SLA_ERRED_CREATE_ACCOUNTING'));
   set_item_result(run_stored_sig('CST_PURGE_SLA_DIAG_HIGH_COUNT'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_95261'));
   set_item_result(run_stored_sig('CST_SLA_AADS_NOT_VALIDATED'));
   set_item_result(run_stored_sig('EBS_MFG_CSTSUTA_CST_SLA_ERROR_95308'));
   set_item_result(run_stored_sig('CST_SLA_ERROR_0_XLA_INTER_BSV_NOT_ASSIGNED'));
end_section;
debug('end section: XLA_Accounting_Details');

debug('begin section: Inventory_Transaction_Details');
start_section('Inventory Transaction Details', 'Inventory_Transaction_Details');
   set_item_result(run_stored_sig('CST_SLA_MTL_TRANSACTION_ACCOUNTS'));
   set_item_result(run_stored_sig('CST_SLA_MTL_MATERIAL_TRANSACTIONS'));
end_section;
debug('end section: Inventory_Transaction_Details');

debug('begin section: WIP_Transaction_Details');
start_section('WIP Transaction Details', 'WIP_Transaction_Details');
   set_item_result(run_stored_sig('CST_SLA_WIP_TRANSACTION_ACCOUNTS'));
   set_item_result(run_stored_sig('CST_SLA_WIP_TRANSACTIONS'));
end_section;
debug('end section: WIP_Transaction_Details');

debug('begin section: Receiving_Transaction_Details');
start_section('Receiving Transaction Details', 'Receiving_Transaction_Details');
   set_item_result(run_stored_sig('CST_SLA_RCV_RECEIVING_SUB_LEDGER'));
   set_item_result(run_stored_sig('CST_SLA_RCV_TRANSACTIONS'));
   set_item_result(run_stored_sig('CST_SLA_RCV_ACCOUNTING_EVENTS'));
end_section;
debug('end section: Receiving_Transaction_Details');
...
start_section('Proactive Recommendations', 'Proactive_Recommendations');
   ...
   set_item_result(run_stored_sig('CST_XLA_POLICY_FUNCTION_XLA_TRANSACTION_ENTITIES AND XLA_EVENTS'));
end_section;
