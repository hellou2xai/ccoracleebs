...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Work in Process (WIP) Analyzer         |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE wip_conf_analyzer_pkg AUTHID DEFINER AS
...
END wip_conf_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY wip_conf_analyzer_pkg AS
-- $Id: wip_analyzer.sql, 200.6 2026/01/28 13:12:47 dbuteica Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2931244.1));
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'WIP_CONF_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'WIP_CONF_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>WIP_CONF Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2931244.1#feedback");
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/mosc/discussion/" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">9016</span>
...
<p>The EBS Work in Process (WIP) Analyzer (Note 2931244.1) is a self-service health-check script that reviews the overall footprint, analyzes current configurations and settings for the environment and provides feedback and recommendations on best practices.</p><p>200.5 MOSFS links</p>
            <br><br>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,9016,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,9016,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},9016,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
debug('begin populate parameters hash table');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Discrete Job Number or Work Order Number';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_Job_Number,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/01/28 13:12:46 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'wip_analyzer.sql';
...
debug('begin parameter validation: p_Job_Number');
IF p_Job_Number IS NOT NULL THEN
BEGIN
SELECT WIP_ENTITY_NAME
INTO l_exists_val
FROM (SELECT WIP_ENTITY_NAME FROM WIP_ENTITIES WHERE ENTITY_TYPE IN (1,3))
WHERE WIP_ENTITY_NAME = p_Job_Number and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Invalid Discrete Job Number parameter provided.  The job related data extraction will not be executed.');
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_Job_Number failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
debug('end parameter validation: p_Job_Number');
...
IF p_Job_Number is not null THEN
    BEGIN
        ...
        INTO   l_t_wip_entity_id
        FROM   WIP_ENTITIES
        WHERE  wip_entity_name = to_char(p_Job_Number)
        AND    ENTITY_TYPE IN (1,3)
        AND    rownum = 1;

    EXCEPTION 
        WHEN NO_DATA_FOUND THEN
            print_log('Invalid Job Number');
            raise invalid_parameters;
      END;
  END IF;
...
debug('begin populate sql tokens hash table');
   ...
   g_sql_tokens('##$$t_wip_entity_id$$##') := l_t_wip_entity_id;
   g_masked_tokens('##$$t_wip_entity_id$$##') := mask_text( g_sql_tokens('##$$t_wip_entity_id$$##'), 'NO_MASK');
...
debug('begin add_signature: EBS_MFG_DM_WIP_PARAMETERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25027',
      p_sig_id                 => 'EBS_MFG_DM_WIP_PARAMETERS',
      p_sig_sql                => 'SELECT WP.ORGANIZATION_ID ORG_ID,
       ...
       DEFAULT_PULL_SUPPLY_LOCATOR_ID SUP_LOC_ID,
       (SELECT CONCATENATED_SEGMENTS
          FROM MTL_ITEM_LOCATIONS_KFV
         WHERE INVENTORY_LOCATION_ID = DEFAULT_PULL_SUPPLY_LOCATOR_ID) SUP_LOCATOR,
 ...
 WHERE WP.ORGANIZATION_ID = MP.ORGANIZATION_ID (+)
   ...
   ORDER BY 2',
      p_title                  => 'WIP Parameters',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'List of WIP organization parameters found.',
      p_solution               => '',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_WIP_PARAMETERS');



debug('begin add_signature: EBS_MFG_DM_INV_PARAMETERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25028',
      p_sig_id                 => 'EBS_MFG_DM_INV_PARAMETERS',
      p_sig_sql                => 'SELECT MP.ORGANIZATION_ID   ORG_ID,
       ...
       MP.COST_CUTOFF_DATE,
       MP.CALENDAR_CODE,
       cctav.cost_type "Average Rates Cost Type",
       MP.WMS_ENABLED_FLAG,
       MP.QA_SKIPPING_INSP_FLAG
  FROM MTL_PARAMETERS MP, 
       cst_cost_types_all_v cctav
 WHERE MP.avg_rates_cost_type_id = cctav.cost_type_id(+)
   AND MP.ORGANIZATION_ID IN (select organization_id from wip_parameters)
 ORDER BY ORGANIZATION_CODE',
      p_title                  => 'INV Parameters for WIP Organizations',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'List of INV Organization Parameters for WIP Organizations.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_INV_PARAMETERS');



debug('begin add_signature: EBS_MFG_DM_SUBINVENTORIES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25029',
      p_sig_id                 => 'EBS_MFG_DM_SUBINVENTORIES',
 ...
 WHERE MP.ORGANIZATION_ID = MSV.ORGANIZATION_ID (+)
   AND MP.ORGANIZATION_ID in (select organization_id from wip_parameters)
 ORDER BY 2,3',
      p_title                  => 'List of Subinventories for WIP Organizations',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'List of Subinventories for WIP organizations.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_SUBINVENTORIES');
...
debug('begin add_signature: FC_WIP_PENDING_MOVE_TNX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10468',
      p_sig_id                 => 'FC_WIP_PENDING_MOVE_TNX',
      p_sig_sql                => 'SELECT WE.WIP_ENTITY_NAME,
       WMTI.TRANSACTION_ID,
       ...
       DECODE(WMTI.TRANSACTION_TYPE,
              ...
              2,              ''COMPLETE'',
              3,              ''RETURN'',
              WMTI.TRANSACTION_TYPE) TRANSACTION_TYPE_MEANING,
       DECODE(WMTI.FM_INTRAOPERATION_STEP_TYPE,
              ...
              4,              ''REJECT'',
              5,              ''SCRAP'',
              WMTI.FM_INTRAOPERATION_STEP_TYPE) FM_OP_STEP,
       WMTI.TO_OPERATION_CODE TO_OP_CODE,
       DECODE(WMTI.TO_INTRAOPERATION_STEP_TYPE,
              1,              ''QUEUE'',
              2,              ''RUN'',
              3,              ''TOMOVE'',
              4,              ''REJECT'',
              5,              ''SCRAP'',
              WMTI.TO_INTRAOPERATION_STEP_TYPE) TO_OP_STEP,
 ...
 WHERE WMTI.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
   ...
   AND ((WMTI.PROCESS_STATUS = 3) 
           OR (WMTI.PROCESS_STATUS in (1,2) 
           AND WMTI.CREATION_DATE < sysdate - 0.25 ))
 ORDER BY 1,2',
      p_title                  => 'WIP Pending Move Transactions',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are erred WIP move transaction and pending/running transactions that are not processed for more than 6 hours.',
      p_solution               => 'For records returning PROCESS_STATUS = ERROR, visit the following document: [457066/KB627612]
For records returning PROCESS_STATUS = PENDING, check if the Move Manager is active and visit the following document: [2102653/KB753608]
For records returning PROCESS_STATUS = ERROR and PROCESS_PHASE = BACKFLUSH SETUP, see Errors document attached in Note: [1311963/KB664314]',
      p_success_msg            => 'No erred WIP move transactions or pending/running transactions that are not processed for more than 6 hours.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FC_WIP_PENDING_MOVE_TNX');



debug('begin add_signature: INVALID SPACE IN WIP_DISCRETE_JOBS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '11624',
      p_sig_id                 => 'INVALID SPACE IN WIP_DISCRETE_JOBS',
      p_sig_sql                => 'select wip_entity_id,
SOURCE_CODE ,
decode(SOURCE_CODE,LTRIM(RTRIM(SOURCE_CODE)), ''No Space Found'', ''Space Found'') SOURCE_CODE_SPACE,
DESCRIPTION ,
decode(DESCRIPTION,LTRIM(RTRIM(DESCRIPTION)), ''No Space Found'', ''Space Found'') DESCRIPTION_SPACE,
CLASS_CODE ,
decode(CLASS_CODE,LTRIM(RTRIM(CLASS_CODE)), ''No Space Found'', ''Space Found'') CLASS_CODE_SPACE,
BOM_REVISION ,
decode(BOM_REVISION,LTRIM(RTRIM(BOM_REVISION)), ''No Space Found'', ''Space Found'') BOM_REVISION_SPACE,
ROUTING_REVISION ,
decode(ROUTING_REVISION,LTRIM(RTRIM(ROUTING_REVISION)), ''No Space Found'', ''Space Found'') ROUTING_REVISION_SPACE,
LOT_NUMBER ,
decode(LOT_NUMBER,LTRIM(RTRIM(LOT_NUMBER)), ''No Space Found'', ''Space Found'') LOT_NUMBER_SPACE,
ALTERNATE_BOM_DESIGNATOR ,
decode(ALTERNATE_BOM_DESIGNATOR,LTRIM(RTRIM(ALTERNATE_BOM_DESIGNATOR)), ''No Space Found'', ''Space Found'') ALTERNATE_BOM_DESIGNATOR_SPACE,
ALTERNATE_ROUTING_DESIGNATOR ,
decode(ALTERNATE_ROUTING_DESIGNATOR,LTRIM(RTRIM(ALTERNATE_ROUTING_DESIGNATOR)), ''No Space Found'', ''Space Found'') ALT_ROUTING_DESIGNATOR_SPACE,
COMPLETION_SUBINVENTORY ,
decode(COMPLETION_SUBINVENTORY,LTRIM(RTRIM(COMPLETION_SUBINVENTORY)), ''No Space Found'', ''Space Found'') COMPLETION_SUBINVENTORY_SPACE,
DEMAND_CLASS ,
decode(DEMAND_CLASS,LTRIM(RTRIM(DEMAND_CLASS)), ''No Space Found'', ''Space Found'') DEMAND_CLASS_SPACE,
ATTRIBUTE_CATEGORY ,
decode(ATTRIBUTE_CATEGORY,LTRIM(RTRIM(ATTRIBUTE_CATEGORY)), ''No Space Found'', ''Space Found'')ATTRIBUTE_CATEGORY_SPACE,
ATTRIBUTE1 ,
decode(ATTRIBUTE1,LTRIM(RTRIM(ATTRIBUTE1)), ''No Space Found'', ''Space Found'') ATTRIBUTE1_SPACE,
ATTRIBUTE2 ,
decode(ATTRIBUTE2,LTRIM(RTRIM(ATTRIBUTE2)), ''No Space Found'', ''Space Found'') ATTRIBUTE2_SPACE,
ATTRIBUTE3 ,
decode(ATTRIBUTE3,LTRIM(RTRIM(ATTRIBUTE3)), ''No Space Found'', ''Space Found'') ATTRIBUTE3_SPACE,
ATTRIBUTE4 ,
decode(ATTRIBUTE4,LTRIM(RTRIM(ATTRIBUTE4)), ''No Space Found'', ''Space Found'') ATTRIBUTE4_SPACE,
ATTRIBUTE5 ,
decode(ATTRIBUTE5,LTRIM(RTRIM(ATTRIBUTE5)), ''No Space Found'', ''Space Found'') ATTRIBUTE5_SPACE,
ATTRIBUTE6 ,
decode(ATTRIBUTE6,LTRIM(RTRIM(ATTRIBUTE6)), ''No Space Found'', ''Space Found'') ATTRIBUTE6_SPACE,
ATTRIBUTE7 ,
decode(ATTRIBUTE7,LTRIM(RTRIM(ATTRIBUTE7)), ''No Space Found'', ''Space Found'') ATTRIBUTE7_SPACE,
ATTRIBUTE8 ,
decode(ATTRIBUTE8,LTRIM(RTRIM(ATTRIBUTE8)), ''No Space Found'', ''Space Found'') ATTRIBUTE8_SPACE,
ATTRIBUTE9 ,
decode(ATTRIBUTE9,LTRIM(RTRIM(ATTRIBUTE9)), ''No Space Found'', ''Space Found'') ATTRIBUTE9_SPACE,
ATTRIBUTE10 ,
decode(ATTRIBUTE10,LTRIM(RTRIM(ATTRIBUTE10)), ''No Space Found'', ''Space Found'') ATTRIBUTE10_SPACE,
ATTRIBUTE11 ,
decode(ATTRIBUTE11,LTRIM(RTRIM(ATTRIBUTE11)), ''No Space Found'', ''Space Found'') ATTRIBUTE11_SPACE,
ATTRIBUTE12 ,
decode(ATTRIBUTE12,LTRIM(RTRIM(ATTRIBUTE12)), ''No Space Found'', ''Space Found'') ATTRIBUTE12_SPACE,
ATTRIBUTE13 ,
decode(ATTRIBUTE13,LTRIM(RTRIM(ATTRIBUTE13)), ''No Space Found'', ''Space Found'') ATTRIBUTE13_SPACE,
ATTRIBUTE14 ,
decode(ATTRIBUTE14,LTRIM(RTRIM(ATTRIBUTE14)), ''No Space Found'', ''Space Found'') ATTRIBUTE14_SPACE,
ATTRIBUTE15 ,
decode(ATTRIBUTE15,LTRIM(RTRIM(ATTRIBUTE15)), ''No Space Found'', ''Space Found'') ATTRIBUTE15_SPACE,
END_ITEM_UNIT_NUMBER ,
decode(END_ITEM_UNIT_NUMBER,LTRIM(RTRIM(END_ITEM_UNIT_NUMBER)), ''No Space Found'', ''Space Found'') END_ITEM_UNIT_NUMBER_SPACE,
ASSET_NUMBER ,
decode(ASSET_NUMBER,LTRIM(RTRIM(ASSET_NUMBER)), ''No Space Found'', ''Space Found'') ASSET_NUMBER_SPACE,
REBUILD_SERIAL_NUMBER ,
decode(REBUILD_SERIAL_NUMBER,LTRIM(RTRIM(REBUILD_SERIAL_NUMBER)), ''No Space Found'', ''Space Found'') REBUILD_SERIAL_NUMBER_SPACE,
MANUAL_REBUILD_FLAG ,
decode(MANUAL_REBUILD_FLAG,LTRIM(RTRIM(MANUAL_REBUILD_FLAG)), ''No Space Found'', ''Space Found'') MANUAL_REBUILD_FLAG_SPACE,
SHUTDOWN_TYPE ,
decode(SHUTDOWN_TYPE,LTRIM(RTRIM(SHUTDOWN_TYPE)), ''No Space Found'', ''Space Found'') SHUTDOWN_TYPE_SPACE,
NOTIFICATION_REQUIRED ,
decode(NOTIFICATION_REQUIRED,LTRIM(RTRIM(NOTIFICATION_REQUIRED)), ''No Space Found'', ''Space Found'') NOTIFICATION_REQUIRED_SPACE,
WORK_ORDER_TYPE ,
decode(WORK_ORDER_TYPE,LTRIM(RTRIM(WORK_ORDER_TYPE)), ''No Space Found'', ''Space Found'') WORK_ORDER_TYPE_SPACE,
ACTIVITY_TYPE ,
decode(ACTIVITY_TYPE,LTRIM(RTRIM(ACTIVITY_TYPE)), ''No Space Found'', ''Space Found'') ACTIVITY_TYPE_SPACE,
ACTIVITY_CAUSE ,
decode(ACTIVITY_CAUSE,LTRIM(RTRIM(ACTIVITY_CAUSE)), ''No Space Found'', ''Space Found'') ACTIVITY_CAUSE_SPACE,
TAGOUT_REQUIRED ,
decode(TAGOUT_REQUIRED,LTRIM(RTRIM(TAGOUT_REQUIRED)), ''No Space Found'', ''Space Found'') TAGOUT_REQUIRED_SPACE,
PLAN_MAINTENANCE ,
decode(PLAN_MAINTENANCE,LTRIM(RTRIM(PLAN_MAINTENANCE)), ''No Space Found'', ''Space Found'') PLAN_MAINTENANCE_SPACE,
ACTIVITY_SOURCE ,
decode(ACTIVITY_SOURCE,LTRIM(RTRIM(ACTIVITY_SOURCE)), ''No Space Found'', ''Space Found'') ACTIVITY_SOURCE_SPACE,
MATERIAL_ISSUE_BY_MO ,
decode(MATERIAL_ISSUE_BY_MO,LTRIM(RTRIM(MATERIAL_ISSUE_BY_MO)), ''No Space Found'', ''Space Found'') MATERIAL_ISSUE_BY_MO_SPACE,
ISSUE_ZERO_COST_FLAG ,
decode(ISSUE_ZERO_COST_FLAG,LTRIM(RTRIM(ISSUE_ZERO_COST_FLAG)), ''No Space Found'', ''Space Found'') ISSUE_ZERO_COST_FLAG_SPACE
from wip_discrete_jobs
where SOURCE_CODE <> LTRIM(RTRIM(SOURCE_CODE))
or DESCRIPTION <> LTRIM(RTRIM(DESCRIPTION))
or LOT_NUMBER <> LTRIM(RTRIM(LOT_NUMBER))
or CLASS_CODE <> LTRIM(RTRIM(CLASS_CODE))
or BOM_REVISION <> LTRIM(RTRIM(BOM_REVISION))
or ROUTING_REVISION <> LTRIM(RTRIM(ROUTING_REVISION))
or ALTERNATE_BOM_DESIGNATOR <> LTRIM(RTRIM(ALTERNATE_BOM_DESIGNATOR))
or ALTERNATE_ROUTING_DESIGNATOR <> LTRIM(RTRIM(ALTERNATE_ROUTING_DESIGNATOR))
or COMPLETION_SUBINVENTORY <> LTRIM(RTRIM(COMPLETION_SUBINVENTORY))
or DEMAND_CLASS <> LTRIM(RTRIM(DEMAND_CLASS))
or ATTRIBUTE_CATEGORY <> LTRIM(RTRIM(ATTRIBUTE_CATEGORY))
or ATTRIBUTE1 <> LTRIM(RTRIM(ATTRIBUTE1))
or ATTRIBUTE2 <> LTRIM(RTRIM(ATTRIBUTE2))
or ATTRIBUTE3 <> LTRIM(RTRIM(ATTRIBUTE3))
or ATTRIBUTE4 <> LTRIM(RTRIM(ATTRIBUTE4))
or ATTRIBUTE5 <> LTRIM(RTRIM(ATTRIBUTE5))
or ATTRIBUTE6 <> LTRIM(RTRIM(ATTRIBUTE6))
or ATTRIBUTE7 <> LTRIM(RTRIM(ATTRIBUTE7))
or ATTRIBUTE8 <> LTRIM(RTRIM(ATTRIBUTE8))
or ATTRIBUTE9 <> LTRIM(RTRIM(ATTRIBUTE9))
or ATTRIBUTE10 <> LTRIM(RTRIM(ATTRIBUTE10))
or ATTRIBUTE11 <> LTRIM(RTRIM(ATTRIBUTE11))
or ATTRIBUTE12 <> LTRIM(RTRIM(ATTRIBUTE12))
or ATTRIBUTE13 <> LTRIM(RTRIM(ATTRIBUTE13))
or ATTRIBUTE14 <> LTRIM(RTRIM(ATTRIBUTE14))
or ATTRIBUTE15 <> LTRIM(RTRIM(ATTRIBUTE15))
or END_ITEM_UNIT_NUMBER <> LTRIM(RTRIM(END_ITEM_UNIT_NUMBER))
or ASSET_NUMBER <> LTRIM(RTRIM(ASSET_NUMBER))
or REBUILD_SERIAL_NUMBER <> LTRIM(RTRIM(REBUILD_SERIAL_NUMBER))
or MANUAL_REBUILD_FLAG <> LTRIM(RTRIM(MANUAL_REBUILD_FLAG))
or SHUTDOWN_TYPE <> LTRIM(RTRIM(SHUTDOWN_TYPE))
or NOTIFICATION_REQUIRED <> LTRIM(RTRIM(NOTIFICATION_REQUIRED))
or WORK_ORDER_TYPE <> LTRIM(RTRIM(WORK_ORDER_TYPE))
or ACTIVITY_TYPE <> LTRIM(RTRIM(ACTIVITY_TYPE))
or ACTIVITY_CAUSE <> LTRIM(RTRIM(ACTIVITY_CAUSE))
or TAGOUT_REQUIRED <> LTRIM(RTRIM(TAGOUT_REQUIRED))
or PLAN_MAINTENANCE <> LTRIM(RTRIM(PLAN_MAINTENANCE))
or ACTIVITY_SOURCE <> LTRIM(RTRIM(ACTIVITY_SOURCE))
or MATERIAL_ISSUE_BY_MO <> LTRIM(RTRIM(MATERIAL_ISSUE_BY_MO))
or ISSUE_ZERO_COST_FLAG <> LTRIM(RTRIM(ISSUE_ZERO_COST_FLAG))',
      p_title                  => 'Invalid Leading or Trailing Space In WIP_DISCRETE_JOBS Table Columns',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Rows returned by the SQL above indicate that wip_discrete_jobs table is including invalid spaces. This causes FRM-40654 error when updating a job.',
      p_solution               => 'Review solution steps in [2477211/KB758285] Script To Find Invalid Space Of WIP_DISCRETE_JOBS To Avoid FRM-40654.',
      p_success_msg            => 'There is no corrupted job.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: INVALID SPACE IN WIP_DISCRETE_JOBS');



debug('begin add_signature: EBS_MFG_DM_CLS_JOB_OPEN_MO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26877',
      p_sig_id                 => 'EBS_MFG_DM_CLS_JOB_OPEN_MO',
      p_sig_sql                => 'SELECT wdj.wip_entity_id,
  we.wip_entity_name,
  we.organization_id,
  we.entity_type,
  wdj.status_type,
  ...
  wdj.date_completed,
  wdj.date_closed,
  mtrh.HEADER_ID MO_Header_id,
  mtrh.REQUEST_NUMBER MO_REQUEST_NUMBER,
  mtrl.line_ID MO_LINE_ID,
  mtrl.line_number MO_LINE_NUMBER,
  mtrl.line_status MO_LINE_STATUS
...
',
      p_title                  => 'Failed Close Jobs due to Open Move Orders',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Failed Close Discrete jobs (wip_discrete_jobs.status_type = 15) because associated move orders are in open status.',
      p_solution               => 'Review [1516429/KB771774] and close the open lines in the move orders and try to close the jobs again.',
      p_success_msg            => 'There is no problematic job.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_CLS_JOB_OPEN_MO');



debug('begin add_signature: EBS_MFG_DM_DJ_PENDING_WJSI');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26924',
      p_sig_id                 => 'EBS_MFG_DM_DJ_PENDING_WJSI',
      p_sig_sql                => 'SELECT WIE.ERROR_TYPE,
       WIE.ERROR,
       WJSI.INTERFACE_ID,
       GROUP_ID,
       HEADER_ID,
       ...
       DECODE(PROCESS_PHASE,
              ...
              3,              ''Explosion'',
              4,              ''Completion'',
              ''Invalid Value'') PROCESS_PHASE_MEANING,
       PROCESS_STATUS,       
       DECODE(PROCESS_STATUS,
              ...
              4,              ''Complete'',
              5,              ''Warning'',
              ''Invalid Value'') PROCESS_STATUS_MEANING,
       SOURCE_CODE,
       SOURCE_LINE_ID,
       ORGANIZATION_ID,
       WIP_ENTITY_ID,
       JOB_NAME WIP_ENTITY_NAME,
       LOAD_TYPE,
       DECODE(LOAD_TYPE,
              ...
              ''Invalid value'') LOAD_TYPE_MEANING,
       DECODE(STATUS_TYPE, 1, ''Unreleased'', 
                           3, ''Released'', 
                           6, ''OnHold'',
                           status_type) STATUS_TYPE,
       PRIMARY_ITEM_ID ASSEMBLY_ID,
       WIP_SUPPLY_TYPE,
       CLASS_CODE ACCOUNTING_CLASS,
       START_QUANTITY,
       FIRST_UNIT_COMPLETION_DATE FUCD,
       FIRST_UNIT_START_DATE FUSD,
       LAST_UNIT_COMPLETION_DATE LUCD,
       LAST_UNIT_START_DATE LUSD,
       DECODE(NVL(SCHEDULING_METHOD, ''0''),
              1,              ''1-Routing'',
              2,              ''2-Item Lead Time'',
              3,              ''3-Manual'',
              0,              ''NULL'',
       ...
       COMPLETION_SUBINVENTORY COMPL_SUBINV,
       DECODE(OVERCOMPLETION_TOLERANCE_TYPE, 1, ''PERCENT'', 2, ''AMOUNT'') TOL_TYPE,
       OVERCOMPLETION_TOLERANCE_VALUE TOL_VALUE,
       ROUTING_REFERENCE_ID ROUT_REF_ID,
       ROUTING_REVISION ROUT_REV,
       ROUTING_REVISION_DATE ROUT_REV_DATE,
       BOM_REFERENCE_ID BOM_REF_ID,
       BOM_REVISION BOM_REV,
       BOM_REVISION_DATE BOM_REV_DATE,
       ...
       DESCRIPTION
  FROM WIP_JOB_SCHEDULE_INTERFACE WJSI, WIP_INTERFACE_ERRORS WIE
 WHERE WJSI.INTERFACE_ID = WIE.INTERFACE_ID(+)
   AND process_status < 4',
      p_title                  => 'WML Interface Data',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are records to be corrected/processed by WIP Mass Load.',
      p_solution               => 'Review solution steps in [1618613/KB762099] - Technical Brief:- Discrete Job Interface (WIP Mass Load).',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_DJ_PENDING_WJSI');



debug('begin add_signature: EBS_MFG_DM_PENDING_CLOSE_JOBS_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25210',
      p_sig_id                 => 'EBS_MFG_DM_PENDING_CLOSE_JOBS_CLOSED',
      p_sig_sql                => 'SELECT WDJ.ORGANIZATION_ID,
       ...
       MLW.meaning WDJ_STATUS_MEANING,
 ...
 WHERE WDJ.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
   ...
   AND WE.ENTITY_TYPE = 3
 ORDER BY 2,4',
      p_title                  => 'Closed Jobs Showing Wrong Status (Job Showing Status <> Closed)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Discrete Jobs in Pending Close or Failed Close status (or any other status but Closed) that cannot be closed. The jobs are actually closed but the status is not correct.',
      p_solution               => 'Review solution steps in [2937565/KB722469] Failed Close Or Pending Close Discrete Jobs Not Able To Get Closed (Jobs Are Actually Closed But The Status Is Not Reflecting It).',
      p_success_msg            => 'There are no Discrete Jobs in Pending Close or Failed Close status (or any other status but Closed) that cannot be closed.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_PENDING_CLOSE_JOBS_CLOSED');



debug('begin add_signature: EBS_MFG_DM_PENDING_CLOSE_JOBS_OPEN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25030',
      p_sig_id                 => 'EBS_MFG_DM_PENDING_CLOSE_JOBS_OPEN',
      p_sig_sql                => 'SELECT WDJ.ORGANIZATION_ID,
       MP.ORGANIZATION_CODE,
       WDJ.WIP_ENTITY_ID,
       WE.WIP_ENTITY_NAME,
 ...
 ORDER BY 2,4',
      p_title                  => 'Open Discrete Jobs Stuck in Pending Close Status',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Unable to Close Discrete jobs because the jobs are stuck in Pending Close status.',
      p_solution               => 'Review solution steps in [2931310/KB739420] WIP Discrete Jobs Remain In Pending Close Status And Cannot Be Resubmitted For Closure.',
      p_success_msg            => 'There are no Discrete Jobs stuck in pending close status that are still open. ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_PENDING_CLOSE_JOBS_OPEN');



debug('begin add_signature: EBS_MFG_DMEF_PNDGCLK');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30039',
      p_sig_id                 => 'EBS_MFG_DMEF_PNDGCLK',
      p_sig_sql                => 'SELECT DISTINCT MP.ORGANIZATION_CODE,
WE.WIP_ENTITY_NAME,
...
WRAT.END_DATE,
WRAT.DURATION
FROM WIP_RESOURCE_ACTUAL_TIMES WRAT, WIP_ENTITIES WE, MTL_PARAMETERS MP
WHERE WRAT.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
AND WRAT.ORGANIZATION_ID = MP.ORGANIZATION_ID
AND WRAT.STATUS_TYPE <> 2
AND WRAT.TIME_ENTRY_MODE = 4
AND WRAT.COST_FLAG = ''Y''
AND WRAT.START_DATE IS NOT NULL
AND WRAT.END_DATE IS NOT NULL',
      p_title                  => 'WIP Pending Clock Entries',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Pending Clock Entries that prevent closing the job.',
      p_solution               => 'Review [2266056/KB685997] -Clock In/Out Times Are Not Processed Correctly and Stop Job Closure',
      p_success_msg            => 'No Pending Clock Entries.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DMEF_PNDGCLK');



debug('begin add_signature: EBS_MFG_DM_ITEMS_TO_PICK_RELEASE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26885',
      p_sig_id                 => 'EBS_MFG_DM_ITEMS_TO_PICK_RELEASE',
 ...
 WHERE WE.WIP_ENTITY_ID = WRO.WIP_ENTITY_ID
   ...
   AND ((WE.ENTITY_TYPE != 6 AND (WRO.WIP_ENTITY_ID in (Select WIP_ENTITY_ID from WIP_JOB_OPEN_ALLOCATIONS_V))) 
   /*OR (WE.ENTITY_TYPE = 6  AND WDJ.STATUS_TYPE in (3, 4))*/)
 ORDER BY WRO.OPERATION_SEQ_NUM, MSIK.CONCATENATED_SEGMENTS',
      p_title                  => 'Items to be Picked by WIP Component Pick Release',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'These are no Material Requirements that are eligible for WIP Component Pick Release to get allocations.',
      p_solution               => 'Review solution steps from WIP related part in [1075181/KB751535] EAM Work Order Material Allocation (Auto-Allocation) Not Working At WO Release. WIP Component Pick Release Not Generating Any Reservations.',
      p_success_msg            => 'There are items eligible for WIP Component Pick Release. Expand the list above.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_ITEMS_TO_PICK_RELEASE');



debug('begin add_signature: EBS_MFG_DM_JOB_RESERVABLE_SUPPLY_SUBINV');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26879',
      p_sig_id                 => 'EBS_MFG_DM_JOB_RESERVABLE_SUPPLY_SUBINV',
      p_sig_sql                => 'select we.wip_entity_name Job_Number,
       we.WIP_ENTITY_ID,
       decode (wdj.status_type, 3, ''3-Released'', 4, ''4-Compelte'') Job_Status,
       msi.secondary_inventory_name Supply_Subinventory,
 ...
 order by organization_code, wip_entity_name',
      p_title                  => 'Reservable Supply Subinventories Exist in WIP Material Requirements',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'For the returned Job(s), one or more material requirements are using a reservable supply subinventory.
IGNORE this output if the business is NOT using WIP Component Pick-Release for generating material reservations.',
      p_solution               => 'Review solution steps in [1075181/KB751535] EAM Work Order Material Allocation (Auto-Allocation) Not Working At WO Release. WIP Component Pick Release Not Generating Any Reservations.',
      p_success_msg            => 'No reservable supply subinventory set in material requirements.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_JOB_RESERVABLE_SUPPLY_SUBINV');



debug('begin add_signature: EBS_MFG_DM_PICK_ELIGIBLE_JOBS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26878',
      p_sig_id                 => 'EBS_MFG_DM_PICK_ELIGIBLE_JOBS',
 ...
 group by we.wip_entity_name,
  ...
  --        ml.meaning,
          ...
          wsg.schedule_group_name,
          wdj.build_sequence',
      p_title                  => 'WIP Discrete Jobs with Open Material Requirements (Eligible WIP Jobs for Pick-Release)',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Displays the jobs with open material requirements that are eligible for WIP component pick-release. In case there are jobs that are not in this list, please check the job''s status. Only jobs in Released or Complete status are eligible.',
      p_solution               => 'The discrete jobs that are not in this list, cannot be pick-released.',
      p_success_msg            => 'Displays the jobs with open material requirements that are eligible for WIP component pick-release. In case there are jobs that are not in this list, please check the job''s status. Only jobs in Released or Complete status are eligible.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_PICK_ELIGIBLE_JOBS');



debug('begin add_signature: EBS_MFG_DM_RESERV_SUPPLY_SUB_WIP_PARAM');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '26881',
      p_sig_id                 => 'EBS_MFG_DM_RESERV_SUPPLY_SUB_WIP_PARAM',
 ...
 where msi.organization_id = mp.organization_id
   ...
   and msi.reservable_type = 1',
      p_title                  => 'Reservable Subinventories Exist in WIP Parameters',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'These Organizations have Reservable Subinventory set for the "Default Supply Subinventory" parameter in WIP Parameters form. This setup is not supported for WIP Component Picking process. (WIP Pick/Release cannot be used when supply subinventory is reservable).',
      p_solution               => 'Review solution steps in [1075181/KB751535] Work Order Material Allocation (Auto-Allocation) Not Working At WO Release. WIP Component Pick Release Not Generating Any Reservations.',
      p_success_msg            => 'No organizations with reservable default supply subinventory.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DM_RESERV_SUPPLY_SUB_WIP_PARAM');



debug('begin add_signature: WIP_DJDC_HEADER_WDJWE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9677',
      p_sig_id                 => 'WIP_DJDC_HEADER_WDJWE',
 ...
 WHERE WE.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   ...
   AND (WDJ.STATUS_TYPE = MLS.LOOKUP_CODE AND MLS.LOOKUP_TYPE=''WIP_JOB_STATUS'')',
      p_title                  => 'Header Information of the Job',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Header details for the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_HEADER_WDJWE');



debug('begin add_signature: WIP_DJDC_HEADER_WDJMSI');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9836',
      p_sig_id                 => 'WIP_DJDC_HEADER_WDJMSI',
      p_sig_sql                => 'SELECT WDJ.PRIMARY_ITEM_ID ASSEMBLY_ID,
       SUBSTRB(MSI.CONCATENATED_SEGMENTS, 1, 30) ASSEMBLY_NAME,
       ...
       (SELECT CONCATENATED_SEGMENTS
          FROM MTL_ITEM_LOCATIONS_KFV
         WHERE INVENTORY_LOCATION_ID = WDJ.COMPLETION_LOCATOR_ID) COMPL_LOCATOR,
       DECODE(MSI.LOT_CONTROL_CODE, 1, ''No'', 2, ''Yes'', MSI.LOT_CONTROL_CODE) LOT_CONTROL,
       DECODE(MSI.SERIAL_NUMBER_CONTROL_CODE,1,''No'',2,''Yes'',MSI.SERIAL_NUMBER_CONTROL_CODE) SERIAL_CONTROL,
       WDJ.ROUTING_REVISION ROUT_REV,
       WDJ.ROUTING_REVISION_DATE ROUT_REV_DATE,
       NVL(WDJ.ALTERNATE_ROUTING_DESIGNATOR, ''Primary'') ALT_ROUT_DES,
       WDJ.BOM_REVISION BOM_REV,
       WDJ.BOM_REVISION_DATE BOM_REV_DATE,
       NVL(WDJ.ALTERNATE_BOM_DESIGNATOR, ''Primary'') ALT_BOM_DES
  FROM WIP_DISCRETE_JOBS WDJ, MTL_SYSTEM_ITEMS_KFV MSI
 WHERE WDJ.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND WDJ.PRIMARY_ITEM_ID = MSI.INVENTORY_ITEM_ID(+)
   AND WDJ.ORGANIZATION_ID = MSI.ORGANIZATION_ID(+)',
      p_title                  => 'Assembly Details for the Job',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Details of the assembly associated to the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_HEADER_WDJMSI');



debug('begin add_signature: WIP_DJDC_HEADER_WDJMSIMSN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9839',
      p_sig_id                 => 'WIP_DJDC_HEADER_WDJMSIMSN',
      p_sig_sql                => 'SELECT WDJ.PRIMARY_ITEM_ID ASSEMBLY_ID,
       MSN.SERIAL_NUMBER,
       DECODE(MSN.CURRENT_STATUS,
              ...
              MSN.CURRENT_STATUS) CURRENT_STATUS_MEANING,
       ...
       DECODE(MSN.INTRAOPERATION_STEP_TYPE,
              ...
              4,''REJECT'',
              5,''SCRAP'',
              MSN.INTRAOPERATION_STEP_TYPE) OP_SEQ_STEP
  FROM WIP_DISCRETE_JOBS    WDJ,
 ...
 WHERE WDJ.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND WDJ.PRIMARY_ITEM_ID = MSI.INVENTORY_ITEM_ID(+)
   AND WDJ.ORGANIZATION_ID = MSI.ORGANIZATION_ID(+)
   AND (WDJ.WIP_ENTITY_ID = MSN.ORIGINAL_WIP_ENTITY_ID OR
       WDJ.WIP_ENTITY_ID = MSN.WIP_ENTITY_ID)
   AND MSI.INVENTORY_ITEM_ID = MSN.INVENTORY_ITEM_ID
   AND WDJ.ORGANIZATION_ID = MSN.CURRENT_ORGANIZATION_ID(+)',
      p_title                  => 'Assembly Serials Linked to the Job',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Displays the serial numbers associated to the discrete job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_HEADER_WDJMSIMSN');



debug('begin add_signature: WIP_DJDC_DETAILS_WO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9844',
      p_sig_id                 => 'WIP_DJDC_DETAILS_WO',
 ...
 WHERE WO.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
 ...
 ORDER BY 1',
      p_title                  => 'Job Operations (WO)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Operation details for the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_WO');



debug('begin add_signature: WIP_DJDC_DETAILS_WOR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9846',
      p_sig_id                 => 'WIP_DJDC_DETAILS_WOR',
      p_sig_sql                => 'SELECT WORV.OPERATION_SEQ_NUM OP_SEQ_NUM,
       WORV.RESOURCE_SEQ_NUM RES_SEQ_NUM,
       WORV.RESOURCE_ID RES_ID,
       WORV.RESOURCE_CODE RES_CODE,
       WORV.AUTOCHARGE_CODE AUTOCHARGE,
       WORV.UOM_CODE UOM_CODE,
       DECODE(WORV.BASIS_TYPE,1,''ITEM'',2,''LOT'',3,''RES UNITS'',4,''RES VALUE'',5,''TOT VALUE'',6,''ACTIVITY'',WORV.BASIS_TYPE) BASIS_TYPE,
       WORV.USAGE_RATE_OR_AMOUNT USAGE_RATE,
       DECODE(WORV.SCHEDULED_FLAG,1,''YES'',2,''NO'',3,''PRIOR'',4,''NEXT'',WORV.SCHEDULED_FLAG) SCHED_FLAG,
       WORV.SCHEDULED_UNITS,
       WORV.START_DATE,
       WORV.COMPLETION_DATE COMPL_DATE,
       WORV.APPLIED_RESOURCE_UNITS APP_RES_UNITS,
       WORV.APPLIED_RESOURCE_VALUE APP_RES_VALUE,
       WORV.SCHEDULE_SEQ_NUM SCHED_SEQ_NUM,
       WORV.PHANTOM_FLAG,
       WORV.PHANTOM_ITEM_ID,
       WORV.PHANTOM_OP_SEQ_NUM,
       WORV.CREATION_DATE
  FROM WIP_OPERATION_RESOURCES_V WORV
 WHERE WORV.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
 ORDER BY 1, 2',
      p_title                  => 'Resource Requirements (WOR) ',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Resource requirements for the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_WOR');



debug('begin add_signature: WIP_DJDC_DETAILS_WORI');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9848',
      p_sig_id                 => 'WIP_DJDC_DETAILS_WORI',
      p_sig_sql                => 'SELECT WORIV.OPERATION_SEQ_NUM OP_SEQ_NUM,
       WORIV.RESOURCE_SEQ_NUM RES_SEQ_NUM,
       WORIV.RESOURCE_ID RES_ID,
       BR.RESOURCE_CODE RES_CODE,
       WORIV.INSTANCE_ID,
       WORIV.INSTANCE_NAME
  FROM WIP_OP_RESOURCE_INSTANCES_V WORIV, BOM_RESOURCES BR
 WHERE WORIV.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND WORIV.RESOURCE_ID = BR.RESOURCE_ID
 ORDER BY 1, 2',
      p_title                  => 'Resource Instances (WORI)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Resource instances associated to the resource requirements of the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_WORI');



debug('begin add_signature: WIP_DJDC_DETAILS_WORU');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9847',
      p_sig_id                 => 'WIP_DJDC_DETAILS_WORU',
      p_sig_sql                => 'SELECT WORU.OPERATION_SEQ_NUM		OP_SEQ_NUM,
       WORU.RESOURCE_SEQ_NUM		RES_SEQ_NUM,
 ...
 WHERE WORU.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
 ...
 ORDER BY 1, 2',
      p_title                  => 'Resource Usage (WORU)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Resource usages of the jobs or jobs created/scheduled by ASCP.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_WORU');



debug('begin add_signature: WIP_DJDC_DETAILS_WRO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9845',
      p_sig_id                 => 'WIP_DJDC_DETAILS_WRO',
 ...
 WHERE WRO.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND WRO.INVENTORY_ITEM_ID = MSI.INVENTORY_ITEM_ID(+)
   AND WRO.ORGANIZATION_ID = MSI.ORGANIZATION_ID(+)
 ORDER BY 1, 3',
      p_title                  => 'Material Requirements (WRO)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Material requirements for the job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_WRO');



debug('begin add_signature: WIP_DJDC_DETAILS_BOM');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9851',
      p_sig_id                 => 'WIP_DJDC_DETAILS_BOM',
      p_sig_sql                => 'SELECT BBOM.ASSEMBLY_ITEM_ID ASSEMBLY_ID,
       SUBSTRB(MSI.CONCATENATED_SEGMENTS, 1, 30) ASSEMBLY_NAME,
       BIC.OPERATION_SEQ_NUM OP_SEQ_NUM,
       BIC.ITEM_NUM ITEM_SEQ,
       BIC.COMPONENT_ITEM_ID COMP_ITEM_ID,
       MSIB.SEGMENT1 COMP_ITEM_NAME,
       BIC.COMPONENT_SEQUENCE_ID COMP_SEQ_ID,
       BIC.COMPONENT_QUANTITY COMP_QTY,
       DECODE(BIC.WIP_SUPPLY_TYPE,1,''PUSH'',2,''ASSEMBLY PULL'',3,''OPERATION PULL'',4,''BULK'',5,''SUPPLIER'',6,''PHANTOM'',7,''BASED ON BILL'',BIC.WIP_SUPPLY_TYPE) WIP_SUPPLY_TYPE,
       BIC.EFFECTIVITY_DATE EFFECT_DATE,
       BIC.IMPLEMENTATION_DATE IMPLEM_DATE,
       BIC.DISABLE_DATE,
       BIC.SUPPLY_SUBINVENTORY,
       BIC.SUPPLY_LOCATOR_ID,
       BIC.SUPPLY_SUBINVENTORY SUPPLY_SUBINV,
       (SELECT CONCATENATED_SEGMENTS FROM MTL_ITEM_LOCATIONS_KFV WHERE INVENTORY_LOCATION_ID = BIC.SUPPLY_LOCATOR_ID) SUPPLY_LOCATOR,
       NVL(BBOM.ALTERNATE_BOM_DESIGNATOR, ''PRIMARY'') ALTERNATE_BOM_DESIGNATOR,
       BBOM.COMMON_ASSEMBLY_ITEM_ID,
       DECODE(BBOM.ASSEMBLY_TYPE,1,''MFG RTG'',2,''ENG RTG'',BBOM.ASSEMBLY_TYPE) ASSEMBLY_TYPE,
 ...
 WHERE BBOM.COMMON_BILL_SEQUENCE_ID = BIC.BILL_SEQUENCE_ID
   AND WDJ.WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND BBOM.ORGANIZATION_ID = WDJ.ORGANIZATION_ID
   AND BBOM.ASSEMBLY_ITEM_ID = WDJ.PRIMARY_ITEM_ID
 ...
 ORDER BY 3, 4',
      p_title                  => 'Assembly BOM Information',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'BOM components for the Discrete Job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_BOM');



debug('begin add_signature: WIP_DJDC_DETAILS_BOR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9849',
      p_sig_id                 => 'WIP_DJDC_DETAILS_BOR',
      p_sig_sql                => 'SELECT BOR.ASSEMBLY_ITEM_ID ASSY_ID,
       ...
       DECODE(BOS.OPERATION_YIELD_ENABLED,1,''YES'',2,''NO'',BOS.OPERATION_YIELD_ENABLED) OP_YIELD_ENABLED,
       BOS.YIELD,
       ...
       (SELECT CONCATENATED_SEGMENTS
          FROM MTL_ITEM_LOCATIONS_KFV
         WHERE INVENTORY_LOCATION_ID = BOR.COMPLETION_LOCATOR_ID) COMPL_LOCATOR,
       BOR.SERIALIZATION_START_OP SERIAL_START_OP,
       NVL(BOR.ALTERNATE_ROUTING_DESIGNATOR, ''PRIMARY'') ALT_ROUT_DES,
       DECODE(NVL(BOR.CFM_ROUTING_FLAG, 2),1,''FLOW'',2,''DISCRETE'',3,''NETWORK'',BOR.CFM_ROUTING_FLAG) CFM_ROUTING_FLAG,
       DECODE(BOR.ROUTING_TYPE,1,''MFG RTG'',2,''ENG RTG'',BOR.ROUTING_TYPE) ROUT_TYPE
 ...
 ORDER BY 2',
      p_title                  => 'Assembly Routing (BOR)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Details of the Routing associated to the discrete job.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_BOR');



debug('begin add_signature: WIP_DJDC_DETAILS_BORR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9850',
      p_sig_id                 => 'WIP_DJDC_DETAILS_BORR',
      p_sig_sql                => 'SELECT BOR.ASSEMBLY_ITEM_ID ASSY_ID,
       BOS.OPERATION_SEQ_NUM OP_SEQ_NUM,
       BOS.OPERATION_SEQUENCE_ID OP_SEQ_ID,
       BOS.DEPARTMENT_ID DEPT_ID,
       ...
       DECODE(BORV.AUTOCHARGE_TYPE,1,''WIP Move'',2,''Manual'',3,''PO Receipt'',4,''PO Move'',BORV.AUTOCHARGE_TYPE) AUTOCHARGE
 ...
 ORDER BY 2, 4',
      p_title                  => 'Assembly Routing Resources (BOR)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Assembly Routing Resources (BOR) Details.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_DETAILS_BORR');



debug('begin add_signature: WIP_DJDC_HEADER_WJSI');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9841',
      p_sig_id                 => 'WIP_DJDC_HEADER_WJSI',
      p_sig_sql                => 'SELECT WIE.ERROR_TYPE,
       WIE.ERROR,
       WJSI.INTERFACE_ID,
       GROUP_ID,
       ...
       DESCRIPTION
  FROM WIP_JOB_SCHEDULE_INTERFACE WJSI, WIP_INTERFACE_ERRORS WIE
 WHERE WIP_ENTITY_ID = ##$$t_wip_entity_id$$##
   AND WJSI.INTERFACE_ID = WIE.INTERFACE_ID(+)
   AND process_status < 4',
      p_title                  => 'Unprocessed records in WML interface',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Unprocessed records in WML Interface.',
      p_solution               => 'Review the technical brief [1618613/KB762099] for how to process interface records via WIP Mass Load.',
      p_success_msg            => 'No unprocessed records in WML Interface exist.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_HEADER_WJSI');



debug('begin add_signature: WIP_DJDC_HEADER_WJDI');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9843',
      p_sig_id                 => 'WIP_DJDC_HEADER_WJDI',
      p_sig_sql                => 'SELECT WIE.ERROR_TYPE,
       WIE.ERROR,
       PARENT_HEADER_ID,
       DECODE(PROCESS_PHASE,2,''Validation'',3,''Explosion'',4,''Completion'',''Invalid Value'') PROCESS_PHASE,
       DECODE(PROCESS_STATUS,1,''Pending'',2,''Running'',3,''Error'',4,''Complete'',5,''Warning'',''Invalid Value'') PROCESS_STATUS,
       WJDI.*
  FROM WIP_JOB_DTLS_INTERFACE WJDI, WIP_INTERFACE_ERRORS WIE
 WHERE WJDI.PARENT_HEADER_ID IN
       (SELECT HEADER_ID
          FROM WIP_JOB_SCHEDULE_INTERFACE
         WHERE WIP_ENTITY_ID = ##$$t_wip_entity_id$$##)
   AND WJDI.INTERFACE_ID = WIE.INTERFACE_ID (+)',
      p_title                  => 'Interface Details',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are unprocessed records in the WIP Mass Load details interface.',
      p_solution               => 'Review the technical brief [1618613/KB762099] for how to process interface records via WIP Mass Load.',
      p_success_msg            => 'There are no unprocessed records in the WIP Mass Load details interface.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: WIP_DJDC_HEADER_WJDI');



debug('begin add_signature: EBS_MFG_DMEF_WIP_JOB_CLOSED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '28202',
      p_sig_id                 => 'EBS_MFG_DMEF_WIP_JOB_CLOSED',
      p_sig_sql                => 'SELECT WDJ.ORGANIZATION_ID,
       MP.ORGANIZATION_CODE,
       WDJ.WIP_ENTITY_ID,
       WE.WIP_ENTITY_NAME,
 ...
 WHERE WDJ.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
   ...
   AND WE.ENTITY_TYPE = 3
 ORDER BY 2,4',
      p_title                  => 'Closed Discrete Job Showing Wrong Status (Job Showing Status <> Closed)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The Discrete Job is in Pending Close or Failed Close status (or any other status but Closed) that cannot be closed. The job is actually closed but the status is not correct.',
      p_solution               => 'Review solution steps in [2349125/KB379836] Open Work Order/Discrete Job with Entity Type Closed [Maintenance Job/Discrete Job] Throw Different Errors within the Application.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_DMEF_WIP_JOB_CLOSED');
...
BEGIN
  ...
  analyzer_title := 'Work in Process (WIP) Analyzer';
...
start_section('Organizations Setup', 'ORG_SETUP');
   set_item_result(run_stored_sig('EBS_MFG_DM_WIP_PARAMETERS'));
   set_item_result(run_stored_sig('EBS_MFG_DM_INV_PARAMETERS'));
   set_item_result(run_stored_sig('EBS_MFG_DM_SUBINVENTORIES'));
end_section;
...
start_section('Stuck Transactions', 'STUCK_TRANSACTIONS');
   ...
   set_item_result(run_stored_sig('FC_WIP_PENDING_MOVE_TNX'));
   set_item_result(run_stored_sig('INVALID SPACE IN WIP_DISCRETE_JOBS'));
   set_item_result(run_stored_sig('EBS_MFG_DM_CLS_JOB_OPEN_MO'));
   set_item_result(run_stored_sig('EBS_MFG_DM_DJ_PENDING_WJSI'));
   set_item_result(run_stored_sig('EBS_MFG_DM_PENDING_CLOSE_JOBS_CLOSED'));
   set_item_result(run_stored_sig('EBS_MFG_DM_PENDING_CLOSE_JOBS_OPEN'));
   set_item_result(run_stored_sig('EBS_MFG_DMEF_PNDGCLK'));
end_section;
debug('end section: STUCK_TRANSACTIONS');

debug('begin section: WIP_PICK_RELEASE');
start_section('WIP Pick Release Section', 'WIP_PICK_RELEASE');
   set_item_result(run_stored_sig('EBS_MFG_DM_ITEMS_TO_PICK_RELEASE'));
   set_item_result(run_stored_sig('EBS_MFG_DM_JOB_RESERVABLE_SUPPLY_SUBINV'));
   set_item_result(run_stored_sig('EBS_MFG_DM_PICK_ELIGIBLE_JOBS'));
   set_item_result(run_stored_sig('EBS_MFG_DM_RESERV_SUPPLY_SUB_WIP_PARAM'));
end_section;
debug('end section: WIP_PICK_RELEASE');

debug('begin section: Job_Related_Information');
IF g_sql_tokens('##$$t_wip_entity_id$$##') IS NOT NULL THEN
   start_section('Job Related Information', 'Job_Related_Information');
      set_item_result(run_stored_sig('WIP_DJDC_HEADER_WDJWE'));
      set_item_result(run_stored_sig('WIP_DJDC_HEADER_WDJMSI'));
      set_item_result(run_stored_sig('WIP_DJDC_HEADER_WDJMSIMSN'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_WO'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_WOR'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_WORI'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_WORU'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_WRO'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_BOM'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_BOR'));
      set_item_result(run_stored_sig('WIP_DJDC_DETAILS_BORR'));
      set_item_result(run_stored_sig('WIP_DJDC_HEADER_WJSI'));
      set_item_result(run_stored_sig('WIP_DJDC_HEADER_WJDI'));
      set_item_result(run_stored_sig('EBS_MFG_DMEF_WIP_JOB_CLOSED'));
   end_section;
END IF;
debug('end section: Job_Related_Information');
