...
REM |                                                                           |
REM | FILENAME                                                                  |
REM |    lcm_integration_analyzer.sql                                           |
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for Landed Cost Management (LCM)           |
...
CREATE OR REPLACE PACKAGE lcm_int_analyzer_pkg AUTHID DEFINER AS
...
(            p_organization_code            IN VARCHAR2    DEFAULT NULL
           ,p_lcm_ship_header_id           IN NUMBER      DEFAULT NULL
...
END lcm_int_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY lcm_int_analyzer_pkg AS
-- $Id: lcm_integration_analyzer.sql, 200.26 2026/01/28 13:28:27 sothman Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(1677693.1));
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'DISLCMANL_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'DISLCMANL_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>DISLCMANL Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=1677693.1#feedback");
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/3724344" class="blacklink" target="new">
...
•&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; Streamlined data analysis: By maintaining the highlighted selection as you scroll, you can effortlessly compare and analyze data across multiple columns.&nbsp;</p><p>This new feature is designed to provide a more intuitive and user-friendly experience, enabling you to work more efficiently with your data.</p><p>Other features:<br>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,913,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,913,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},913,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
BEGIN
    ...
    print_log('Processing recommended patches signature: INL_REC_PATCHS (check_rec_patches_1) "Recommended Patches" (version 2)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_col_rows(1)(l_rows) := '24665183';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Oracle Landed Cost Management Recommended Patch Collection (Sep. 2016)';
   l_col_rows(5)(l_rows) := '[1989683.1]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '25796061';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Oracle Landed Cost Management Recommended Patch Collection (Mar. 2017)';
   l_col_rows(5)(l_rows) := '[1989683.1]';
...
Best Practice: Always ensure that you are on the latest code with all recommended patches applied. This will help avoid known issues, prevent potential data corruption and provide the latest features and functionality. Having a proactive patching plan and periodic proactive patching process in place are the best ways to stay current.';
  ...
  -- Register
  l_step := '60';
  g_signatures('INL_REC_PATCHS') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'INL_REC_PATCHS',     -- sig ID
...
FUNCTION get_GMF
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: GMF');
BEGIN
     select fpi.status into g_gmf_id 
       from fnd_application_vl fav, fnd_product_installations fpi
      where fav.application_short_name = 'GMF'
      and fav.application_id = fpi.application_id;
     return g_gmf_id;
EXCEPTION WHEN NO_DATA_FOUND THEN
     return null;
END; 
debug('end sql token function: GMF');
END get_GMF;

FUNCTION get_BOM
   return VARCHAR2 IS
BEGIN
debug('begin sql token function: BOM');
BEGIN
     select fpi.status into g_bom_id 
       from fnd_application_vl fav, fnd_product_installations fpi
      where fav.application_short_name = 'BOM'
      and fav.application_id = fpi.application_id;
     return g_bom_id;
EXCEPTION WHEN NO_DATA_FOUND THEN
     return null;
END;
debug('end sql token function: BOM');
END get_BOM;
...
debug('begin populate parameters hash table');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'organization_code';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_organization_code,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'ship_header_id';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_lcm_ship_header_id,'NO_MASK');
   g_parameters.extend();
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/01/28 13:28:26 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'lcm_integration_analyzer.sql';
...
debug('begin parameter validation: p_organization_code');
IF p_organization_code IS NOT NULL THEN
BEGIN
SELECT SP.ORGANIZATION_CODE
INTO l_exists_val
FROM MTL_PARAMETERS SP
WHERE SP.ORGANIZATION_CODE = p_organization_code and rownum < 2;
EXCEPTION
   WHEN NO_DATA_FOUND THEN
   print_error('INVALID ARGUMENT: Please enter a valid ORGANIZATION_CODE (MTL_PARAMETERS.ORGANIZATION_CODE). ');
   raise invalid_parameters;
   WHEN OTHERS THEN
   print_error('INVALID ARGUMENT: Validation query for p_organization_code failed with '||sqlerrm||'.');
   raise invalid_parameters;
END;
END IF;
debug('end parameter validation: p_organization_code');
...
debug('begin populate sql tokens hash table');
   g_sql_tokens('##$$GMF$$##') := get_GMF();
   l_system_function_var := null;
   g_masked_tokens('##$$GMF$$##') := mask_text( g_sql_tokens('##$$GMF$$##'), 'NO_MASK');
   g_sql_tokens('##$$BOM$$##') := get_BOM();
   l_system_function_var := null;
   g_masked_tokens('##$$BOM$$##') := mask_text( g_sql_tokens('##$$BOM$$##'), 'NO_MASK');
   g_sql_tokens('##$$APPS_PARAM1$$##') := '9004';
   ...
   g_sql_tokens('##$$ship_header_id$$##') := p_lcm_ship_header_id;
   g_masked_tokens('##$$ship_header_id$$##') := mask_text( g_sql_tokens('##$$ship_header_id$$##'), 'NO_MASK');
...
debug('begin add_signature: INV_ORG_PARAMS_PROCESS_LCM_ENABLED_CHILD1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5762',
      p_sig_id                 => 'INV_ORG_PARAMS_PROCESS_LCM_ENABLED_CHILD1',
      p_sig_sql                => 'SELECT count(*) count
    FROM  MTL_PARAMETERS MP
    WHERE MP.LCM_ENABLED_FLAG = ''Y''
    AND mp.LCM_Var_account is null
    AND MP.organization_id = ''##$$FK1$$##''',
      p_title                  => 'Inventory Organization Parameters - Process and LCM enabled',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => 'Inventory Organization Parameters - Process and LCM enabled',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763]</li>
   <li> Section Setup - Setting Up Inventory Organization Options for more details on Defining the LCM Variance Account </li>
   </ul>',
      p_success_msg            => 'LCM Variance Accounts Have Been Defined.',
   ...
   l_info.delete;
debug('end add_signature: INV_ORG_PARAMS_PROCESS_LCM_ENABLED_CHILD1');



debug('begin add_signature: INV_ORG_PARAMS_PROCESS_LCM_ENABLED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5760',
      p_sig_id                 => 'INV_ORG_PARAMS_PROCESS_LCM_ENABLED',
      p_sig_sql                => 'SELECT MP.organization_code, 
           MP.organization_id, MP.organization_id "##$$FK1$$##", 
           MP.process_enabled_flag, 
           MP.lcm_enabled_flag,
           LCM_Var_account AS LCM_Account_ID,
           gcc.segment1||''-''|| gcc.segment2||''-''|| gcc.segment3||''-''|| gcc.segment4||''-''|| gcc.segment5||''-''|| gcc.segment6||''-''|| gcc.segment7 AS LCM_Account_Combination 
    FROM MTL_PARAMETERS MP,
         GL_CODE_COMBINATIONS GCC
    WHERE MP. LCM_VAR_ACCOUNT = GCC.CODE_COMBINATION_ID(+)
    AND   MP.lcm_enabled_flag = ''Y''',
      p_title                  => 'Inventory organizations Enabled for LCM Details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no Organizations enabled for LCM',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763], Section Setup - Setting Up Inventory Organization Options</B> for more details on enabling Inventory organization for LCM </li>
   </ul>',
      ...
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('INV_ORG_PARAMS_PROCESS_LCM_ENABLED_CHILD1'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '15'
      );
   l_info.delete;
debug('end add_signature: INV_ORG_PARAMS_PROCESS_LCM_ENABLED');



debug('begin add_signature: RECEIVING_OPTIONS_CHILD1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5765',
      p_sig_id                 => 'RECEIVING_OPTIONS_CHILD1',
      p_sig_sql                => 'SELECT count(*) count
    FROM  RCV_PARAMETERS RP,
          MTL_PARAMETERS MP
    where MP.ORGANIZATION_ID = RP.ORGANIZATION_ID
    AND MP.organization_id = ''##$$FK2$$##''
    AND MP.LCM_ENABLED_FLAG = ''Y''
    AND ( RP.LCM_ACCOUNT_ID IS NULL OR 
         RP.INV_VARIANCE_ACCOUNT_ID IS NULL OR 
         RP.EXCHANGE_VARIANCE_ACCOUNT_ID IS NULL OR 
         RP.TAX_VARIANCE_ACCOUNT_ID IS NULL  OR 
         RP.DEF_CHARGES_ACCOUNT_ID IS NULL)',
      p_title                  => 'LCM Accounts Defined',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => 'All the 5 LCM accounts </br>
          a. Landed Cost Absorption Account </br>
          b. Invoice Price Variance Account </br>
          c. Exchange Rate Variance Account </br>
          d. Tax Variance Account </br>
          e. Default Charge Account MUST be defined. </br>''',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#ReceivingOptionsSetup]</li>
   <li> Section Setup -  Setting Up Receiving Options for more details. </li>
   </ul>',
      p_success_msg            => 'All the 5 LCM accounts Have Been Defined.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: RECEIVING_OPTIONS_CHILD1');



debug('begin add_signature: RECEIVING_OPTIONS_CHILD2');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5766',
      p_sig_id                 => 'RECEIVING_OPTIONS_CHILD2',
      p_sig_sql                => 'SELECT count(*) count
    FROM  RCV_PARAMETERS RP,
          MTL_PARAMETERS MP
    WHERE MP.ORGANIZATION_ID = RP.ORGANIZATION_ID
    AND MP.organization_id = ''##$$FK2$$##''
    AND MP.LCM_ENABLED_FLAG = ''Y''
    and NVL(RP.advanced_pricing, ''N'') = ''N''',
      p_title                  => 'Advanced Pricing integration with LCM',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => 'Advanced Pricing integration with LCM ',
      p_solution               => '<ul>
   <li>Interface To Advanced Pricing is not Selected.  Select the "Interface To Advanced Pricing" check box in the "Cost Factors" region of the Receiving Options page,for automatic charge lines generation</li></ul>',
      p_success_msg            => 'Advanced Pricing integration with LCM is enabled.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: RECEIVING_OPTIONS_CHILD2');



debug('begin add_signature: RECEIVING_OPTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5764',
      p_sig_id                 => 'RECEIVING_OPTIONS',
      p_sig_sql                => 'SELECT mp.organization_code, 
           mp.organization_id, MP.organization_id "##$$FK2$$##", 
           rp.enforce_rma_serial_num, 
           Decode(rp.enforce_rma_lot_num, ''R'', ''Restricted'', ''W'', ''Restricted with Warning'', ''Y'', ''Unrestricted'') AS "Validate Lots on RMA Receipts", 
           PRE_RECEIVE "Pre-Receiving in LCM", 
           advanced_pricing "Interface to Advanced Pricing", 
           LCM_ACCOUNT_ID, 
           GCC1.SEGMENT1||''-''|| GCC1.SEGMENT2||''-''|| GCC1.SEGMENT3||''-''|| GCC1.SEGMENT4||''-''|| GCC1.SEGMENT5||''-''|| GCC1.SEGMENT6||''-''|| GCC1.SEGMENT7 "Landed Cost Absorption Account" , 
           INV_VARIANCE_ACCOUNT_ID, GCC1.SEGMENT1||''-''|| GCC1.SEGMENT2||''-''|| GCC1.SEGMENT3||''-''|| GCC1.SEGMENT4||''-''|| GCC1.SEGMENT5||''-''|| GCC1.SEGMENT6||''-''|| GCC1.SEGMENT7 "Invoice Price Variance Account" , 
           EXCHANGE_VARIANCE_ACCOUNT_ID, GCC1.SEGMENT1||''-''|| GCC1.SEGMENT2||''-''|| GCC1.SEGMENT3||''-''|| GCC1.SEGMENT4||''-''|| GCC1.SEGMENT5||''-''|| GCC1.SEGMENT6||''-''|| GCC1.SEGMENT7 "Exchange Rate Variance Account" , 
           TAX_VARIANCE_ACCOUNT_ID, GCC1.SEGMENT1||''-''|| GCC1.SEGMENT2||''-''|| GCC1.SEGMENT3||''-''|| GCC1.SEGMENT4||''-''|| GCC1.SEGMENT5||''-''|| GCC1.SEGMENT6||''-''|| GCC1.SEGMENT7 "Tax Variance Account" , 
           DEF_CHARGES_ACCOUNT_ID, GCC1.SEGMENT1||''-''|| GCC1.SEGMENT2||''-''|| GCC1.SEGMENT3||''-''|| GCC1.SEGMENT4||''-''|| GCC1.SEGMENT5||''-''|| GCC1.SEGMENT6||''-''|| GCC1.SEGMENT7 "Default Charge Account" 
    FROM RCV_PARAMETERS RP, 
         MTL_PARAMETERS MP,
    ...
    WHERE MP.ORGANIZATION_ID = RP.ORGANIZATION_ID
    AND MP.LCM_ENABLED_FLAG = ''Y''
    and RP.LCM_ACCOUNT_ID = GCC1.CODE_COMBINATION_ID(+)
    AND RP.INV_VARIANCE_ACCOUNT_ID = GCC2.CODE_COMBINATION_ID(+)
    and RP.EXCHANGE_VARIANCE_ACCOUNT_ID = GCC3.CODE_COMBINATION_ID(+)
    AND RP.TAX_VARIANCE_ACCOUNT_ID = GCC4.CODE_COMBINATION_ID(+)
    AND RP.DEF_CHARGES_ACCOUNT_ID = GCC5.CODE_COMBINATION_ID(+)',
      p_title                  => 'Receiving Options',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No Receiving Options</B> defined for LCM enabled Organizations.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#ReceivingOptionsSetup], Section Setup - Setting Up Receiving Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'No possible problems found.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      ...
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('RECEIVING_OPTIONS_CHILD1','RECEIVING_OPTIONS_CHILD2'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '21'
      );
   l_info.delete;
debug('end add_signature: RECEIVING_OPTIONS');



debug('begin add_signature: UOM_CLASSES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5763',
      p_sig_id                 => 'UOM_CLASSES',
      p_sig_sql                => 'SELECT UOM_CLASS, 
           DESCRIPTION, 
           UNIT_OF_MEASURE, 
           UOM_CODE, 
           DISABLE_DATE 
    ...
    AND exists ( select 1
              ...
              where FPO.PROFILE_OPTION_ID   = FPOV.PROFILE_OPTION_ID
               and FPOV.PROFILE_OPTION_VALUE = MUM.UOM_CLASS
               and FPO.PROFILE_OPTION_NAME  in (''INL_QUANTITY_UOM_CLASS'', ''INL_VOLUME_UOM_CLASS'', ''INL_WEIGHT_UOM_CLASS''))',
      p_title                  => 'UOM Classes',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'UOM Classes',
      p_solution               => '<ul>
   <li>UOM Class Definition for Quantity, Weight and Volume should exist.</li>
   <li>Refer [1498479/KB662531] for more details on this setup. </li>
   </ul>',
      p_success_msg            => 'UOM Classes are defined.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: UOM_CLASSES');



debug('begin add_signature: COST_FACTORS_CHILD1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '6142',
      p_sig_id                 => 'COST_FACTORS_CHILD1',
      p_sig_sql                => 'SELECT count(*) "NO_OF_RECORDS"
    FROM   PON_COST_FACTORS_VL PCF,
           CM_CMPT_MST CCM
    WHERE PCF.COST_COMPONENT_CLASS_ID = CCM.COST_CMPNTCLS_ID (+)
    AND pcf.price_element_type_id= ''##$$FK5$$##''
    AND NVL(COST_ACQUISITION_CODE,''E'') <> ''I''
    AND PCF.enabled_flag = ''Y''',
      p_title                  => 'Acquisition Cost Indicator is not selected as Inventory',
      p_fail_condition         => '[NO_OF_RECORDS] > [0]',
      p_problem_descr          => 'Acquisition Cost Indicator is not selected as Inventory',
      p_solution               => '<ul>
   <li> Select the Acquisition Cost Indicator as Expense</B> to not apply the cost factor in cost calculations or Inventory</B> to apply the cost factor in cost calculations, in case you are using OPM Financials Review Note - What is The Difference Between Expense And Inventory Type Of Acquisition Cost Indicator? [1511681/KB614503]</li>
   </ul>',
      p_success_msg            => 'Acquisition Cost Indicator is not selected as Inventory',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: COST_FACTORS_CHILD1');



debug('begin add_signature: COST_FACTORS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5767',
      p_sig_id                 => 'COST_FACTORS',
      p_sig_sql                => 'SELECT pcf.price_element_type_id, pcf.price_element_type_id "##$$FK5$$##", 
          PCF.PRICE_ELEMENT_CODE, 
          PCF.name, 
          PCF.DESCRIPTION, 
          PCF.ENABLED_FLAG, 
          PCF.SYSTEM_FLAG, 
          PCF.PRICING_BASIS, 
          PCF.ALLOCATION_BASIS, 
          ccm.cost_cmpntcls_code, 
          COST_ANALYSIS_CODE, 
          DECODE(COST_ACQUISITION_CODE, ''I'', ''Inventory'', ''E'', ''Expense'',COST_ACQUISITION_CODE) "Acquisition Cost Indicator", 
          INVOICE_LINE_TYPE 
    FROM PON_COST_FACTORS_VL PCF,
         CM_CMPT_MST CCM
    WHERE PCF.COST_COMPONENT_CLASS_ID = CCM.COST_CMPNTCLS_ID (+)
    AND PCF.enabled_flag = ''Y''',
      p_title                  => 'Cost Factors',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No Cost Factors </B> setup to Define LCM Charge Types.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#CostFactorsSetup], Section Setup -  Setting Up Cost Factors </B> for more details. </li>
   </ul>',
      p_success_msg            => 'All Cost Factors have been defined to Define LCM Charge Types.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('COST_FACTORS_CHILD1'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '17'
      );
   l_info.delete;
debug('end add_signature: COST_FACTORS');



debug('begin add_signature: LANDED_COST_ADJ_COST_FACTOR_GREATER_30');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '1007',
      p_sig_id                 => 'LANDED_COST_ADJ_COST_FACTOR_GREATER_30',
      p_sig_sql                => 'SELECT PRICE_ELEMENT_TYPE_ID "PRICE_ELEMENT_TYPE_ID",
           TRADING_PARTNER_ID "TRADING_PARTNER_ID",
           PRICE_ELEMENT_CODE "PRICE_ELEMENT_ID",
           length(PRICE_ELEMENT_CODE) "LENGTH"
    FROM PON_PRICE_ELEMENT_TYPES 
    WHERE LENGTH(PRICE_ELEMENT_CODE) > 30',
      p_title                  => 'Landed Cost Adjustments Import cannot handle Cost Factor names longer than 30 Characters',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'The Landed Cost Adjustments Import concurrent program takes its values for the column COST_FACTOR from
    the column PRICE_ELEMENT_CODE in Table PON_PRICE_ELEMENT_TYPES.</BR>
    The database schema allows PRICE_ELEMENT_CODE to be up to 80 characters in length, but the Adjustments
    Import process can only handle 30 character COST_FACTOR names and will give the following error if
    presented with a longer name :</BR></BR>

    ORA-06502: PL/SQL: numeric or value error: character string buffer too small</BR></BR>

    Note that you cannot by default see the PRICE_ELEMENT_CODE value from the setup screen</BR>
    
    (Navigation : Sourcing SuperUser > Sourcing HomePage > Negotiations > Reusable Lists > Cost Factors).',
      p_solution               => 'Change the name of the cost factor codes returned by the above to 30 or less characters.  See [1483335/KB458606] for additional details.</BR></BR>
Navigation: Purchasing Super User -> Setup -> Purchasing -> Cost Factors)</BR></BR>

Re-run ''Landed Cost Adjustments Import Process'' and test if the issue exist. If the issue still exist, provide trace with binds collected for the same program.',
   ...
   l_info.delete;
debug('end add_signature: LANDED_COST_ADJ_COST_FACTOR_GREATER_30');



debug('begin add_signature: LCM_OPTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5772',
      p_sig_id                 => 'LCM_OPTIONS',
      p_sig_sql                => 'SELECT MP.organization_code, 
          mp.organization_id, 
          USER_DEFINED_SHIP_NUM_CODE "Shipment Number Generation", 
          MANUAL_SHIP_NUM_TYPE "Shipment Number Type", 
          NEXT_SHIP_NUM "Next Shipment Number", 
          PO_PRICE_TOLER_PERC "PO Price Tolerance%" 
    FROM INL_PARAMETERS IP,
         MTL_PARAMETERS MP
    WHERE MP.ORGANIZATION_ID = IP.ORGANIZATION_ID
    AND MP.lcm_enabled_flag = ''Y''',
      p_title                  => 'LCM Options',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No LCM Options</B> Defined.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup] , Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li></ul>',
      p_success_msg            => 'Defined LCM Options</B>.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_OPTIONS');



debug('begin add_signature: LCM_SHIPMENT_LINE_TYPES_CHILD1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5773',
      p_sig_id                 => 'LCM_SHIPMENT_LINE_TYPES_CHILD1',
      p_sig_sql                => 'SELECT count(*) count
    FROM   INL_SHIP_LINE_TYPES_VL
    WHERE Ship_line_type_code =''##$$FK4$$##''
    AND NVL(DFLT_landed_cost_flag,''N'') <> ''Y''',
      p_title                  => 'Included in Landed Cost Checkbox',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => 'Included in Landed Cost</B> check box is not selected.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'Included in Landed Cost</B> check box is selected.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_SHIPMENT_LINE_TYPES_CHILD1');



debug('begin add_signature: LCM_SHIPMENT_LINE_TYPES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5774',
      p_sig_id                 => 'LCM_SHIPMENT_LINE_TYPES',
      p_sig_sql                => 'SELECT  Ship_line_type_code, Ship_line_type_code "##$$FK4$$##", 
            Ship_line_type_name, 
            DFLT_landed_cost_flag "Included in the Landed Cost?", 
            Dflt_allocation_enabled_flag "Associable Line", 
            active_from_date, 
            active_to_date 
    FROM INL_Ship_line_types_VL',
      p_title                  => 'LCM Shipment Line Types',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no LCM Shipment Line Types</B> Defined.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li></ul>',
      p_success_msg            => 'Defined LCM shipment line types.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('LCM_SHIPMENT_LINE_TYPES_CHILD1'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '14'
      );
   l_info.delete;
debug('end add_signature: LCM_SHIPMENT_LINE_TYPES');



debug('begin add_signature: LCM_SHIPMENT_TYPES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5775',
      p_sig_id                 => 'LCM_SHIPMENT_TYPES',
      p_sig_sql                => 'SELECT Ship_type_id, 
          Ship_type_code, 
          ship_type_name, 
          Decode(trd_pties_alwd_code, 1, ''Domestic'', 2, ''Foreign'', 3, ''Both''), 
          active_from_date, 
          Active_to_date
FROM INL_SHIP_TYPES_VL',
      p_title                  => 'LCM Shipment Types',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no LCM Shipment Types Defined</B>.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'All LCM shipment types defined.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_SHIPMENT_TYPES');



debug('begin add_signature: LCM_SHIPMENT_TYPES_ALLOWED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5776',
      p_sig_id                 => 'LCM_SHIPMENT_TYPES_ALLOWED',
      p_sig_sql                => 'SELECT IST.ship_type_code, 
          ISLT.Ship_line_type_code, 
          IALT.DFLT_Ship_line_type_flag 
   FROM INL_ALWD_LINE_TYPES IALT, 
        INL_SHIP_TYPES_B IST, 
        INL_SHIP_LINE_TYPES_B ISLT 
   WHERE IALT.PARENT_TABLE_ID = IST.SHIP_TYPE_ID 
   and IALT.SHIP_LINE_TYPE_ID = ISLT.SHIP_LINE_TYPE_ID 
   order by IST.ship_type_code, 
            ISLT.Ship_line_type_code',
      p_title                  => 'LCM Shipment Types Allowed',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No LCM Shipment Types Allowed</B> Defined.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'Defined LCM shipment types allowed.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_SHIPMENT_TYPES_ALLOWED');



debug('begin add_signature: LCM_PARTY_TYPES_ALLOWED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5777',
      p_sig_id                 => 'LCM_PARTY_TYPES_ALLOWED',
      p_sig_sql                => 'SELECT IST.ship_type_code, 
          IAPT.party_type_code 
   FROM INL_ALWD_PARTY_TYPES IAPT, 
        INL_SHIP_TYPES_B IST 
   WHERE IAPT.PARENT_TABLE_ID = IST.SHIP_TYPE_ID 
   order by IST.ship_type_code',
      p_title                  => 'LCM Party Types Allowed',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No LCM Party Types Allowed</B>.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'Defined LCM party types allowed.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_PARTY_TYPES_ALLOWED');



debug('begin add_signature: LCM_PARTY_USAGES_ALLOWED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5778',
      p_sig_id                 => 'LCM_PARTY_USAGES_ALLOWED',
      p_sig_sql                => 'SELECT IST.ship_type_code, 
          IAPU.party_usage_code 
   FROM INL_ALWD_PARTY_USAGES IAPU, 
        INL_SHIP_TYPES_B IST 
   WHERE IAPU.PARENT_TABLE_ID = IST.SHIP_TYPE_ID 
   order by IST.ship_type_code',
      p_title                  => 'LCM Party Usages Allowed',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No LCM Party Usages Allowed</B>',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li>
   </ul>',
      p_success_msg            => 'Defined LCM party usages allowed.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_PARTY_USAGES_ALLOWED');



debug('begin add_signature: LCM_SOURCE_TYPES_ALLOWED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5779',
      p_sig_id                 => 'LCM_SOURCE_TYPES_ALLOWED',
      p_sig_sql                => 'SELECT IST.ship_type_code, 
          IAST.ship_line_src_type_code, 
          IAST.DFLT_source_type_flag 
   FROM INL_ALWD_SOURCE_TYPES IAST, 
        INL_SHIP_TYPES_B IST 
   where IAST.ship_type_id = IST.SHIP_TYPE_ID 
   order by IST.ship_type_code',
      p_title                  => 'LCM Source Types Allowed',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No LCM Source Types Allowed</B>',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#LCMOptionsSetup], Section Setup - Setting Up Landed Cost Management Options</B> for more details. </li></ul>',
      p_success_msg            => 'Defined LCM source types allowed.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: LCM_SOURCE_TYPES_ALLOWED');



debug('begin add_signature: PRICE_MODIFIER_HEADERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5768',
      p_sig_id                 => 'PRICE_MODIFIER_HEADERS',
      p_sig_sql                => 'SELECT QSLH.list_header_id, 
          QSLH.LIST_TYPE_CODE, 
          QSLH.name, 
          QSLH.DESCRIPTION, 
          QSLH.automatic_flag, 
          ...
          QSLH.currency_code, 
          QSLH.global_flag, 
          QSLH.start_date_active, 
          QSLH.end_date_active 
    FROM QP_SECU_LIST_HEADERS_VL QSLH
    WHERE QSLH.PTE_CODE = ''PO''
    and QSLH.SOURCE_SYSTEM_CODE = ''PO''
    AND QSLH.LIST_TYPE_CODE = ''CHARGES''
    AND QSLH.ACTIVE_FLAG = ''Y''',
      p_title                  => 'Price Modifier Headers',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No Price Modifiers</B> setup to Define LCM Charge Types.',
      p_solution               => '<ul>
<li> Review Note - EBS LCM OPM integration[978607/KB271880#QPModifiers], Section  5. Define Advanced Pricing Modifiers for more details. </li>
</ul>
',
      p_success_msg            => 'Price Modifiers have been setup to Define LCM charge types.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: PRICE_MODIFIER_HEADERS');



debug('begin add_signature: PRICE_MODIFIER_LINES_CHILD1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5770',
      p_sig_id                 => 'PRICE_MODIFIER_LINES_CHILD1',
      p_sig_sql                => 'SELECT count(*) count
    FROM  QP_MODIFIER_SUMMARY_V QMS,
          qp_pricing_phases QPP
    WHERE QMS.PRICING_PHASE_ID = QPP.PRICING_PHASE_ID
    AND QMS.LIST_LINE_ID =''##$$FK3$$##''
    AND NVL(QMS.AUTOMATIC_FLAG, ''N'') <> ''Y''
    AND exists ( select 1
                from QP_SECU_LIST_HEADERS_VL QSLH
                where QMS.list_header_id = QSLH.list_header_id
                  and QSLH.PTE_CODE = ''PO''
                  and QSLH.SOURCE_SYSTEM_CODE = ''PO''
                  and QSLH.LIST_TYPE_CODE = ''CHARGES''
                  and QSLH.ACTIVE_FLAG = ''Y'')',
      p_title                  => 'Automatic Flag',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => '<B>Automatic Flag</B> is not enabled.',
      p_solution               => '<ul>
   <li>Select <B>Automatic Flag</B> for automatic charge lines generation.</li>
   </ul>',
      p_success_msg            => 'Automatic Flag is enabled.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: PRICE_MODIFIER_LINES_CHILD1');



debug('begin add_signature: PRICE_MODIFIER_LINES_CHILD2');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5771',
      p_sig_id                 => 'PRICE_MODIFIER_LINES_CHILD2',
      p_sig_sql                => 'SELECT count(*) count
    FROM  QP_MODIFIER_SUMMARY_V QMS,
          qp_pricing_phases QPP
    WHERE QMS.PRICING_PHASE_ID = QPP.PRICING_PHASE_ID
    AND QMS.LIST_LINE_ID = ''##$$FK3$$##''
    and QPP.name NOT LIKE ''PO:%''
    AND exists ( select 1
                from QP_SECU_LIST_HEADERS_VL QSLH
                where QMS.list_header_id = QSLH.list_header_id
                  and QSLH.PTE_CODE = ''PO''
                  ...
                  and QSLH.LIST_TYPE_CODE = ''CHARGES''
                  AND QSLH.ACTIVE_FLAG = ''Y'')',
      p_title                  => 'Relevant Pricing Phase',
      p_fail_condition         => '[count] > [0]',
      p_problem_descr          => 'Relevant Pricing Phase',
      p_solution               => '<ul>
   <li>Choose relevant Pricing Phase starting with " PO: "</li>
   </ul>',
      p_success_msg            => 'Relevant Pricing Phase is Correct.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: PRICE_MODIFIER_LINES_CHILD2');



debug('begin add_signature: PRICE_MODIFIER_LINES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5769',
      p_sig_id                 => 'PRICE_MODIFIER_LINES',
      p_sig_sql                => 'SELECT QMS.list_header_id, 
           QMS.LIST_LINE_ID, QMS.LIST_LINE_ID "##$$FK3$$##", 
           QMS.MODIFIER_LEVEL_CODE, 
           QMS.LIST_LINE_TYPE, 
           QMS.START_DATE_ACTIVE, 
           QMS.END_DATE_ACTIVE, 
           QMS.AUTOMATIC_FLAG, 
           QMS.OVERRIDE_FLAG, 
           QMS.INCLUDE_ON_RETURNS_FLAG, 
           QPP.name AS "Pricing Phase", 
           PRODUCT_ATTRIBUTE_TYPE, 
           PRODUCT_ATTR_VALUE, 
           PRODUCT_PRECEDENCE, 
           charge_name, 
           ARITHMETIC_OPERATOR_TYPE "Application Method", 
           operand "Value" 
    FROM  QP_MODIFIER_SUMMARY_V QMS,
          qp_pricing_phases QPP
    WHERE QMS.PRICING_PHASE_ID = QPP.PRICING_PHASE_ID
    AND exists ( select 1
                 ...
                 and QSLH.LIST_TYPE_CODE = ''CHARGES''
                 and QSLH.ACTIVE_FLAG = ''Y'')',
      p_title                  => 'Price Modifier Lines',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are No Price Modifier Lines</B> defined to Define LCM Charge Types.',
      p_solution               => '<ul>
<li> Review Note - EBS LCM OPM integration[978607/KB271880#QPModifiers], Section  5. Define Advanced Pricing Modifiers for more details. </li>
</ul>
',
      p_success_msg            => 'Price Modifier Lines have been setup to Define LCM charge types.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
      p_extra_info             => l_info,
      p_child_sigs             => VARCHAR_TBL('PRICE_MODIFIER_LINES_CHILD1','PRICE_MODIFIER_LINES_CHILD2'),
      p_include_in_dx_summary  => nvl('P','N'),
      p_version                => '16'
      );
   l_info.delete;
debug('end add_signature: PRICE_MODIFIER_LINES');



debug('begin add_signature: INL_AC_UPDATE_FROM_LCM_ADJ');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13804',
      p_sig_id                 => 'INL_AC_UPDATE_FROM_LCM_ADJ',
      p_sig_sql                => 'SELECT * FROM
(
/*  Cost Update Accounting */
SELECT ph.segment1 po_number,
       rt.transaction_id rcv_transaction_id,
       mmt.transaction_id mmt_transaction_id,
       mmt.transaction_type_id mmt_ransaction_type_id,
       mmt.transaction_date mmt_transaction_date,
       gl.padded_concatenated_segments account_num,
       DECODE(mta.accounting_line_type, 1, ''Inventory'', 2, ''Landed Cost Absorption'', 13, ''Cost Variance'', 20, ''LCM Variance'', ''Other'') accounting_line_type,
       DECODE(mta.accounting_line_type,1,mta.base_transaction_value,13,mta.base_transaction_value,20,mta.base_transaction_value,0,0) dr,
       DECODE(mta.accounting_line_type,1,0,13,0,20,0,mta.base_transaction_value) cr,      
       NULL ship_line_id,
       NULL ship_num,
...
AND mta.reference_account = gl.code_combination_id
AND mmt.trx_source_line_id IN
  (SELECT trx_source_line_id
   FROM mtl_material_transactions
   WHERE transaction_type_id = 80
   AND source_code = ''LCMADJ''
   AND transaction_date BETWEEN sysdate-90 AND sysdate)
UNION
/* Item landed Cost portions, with or without invoices */
SELECT ph.segment1 po_number,
       (SELECT MAX(rt.transaction_id) -- This MAX is for cases where one ship line has multiple RTs (PLL broken into multiple PDs)
        FROM rcv_transactions rt
        WHERE rt.lcm_shipment_line_id = NVL(sl.parent_ship_line_id,sl.ship_line_id)       
        AND rt.transaction_type = ''RECEIVE''
        ) rcv_transaction_id,       
       NULL mmt_transaction_id,
       NULL mmt_ransaction_type_id,      
       NULL mmt_transaction_date,      
       NULL account_num,
       NULL accounting_line_type,
       NULL dr,
       NULL cr,               
       NVL(sl.parent_ship_line_id,sl.ship_line_id) ship_line_id,
       sh.ship_num,
       ...
       a.allocation_id,                
       a.adjustment_num,
       DECODE(a.adjustment_num,0,a.allocation_amt,0) first_estimated_amt,
       DECODE(a.adjustment_num,0,0,a.allocation_amt) adjusted_amt,      
       ai.invoice_num invoice_num,
       m.creation_date match_date
FROM mtl_parameters mp,
     po_headers_all ph,
...
WHERE mp.organization_id = sh.organization_id
...
AND aid.invoice_distribution_id(+) = m.from_parent_table_id
AND m.from_parent_table_name(+) = ''AP_INVOICE_DISTRIBUTIONS''
AND m.match_id(+) = sl.match_id
AND msi.organization_id = sh.organization_id
AND msi.inventory_item_id = sl.inventory_item_id
AND a.ship_line_id = sl.ship_line_id
AND a.from_parent_table_name = ''INL_SHIP_LINES''
AND a.landed_cost_flag = ''Y''
AND a.allocation_amt <> 0
AND (a.adjustment_num = 0 OR a.adjustment_num = sh.adjustment_num)
AND sh.ship_header_id = sl.ship_header_id
AND NVL(sl.parent_ship_line_id,sl.ship_line_id) IN
(SELECT sl2.ship_line_id
  FROM inl_ship_lines_all sl2,
       rcv_transactions rt2,
       mtl_material_transactions mmt2
  WHERE sl2.ship_line_id = rt2.lcm_shipment_line_id
  AND rt2.transaction_id = mmt2.trx_source_line_id
  AND mmt2.transaction_type_id = 80
  AND mmt2.source_code = ''LCMADJ''
  AND mmt2.transaction_date BETWEEN sysdate-90 AND sysdate
  )
UNION
/* Charge and tax landed Cost portions, without invoices */
SELECT x.po_number,
       ...
       x.allocation_id,                
       x.adjustment_num,
       DECODE(x.adjustment_num,0,x.allocation_amt,0) first_estimated_amt,
       DECODE(x.adjustment_num,0,0,x.allocation_amt) adjusted_amt,      
       x.invoice_num,
       NULL match_date             
FROM
(SELECT ph.segment1 po_number,
       ...
       sh.ship_num,
       mp.organization_code,      
       DECODE(a.from_parent_table_name,''INL_CHARGE_LINES'',''Charge'',''INL_TAX_LINES'',''Tax'') component_type,
       DECODE(a.from_parent_table_name,''INL_CHARGE_LINES'',clt.charge_line_type_code,''INL_TAX_LINES'',tl.tax_code) component_code,
       a.allocation_id,      
       a.adjustment_num,
       a.allocation_amt,
       a.creation_date,
       NULL invoice_num,
       cl.match_id cl_match_id,
       cl.match_amount_id cl_match_amount_id,      
       tl.match_id tl_match_id,
       tl.match_amount_id tl_match_amount_id    
...
AND pll.line_location_id = sl.ship_line_source_id
AND sl.ship_line_src_type_code = ''PO''
AND clt.charge_line_type_id(+) = cl.charge_line_type_id
AND cl.charge_line_id(+) = DECODE(a.from_parent_table_name,''INL_CHARGE_LINES'',a.from_parent_table_id,-1)
AND tl.tax_line_id(+) = DECODE(a.from_parent_table_name,''INL_TAX_LINES'',a.from_parent_table_id,-1)      
AND a.ship_line_id = sl.ship_line_id
AND a.from_parent_table_name IN (''INL_CHARGE_LINES'',''INL_TAX_LINES'')
AND a.landed_cost_flag = ''Y''
AND a.allocation_amt <> 0
AND (a.adjustment_num = 0 OR a.adjustment_num = sh.adjustment_num)
AND sh.ship_header_id = sl.ship_header_id
...
WHERE x.cl_match_id IS NULL
AND x.cl_match_amount_id IS NULL
AND x.tl_match_id IS NULL
AND x.tl_match_amount_id IS NULL
UNION
/* Charge and tax landed Cost portions, with invoices (matched to single RT) */
SELECT ph.segment1 po_number,
       (SELECT MAX(rt.transaction_id) -- This MAX is for cases where one ship line has multiple RTs (PLL broken into multiple PDs)
        FROM rcv_transactions rt
        WHERE rt.lcm_shipment_line_id = NVL(sl.parent_ship_line_id,sl.ship_line_id)       
...
AND ph.po_header_id = pll.po_header_id
AND pll.line_location_id = sl.ship_line_source_id
AND sl.ship_line_src_type_code = ''PO''AND ai.invoice_id = aid.invoice_id
AND aid.invoice_distribution_id = m.from_parent_table_id
AND m.from_parent_table_name = ''AP_INVOICE_DISTRIBUTIONS''
AND m.to_parent_table_name = ''INL_SHIP_LINES''
AND m.to_parent_table_id = NVL(sl.parent_ship_line_id,sl.ship_line_id)
AND (m.match_id = DECODE(cl.charge_line_id,NULL,-1,cl.match_id) OR
     m.match_id = DECODE(tl.tax_line_id,NULL,-1,tl.match_id))
AND clt.charge_line_type_id(+) = cl.charge_line_type_id                   
AND cl.charge_line_id(+) = DECODE(a.from_parent_table_name,''INL_CHARGE_LINES'',a.from_parent_table_id,-1)
AND tl.tax_line_id(+) = DECODE(a.from_parent_table_name,''INL_TAX_LINES'',a.from_parent_table_id,-1)
AND a.ship_line_id = sl.ship_line_id
...
(SELECT sl2.ship_line_id
  ...
  AND mmt2.transaction_date BETWEEN sysdate-90 AND sysdate)
UNION
/* Charge and tax landed Cost portions, with invoices (matched to multiple RTs) */
SELECT ph.segment1 po_number,
       (SELECT MAX(rt.transaction_id) -- This MAX is for cases where one ship line has multiple RTs (PLL broken into multiple PDs)
        FROM rcv_transactions rt
        WHERE rt.lcm_shipment_line_id = NVL(sl.parent_ship_line_id,sl.ship_line_id)       
...
AND m.to_parent_table_name = ''INL_SHIP_LINES''
AND m.to_parent_table_id = NVL(sl.parent_ship_line_id,sl.ship_line_id)
AND (ma.match_amount_id = DECODE(cl.charge_line_id,NULL,-1,cl.match_amount_id) OR
     ma.match_amount_id = DECODE(tl.tax_line_id,NULL,-1,tl.match_amount_id))
AND clt.charge_line_type_id(+) = cl.charge_line_type_id    
AND cl.charge_line_id(+) = DECODE(a.from_parent_table_name,''INL_CHARGE_LINES'',a.from_parent_table_id,-1)
AND tl.tax_line_id(+) = DECODE(a.from_parent_table_name,''INL_TAX_LINES'',a.from_parent_table_id,-1)
AND a.ship_line_id = sl.ship_line_id
...
ORDER BY y.po_number,
         y.rcv_transaction_id,
         y.mmt_transaction_id,   
         y.accounting_line_type,
         y.adjustment_num,
         DECODE(y.component_type,''Item'',1,''Charge'',2,''Tax'',3,4),
         y.allocation_id',
      p_title                  => 'Source of Average Cost Update from LCM Adjustments',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'The query lists the Average Cost Update transactions generated from LCM adjustments along with the Invoices that caused the adjustments for the prior 90 days.',
      p_solution               => 'Average Cost Updates will happen in the system as a consequence of the processing of AP invoices in LCM (Actual Landed Cost calculation).

The query lists the Average Cost Update transactions generated from LCM adjustments along with the Invoices that caused the adjustments. [2008564/KB735899]',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
...
and creation_date >= sysdate-90',
   ...
   l_info.delete;
debug('end add_signature: INL_PO_LINE_LOCATIONS_ALL_NULL_NEED_BY_DATE');



debug('begin add_signature: INL_INL_SHIP_HEADERS_INT_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13806',
      p_sig_id                 => 'INL_INL_SHIP_HEADERS_INT_ERROR',
      p_sig_sql                => 'select SHIP_HEADER_INT_ID,GROUP_ID,PROCESSING_STATUS_CODE,TRANSACTION_TYPE,INTERFACE_SOURCE_CODE,INTERFACE_SOURCE_TABLE,INTERFACE_SOURCE_LINE_ID,VALIDATION_FLAG,SHIP_NUM,SHIP_DATE,SHIP_TYPE_ID,SHIP_TYPE_CODE,LEGAL_ENTITY_ID,LEGAL_ENTITY_NAME,ORGANIZATION_ID,ORGANIZATION_CODE,LOCATION_ID,LOCATION_CODE,ORG_ID,TAXATION_COUNTRY,DOCUMENT_SUB_TYPE,SHIP_HEADER_ID,LAST_TASK_CODE,CREATION_DATE,LAST_UPDATE_DATE,LAST_UPDATE_LOGIN,REQUEST_ID,PROGRAM_ID,PROGRAM_APPLICATION_ID,PROGRAM_UPDATE_DATE,RCV_ENABLED_FLAG
from inl_ship_headers_int
where processing_status_code = ''ERROR''',
      p_title                  => 'INL_SHIP_HEADERS_INT shipment transaction are stuck',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Shipment transaction are stuck.',
      p_solution               => 'Datafix for Receipt Transaction Stuck With Status Status LC Interfaced [2288616/KB698974]',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: INL_INL_SHIP_HEADERS_INT_ERROR');



debug('begin add_signature: INL_INL_SHIP_LINES_INT_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13807',
      p_sig_id                 => 'INL_INL_SHIP_LINES_INT_ERROR',
      p_sig_sql                => 'select SHIP_HEADER_INT_ID,SHIP_LINE_INT_ID,GROUP_ID,PROCESSING_STATUS_CODE,VALIDATION_FLAG,SHIP_LINE_GROUP_REFERENCE,PARTY_ID,PARTY_NUMBER,PARTY_SITE_ID,PARTY_SITE_NUMBER,SOURCE_ORGANIZATION_ID,SOURCE_ORGANIZATION_CODE,SHIP_LINE_NUM,SHIP_LINE_TYPE_ID,SHIP_LINE_TYPE_CODE,SHIP_LINE_SRC_TYPE_CODE,SHIP_LINE_SOURCE_ID,CURRENCY_CODE,CURRENCY_CONVERSION_TYPE,CURRENCY_CONVERSION_DATE,CURRENCY_CONVERSION_RATE,INVENTORY_ITEM_ID,TXN_QTY,TXN_UOM_CODE,TXN_UNIT_PRICE,PRIMARY_QTY,PRIMARY_UOM_CODE,PRIMARY_UNIT_PRICE,SECONDARY_QTY,SECONDARY_UOM_CODE,SECONDARY_UNIT_PRICE,LANDED_COST_FLAG,ALLOCATION_ENABLED_FLAG,TRX_BUSINESS_CATEGORY,INTENDED_USE,PRODUCT_FISCAL_CLASS,PRODUCT_CATEGORY,PRODUCT_TYPE,USER_DEF_FISCAL_CLASS,TAX_CLASSIFICATION_CODE,ASSESSABLE_VALUE,SHIP_FROM_PARTY_ID,SHIP_FROM_PARTY_NUMBER,SHIP_FROM_PARTY_SITE_ID,SHIP_FROM_PARTY_SITE_NUMBER,SHIP_TO_ORGANIZATION_ID,SHIP_TO_ORGANIZATION_CODE,SHIP_TO_LOCATION_ID,SHIP_TO_LOCATION_CODE,BILL_FROM_PARTY_ID,BILL_FROM_PARTY_NUMBER,BILL_FROM_PARTY_SITE_ID,BILL_FROM_PARTY_SITE_NUMBER,BILL_TO_ORGANIZATION_ID,BILL_TO_ORGANIZATION_CODE,BILL_TO_LOCATION_ID,BILL_TO_LOCATION_CODE,POA_PARTY_ID,POA_PARTY_NUMBER,POA_PARTY_SITE_ID,POA_PARTY_SITE_NUMBER,POO_ORGANIZATION_ID,POO_TO_ORGANIZATION_CODE,POO_LOCATION_ID,POO_LOCATION_CODE,ORG_ID,SHIP_HEADER_ID,SHIP_LINE_ID,INTERFACE_SOURCE_TABLE,INTERFACE_SOURCE_LINE_ID,CREATED_BY,CREATION_DATE,LAST_UPDATED_BY,LAST_UPDATE_DATE,LAST_UPDATE_LOGIN,REQUEST_ID,PROGRAM_ID,PROGRAM_APPLICATION_ID,PROGRAM_UPDATE_DATE,SHIP_LINE_GROUP_NUM,INVENTORY_ITEM_NAME,SHIP_LINE_GROUP_ID
from inl_ship_lines_int
where processing_status_code = ''ERROR''',
      p_title                  => 'INL_SHIP_LINES_INT_ERROR transaction are stuck',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Shipment transaction are stuck.',
      p_solution               => 'Datafix for Receipt Transaction Stuck With Status Status LC Interfaced [2288616/KB698974]',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: INL_INL_SHIP_LINES_INT_ERROR');



debug('begin add_signature: LCM_GMF_LC_ADJ_TRANSACTIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '12588',
      p_sig_id                 => 'LCM_GMF_LC_ADJ_TRANSACTIONS',
      p_sig_sql                => 'select  ADJ_TRANSACTION_ID,
        RCV_TRANSACTION_ID,
        EVENT_TYPE,
        EVENT_SOURCE,
        EVENT_SOURCE_ID,
 ...
 WHERE rcv_transaction_id IN
   ...
   AND TRANSACTION_DATE >= sysdate-90',
      p_title                  => 'Adjustment transactions for OPM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Adjustment Transactions in OPM',
      p_solution               => 'You should review adjustment transactions created in OPM for the PO in case. 
Please review OPM LCM Transactional Data Diagnostic [1664499/KB772247]',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: LCM_GMF_LC_ADJ_TRANSACTIONS');



debug('begin add_signature: INL_Shipment_Accounting_Transactions');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '20598',
      p_sig_id                 => 'INL_Shipment_Accounting_Transactions',
      p_sig_sql                => 'SELECT DECODE (raet.event_type_name, ''RECEIVE'', 1, ''LOGICAL_RECEIVE'', 1, ''LDD_COST_ADJ_RCV'', 3.1, ''LDD_COST_ADJ_DEL_ASSET'', 3.2, ''RETURN_TO_RECEIVING'', 6.1, ''RETURN_TO_VENDOR'', 6.2) event_type,
       rsl.creation_date,
       sh.ship_header_id,
       mp.organization_id,
       mp.organization_code,
...
ORDER BY ship_line_num, event_type, creation_date, mmt_transaction_id',
      p_title                  => 'Accounting Transactions that Occur for a Particular LCM shipment',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'This script lists all the accounting transactions that occur for a particular LCM shipment in the Procure to Pay cycle.',
      p_solution               => 'Review LCM Script for Accounting transactions that occur for a particular LCM shipment [2012856/KB752659]',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: INL_Shipment_Accounting_Transactions');



debug('begin add_signature: INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ON_HOLD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9382',
      p_sig_id                 => 'INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ON_HOLD',
      p_sig_sql                => 'SELECT mp.organization_code,
       mp.organization_id,
       sh.ship_num,
       sh.ship_status_code
...
WHERE mp.organization_id = sh.organization_id 
AND sh.ship_header_id = sl.ship_header_id
AND sh.ship_status_code = ''ON HOLD'' 
AND sl.ship_line_id = rti.shipment_line_id
AND rti.processing_status_code = ''LC_INTERFACED''',
      p_title                  => 'Transactions with inl_ship_headers_all.ship_status_code = ON HOLD',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions with  inl_ship_headers_all.ship_status_code = ON HOLD.',
      p_solution               => '<ul>
<li> For resolution review [2346779/KB722683#Onhold] LCM as a Service flow – Stuck rcv_transactions_interface.processing_status_code = LC_INTERFACED and How to Solve Them
, Query 1.</li></ul>',
      p_success_msg            => 'There are no Error Transactions with inl_ship_headers_all.ship_status_code = ON HOLD.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ON_HOLD');



debug('begin add_signature: INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9383',
      p_sig_id                 => 'INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ERROR',
      p_sig_sql                => 'SELECT  ishi.ship_header_int_id,ishi.processing_status_code,
        isli.ship_line_int_id,isli.processing_status_code isli_processing_status_code
FROM    inl_ship_headers_int ishi,inl_ship_lines_int isli,rcv_transactions_interface rti
WHERE   ishi.ship_header_int_id = isli.ship_header_int_id
AND     ishi.processing_status_code =''ERROR''
AND     isli.ship_line_source_id = rti.po_line_location_id
AND     rti.processing_status_code =''LC_INTERFACED''',
      p_title                  => 'Transactions with inl_ship_headers_all.ship_status_code = ERROR',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transactions with inl_ship_headers_all.ship_status_code = ERROR.',
      p_solution               => '<ul>
<li> Review [2346779/KB722683#Error] LCM as a Service flow – Stuck rcv_transactions_interface.processing_status_code = LC_INTERFACED and How to Solve Them
, Query 2.</li></ul>',
      p_success_msg            => 'There are no Error Transactions with inl_ship_headers_all.ship_status_code = ERROR.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ERROR');



debug('begin add_signature: LCM_STUCK_RTI_RECORDS_DP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10769',
      p_sig_id                 => 'LCM_STUCK_RTI_RECORDS_DP',
      p_sig_sql                => 'SELECT INTERFACE_TRANSACTION_ID,
  CREATION_DATE,
  TRANSACTION_TYPE,
...
(SELECT TRANSACTION_ID
FROM RCV_TRANSACTIONS
WHERE PROCESSING_STATUS_CODE IN (''LC_PENDING'',''LC_INTERFACED''))
ORDER BY INTERFACE_TRANSACTION_ID DESC',
      p_title                  => 'LCM Records stuck in RTI with status LC_PENDING or LC_INTERFACED',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This signature displays the LCM records that are stuck in interface table RCV_TRANSACTIONS_INTERFACE RTI during the receiving processes, with a status of LC_PENDING or LC_INTERFACED and for which the usual delete script used in receiving area ([303544/KB162444] Delete_stuck_rti_for_R121_and_above.sql) cannot be used as the identification script will fail to pick up such records.',
      p_solution               => 'There are LCM records stuck in RCV_TRANSACTIONS_INTERFACE.
These records CANNOT be deleted using the generic datafix script as provided by [303544/KB162444].
Service Request needs to be routed to Landed Cost Management support team for further assistance. For Support reference only, use for assistance [2081703/KB198268],[1953079/KB723047],[2146885/KB422497]',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: LCM_STUCK_RTI_RECORDS_DP');
...
debug('begin add_signature: MANDATORY_PROFILES_LCM_DISCRETE');
   l_info('##MASK##FPOV.PROFILE_OPTION_VALUE'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5759',
      p_sig_id                 => 'MANDATORY_PROFILES_LCM_DISCRETE',
      p_sig_sql                => 'SELECT TEMP.USER_PROFILE_OPTION_NAME,
          TEMP.PROFILE_OPTION_NAME,
          TEMP.PROFILE_LEVEL,
          TEMP.Level_Value,
          TEMP.PROFILE_OPTION_VALUE,
          TEMP.PROFILE_OPTION_ID,
          TEMP.UPDATED_DATE
  FROM(
  ...
  WHERE FPO.PROFILE_OPTION_ID   = FPOV.PROFILE_OPTION_ID
  and FPO.PROFILE_OPTION_NAME  in (''QP_BLIND_DISCOUNT'', ''QP_LICENSED_FOR_PRODUCT'', ''QP_PRICING_PERSPECTIVE_REQUEST_TYPE'', ''QP_PRICING_TRANSACTION_ENTITY'',''QP_SOURCE_SYSTEM_CODE'', ''INL_QP_CURRENCY_CODE'', ''INL_QP_CURRENCY_CONVERSION_TYPE'', ''INL_SHIP_TYPE_ID_OI'', ''INL_QUANTITY_UOM_CLASS'', ''INL_VOLUME_UOM_CLASS'', ''INL_WEIGHT_UOM_CLASS'')
  ...
  order by 1,  FPOV.LEVEL_ID,  4) TEMP
  where ( TEMP.PROFILE_LEVEL= ''Site'' or TEMP.PROFILE_LEVEL= ''Responsibility'' 
	  or TEMP.PROFILE_LEVEL= ''User'' or TEMP.PROFILE_LEVEL= ''Server'' 
	  or TEMP.PROFILE_LEVEL= ''Organization'' or TEMP.PROFILE_LEVEL= ''Unknown''
	  or (TEMP.PROFILE_LEVEL= ''Application'' and TEMP.LEVEL_VALUE in ( ''INL'', ''QP'', ''PO''))
      )',
      p_title                  => 'Mandatory Profiles for LCM Integration - Discrete',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This shows specific profiles that should be reviewed to ensure proper setup.',
      p_solution               => '<ul>
   <li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#QPProfile], Section Setup - Setting Up Advanced Pricing Profile Options </B> and [862452/KB729763#LCMProfile] Setting Up Landed Cost Management Profile Options</B>.</li> <li>Also refer to [1498479/KB662531], UOM Calculation with different Allocation Basis. </li>
   </ul>',
      p_success_msg            => 'This shows specific profiles that should be reviewed to ensure proper setup.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: MANDATORY_PROFILES_LCM_DISCRETE');



debug('begin add_signature: VERSIONS_IMPORTANT_PACKAGES_DISCRETE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '5780',
      p_sig_id                 => 'VERSIONS_IMPORTANT_PACKAGES_DISCRETE',
      p_sig_sql                => 'SELECT srce.name sname, 
          srce.type stype,
          substr(srce.text,instr(srce.text,'':'')+2, (instr(srce.text, '' '', 10, 2)-1) - (instr(srce.text,'':'')+1)) SFile,
          substr(srce.text,instr(srce.text,'' '', 10, 2)+1, (instr(srce.text, '' '', 10, 3)-1) - (instr(srce.text,'' '', 10, 2))) SVer,
          obj.status ostatus
    FROM dba_source srce, dba_objects obj
    where srce.name = obj.object_name and srce.type = obj.object_type
    and srce.name in (''CST_LCMADJUSTMENTS_PVT'', ''CST_LCMADJUSTMENTS_PUB'', ''CST_LANDEDCOSTHOOK_PUB'',
                      ''RCV_CALL_LCM_WS'', ''INL_INTERFACE_PVT'', ''INL_SHIPMENT_PVT'', ''INL_INTEGRATION_GRP'', ''RCV_UPDATE_RTI_LC'', ''INL_LANDEDCOST_PUB'')
    and srce.text like ''%$Header%''
    and SRCE.LINE < 10
    order by srce.name, srce.type',
      p_title                  => 'Versions of Important Discrete Costing/LCM PL/SQL Packages',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This shows Important Discrete Costing/LCM PL/SQL Packages.',
      p_solution               => '<ul>
   <li>Review the Latest Recommended Patches of LCM and CST as per [1989683/KB548692] Landed Cost Management (LCM) Recommended Patch Collection (RPC) Priority Patches for release 12.1 & 12.2 and [1213663/KB269969] Oracle Cost Management Recommended Patch Collection (RPC) Priority Patches.</li>
   </ul>',
      p_success_msg            => 'This shows Important Discrete Costing/LCM PL/SQL Packages.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: VERSIONS_IMPORTANT_PACKAGES_DISCRETE');



debug('begin add_signature: MANDATORY_PROFILES_LCM_OPM');
   l_info('##MASK##LEVEL_VALUE'):= 'DISPLAY_BOTH_25_PCNT_WORD';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '6140',
      p_sig_id                 => 'MANDATORY_PROFILES_LCM_OPM',
      p_sig_sql                => 'SELECT USER_PROFILE_OPTION_NAME,PROFILE_OPTION_NAME,PROFILE_LEVEL,LEVEL_VALUE,PROFILE_OPTION_VALUE,PROFILE_OPTION_ID,UPDATED_DATE
   FROM( SELECT FPO.USER_PROFILE_OPTION_NAME, 
                FPO.PROFILE_OPTION_NAME, 
                DECODE(FPOV.LEVEL_ID, 10001, ''Site'', 10002, ''Application'', 10003, ''Responsibility'', 10004, ''User'', 10005, ''Server'', 10006, ''Organization'', ''Unknown'') AS PROFILE_LEVEL , 
                DECODE(FPOV.level_id, 10001, ''Site'', 10002, FA.application_short_name, 10003, FR.RESPONSIBILITY_NAME, 10004, FU.USER_NAME, 10005, FN.NODE_NAME, 10006, HOU.name, ''Unknown'') AS Level_Value , 
                FPOV.PROFILE_OPTION_VALUE , 
                FPOV.PROFILE_OPTION_ID , 
                TO_CHAR(FPOV.LAST_UPDATE_DATE,''DD-MON-YYYY HH24:MI'') AS UPDATED_DATE , 
                FU_UPD.user_name AS updated_by FROM FND_PROFILE_OPTIONS_VL FPO , 
                FND_PROFILE_OPTION_VALUES FPOV , 
                FND_USER FU_UPD , 
                FND_USER FU , 
                FND_RESPONSIBILITY_VL FR , 
                FND_APPLICATION FA , 
                FND_NODES FN , 
                HR_OPERATING_UNITS HOU WHERE FPO.PROFILE_OPTION_ID = FPOV.PROFILE_OPTION_ID 
                and FPO.PROFILE_OPTION_NAME in (''QP_BLIND_DISCOUNT'', ''QP_LICENSED_FOR_PRODUCT'', ''QP_PRICING_PERSPECTIVE_REQUEST_TYPE'', ''QP_PRICING_TRANSACTION_ENTITY'', 
                                                ''QP_SOURCE_SYSTEM_CODE'', ''INL_QP_CURRENCY_CODE'', ''INL_QP_CURRENCY_CONVERSION_TYPE'', ''INL_SHIP_TYPE_ID_OI'',  
                                                ''INL_QUANTITY_UOM_CLASS'', ''INL_VOLUME_UOM_CLASS'', ''INL_WEIGHT_UOM_CLASS'', ''GMF_LC_IV_TRANSFER'') 
                and FPO.APPLICATION_ID = FPOV.APPLICATION_ID 
                and FPOV.LAST_UPDATED_BY = FU_UPD.USER_ID (+) 
                AND FPOV.LEVEL_VALUE = FU.USER_ID (+)  
                and FPOV.LEVEL_VALUE = FN.NODE_ID (+)  
                ...
                TEMP where ( TEMP.PROFILE_LEVEL= ''Site'' or TEMP.PROFILE_LEVEL= ''Responsibility'' or TEMP.PROFILE_LEVEL= ''User'' or TEMP.PROFILE_LEVEL= ''Server'' 
                             or TEMP.PROFILE_LEVEL= ''Organization'' or TEMP.PROFILE_LEVEL= ''Unknown'' or (TEMP.PROFILE_LEVEL= ''Application'' 
                             and TEMP.LEVEL_VALUE in ( ''INL'', ''QP'', ''GMF'', ''PO'')) )',
      p_title                  => 'Mandatory profiles for LCM Integration with OPM',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This shows specific profiles that should be reviewed to Ensure Proper Setup.',
      p_solution               => '<ul><li> Review Note - EBS LCM :How to Diagnose Issues Related to the Landed Cost Management (LCM) [862452/KB729763#QPProfile], Section Setup - Setting Up Advanced Pricing Profile Options </B> and[862452/KB729763#LCMProfile] Setting Up Landed Cost Management Profile Options</B>.</li> <li>Also refer to [1498479/KB662531] UOM Calculation with different Allocation Basis.</li></ul>',
      p_success_msg            => 'This shows specific profiles that should be reviewed to ensure proper setup.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: MANDATORY_PROFILES_LCM_OPM');



debug('begin add_signature: VERSIONS_IMPORTANT_PACKAGES_OPM');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '6141',
      p_sig_id                 => 'VERSIONS_IMPORTANT_PACKAGES_OPM',
      p_sig_sql                => 'SELECT srce.name sname, 
          srce.type stype,
          substr(srce.text,instr(srce.text,'':'')+2, (instr(srce.text, '' '', 10, 2)-1) - (instr(srce.text,'':'')+1)) SFile,
          substr(srce.text,instr(srce.text,'' '', 10, 2)+1, (instr(srce.text, '' '', 10, 3)-1) - (instr(srce.text,'' '', 10, 2))) SVer,
    ...
    FROM dba_source srce, dba_objects obj
    where srce.name = obj.object_name and srce.type = obj.object_type
    and srce.name in (''GMF_LC_ADJ_TRANSACTIONS_PKG'', ''GMF_LOT_COSTING_PUB'', ''GMF_LAYERS'', ''GMF_LOTCOSTADJUSTMENT_PUB'', ''GMF_LOTCOSTADJUSTMENT_PVT'', ''RCV_CALL_LCM_WS'', 
                       ''INL_INTERFACE_PVT'', ''INL_SHIPMENT_PVT'', ''INL_INTEGRATION_GRP'', ''RCV_UPDATE_RTI_LC'', ''INL_LANDEDCOST_PUB'') 
    and srce.text like ''%$Header%'' 
    and srce.line < 10 
    order by srce.name, srce.type',
      p_title                  => 'Versions of Important OPM/LCM PL/SQL Packages',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'This shows Important OPM/LCM PL/SQL Packages.',
      p_solution               => '<ul>
   <li>Refer to [1935888/KB230972] recommended File Versions and the associated patches.</li>
   </ul>',
      p_success_msg            => 'This shows Important OPM/LCM PL/SQL Packages.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: VERSIONS_IMPORTANT_PACKAGES_OPM');
...
debug('begin section: Inventory_Org_Parameters');
start_section('Inventory Organization Parameters', 'Inventory_Org_Parameters');
   set_item_result(run_stored_sig('INV_ORG_PARAMS_PROCESS_LCM_ENABLED'));
end_section;
debug('end section: Inventory_Org_Parameters');

debug('begin section: Receiving_Org_Parameters');
start_section('Receiving Organization Parameters', 'Receiving_Org_Parameters');
   set_item_result(run_stored_sig('RECEIVING_OPTIONS'));
end_section;
debug('end section: Receiving_Org_Parameters');

debug('begin section: UOM_CLASSES');
start_section('UOM Classes', 'UOM_CLASSES');
   set_item_result(run_stored_sig('UOM_CLASSES'));
end_section;
debug('end section: UOM_CLASSES');

debug('begin section: Cost_Factors_Setup');
start_section('Cost Factors Setup', 'Cost_Factors_Setup');
   set_item_result(run_stored_sig('COST_FACTORS'));
   set_item_result(run_stored_sig('LANDED_COST_ADJ_COST_FACTOR_GREATER_30'));
end_section;
debug('end section: Cost_Factors_Setup');

debug('begin section: LCM_Setup');
start_section('LCM Setup', 'LCM_Setup');
   set_item_result(run_stored_sig('LCM_OPTIONS'));
   set_item_result(run_stored_sig('LCM_SHIPMENT_LINE_TYPES'));
   set_item_result(run_stored_sig('LCM_SHIPMENT_TYPES'));
   set_item_result(run_stored_sig('LCM_SHIPMENT_TYPES_ALLOWED'));
   set_item_result(run_stored_sig('LCM_PARTY_TYPES_ALLOWED'));
   set_item_result(run_stored_sig('LCM_PARTY_USAGES_ALLOWED'));
   set_item_result(run_stored_sig('LCM_SOURCE_TYPES_ALLOWED'));
end_section;
debug('end section: LCM_Setup');

debug('begin section: Price_Modifier_Setup');
start_section('Price Modifier Setup', 'Price_Modifier_Setup');
   set_item_result(run_stored_sig('PRICE_MODIFIER_HEADERS'));
   set_item_result(run_stored_sig('PRICE_MODIFIER_LINES'));
end_section;
debug('end section: Price_Modifier_Setup');

debug('begin section: Transaction_Details');
start_section('Transaction Details', 'Transaction_Details');
   set_item_result(run_stored_sig('INL_AC_UPDATE_FROM_LCM_ADJ'));
   set_item_result(run_stored_sig('INL_PO_LINE_LOCATIONS_ALL_NULL_NEED_BY_DATE'));
   set_item_result(run_stored_sig('INL_INL_SHIP_HEADERS_INT_ERROR'));
   set_item_result(run_stored_sig('INL_INL_SHIP_LINES_INT_ERROR'));
   set_item_result(run_stored_sig('LCM_GMF_LC_ADJ_TRANSACTIONS'));
   set_item_result(run_stored_sig('INL_Shipment_Accounting_Transactions'));
end_section;
debug('end section: Transaction_Details');

debug('begin section: Pending_Transactions');
start_section('Pending Transactions', 'Pending_Transactions');
   set_item_result(run_stored_sig('INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ON_HOLD'));
   set_item_result(run_stored_sig('INL_SHIP_HEADERS_ALL_SHIP_STATUS_CODE_ERROR'));
   set_item_result(run_stored_sig('LCM_STUCK_RTI_RECORDS_DP'));
   set_item_result(run_stored_sig('CST_LC_ADJ_INTERFACE_COLUMN CANNOT HAVE NEGATIVE VALUE'));
end_section;
debug('end section: Pending_Transactions');
...
start_section('Proactive Recommendations', 'Proactive_Recommendations');
   IF '##$$BOM$$##' <> 'N' THEN
      set_item_result(run_stored_sig('MANDATORY_PROFILES_LCM_DISCRETE'));
      set_item_result(run_stored_sig('VERSIONS_IMPORTANT_PACKAGES_DISCRETE'));
   END IF;
   set_item_result(check_rec_patches_1); /* Signature INL_REC_PATCHS */
   IF '##$$GMF$$##' <> 'N' THEN
      set_item_result(run_stored_sig('MANDATORY_PROFILES_LCM_OPM'));
      set_item_result(run_stored_sig('VERSIONS_IMPORTANT_PACKAGES_OPM'));
   END IF;
end_section;
