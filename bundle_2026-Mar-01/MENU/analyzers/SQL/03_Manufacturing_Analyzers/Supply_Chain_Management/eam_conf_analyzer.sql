...
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to install package used for EAM Configuration Analyzer             |
REM |                                                                           |
REM | HISTORY                                                                   |
REM +===========================================================================+
...
CREATE OR REPLACE PACKAGE eam_conf_analyzer_pkg AUTHID DEFINER AS
...
END eam_conf_analyzer_pkg;
/
show errors


CREATE OR REPLACE PACKAGE BODY eam_conf_analyzer_pkg AS
-- $Id: eam_conf_analyzer.sql, 200.30 2026/01/28 12:51:55 dbuteica Exp $
...
g_analyzer_doc_id VARCHAR2(15) := TO_CHAR(TRUNC(2352539.1));
...
BEGIN
  ...
  l_step := '2';
    ...
    l_log_file := 'EAM_CONF_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.log';
    l_out_file := 'EAM_CONF_Analyzer_'||l_db_name||'_'||g_params_string||l_date_char||'.html';
    l_step := '4';
...
BEGIN
    ...
    print_out('<TITLE>EAM_CONF Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    ...
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2352539.1#feedback");
...
-- top right menu
    ...
    print_out('<div class="menubutton" id="execParameters" title="Shows the analyzer''s execution parameters" data-popup-open="popup-2">
          ...
          Parameters</div>');
    print_out('<div class="menubutton" id="feedback" title="Opens the Oracle Community feedback thread"><a href="https://community.oracle.com/thread/4116596" class="blacklink" target="new">
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">1394</span>
...
<ul><li><span style="font-family:Arial,Helvetica,sans-serif;font-size:12px;">200.29 MOSFS links</span></li>
<li><span style="font-family:Arial,Helvetica,sans-serif;font-size:12px;">This is version of the EAM Information Analyzer.&nbsp;</span></li>
<li><span style="font-family:Arial,Helvetica,sans-serif;font-size:12px;">PIW review.</span></li>
<li>Premier Support Bulletin added.</li>
<li>Framework was upgraded to 4.5.6.</li>
<li>Added new signatures.</li>
</ul>
...
<!-- '||l_current_sig||' -->
    ...
    -- Add feedback
	l_html := l_html || '
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},up,Analyzer,1394,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_up" title="Signature Feedback: Thumbs Up"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:17::::11,RIR,RP:P17_SR_NUMBER,P17_THUMB,P17_TYPE,P17_ANALYZER_ID,P17_SIGNATURE_ID,P17_PLA_LINE,P17_PLA_FAMILY,P17_PLA_Area:{SR},dn,Analyzer,1394,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="thumb_dn" title="Signature Feedback: Thumbs Down"></span></a>
			<a class="detailsmall internal" data-sr-href="https://aseobs.oraclecorp.com/ords/f?p=345:56::::11,RIR,RP:P56_SR,P56_ANALYZER_ID,P56_SIGNATURE_ID,P56_PLA_LINE_ANALYTICS,P56_PLA_FAMILY_1,P56_PLA_Area_1:{SR},1394,'||p_sig.sigrepo_id||',EBS,EBS MFG,EBS - Discrete MFG Execution and Financials" target="_blank"><span class="af_idea" title="Signature Idea"></span></a>
	</div>';
...
FUNCTION check_rec_patches_1 RETURN VARCHAR2 IS
  /* Signature EAM_AN_RECOMMENDED_PATCHES */
...
BEGIN
    ...
    print_log('Processing recommended patches signature: EAM_AN_RECOMMENDED_PATCHES (check_rec_patches_1) "Recommended Patches" (version 6)');
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.1' THEN
   ...
   l_col_rows(1)(l_rows) := '14017681';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'Oracle Enterprise Asset Management: Post 12.1.1 consolidation Patch which is Applicable On top of 12.1.1, 12.1.2, 12.1.3';
   l_col_rows(5)(l_rows) := '[1938936.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '24396610';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.1.3 - EAM 2016 CONSOLIDATION PATCH';
   l_col_rows(5)(l_rows) := '[1938936.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '26583268';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'After this Patch, if one PMSchedule ends in error, instead of whole PM Run ending with exception, the problematic PMSchedule is skipped and PM generates WOs for other valid PMSchedules successfully';
   l_col_rows(5)(l_rows) := '[1938936.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '26353272';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := 'PM CREATES DUP WO IF PM RAN FOR FUTURE SKIPPING CURRENT CYCLE WITH NONMOV MOVED';
   l_col_rows(5)(l_rows) := '[1938936.1]';
...
IF substr(g_rep_info('Apps Version'),1,4) = '12.2' THEN
   ...
   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '24498252';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.2.x consolidation Applicable all 12.2.X releases till 12.2.6';
   l_col_rows(5)(l_rows) := '[1938936.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '26983458';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '12.2.4 or above ORA-01000: MAXIMUM OPEN CURSORS EXCEEDED';
   l_col_rows(5)(l_rows) := '[1938936.1]';

   l_rows := l_rows + 1;
   l_rel_rows(1)(l_rows) := l_rel;
   l_col_rows(1)(l_rows) := '28354685';
   l_col_rows(2)(l_rows) := 'No';
   l_col_rows(3)(l_rows) := NULL;
   l_col_rows(4)(l_rows) := '(12.2.6+) EAM PROGRAM GENERATE PREVENTIVE MAINTENANCE WORK ORDERS GENERATE DUPLICATE WO';
   l_col_rows(5)(l_rows) := '[1938936.1]';
...
<ul>
<li>[1938936/KB521788] Enterprise Asset Management Latest Patch Level for R12 (12.1.X/12.2.X)</li>
...
<li>[10940175/KA580] Patch Wizard FAQ</li>
   ...
   l_sig.success_msg := 'All recommended patches are applied.';
  ...
  -- Register
  l_step := '60';
  g_signatures('EAM_AN_RECOMMENDED_PATCHES') := l_sig;

  --Render
  l_step := '70';
  RETURN process_signature_results(
    'EAM_AN_RECOMMENDED_PATCHES',     -- sig ID
...
debug('begin populate parameters hash table');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Maximum Rows to Display (Default 500):';
   g_parameters(g_parameters.LAST).pvalue := mask_text(p_max_output_rows,'NO_MASK');
   g_parameters.extend();
   g_parameters(g_parameters.LAST).pname := 'Debug Mode, Defaults to Yes or Y';
...
-- Revision and date values can be populated by RCS
  ...
  l_date_char := rtrim(replace('$Date: 2026/01/28 12:51:54 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'eam_conf_analyzer.sql';
...
debug('begin add_signature: EAM_ORG_STRUCTURE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9371',
      p_sig_id                 => 'EAM_ORG_STRUCTURE',
      p_sig_sql                => 'SELECT MP.ORGANIZATION_ID   ORG_ID,
       MP.ORGANIZATION_CODE ORG,
       ML1.MEANING "Serial Uniqueness",
       ML2.MEANING "Serial ",
       MP.MAINT_ORGANIZATION_ID "Maintenance OrgID",
       MP.COST_ORGANIZATION_ID "Costing OrgID",
       MP.MASTER_ORGANIZATION_ID "Master OrgID",
       DECODE(MP.EAM_ENABLED_FLAG, ''Y'', ''Yes'', ''N'', ''No'') "EAM Enabled",
       DECODE(MP.PROJECT_REFERENCE_ENABLED, 1, ''Yes'', 2, ''No'') "Project Enabled",
       DECODE(MP.PRIMARY_COST_METHOD,1,''Standard'',2,''Average'',5,''FIFO'',6,''LIFO'') "Costing Method",
       MP.COST_CUTOFF_DATE "Cost Cutoff Date"
  FROM MTL_PARAMETERS MP, MFG_LOOKUPS ML1, MFG_LOOKUPS ML2
 WHERE MP.SERIAL_NUMBER_TYPE = ML1.LOOKUP_CODE
   AND ML1.LOOKUP_TYPE = ''MTL_SERIAL_NUMBER_TYPE''
   AND MP.SERIAL_NUMBER_TYPE = ML2.LOOKUP_CODE
   AND ML2.LOOKUP_TYPE = ''MTL_SERIAL_GENERATION''
   AND MP.EAM_ENABLED_FLAG = ''Y''
 ORDER BY ORGANIZATION_CODE',
      p_title                  => 'EAM Enabled Organizations',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'EAM enabled organizations.',
      p_solution               => '',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EAM_ORG_STRUCTURE');



debug('begin add_signature: EAM_WODC_SETUP_EAM');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9627',
      p_sig_id                 => 'EAM_WODC_SETUP_EAM',
      p_sig_sql                => 'SELECT WEP.ORGANIZATION_ID ORG_ID,
       MP.ORGANIZATION_CODE ORG_CODE,
       DECODE(SERIAL_NUMBER_ENABLED, ''Y'', ''Yes'', ''No'') SERIAL_AUTOGEN,
       DECODE(DEF_EAM_COST_ELEMENT_ID,
              1,''Element'',
              2,''Labor'',
              3,''Material'') DEF_COST_ELEMENT,
       DECODE(DEF_MAINT_COST_CATEGORY,
              1,''Maintenance'',
              2,''Operations'',
              3,''Contract'') DEF_COST_CATEGORY,
       DECODE(PM_IGNORE_MISSED_WO, ''Y'', ''Yes'', ''No'') PM_IMPL_HORIZON_START,
       DECODE(WORK_REQUEST_AUTO_APPROVE, ''N'', ''No'', ''Yes'') WR_AUTO_APPROVE,
       DECODE(WORK_REQ_EXTENDED_LOG_FLAG, ''N'', ''No'', ''Yes'') WR_EXT_LOG_FLAG,
       DECODE(DEFAULT_ASSET_FLAG, ''Y'', ''Yes'', ''No'') WR_DEF_ASSET,
       DECODE(WORK_REQUEST_ASSET_NUM_REQD, ''Y'', ''Yes'', ''No'') WR_ASSET_MAND,
       DEFAULT_EAM_CLASS WIP_ACC_CLASS,
       EASY_WORK_ORDER_PREFIX EXPRESS_WO_PREFIX,
       WORK_ORDER_PREFIX WO_PREFIX,
       BD.DEPARTMENT_CODE DEF_DEPARTMENT,
       DECODE(EAM_WO_WORKFLOW_ENABLED, ''Y'', ''Yes'', ''No'') WO_WORKFLOW,
       DECODE(MATERIAL_ISSUE_BY_MO, ''Y'', ''Yes'', ''No'') MAT_ISSUE_REQ,
       DECODE(ISSUE_ZERO_COST_FLAG, ''N'', ''No'', ''Yes'') REBUILD_ZERO_COST,
       DECODE(INVOICE_BILLABLE_ITEMS_ONLY, ''Y'', ''Yes'', ''No'') INV_BILL_ITEM_ONLY,
       DECODE(AUTO_FIRM_FLAG, ''Y'', ''Yes'', ''No'') AUTO_FIRM_RELEASE,
       DECODE(AUTO_FIRM_ON_CREATE, ''Y'', ''Yes'', ''No'') AUTO_FIRM_CREATE,
       MAINTENANCE_OFFSET_ACCOUNT MAINT_OFFSET_ACC,
       INTERMEDIATE_SUBINVENTORY MOVE_INTER_SUBINV,
       INTERMEDIATE_LOCATOR MOVE_INTER_LOC
  FROM WIP_EAM_PARAMETERS WEP, APPS.MTL_PARAMETERS MP, BOM_DEPARTMENTS BD
 WHERE MP.EAM_ENABLED_FLAG = ''Y''
   AND WEP.ORGANIZATION_ID = MP.ORGANIZATION_ID (+)
   AND WEP.DEFAULT_DEPARTMENT_ID = BD.DEPARTMENT_ID (+)
   ORDER BY 2',
      p_title                  => 'EAM Enabled Organizations - EAM Parameters',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No EAM enabled organizations are defined.',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WODC_SETUP_EAM');



debug('begin add_signature: EAM_WODC_SETUP_WIP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9626',
      p_sig_id                 => 'EAM_WODC_SETUP_WIP',
      p_sig_sql                => 'SELECT WP.ORGANIZATION_ID ORG_ID,
       MP.ORGANIZATION_CODE ORG_CODE,
       DEFAULT_DISCRETE_CLASS DEF_DISCRETE_CLASS,
       DECODE(LOT_NUMBER_DEFAULT_TYPE,
              1,''Job Name'',
              2,''Based on Inventory Rules'',
              3,''No Default'') DEF_LOT_NUMBER_TYPE,
       DECODE(SO_CHANGE_RESPONSE_TYPE, 1, ''Never'', 2, ''Always'', 3, ''1 to 1'') RESPOND_SO_CHANGE,
       DECODE(REPETITIVE_VARIANCE_TYPE,
              1,''All Schedules'',
              2,''Canceleed and CompleteNoChg Only'') RECOGNIZE_PERIOD_VARIANCE,
       AUTORELEASE_DAYS,
       DECODE(AUTO_COMPUTE_FINAL_COMPLETION, 1, ''Yes'', ''No'') AUTO_COMPUTE_FINAL_COMPL,
       DECODE(MANDATORY_SCRAP_FLAG, 1, ''Yes'', ''No'') REQUIRE_SCRAP_ACC,
       DECODE(DYNAMIC_OPERATION_INSERT_FLAG, 1, ''Yes'', ''No'') ALLOW_CREATION_NEW_OP,
       DECODE(MOVES_OVER_NO_MOVE_STATUSES, 1, ''Yes'', ''No'') ALLOW_OVER_NOMOVE_SF_STATUS,
       DEFAULT_PULL_SUPPLY_SUBINV SUP_SUBINV,
       DEFAULT_PULL_SUPPLY_LOCATOR_ID SUP_LOC,
       ...
       DECODE(ALLOW_BACKFLUSH_QTY_CHANGE, ''Y'', ''Yes'', ''No'') ALLOW_QTY_CHANGE_BF,
       DECODE(PO_CREATION_TIME,
              1,''Job Release'',
              2,''Operation'',
              3,''Manual'') PR_CREATION,
       DECODE(PROPAGATE_JOB_CHANGE_TO_PO, 1, ''Auto'', 2, ''Manual'') PROPAGATE_JOB_PO,
       DECODE(DEFAULT_SERIALIZATION_START_OP, 1, ''Yes'', 2, ''No'') SERIAL_START_OP,
       DEFAULT_OVERCOMPL_TOLERANCE DEF_OVERCOMPL_TOL,
       WPGR.NAME DEF_PICK_RULE
 ...
 WHERE MP.EAM_ENABLED_FLAG = ''Y''
   ...
   ORDER BY 2   ',
      p_title                  => 'EAM Enabled Organizations - WIP Parameters',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No EAM enabled organizations are defined.',
      p_solution               => '',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EAM_WODC_SETUP_WIP');



debug('begin add_signature: EAM_WODC_SETUP_SUBINV');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9625',
      p_sig_id                 => 'EAM_WODC_SETUP_SUBINV',
      p_sig_sql                => 'SELECT MSV.ORGANIZATION_ID ORG_ID,
       MP.ORGANIZATION_CODE ORG_CODE,
       MSV.SECONDARY_INVENTORY_NAME SUBINV_NAME,
       DECODE(MSV.RESERVABLE_TYPE, 1, ''YES'', 2, ''NO'', RESERVABLE_TYPE) RESERVABLE,
       DECODE(MSV.QUANTITY_TRACKED, 1, ''YES'', ''NO'') QtyTracked,
       DECODE(MSV.ASSET_INVENTORY, 1, ''Asset'', ''Expense'') SUBINV_TYPE,
       MSV.DISABLE_DATE,
       DECODE(MSV.INVENTORY_ATP_CODE,
              1,''INCL IN ATP CALC'',
              2,''NOT INCL IN ATP CALC'',
              INVENTORY_ATP_CODE) INV_ATP_CODE_MNG,
       DECODE(MSV.LOCATOR_TYPE,
              1,''NO LOC CONTROL'',
              2,''PRESPECIFIED'',
              3,''DYNAMIC'',
       ...
       MSV.PICKING_ORDER,
       MSV.SOURCE_SUBINVENTORY SRC_SUBINV
  FROM MTL_SECONDARY_INVENTORIES MSV, APPS.MTL_PARAMETERS MP
 WHERE MP.ORGANIZATION_ID = MSV.ORGANIZATION_ID (+)
   AND MP.EAM_ENABLED_FLAG = ''Y''
 ORDER BY 1',
      p_title                  => 'EAM Enabled Organizations - Subinventories',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No EAM enabled organizations are defined.',
      p_solution               => '',
      p_success_msg            => '',
   ...
   l_info.delete;
debug('end add_signature: EAM_WODC_SETUP_SUBINV');



debug('begin add_signature: EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7433',
      p_sig_id                 => 'EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED',
      p_sig_sql                => 'SELECT ROW_ID,ASSOCIATION_ID,INITIAL_READING,METER_ID,ORGANIZATION_ID,ASSET_GROUP_ID,ASSET_NUMBER,METER_NAME,METER_TYPE,METER_UOM,VALUE_CHANGE_DIR,EAM_REQUIRED_FLAG,PRIMARY_FAILURE_FLAG,USED_IN_SCHEDULING,USER_DEFINED_RATE,USE_PAST_READING,DESCRIPTION,FROM_EFFECTIVE_DATE,TO_EFFECTIVE_DATE,CREATED_BY,CREATION_DATE,LAST_UPDATE_DATE,LAST_UPDATED_BY,MAINTENANCE_OBJECT_TYPE,MAINTENANCE_OBJECT_ID,CREATION_ORGANIZATION_ID,TMPL_FLAG,SOURCE_TMPL_ID,EAM_ITEM_TYPE,ASSET_GROUP,ORGANIZATION_CODE,ASSET_DESCRIPTION,SOURCE_METER_NAME,FACTOR,INITIAL_READING_DATE,RELATIONSHIP_START_DATE,MAINT_ORGANIZATION_ID
  FROM EAM_ASSET_METERS_V
 WHERE MAINT_ORGANIZATION_ID IS NOT NULL
   AND MAINTENANCE_OBJECT_ID IN
       (SELECT MAINTENANCE_OBJECT_ID
          FROM EAM_PM_SCHEDULINGS
         WHERE TMPL_FLAG = ''Y''
           AND PM_SCHEDULE_ID IN (SELECT PM_SCHEDULE_ID
                                    FROM EAM_PM_SCHEDULING_RULES
                                   WHERE METER_ID = 1
                                     AND RULE_TYPE = 2))',
      p_title                  => 'EAM_ABO_INST_API_FAILED',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'When attempting to create a new asset number, the following error occurs.

Insert procedure failed with the following message: EAM_ABO_INST_API_FAILED',
      p_solution               => 'Log a SR with the EAM support team for a data fix.  Reference:
Error EAM_ABO_INST_API_FAILED In Asset Number Form For Asset Group With Activity Association [1491629/KB546878]',
      p_success_msg            => 'Data check passed',
   ...
   l_info.delete;
debug('end add_signature: EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED');



debug('begin add_signature: EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED_1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7434',
      p_sig_id                 => 'EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED_1',
      p_sig_sql                => 'SELECT CCT.COUNTER_ID,CCT.NAME,CCT.DESCRIPTION,CCT.LANGUAGE,CCT.SOURCE_LANG,CCT.CREATION_DATE,CCT.CREATED_BY,CCT.LAST_UPDATED_BY,CCT.LAST_UPDATE_DATE,CCT.LAST_UPDATE_LOGIN,CCT.SECURITY_GROUP_ID,CCT.MIGRATED_FLAG
  FROM CSI_COUNTERS_TL CCT, CSI_COUNTERS_B CCB
 WHERE CCT.COUNTER_ID = CCB.COUNTER_ID
   AND CCB.CREATED_FROM_COUNTER_TMPL_ID = 1',
      p_title                  => 'EAM_ABO_INST_API_FAILED_1',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'When attempting to create a new asset number, the following error occurs.

Insert procedure failed with the following message: EAM_ABO_INST_API_FAILED',
      p_solution               => 'Log a SR with the EAM support team to request a data fix.  Reference:
Error EAM_ABO_INST_API_FAILED In Asset Number Form For Asset Group With Activity Association [1491629/KB546878]',
      p_success_msg            => 'Data check passed',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
   ...
   l_info.delete;
debug('end add_signature: EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED_1');



debug('begin add_signature: EAM-LOOKUP-TYPE-ERROR-JBO-27122');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9492',
      p_sig_id                 => 'EAM-LOOKUP-TYPE-ERROR-JBO-27122',
      p_sig_sql                => 'SELECT LOOKUP_TYPE "Lookup Type",
LOOKUP_CODE "Lookup Code"
FROM fnd_lookup_values
WHERE view_application_id = 700
AND lookup_type IN ( ''%_EAM_%'', ''WIP_%'')
AND lookup_code is not null 
AND NOT regexp_like(lookup_code, ''^[0-9]+$'')',
      p_title                  => 'EAM LOOKUP_TYPEs that Contain Non Numeric Codes - Verification',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'All EAM LOOKUP_CODE values must be numeric only.  The error JBO-27122: SQL error during statement preparation, occurs when alphanumeric values are present.',
      p_solution               => 'See the Solutions in [832838/KB670140] Using EAM Functions - Work Request results in Error JBO-27122',
      p_success_msg            => 'There are no alpha codes returned in EAM lookups.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: EAM-LOOKUP-TYPE-ERROR-JBO-27122');



debug('begin add_signature: EAM_DATA_CHECK_DUPLICATE_OPERATION_RESOURCE_IDS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7441',
      p_sig_id                 => 'EAM_DATA_CHECK_DUPLICATE_OPERATION_RESOURCE_IDS',
      p_sig_sql                => 'SELECT  A.OPERATION_SEQUENCE_ID, A.RESOURCE_SEQ_NUM, A.RESOURCE_ID, A.LAST_UPDATE_DATE, a.basis_type
...
WHERE a.rowid        > ANY
 ...
 AND A.resource_seq_num        = B.resource_seq_num)',
      p_title                  => 'Duplicate Operation Resource IDs',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are duplicate sequence IDs for a routing operation.  This causes the form to hang.',
      p_solution               => '[1504686/KB731212]',
      p_success_msg            => 'There are no duplicate sequence IDs for routing operations.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EAM_DATA_CHECK_DUPLICATE_OPERATION_RESOURCE_IDS');



debug('begin add_signature: EAM_DATA_CHECK_DUPLICATE_OPERATION_SEQUENCE_NUMBERS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '7442',
      p_sig_id                 => 'EAM_DATA_CHECK_DUPLICATE_OPERATION_SEQUENCE_NUMBERS',
      p_sig_sql                => 'select bos1.OPERATION_SEQUENCE_ID, bos1.ROUTING_SEQUENCE_ID, bos1.OPERATION_SEQ_NUM, bos1.LAST_UPDATE_DATE, bos1.STANDARD_OPERATION_ID, bos1.DEPARTMENT_ID, bos1.OPERATION_DESCRIPTION
...
and (bos1.disable_date is null and bos2.disable_date is null)',
      p_title                  => 'Duplicate operation sequence numbers',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are duplicate operation sequence numbers.  This causes the form to hang.',
      p_solution               => 'See [1504686/KB731212] for details to implement the solution.',
      p_success_msg            => 'There are no duplicate operation sequence numbers.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EAM_DATA_CHECK_DUPLICATE_OPERATION_SEQUENCE_NUMBERS');



debug('begin add_signature: EAM_LAST_CYCLIC_ACT_IS_NULL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9281',
      p_sig_id                 => 'EAM_LAST_CYCLIC_ACT_IS_NULL',
      p_sig_sql                => 'select PM_SCHEDULE_ID,ACTIVITY_ASSOCIATION_ID,NON_SCHEDULED_FLAG,FROM_EFFECTIVE_DATE,TO_EFFECTIVE_DATE,RESCHEDULING_POINT,LEAD_TIME,CREATED_BY,CREATION_DATE,LAST_UPDATE_LOGIN,LAST_UPDATE_DATE,LAST_UPDATED_BY,ATTRIBUTE_CATEGORY,NAME,TMPL_FLAG,SOURCE_TMPL_ID,AUTO_INSTANTIATION_FLAG,SOURCE_CODE,SOURCE_LINE,NEXT_SERVICE_START_DATE,NEXT_SERVICE_END_DATE,SET_NAME_ID,TYPE_CODE,SCHEDULING_METHOD_CODE,WHICHEVER_FIRST,INCLUDE_MANUAL,DAY_TOLERANCE,DEFAULT_IMPLEMENT,GENERATE_WO_STATUS,INTERVAL_PER_CYCLE,CURRENT_CYCLE,CURRENT_SEQ,CURRENT_WO_SEQ,BASE_DATE,BASE_READING,EAM_LAST_CYCLIC_ACT,MAINTENANCE_OBJECT_ID,MAINTENANCE_OBJECT_TYPE,GENERATE_NEXT_WORK_ORDER,LAST_REVIEWED_BY,LAST_REVIEWED_DATE
from eam_pm_schedulings 
where eam_last_cyclic_act is null',
      p_title                  => 'EAM_PM_SCHEDULINGS List Where Last Service Date of Activity (EAM_LAST_CYCLIC_ACT) is Null',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Generate Preventive Maintenance Work Orders, EAMPMSDU, does not generate the work order for a specific Active Maintenance.<br>
When trying to perform the forecast in the Maintenance Workbench for reported assets, the forecast will not be created.',
      p_solution               => 'To locate the root cause and repair see the document below:<br>
[1960340/KB608691] EAMPMSDU Generate Preventive Maintenance Work Orders Not Creating Forecast and Work Orders For Specific Asset.',
      p_success_msg            => 'There are no results to report where last service date of activity is null.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EAM_LAST_CYCLIC_ACT_IS_NULL');



debug('begin add_signature: EAM_PM_ACTIVITY_CHECK_ASSET-ACTIVITY_NULL_LAST_SERVICE_DATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9293',
      p_sig_id                 => 'EAM_PM_ACTIVITY_CHECK_ASSET-ACTIVITY_NULL_LAST_SERVICE_DATE',
      p_sig_sql                => 'select epa.ACTIVITY_ASSOCIATION_ID,epa.PM_SCHEDULE_ID,epa.INTERVAL_MULTIPLE,epa.ALLOW_REPEAT_IN_CYCLE,epa.DAY_TOLERANCE,epa.NEXT_SERVICE_START_DATE,epa.NEXT_SERVICE_END_DATE,epa.LAST_UPDATE_DATE,epa.LAST_UPDATED_BY,epa.CREATION_DATE,epa.CREATED_BY,epa.LAST_UPDATE_LOGIN,meaa.ORGANIZATION_ID,meaa.ASSET_ACTIVITY_ID,meaa.START_DATE_ACTIVE,meaa.END_DATE_ACTIVE,meaa.PRIORITY_CODE,meaa.LAST_UPDATE_DATE,meaa.LAST_UPDATED_BY,meaa.CREATION_DATE,meaa.CREATED_BY,meaa.LAST_UPDATE_LOGIN,meaa.REQUEST_ID,meaa.PROGRAM_APPLICATION_ID,meaa.PROGRAM_ID,meaa.PROGRAM_UPDATE_DATE,meaa.OWNING_DEPARTMENT_ID,meaa.ACTIVITY_CAUSE_CODE,meaa.ACTIVITY_TYPE_CODE,meaa.INVENTORY_ITEM_ID,meaa.SERIAL_NUMBER,meaa.ACTIVITY_ASSOCIATION_ID,meaa.TAGGING_REQUIRED_FLAG,meaa.SHUTDOWN_TYPE_CODE,meaa.CLASS_CODE,meaa.MAINTENANCE_OBJECT_TYPE,meaa.MAINTENANCE_OBJECT_ID,meaa.CREATION_ORGANIZATION_ID,meaa.TMPL_FLAG,meaa.SOURCE_TMPL_ID,meaa.ACTIVITY_SOURCE_CODE,meaa.LAST_SERVICE_START_DATE,meaa.LAST_SERVICE_END_DATE,meaa.PREV_SERVICE_START_DATE,meaa.PREV_SERVICE_END_DATE,meaa.LAST_SCHEDULED_START_DATE,meaa.LAST_SCHEDULED_END_DATE,meaa.PREV_SCHEDULED_START_DATE,meaa.PREV_SCHEDULED_END_DATE,meaa.WIP_ENTITY_ID,meaa.LAST_PM_SUGGESTED_START_DATE,meaa.LAST_PM_SUGGESTED_END_DATE,meaa.PREV_PM_SUGGESTED_START_DATE,meaa.PREV_PM_SUGGESTED_END_DATE
from eam_pm_activities epa, mtl_eam_asset_activities meaa
where epa.pm_schedule_id IN (select pm_schedule_id from eam_pm_schedulings
                             where nvl(from_effective_date,sysdate-1)<=sysdate
                               and nvl(to_effective_date,sysdate+1)>=sysdate
                               and nvl(tmpl_flag,''N'')=''N'')
  and epa.activity_association_id = meaa.activity_association_id
  and nvl(meaa.start_date_active,sysdate-1)<=sysdate
  and nvl(meaa.end_date_active,sysdate+1)>=sysdate
  and meaa.last_service_start_date is null
  and meaa.last_service_end_date is null',
      p_title                  => 'Asset Activity Whose Last Service Date is Null',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'These PM schedules have at least one asset-activity whose last service date is null.  This indicates suggestions were never created from this asset activity in any previous PM runs.


',
      p_solution               => 'Review the following documents:

Missing Last Service Information [2527742/KB349747]
How To Update Last Service Information After Creating PM Schedule? (EAM_ITEM_ACTIVITIES_PUB.update_item_activities API) [2032600/KB529926]',
      p_success_msg            => 'All PM schedules have asset-activity set for last service date.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EAM_PM_ACTIVITY_CHECK_ASSET-ACTIVITY_NULL_LAST_SERVICE_DATE');



debug('begin add_signature: EAM_METER_TO_ASSET_ACTIVITY_ASSOCIATIONS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9125',
      p_sig_id                 => 'EAM_METER_TO_ASSET_ACTIVITY_ASSOCIATIONS',
      p_sig_sql                => 'select ROW_ID,ASSOCIATION_ID,INITIAL_READING,METER_ID,ORGANIZATION_ID,ASSET_GROUP_ID,ASSET_NUMBER,METER_NAME,METER_TYPE,METER_UOM,VALUE_CHANGE_DIR,EAM_REQUIRED_FLAG,PRIMARY_FAILURE_FLAG,USED_IN_SCHEDULING,USER_DEFINED_RATE,USE_PAST_READING,DESCRIPTION,FROM_EFFECTIVE_DATE,TO_EFFECTIVE_DATE,CREATION_DATE,LAST_UPDATE_DATE,MAINTENANCE_OBJECT_TYPE,MAINTENANCE_OBJECT_ID,CREATION_ORGANIZATION_ID,TMPL_FLAG,SOURCE_TMPL_ID,EAM_ITEM_TYPE,ASSET_GROUP,ORGANIZATION_CODE,ASSET_DESCRIPTION,SOURCE_METER_NAME,FACTOR,INITIAL_READING_DATE,RELATIONSHIP_START_DATE,MAINT_ORGANIZATION_ID,RNUM
  from ( select
  a.*, ROWNUM rnum 
      from (select * from eam_asset_meters_v where meter_id in (select meter_id from eam_pm_scheduling_rules)) a
      where ROWNUM <= 2000) 
where rnum  >= 1',
      p_title                  => 'Meter to Asset Activity Associations',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No rows selected from EAM_ASSET_METERS with associated Meter in EAM_PM_SCHEDULINGS_RULES.  Unless this installation is fresh, there should be rows in this report. Please see the note below.<BR><BR>Please Note: The output is limited to the first 2000 rows.',
      p_solution               => 'New assets defined in an asset group after the templates have been setup may need to be reviewed.  Please click on the development recommended note below to verify this setup.<br>
Please see [298818/KB273900] How to Link Existing Asset Groups with Activity Association Templates',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_METER_TO_ASSET_ACTIVITY_ASSOCIATIONS');



debug('begin add_signature: EAM_PM_ACTIVITIES_LIST');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9120',
      p_sig_id                 => 'EAM_PM_ACTIVITIES_LIST',
      p_sig_sql                => 'select ACTIVITY_ASSOCIATION_ID,PM_SCHEDULE_ID,INTERVAL_MULTIPLE,ALLOW_REPEAT_IN_CYCLE,DAY_TOLERANCE,NEXT_SERVICE_START_DATE,NEXT_SERVICE_END_DATE,LAST_UPDATE_DATE,LAST_UPDATED_BY,CREATION_DATE,CREATED_BY,LAST_UPDATE_LOGIN,RNUM
  from ( select /*+ FIRST_ROWS(n) */ 
  a.*, ROWNUM rnum 
      from (select * from eam_pm_activities) a
      where ROWNUM <= 2000) 
where rnum  >= 1',
      p_title                  => 'PM Activities List',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No rows selected.  There should be EAM PM Activities.  Without PM Activities there is no need for a schedule.  For additional details, click into the notes below. <BR><BR>Please Note: The output is limited to the first 2000 rows.',
      p_solution               => 'Additional details to understand proper expectations.<br>
For R11:<br>
[1268738/KB213799] Can one change the EAM Activity in the Preventive Maintenance (Schedule Definition) form (EAMPMDEF)
For R12:<br>
[1514403/KB569453] ACTIVITY_ASSOCIATION_ID from PM Schedule is Kept in table EAM_PM_ACTIVITIES',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_PM_ACTIVITIES_LIST');



debug('begin add_signature: EAM_METER_SCHEDULING_COUNTER_READINGS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9127',
      p_sig_id                 => 'EAM_METER_SCHEDULING_COUNTER_READINGS',
      p_sig_sql                => 'select COUNTER_VALUE_ID,COUNTER_ID,VALUE_TIMESTAMP,COUNTER_READING,RESET_MODE,RESET_REASON,ADJUSTMENT_TYPE,ADJUSTMENT_READING,NET_READING,LIFE_TO_DATE_READING,AUTOMATIC_ROLLOVER_FLAG,INCLUDE_TARGET_RESETS,SOURCE_COUNTER_VALUE_ID,TRANSACTION_ID,DISABLED_FLAG,COMMENTS,SECURITY_GROUP_ID,OBJECT_VERSION_NUMBER,LAST_UPDATE_DATE,LAST_UPDATED_BY,CREATION_DATE,CREATED_BY,LAST_UPDATE_LOGIN,MIGRATED_FLAG,SOURCE_CODE,SOURCE_LINE_ID,INITIAL_READING_FLAG,RNUM
  from ( select
  a.*, ROWNUM rnum 
      from (select * from csi_counter_readings where counter_id in (select meter_id from eam_pm_scheduling_rules) order by counter_id, value_timestamp desc) a
      where ROWNUM <= 2000) 
where rnum  >= 1',
      p_title                  => 'Meter Scheduling Counter Readings',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'No rows selected from CSI_COUNTER_READINGS with associated EAM_PM_SCHEDULING_RULES.  Unless this installation is fresh, there should be rows in this report. Please see the note below.<BR><BR>Please Note: The output is limited to the first 2000 rows.',
      p_solution               => 'Please see [1949501/KB503423] How to Use Meters in Enterprise Asset Management, page 17<br>
The objectives of this Technical Brief are to provide an overview of what meters are and how they are used in EAM.  The document will look at the setups required and look at some examples of using meters.<br>
There will also be a brief discussion of APIs and tables related to meters. The topics covered in the paper are:
<ul>
<li>What are meters?</li>
<li>Basic Setups</li>
<li>Some examples of using Meters</li>
<li>Tables/APIs  used for meters</li>
</ul>',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EAM_METER_SCHEDULING_COUNTER_READINGS');



debug('begin add_signature: EAM_METER_LAST_SERVICE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9128',
      p_sig_id                 => 'EAM_METER_LAST_SERVICE',
      p_sig_sql                => 'select METER_ID,ACTIVITY_ASSOCIATION_ID,LAST_SERVICE_READING,PREV_SERVICE_READING,WIP_ENTITY_ID,LAST_UPDATE_DATE,CREATION_DATE,MIGRATED_FLAG,RNUM
  from ( select
  a.*, ROWNUM rnum 
      from (select * from eam_pm_last_service) a
      where ROWNUM <= 2000) 
where rnum  >= 1',
      p_title                  => 'Meter Last Service Report',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'EAM_PM_LAST_SERVICE has the information about last meter service.  Unless this installation is fresh, there should be rows in this report.  Please see the note below.<BR><BR>Please Note: The output is limited to the first 2000 rows.',
      p_solution               => 'There is a way to validate the last service data.   Please see the note.<br>
[1641378/KB406575] Are there any Steps And Scripts To Validate 11i Vs R12 EAM Data?
',
      p_success_msg            => 'Rows selected from EAM_PM_LAST_SERVICE.  Please review [1641378/KB406575] Are there any Steps And Scripts To Validate 11i Vs R12 EAM Data?<BR><BR>Please Note: The output is limited to the first 2000 rows.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: EAM_METER_LAST_SERVICE');



debug('begin add_signature: EAM_ALL_EXISTING_PM_SET_NAMES');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9129',
      p_sig_id                 => 'EAM_ALL_EXISTING_PM_SET_NAMES',
      p_sig_sql                => 'Select SET_NAME_ID,SET_NAME,DESCRIPTION,END_DATE,LAST_UPDATE_DATE,CREATION_DATE,OWNING_ORGANIZATION_ID,LOCAL_FLAG
from eam_pm_set_names',
      p_title                  => 'PM Set Names',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There are no PM set names.',
      p_solution               => 'Review the solution steps in [1257206/KB381774] Fundamentals of Preventive Maintenance Schedule Definition with Oracle Enterprise Asset Management (eAM).',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_ALL_EXISTING_PM_SET_NAMES');



debug('begin add_signature: EAM_WODC_HEADER_WDJWE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9561',
      p_sig_id                 => 'EAM_WODC_HEADER_WDJWE',
      p_sig_sql                => 'SELECT WDJ.ORGANIZATION_ID ORG_ID,
       MP.ORGANIZATION_CODE ORG_CODE,
       OOD.OPERATING_UNIT OP_UNIT,
       WDJ.WIP_ENTITY_ID,
       WIP_ENTITY_NAME,
       WE.ENTITY_TYPE ENTITY_TYPE_ID,
       MLE.MEANING ENTITY_TYPE,
       WDJ.STATUS_TYPE STATUS_TYPE_ID,
       ...
       DECODE(PO_CREATION_TIME, 1, ''Job Release'',  2, ''Operation'', 3  ''Manual'') PR_CREATION,
       DECODE(MATERIAL_ISSUE_BY_MO, ''N'', ''No'', ''Y'', ''Yes'') MAT_REQ,
       SUBSTRB(WDJ.DESCRIPTION, 1, 30) WO_DESC */
 ...
 WHERE WE.WIP_ENTITY_ID = WDJ.WIP_ENTITY_ID
   ...
   AND WE.ENTITY_TYPE = MLE.LOOKUP_CODE
   AND MLE.LOOKUP_TYPE = ''WIP_ENTITY''
   AND WDJ.STATUS_TYPE = MLS.LOOKUP_CODE
   AND MLS.LOOKUP_TYPE = ''WIP_JOB_STATUS''
   AND WE.ENTITY_TYPE = 7
   AND WDJ.STATUS_TYPE != 12
   AND exists (Select 1
          from WIP_TRANSACTIONS WT
         WHERE WT.TRANSACTION_TYPE = 6
           AND WT.TRANSACTION_DATE > SYSDATE - 365
           AND WT.WIP_ENTITY_ID = WDJ.WIP_ENTITY_ID)',
      p_title                  => 'Header Details (WE, WDJ)',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Work Order Has Entity Type Closed Maintenance Job. Please note that only work orders with job close variance transaction in less than 1 year are displayed.',
      p_solution               => 'Open Work Order with Entity Type Closed Maintenance Job Throws Different Errors within the Application [2349125/KB379836]',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WODC_HEADER_WDJWE');



debug('begin add_signature: EAM_WR_NOTCOMPLETED_WO_COMPLETED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13730',
      p_sig_id                 => 'EAM_WR_NOTCOMPLETED_WO_COMPLETED',
      p_sig_sql                => 'select WORK_REQUEST_ID,WORK_REQUEST_NUMBER,LAST_UPDATE_DATE,CREATION_DATE,ASSET_NUMBER,ASSET_GROUP,ORGANIZATION_ID,WORK_REQUEST_STATUS_ID,WORK_REQUEST_PRIORITY_ID,WORK_REQUEST_OWNING_DEPT,EXPECTED_RESOLUTION_DATE,WIP_ENTITY_ID,DESCRIPTION,WORK_REQUEST_TYPE_ID,WORK_REQUEST_AUTO_APPROVE,MAINTENANCE_OBJECT_TYPE,MAINTENANCE_OBJECT_ID,WF_ITEM_TYPE,WF_ITEM_KEY,EAM_LINEAR_LOCATION_ID
from WIP_EAM_WORK_REQUESTS
where WIP_ENTITY_ID IN (SELECT WIP_ENTITY_ID FROM WIP_DISCRETE_JOBS WHERE STATUS_TYPE=12)
AND WORK_REQUEST_STATUS_ID!=6',
      p_title                  => 'Work Order is closed but Work Request remains On Work Order',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'When Work Order is closed, Work Request status still On Work Order.',
      p_solution               => '(Data Fix) How to change Work Request status [763745/KB609735]',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WR_NOTCOMPLETED_WO_COMPLETED');



debug('begin add_signature: EAM_WO_PENDING CLOSE STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10096',
      p_sig_id                 => 'EAM_WO_PENDING CLOSE STATUS',
...
WHERE WDJ.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
  ...
  and WDJ.WIP_ENTITY_ID not in (select wip_entity_id from wip_transactions wt where wt.transaction_type = 6)
  and (WDJ.STATUS_TYPE = 14 OR EWOD.USER_DEFINED_STATUS_ID = 14)
  and we.entity_type = 6
  and wdj.date_closed is null',
      p_title                  => 'Work Orders Stuck in Pending Close Status',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Work orders that are in Pending close status.

',
      p_solution               => 'Please apply solution from the following document:
Work Orders Stuck in Pending Closed Status. [2527704/KB699976]
',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WO_PENDING CLOSE STATUS');



debug('begin add_signature: EAM_WODC_PO_PR_DELIVER_TO_LOCATION_ID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9665',
      p_sig_id                 => 'EAM_WODC_PO_PR_DELIVER_TO_LOCATION_ID',
      p_sig_sql                => 'select PIE.INTERFACE_TRANSACTION_ID INTF_TRANS_ID,
            PIE.COLUMN_NAME              COLUMN_NAME,
            PIE.ERROR_MESSAGE            ERROR,
            PIE.INTERFACE_TYPE           INTF_TYPE,
            PIE.REQUEST_ID               REQUEST_ID,
            PIE.TABLE_NAME               TABLE_NAME,
           WE.WIP_ENTITY_NAME           Work_Order
  from po_requisitions_interface_all pria,
            po_interface_errors           pie,
            wip_entities                  we
where pria.process_flag in (''ERROR'', ''PENDING'')
    and pria.interface_source_code in (''WIP'', ''EAM'')
    and PRIA.TRANSACTION_ID = PIE.INTERFACE_TRANSACTION_ID
    and PIE.ERROR_MESSAGE = ''Please enter a valid Deliver to Location''
    and pria.wip_entity_id = we.wip_entity_id',
      p_title                  => 'Purchase Requisition Interface Error - Deliver to Location',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Incorrect setup for OSP or Direct Item Department.',
      p_solution               => 'Review solution steps in [968639/KB713729].',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WODC_PO_PR_DELIVER_TO_LOCATION_ID');



debug('begin add_signature: EAM_WORK_ORDER_STATUS_CANCELLED_MISMATCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '15025',
      p_sig_id                 => 'EAM_WORK_ORDER_STATUS_CANCELLED_MISMATCH',
      p_sig_sql                => 'SELECT WDJ.ORGANIZATION_ID,
       MP.organization_code,
       WDJ.WIP_ENTITY_ID,
       WE.WIP_ENTITY_NAME Work_Order_Number,
       WE.ENTITY_TYPE,
       MLE.meaning Entity_Type_Meaning,
       WDJ.DATE_COMPLETED,
       WDJ.DATE_CLOSED,
       WDJ.DATE_RELEASED,
       WDJ.STATUS_TYPE WDJ_STATUS_ID,
       MLW.meaning WDJ_STATUS,
       EWOD.user_defined_status_id wo_status,
       mlo.meaning EWOD_Status
FROM WIP_DISCRETE_JOBS WDJ,
     WIP_ENTITIES WE,
     mtl_parameters mp,
     eam_work_order_details ewod,
     ...
     mfg_lookups mlw,
     mfg_lookups mlo,
     eam_wo_statuses_v ewst
WHERE WDJ.WIP_ENTITY_ID = WE.WIP_ENTITY_ID
  and wdj.organization_id = mp.organization_id
  and ewod.wip_entity_id = wdj.wip_entity_id
  and WDJ.STATUS_TYPE = 7
  and to_number(mle.LOOKUP_code) = to_number(we.entity_type)
  AND MLE.LOOKUP_TYPE = ''WIP_ENTITY''
  AND to_number(WDJ.STATUS_TYPE) = to_number(MLW.LOOKUP_CODE)
  AND MLW.LOOKUP_TYPE = ''WIP_JOB_STATUS''
  AND mlo.lookup_type = ''WIP_JOB_STATUS''
  AND mlo.lookup_code = ewod.user_defined_status_id
  and ewod.user_defined_status_id = ewst.status_id
  and ewst.system_status != 7',
      p_title                  => 'Work Order Status not Cancelled in EWOD while Cancelled in WDJ',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Work Order Status is Not Cancelled Nor Cancelled by PM Nor Cancelled By Approver or any other user defined cancelled status while the System Status is Cancelled.',
      p_solution               => 'Review [2879194/KB448964] for solution details.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_WORK_ORDER_STATUS_CANCELLED_MISMATCH');



debug('begin add_signature: EBS_MFG_EAM_CONF_PENDING_RELEASE_WF_ERROR');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22737',
      p_sig_id                 => 'EBS_MFG_EAM_CONF_PENDING_RELEASE_WF_ERROR',
      p_sig_sql                => 'select we.wip_entity_name Work_Order_Number,
       mp.organization_code,
       ml.meaning previous_status,
  ...
  from wip_discrete_jobs         wdj,
       ...
       eam_wo_statuses_vl        ewsv,
       eam_wo_workflows          eww
 where wdj.wip_entity_id = we.wip_entity_id
   and mp.organization_id = we.organization_id
   and we.wip_entity_id = ewod.wip_entity_id
   and wdj.status_type = to_number(ml.lookup_code)
   and ml.lookup_type = ''WIP_JOB_STATUS''
   and ewod.pending_flag = ''Y''
   and ewod.wip_entity_id = eww.wip_entity_id
   and ewsv.status_id = ewod.user_defined_status_id
   and eww.wf_item_type = ''EAMWOREL''
   and to_char(eww.wf_item_key) = wias.item_key
   and wias.activity_status = ''ERROR''
   and wias.item_type = eww.wf_item_type
  order by we.wip_entity_id',
      p_title                  => 'EAM WOs In Pending Release With Workflow in Error',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'EAM Work Orders stuck in Pending - Release status (Released status with Pending flag is ON) with Workflow in Error.',
      p_solution               => 'Review CASE 2 in [2464333/KB320961].',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_EAM_CONF_PENDING_RELEASE_WF_ERROR');



debug('begin add_signature: EBS_MFG_EAM_CONF_PENDING_RELEASE_NO_WF');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22736',
      p_sig_id                 => 'EBS_MFG_EAM_CONF_PENDING_RELEASE_NO_WF',
      p_sig_sql                => 'select we.wip_entity_name Work_Order_Number,
       mp.organization_code,
       ml.meaning System_status
  from wip_discrete_jobs wdj,
       wip_entities      we,
       mtl_parameters    mp,
       mfg_lookups       ml
 where wdj.wip_entity_id = we.wip_entity_id
   ...
   and we.wip_entity_id in
       ((select ewod.wip_entity_id
           from eam_work_order_details ewod
          where ewod.user_defined_status_id = 3
            and ewod.pending_flag = ''Y''
            and not exists
          (select 1
                   from eam_wo_workflows eww
                  where ewod.wip_entity_id = eww.wip_entity_id)))',
      p_title                  => 'EAM WOs In Pending Release With No Existing Workflow',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'EAM Work Orders stuck in Pending - Release status (Released status with Pending flag is ON) with no Workflow initialized.',
      p_solution               => 'Review CASE 1 in [2464333/KB320961].',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EBS_MFG_EAM_CONF_PENDING_RELEASE_NO_WF');



debug('begin add_signature: DMEF TXN - CST_INVALID_WIP');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21689',
      p_sig_id                 => 'DMEF TXN - CST_INVALID_WIP',
      p_sig_sql                => 'select ''ResTxn'' as TxnType,WCTI.ORGANIZATION_ID, WCTI.WIP_ENTITY_ID, WCTI.TRANSACTION_ID, WCTI.TRANSACTION_DATE,WCTI.ACCT_PERIOD_ID,WCTI.OPERATION_SEQ_NUM
from WIP_COST_TXN_INTERFACE WCTI, WIP_TXN_INTERFACE_ERRORS WTIE
where WCTI.TRANSACTION_ID = WTIE.TRANSACTION_ID(+) 
and (WTIE.ERROR_MESSAGE LIKE ''%WIP_NO_BALANCE%'' or WTIE.ERROR_MESSAGE LIKE ''%CST_NO_BALANCE_ROW%'' or WTIE.ERROR_MESSAGE LIKE ''%CST_INVALID_WIP%'')
UNION ALL
select ''MatTxn'' as TxnType,MMT.ORGANIZATION_ID, MMT.TRANSACTION_SOURCE_ID as WIP_ENTITY_ID, MMT.TRANSACTION_ID, MMT.TRANSACTION_DATE,MMT.ACCT_PERIOD_ID,MMT.OPERATION_SEQ_NUM 
from MTL_MATERIAL_TRANSACTIONS MMT
where MMT.COSTED_FLAG = ''E'' and MMT.TRANSACTION_SOURCE_TYPE_ID = 5 
and (MMT.ERROR_CODE LIKE ''%WIP_NO_BALANCE%'' or MMT.ERROR_CODE LIKE ''%CST_NO_BALANCE_ROW%'' or MMT.ERROR_CODE LIKE ''%CST_INVALID_WIP%'')',
      p_title                  => 'Resource/Material Transactions with error CST_INVALID_WIP - The wip entity is either not defined or does not have a period balance entry.',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Resource/ Material transactions with the following error:
Error Code: CST_INVALID_WIP
Error Explanation: The wip entity is either not defined or does not have a period balance entry.',
      p_solution               => 'Review solution steps in [2052420/KB325716] [Data Fix] Resource [WIP_COST_TXN_INTERFACE (WCTI)]/ Material Transactions [MTL_MATERIAL_TRANSACTIONS (MMT)] Having Errors Related to WIP_PERIOD_BALANCES Table.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: DMEF TXN - CST_INVALID_WIP');



debug('begin add_signature: DMEF TXN - CST_INVALID_JOB_DATE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21688',
      p_sig_id                 => 'DMEF TXN - CST_INVALID_JOB_DATE',
      p_sig_sql                => 'select ''ResTxn'' as TxnType,WCTI.ORGANIZATION_ID, WCTI.WIP_ENTITY_ID, WCTI.TRANSACTION_ID, WCTI.TRANSACTION_DATE,WCTI.ACCT_PERIOD_ID,WCTI.OPERATION_SEQ_NUM
from WIP_COST_TXN_INTERFACE WCTI, WIP_TXN_INTERFACE_ERRORS WTIE
where WCTI.TRANSACTION_ID = WTIE.TRANSACTION_ID(+) and WTIE.ERROR_MESSAGE LIKE ''%CST_INVALID_JOB_DATE%''
UNION ALL
select ''MatTxn'' as TxnType,MMT.ORGANIZATION_ID, MMT.TRANSACTION_SOURCE_ID as WIP_ENTITY_ID, MMT.TRANSACTION_ID, MMT.TRANSACTION_DATE,MMT.ACCT_PERIOD_ID,MMT.OPERATION_SEQ_NUM 
from MTL_MATERIAL_TRANSACTIONS MMT
where MMT.COSTED_FLAG = ''E'' and MMT.TRANSACTION_SOURCE_TYPE_ID = 5 and MMT.ERROR_CODE LIKE ''%CST_INVALID_JOB_DATE%''',
      p_title                  => 'Resource/Material Transactions with error CST_INVALID_JOB_DATE - The transaction date precedes the release date of the job or schedule.',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are Resource/ Material transactions with the following error:
...
Error Explanation: The transaction date precedes the release date of the job or schedule',
   ...
   l_info.delete;
debug('end add_signature: DMEF TXN - CST_INVALID_JOB_DATE');
...
debug('begin add_signature: DMEF RES TXN - ORGANIZATION CODE NOT VALID. PLEASE RE-ENTER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21741',
      p_sig_id                 => 'DMEF RES TXN - ORGANIZATION CODE NOT VALID. PLEASE RE-ENTER',
      p_sig_sql                => 'SELECT WCTI.PROCESS_STATUS, WTIE.ERROR_COLUMN, WTIE.ERROR_MESSAGE, WCTI.*
FROM WIP_COST_TXN_INTERFACE WCTI, WIP_TXN_INTERFACE_ERRORS WTIE
WHERE WCTI.TRANSACTION_ID = WTIE.TRANSACTION_ID(+)
AND WTIE.ERROR_MESSAGE LIKE ''%Organization Code not valid. Please re-enter%''
		   ',
      p_title                  => 'Resource Transactions with Error "Organization Code not valid. Please re-enter"',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Resource transactions failed due to mismatch between Organization Id and Organization Name.',
      p_solution               => 'Review solution in [2363189/KB739114][Data Fix] WIP_COST_TXN_INTERFACE (WCTI) Table with Error "Organization Code not valid. Please re-enter"',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: DMEF RES TXN - ORGANIZATION CODE NOT VALID. PLEASE RE-ENTER');



debug('begin add_signature: EAM_RESOURCE_TRX_ERROR_RESOURCE CODE IS NOT VALID. PLEASE RE-ENTER');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13897',
      p_sig_id                 => 'EAM_RESOURCE_TRX_ERROR_RESOURCE CODE IS NOT VALID. PLEASE RE-ENTER',
      p_sig_sql                => 'select TRANSACTION_ID,ERROR_MESSAGE,ERROR_COLUMN,LAST_UPDATE_DATE,CREATION_DATE,REQUEST_ID,PROGRAM_APPLICATION_ID,PROGRAM_ID,PROGRAM_UPDATE_DATE
from wip_txn_interface_errors 
where error_message like ''%Resource Code not valid%Please re-enter%''
and transaction_id in (select transaction_id from wip_cost_txn_interface where process_status = 3)',
      p_title                  => 'Resource Transactions with Error "Resource Code not valid. Please re-enter"',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are resource transactions with the following error: Resource Code not valid. Please re-enter.
There are two causes for this.
- The resource has been inactivated on the Define Resource Form or from the operation resources against the department.
- The resource_code entered in the interface is not matching the resource_id.',
      p_solution               => 'See solution steps from [2988180/KB764590].',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_RESOURCE_TRX_ERROR_RESOURCE CODE IS NOT VALID. PLEASE RE-ENTER');



debug('begin add_signature: EAM_RESOURCE_TRX_ERROR_EMPLOYEE NUMBER NOT VALID');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '13900',
      p_sig_id                 => 'EAM_RESOURCE_TRX_ERROR_EMPLOYEE NUMBER NOT VALID',
      p_sig_sql                => 'select TRANSACTION_ID,ERROR_MESSAGE,ERROR_COLUMN,LAST_UPDATE_DATE,CREATION_DATE,REQUEST_ID,PROGRAM_APPLICATION_ID,PROGRAM_ID,PROGRAM_UPDATE_DATE
from wip_txn_interface_errors 
where error_message like ''Employee Number not valid''
and transaction_id in (select transaction_id from wip_cost_txn_interface where process_status = 3)',
      p_title                  => 'Employee Number not valid',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'A resource transaction is stuck with the error ''Employee Number not valid'' because the employee details for the resource_id being used are missing.',
      p_solution               => '- Check that valid employees have been identified for the resource and they are active.
- Stop the Cost Manager.
- Run datafix found in [1672312/KB367998#aref_section26] for the respective error
- Now restart the Cost Manager and check transactions are processed.',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('E','W'),
      p_print_sql_output       => nvl('RS','RS'),
   ...
   l_info.delete;
debug('end add_signature: EAM_RESOURCE_TRX_ERROR_EMPLOYEE NUMBER NOT VALID');



debug('begin add_signature: CSI_TXN_ERRORS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '9130',
      p_sig_id                 => 'CSI_TXN_ERRORS',
      p_sig_sql                => 'select TRANSACTION_ERROR_ID,TRANSACTION_ID,MESSAGE_ID,ERROR_TEXT,SOURCE_TYPE,SOURCE_ID,PROCESSED_FLAG,CREATED_BY,CREATION_DATE,LAST_UPDATED_BY,LAST_UPDATE_DATE,LAST_UPDATE_LOGIN,OBJECT_VERSION_NUMBER,SECURITY_GROUP_ID,TRANSACTION_TYPE_ID,SOURCE_GROUP_REF,SOURCE_GROUP_REF_ID,SOURCE_HEADER_REF,SOURCE_HEADER_REF_ID,SOURCE_LINE_REF,SOURCE_LINE_REF_ID,SOURCE_DIST_REF_ID1,SOURCE_DIST_REF_ID2,INV_MATERIAL_TRANSACTION_ID,ERROR_STAGE,MESSAGE_STRING,TRANSACTION_ERROR_DATE,SERIAL_NUMBER,LOT_NUMBER,INSTANCE_ID,INVENTORY_ITEM_ID,SRC_SERIAL_NUM_CTRL_CODE,DST_SERIAL_NUM_CTRL_CODE,SRC_LOT_CTRL_CODE,DST_LOT_CTRL_CODE,SRC_LOCATION_CTRL_CODE,DST_LOCATION_CTRL_CODE,SRC_REV_QTY_CTRL_CODE,DST_REV_QTY_CTRL_CODE,COMMS_NL_TRACKABLE_FLAG,RNUM
  from ( select
  a.*, ROWNUM rnum 
      from (select * from CSI_TXN_ERRORS where source_type is null) a 
      where ROWNUM <= 2000) 
where rnum  >= 1',
      p_title                  => 'CSI_TXN_ERRORS Whose SOURCE_TYPE is Null Report',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'There are transaction errors found in the CSI_TXN_ERRORS table where the SOURCE_TYPE is null.  SOURCE_TYPE should not be null.  This will prevent successful transaction processing.<BR><BR>
Please Note: The output is limited to the first 2000 rows.
',
      p_solution               => 'Please follow [1665334/KB283566] to Resubmit the Installed Base Erred Transactions.
If this issue persists, log a Service Request with Oracle Installed Base Support Team. Include this HTML output report showing the Error Transaction Ids in Transaction_Error_Id column.',
      p_success_msg            => 'There are no transaction errors in CSI_TXN_ERRORS whose SOURCE_TYPE is null.<BR><BR>
Please Note: The output is limited to the first 2000 rows.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: CSI_TXN_ERRORS');



debug('begin add_signature: ASSET_WO_WR _STATUS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '10131',
      p_sig_id                 => 'ASSET_WO_WR _STATUS',
      p_sig_sql                => 'select WEWR.WORK_REQUEST_ID,WEWR.WORK_REQUEST_NUMBER,WEWR.LAST_UPDATE_DATE,WEWR.CREATION_DATE,WEWR.ASSET_NUMBER,WEWR.ASSET_GROUP,WEWR.ORGANIZATION_ID,WEWR.WORK_REQUEST_STATUS_ID,WEWR.WORK_REQUEST_PRIORITY_ID,WEWR.WORK_REQUEST_OWNING_DEPT,WEWR.EXPECTED_RESOLUTION_DATE,WEWR.WIP_ENTITY_ID,WEWR.DESCRIPTION,WEWR.WORK_REQUEST_TYPE_ID,WEWR.WORK_REQUEST_CREATED_BY,WEWR.WORK_REQUEST_AUTO_APPROVE,WEWR.MAINTENANCE_OBJECT_TYPE,WEWR.MAINTENANCE_OBJECT_ID,WEWR.CREATED_FOR,WEWR.PHONE_NUMBER,WEWR.E_MAIL,WEWR.CONTACT_PREFERENCE,WEWR.NOTIFY_ORIGINATOR,WEWR.WF_ITEM_TYPE,WEWR.WF_ITEM_KEY,WEWR.EAM_LINEAR_LOCATION_ID,
       WDJ.WIP_ENTITY_ID,WDJ.ORGANIZATION_ID,WDJ.LAST_UPDATE_DATE,WDJ.CREATION_DATE,WDJ.REQUEST_ID,WDJ.PROGRAM_APPLICATION_ID,WDJ.PROGRAM_ID,WDJ.PROGRAM_UPDATE_DATE,WDJ.SOURCE_LINE_ID,WDJ.SOURCE_CODE,WDJ.DESCRIPTION,WDJ.STATUS_TYPE,WDJ.PRIMARY_ITEM_ID,WDJ.FIRM_PLANNED_FLAG,WDJ.JOB_TYPE,WDJ.WIP_SUPPLY_TYPE,WDJ.CLASS_CODE,WDJ.MATERIAL_ACCOUNT,WDJ.MATERIAL_OVERHEAD_ACCOUNT,WDJ.RESOURCE_ACCOUNT,WDJ.OUTSIDE_PROCESSING_ACCOUNT,WDJ.MATERIAL_VARIANCE_ACCOUNT,WDJ.RESOURCE_VARIANCE_ACCOUNT,WDJ.OUTSIDE_PROC_VARIANCE_ACCOUNT,WDJ.STD_COST_ADJUSTMENT_ACCOUNT,WDJ.OVERHEAD_ACCOUNT,WDJ.OVERHEAD_VARIANCE_ACCOUNT,WDJ.SCHEDULED_START_DATE,WDJ.DATE_RELEASED,WDJ.SCHEDULED_COMPLETION_DATE,WDJ.DATE_COMPLETED,WDJ.DATE_CLOSED,WDJ.START_QUANTITY,WDJ.QUANTITY_COMPLETED,WDJ.QUANTITY_SCRAPPED,WDJ.NET_QUANTITY,WDJ.BOM_REFERENCE_ID,WDJ.ROUTING_REFERENCE_ID,WDJ.COMMON_BOM_SEQUENCE_ID,WDJ.COMMON_ROUTING_SEQUENCE_ID,WDJ.BOM_REVISION,WDJ.ROUTING_REVISION,WDJ.BOM_REVISION_DATE,WDJ.ROUTING_REVISION_DATE,WDJ.LOT_NUMBER,WDJ.ALTERNATE_BOM_DESIGNATOR,WDJ.ALTERNATE_ROUTING_DESIGNATOR,WDJ.COMPLETION_SUBINVENTORY,WDJ.COMPLETION_LOCATOR_ID,WDJ.MPS_SCHEDULED_COMPLETION_DATE,WDJ.MPS_NET_QUANTITY,WDJ.DEMAND_CLASS,WDJ.SCHEDULE_GROUP_ID,WDJ.BUILD_SEQUENCE,WDJ.LINE_ID,WDJ.PROJECT_ID,WDJ.TASK_ID,WDJ.KANBAN_CARD_ID,WDJ.OVERCOMPLETION_TOLERANCE_TYPE,WDJ.OVERCOMPLETION_TOLERANCE_VALUE,WDJ.END_ITEM_UNIT_NUMBER,WDJ.PO_CREATION_TIME,WDJ.PRIORITY,WDJ.DUE_DATE,WDJ.EST_SCRAP_ACCOUNT,WDJ.EST_SCRAP_VAR_ACCOUNT,WDJ.EST_SCRAP_PRIOR_QTY,WDJ.DUE_DATE_PENALTY,WDJ.DUE_DATE_TOLERANCE,WDJ.COPRODUCTS_SUPPLY,WDJ.PARENT_WIP_ENTITY_ID,WDJ.ASSET_NUMBER,WDJ.ASSET_GROUP_ID,WDJ.REBUILD_ITEM_ID,WDJ.REBUILD_SERIAL_NUMBER,WDJ.MANUAL_REBUILD_FLAG,WDJ.SHUTDOWN_TYPE,WDJ.ESTIMATION_STATUS,WDJ.REQUESTED_START_DATE,WDJ.NOTIFICATION_REQUIRED,WDJ.WORK_ORDER_TYPE,WDJ.OWNING_DEPARTMENT,WDJ.ACTIVITY_TYPE,WDJ.ACTIVITY_CAUSE,WDJ.TAGOUT_REQUIRED,WDJ.PLAN_MAINTENANCE,WDJ.PM_SCHEDULE_ID,WDJ.LAST_ESTIMATION_DATE,WDJ.LAST_ESTIMATION_REQ_ID,WDJ.ACTIVITY_SOURCE,WDJ.SERIALIZATION_START_OP,WDJ.MAINTENANCE_OBJECT_ID,WDJ.MAINTENANCE_OBJECT_TYPE,WDJ.MAINTENANCE_OBJECT_SOURCE,WDJ.MATERIAL_ISSUE_BY_MO,WDJ.SCHEDULING_REQUEST_ID,WDJ.ISSUE_ZERO_COST_FLAG,WDJ.EAM_LINEAR_LOCATION_ID,WDJ.ACTUAL_START_DATE,WDJ.EXPECTED_HOLD_RELEASE_DATE,WDJ.EXPEDITED,WDJ.JOB_NOTE
from wip_eam_work_requests wewr, wip_discrete_jobs wdj
where wewr.wip_entity_id = wdj.wip_entity_id
and wdj.status_type in (4,5,7,12)
AND wewr.work_request_status_id<>6
and wewr.asset_number in (''list of the problematic asset numbers'')',
      p_title                  => 'ASSET_WO_WR _STATUS',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Provides details whether any Work order or Work request exist for the Asset.

The Deactivation of the Asset will be done based on the values.

Error You cannot deactivate an asset for which there are open work requests or service requests or work orders [1533588/KB662814]',
      p_solution               => '',
      p_success_msg            => '',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('I','W'),
   ...
   l_info.delete;
debug('end add_signature: ASSET_WO_WR _STATUS');
...
BEGIN
  ...
  analyzer_title := 'EAM Configuration Analyzer';
...
debug('begin section: ORG_SETUP');
start_section('Organizations Setup', 'ORG_SETUP');
   set_item_result(run_stored_sig('EAM_ORG_STRUCTURE'));
   set_item_result(run_stored_sig('EAM_WODC_SETUP_EAM'));
   set_item_result(run_stored_sig('EAM_WODC_SETUP_WIP'));
   set_item_result(run_stored_sig('EAM_WODC_SETUP_SUBINV'));
end_section;
debug('end section: ORG_SETUP');

debug('begin section: DATA_INTEGRITY');
start_section('Data Integrity', 'DATA_INTEGRITY');
   set_item_result(run_stored_sig('EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED'));
   set_item_result(run_stored_sig('EAM_DATA_CHECK_EAM_ABO_INST_API_FAILED_1'));
   set_item_result(run_stored_sig('EAM-LOOKUP-TYPE-ERROR-JBO-27122'));
   set_item_result(run_stored_sig('EAM_DATA_CHECK_DUPLICATE_OPERATION_RESOURCE_IDS'));
   set_item_result(run_stored_sig('EAM_DATA_CHECK_DUPLICATE_OPERATION_SEQUENCE_NUMBERS'));
end_section;
debug('end section: DATA_INTEGRITY');

debug('begin section: PM_DATA');
start_section('Preventative Maintenance', 'PM_DATA');
   set_item_result(run_stored_sig('EAM_LAST_CYCLIC_ACT_IS_NULL'));
   set_item_result(run_stored_sig('EAM_PM_ACTIVITY_CHECK_ASSET-ACTIVITY_NULL_LAST_SERVICE_DATE'));
   set_item_result(run_stored_sig('EAM_METER_TO_ASSET_ACTIVITY_ASSOCIATIONS'));
   set_item_result(run_stored_sig('EAM_PM_ACTIVITIES_LIST'));
   set_item_result(run_stored_sig('EAM_METER_SCHEDULING_COUNTER_READINGS'));
   set_item_result(run_stored_sig('EAM_METER_LAST_SERVICE'));
   set_item_result(run_stored_sig('EAM_ALL_EXISTING_PM_SET_NAMES'));
end_section;
debug('end section: PM_DATA');

debug('begin section: WORK_ORDERS');
start_section('Work Orders', 'WORK_ORDERS');
   set_item_result(run_stored_sig('EAM_WODC_HEADER_WDJWE'));
   set_item_result(run_stored_sig('EAM_WR_NOTCOMPLETED_WO_COMPLETED'));
   set_item_result(run_stored_sig('EAM_WO_PENDING CLOSE STATUS'));
   set_item_result(run_stored_sig('EAM_WODC_PO_PR_DELIVER_TO_LOCATION_ID'));
   set_item_result(run_stored_sig('EAM_WORK_ORDER_STATUS_CANCELLED_MISMATCH'));
   set_item_result(run_stored_sig('EBS_MFG_EAM_CONF_PENDING_RELEASE_WF_ERROR'));
   set_item_result(run_stored_sig('EBS_MFG_EAM_CONF_PENDING_RELEASE_NO_WF'));
end_section;
debug('end section: WORK_ORDERS');

debug('begin section: STUCK_TRANSACTIONS');
start_section('Stuck Transactions', 'STUCK_TRANSACTIONS');
   set_item_result(run_stored_sig('DMEF TXN - CST_INVALID_WIP'));
   set_item_result(run_stored_sig('DMEF TXN - CST_INVALID_JOB_DATE'));
   set_item_result(run_stored_sig('DMEF RES TXN - USER NAME NOT VALID. PLEASE RE-ENTER'));
   set_item_result(run_stored_sig('DMEF RES TXN - ORGANIZATION CODE NOT VALID. PLEASE RE-ENTER'));
   set_item_result(run_stored_sig('EAM_RESOURCE_TRX_ERROR_RESOURCE CODE IS NOT VALID. PLEASE RE-ENTER'));
   set_item_result(run_stored_sig('EAM_RESOURCE_TRX_ERROR_EMPLOYEE NUMBER NOT VALID'));
end_section;
debug('end section: STUCK_TRANSACTIONS');

debug('begin section: PATCH_REPORT');
start_section('Patch Details', 'PATCH_REPORT');
   set_item_result(check_rec_patches_1); /* Signature EAM_AN_RECOMMENDED_PATCHES */
end_section;
debug('end section: PATCH_REPORT');

debug('begin section: ASSET');
start_section('Asset Details', 'ASSET');
   set_item_result(run_stored_sig('CSI_TXN_ERRORS'));
   set_item_result(run_stored_sig('ASSET_WO_WR _STATUS'));
end_section;
debug('end section: ASSET');
